
module Elixir.Data.Buckwalter.Lexicon14 where

import Elixir.Lexicon


version = revised "$Revision$"

lexicon = include section


cluster_1   = listing "Lexicon's properties"


 |> "'a.sw" <| [

    Identity |< At |< Iy      `adj`     {- OaSowAtiy~ -}       [ "phonetician" ] ]

 |> ".s ' .g" <| [

    FAL                       `noun`    {- SAg -}              [ "standard" ],

    FACiL                     `noun`    {- SA}ig -}            [ "jeweler" ]
                              `plural`     FuyyAL
                              `plural`     FAL |< aT
                           {- `others`  [ ".suwwA.g N" ] -} ]

 |> ".s ' .h" <| [

    FACiL                     `adj`     {- SA}iH -}            [ "shouting", "screaming" ] ]

 |> ".s ' ^g" <| [

    FAL                       `noun`    {- SAj -}              [ unwords [ "sheet", "metal" ] ] ]

 |> ".s ' b" <| [

    FuCAL |< aT               `noun`    {- SuWAbap -}          [ "nit", unwords [ "louse", "egg" ], unwords [ "lice", "eggs" ] ]
                              `plural`     FiCLAn
                              `plural`     FuCAL
                              `plural`     FILAn,

    FAL |< At                 `noun`    {- SAbAt -}            [ "Sabat" ],

    FACiL                     `noun`    {- SA}ib -}            [ "Saeb", "Sa'ib" ],

    FACiL                     `adj`     {- SA}ib -}            [ "correct", "accurate", unwords [ "on", "target" ], "opportune" ] ]

 |> ".s ' d" <| [

    FAL                       `noun`    {- SAd -}              [ unwords [ "Sad", "(", "Arabic", "letter", ")" ] ]
                              `plural`     FAL |< At ]

 |> ".s ' l" <| [

    FAL |< aT                 `noun`    {- SAlap -}            [ "hall" ],

    FACiL |< aT               `noun`    {- SA}ilap -}          [ unwords [ "hostile", "act" ] ]
                              `plural`     FawACiL,

    FACiL |< aT               `noun`    {- SA}ilap -}          [ "violence" ]
                              `plural`     FawACiL ]

 |> ".s ' m" <| [

    FACiL                     `adj`     {- SA}im -}            [ "fasting", "abstinent" ]
                              `plural`     FACiL |< Un
                              `plural`     FiyAL
                              `plural`     FuyyaL
                           {- `others`  [ ".suwwam N" ] -} ]

 |> ".s ' n" <| [

    FACiL                     `noun`    {- SA}in -}            [ "preserver", "protector", "maintainer" ]
                              `plural`     FACiL |< Un,

    FACiL |< aT               `noun`    {- DERIVED -}          [ "preserver", "protector", "maintainer" ]
                              `plural`     FACiL |< At ]

 |> ".s ' t" <| [

    FACiL                     `adj`     {- SA}it -}            [ "voiced", "sounded" ] ]

 |> ".s ' y" <| [

    FaCY                      `verb`    {- SaOaY-a -}          [ "twitter", "chirp" ]
                              `imperf`     FCY,

    FaCY                      `verb`    {- SaOaY-i -}          [ "twitter", "chirp" ]
                              `imperf`     FCI,

    FaCIL                     `noun`    {- Sa}iy~ -}           [ "twittering", "chirping" ] ]

 |> ".s .g '" <| [

    HiFCAL                    `noun`    {- IiSogA' -}          [ "attentiveness", "listening" ]
                              `plural`     HiFCAL |< At ]


cluster_2   = listing "Lexicon's properties"


 |> ".s .g .g" <| [

    FaL |< aN                 `noun`    {- SagAF -}            [ "inclination", "disposition", "benevolence" ] ]

 |> ".s .g r" <| [

    FaCuL                     `verb`    {- Sagur-u -}          [ unwords [ "be", "small" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sag~ar -}           [ "diminish", "belittle" ],

    TaFACaL                   `verb`    {- taSAgar -}          [ unwords [ "be", "humble" ] ],

    IstaFCaL                  `verb`    {- AisotaSogar -}      [ unwords [ "look", "down", "on" ] ],

    FiCaL                     `noun`    {- Sigar -}            [ "smallness", "humility" ],

    FiCaL                     `noun`    {- Sigar -}            [ "youth", "infancy" ],

    FaCAL                     `noun`    {- SagAr -}            [ "smallness", "humility" ],

    FaCIL                     `noun`    {- Sagiyr -}           [ "small", "young" ]
                              `plural`     FiCAL,

    FaCIL |< aT               `noun`    {- Sagiyrap -}         [ unwords [ "venial", "sin" ] ]
                              `plural`     FaCA'iL,

    HaFCaL                    `adj`     {- OaSogar -}          [ unwords [ "smaller", "/", "smallest" ], unwords [ "younger", "/", "youngest" ], "minor" ]
                              `plural`     FuCLY |< At,

    TaFCIL                    `noun`    {- taSogiyr -}         [ "diminution", "reduction" ]
                              `plural`     TaFCIL |< At,

    HiFCAL                    `noun`    {- IiSogAr -}          [ "contempt", "disregard" ]
                              `plural`     HiFCAL |< At,

    TaFACuL                   `noun`    {- taSAgur -}          [ "servility", "humility" ]
                              `plural`     TaFACuL |< At,

    FACiL                     `adj`     {- SAgir -}            [ "contemptible", "dejected" ],

    MuFaCCaL                  `adj`     {- muSag~ar -}         [ "diminished", "reduced" ] ]


cluster_3   = listing "Lexicon's properties"


 |> ".s .g w" <| [

    FaCA                      `verb`    {- SagA-u -}           [ unwords [ "lean", "over" ], "incline", unwords [ "be", "leaned", "over" ] ]
                              `imperf`     FCU,

    FaCI                      `verb`    {- Sagiy-a -}          [ unwords [ "lean", "over" ], "incline" ]
                              `imperf`     FCY,

    HaFCY                     `verb`    {- OaSogaY -}          [ unwords [ "pay", "attention" ], "heed", unwords [ "be", "listened", "to" ] ],

    FaCL                      `noun`    {- Sagow -}            [ "inclination", "disposition", "benevolence" ],

    FaC |< aN                 `noun`    {- SagAF -}            [ "inclination", "disposition", "benevolence" ],

    FaCY                      `noun`    {- SagaY -}            [ "inclination", "disposition", "benevolence" ]
                              `plural`     FaCA,

    HiFCA'                    `noun`    {- IiSogA' -}          [ "attentiveness", "listening" ]
                              `plural`     HiFCA' |< At,

    FACI                      `adj`     {- SAgiy -}            [ "attentive", "listening" ],

    MuFCI                     `adj`     {- muSogiy -}          [ "attentive", "listening" ]
                              `plural`     MuFCI |< At ]

 |> ".s .g y" <| [

    FaCI                      `verb`    {- Sagiy-a -}          [ unwords [ "lean", "over" ], "incline" ]
                              `imperf`     FCY,

    FACI                      `adj`     {- SAgiy -}            [ "attentive", "listening" ],

    MuFCI                     `adj`     {- muSogiy -}          [ "attentive", "listening" ]
                              `plural`     MuFCI |< At ]


cluster_4   = listing "Lexicon's properties"


 |> ".s .h .h" <| [

    FaCL                      `verb`    {- SaH~-i -}           [ unwords [ "be", "correct" ], unwords [ "be", "true" ] ]
                              `imperf`     FiCL
                              `pfirst`     FaCaL,

    FaCCaL                    `verb`    {- SaH~aH -}           [ "correct", "confirm" ],

    TaFaCCaL                  `verb`    {- taSaH~aH -}         [ unwords [ "be", "corrected" ], unwords [ "be", "confirmed" ] ],

    FiCL |< aT                `noun`    {- SiH~ap -}           [ "health", "truth", "correctness" ],

    FiCL |< Iy                `adj`     {- SiH~iy~ -}          [ "health", "healthy", "sanitary" ],

    FiCL |< Iy |< aT          `noun`    {- SiH~iy~ap -}        [ "healthiness", "salubriousness" ],

    FaCIL                     `adj`     {- SaHiyH -}           [ "true", "correct" ],

    HaFaCL                    `noun`    {- OaSaH~ -}           [ unwords [ "more", "correct" ] ],

    MaFaCL                    `noun`    {- maSaH~ -}           [ "sanatorium" ]
                              `plural`     MaFaCL |< At,

    MaFaCL |< aT              `noun`    {- maSaH~ap -}         [ "sanatorium" ],

    TaFCIL                    `noun`    {- taSoHiyH -}         [ "correction" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- taSoHiyHiy~ -}      [ "correction", "corrective" ],

    MuFaCCiL                  `noun`    {- muSaH~iH -}         [ "correcting", "corrective" ] ]


cluster_5   = listing "Lexicon's properties"


 |> ".s .h b" <| [

    FaCiL                     `verb`    {- SaHib-a -}          [ "befriend", "accompany" ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- SAHab -}            [ "befriend", "accompany" ],

    HaFCaL                    `verb`    {- OaSoHab -}          [ "escort" ],

    IFtaCaL                   `verb`    {- AiSoTaHab -}        [ "accompany", "escort" ],

    IstaFCaL                  `verb`    {- AisotaSoHab -}      [ "accompany", "escort" ],

    FuCL |< aT                `noun`    {- SuHobap -}          [ "company", "accompanied", "friendship" ],

    FaCAL |< aT               `noun`    {- SaHAbap -}          [ unwords [ "companions", "of", "the", "Prophet" ] ],

    FaCAL |< Iy               `adj`     {- SaHAbiy~ -}         [ unwords [ "companion", "of", "the", "Prophet" ] ],

    MuFACaL |< aT             `noun`    {- muSAHabap -}        [ "escort", "accompanying" ],

    IFtiCAL                   `noun`    {- AiSoTiHAb -}        [ "accompanying", "escort" ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `noun`    {- SAHib -}            [ "owner", "originator" ]
                              `plural`     FuCLAn
                              `plural`     HaFCAL,

    FACiL                     `noun`    {- SAHib -}            [ "friend", "companion" ]
                              `plural`     FuCLAn
                              `plural`     HaFCAL,

    MaFCUL                    `adj`     {- maSoHuwb -}         [ "accompanied" ],

    FuwayCiL                  `noun`    {- SuwayoHib -}        [ unwords [ "little", "friend" ] ]
                              `plural`     FuwayCiL |< Un,

    FuwayCiL |< aT            `noun`    {- DERIVED -}          [ unwords [ "little", "friend" ] ]
                              `plural`     FuwayCiL |< At ]


cluster_6   = listing "Lexicon's properties"


 |> ".s .h f" <| [

    FaCCaL                    `verb`    {- SaH~af -}           [ "mispronounce", "misspell", "misrepresent" ],

    TaFaCCaL                  `verb`    {- taSaH~af -}         [ unwords [ "be", "mispronounced" ], unwords [ "be", "misspelled" ], unwords [ "be", "misrepresented" ] ],

    FaCL |< aT                `noun`    {- SaHofap -}          [ "bowl", "platter" ]
                              `plural`     FiCAL,

    FaCIL |< aT               `noun`    {- SaHiyfap -}         [ "newspaper" ]
                              `plural`     FuCuL,

    FaCA'iL                   `noun`    {- SaHA}if -}          [ unwords [ "sheets", "(", "paper", ")" ] ],

    FuCuL |< Iy               `adj`     {- SuHufiy~ -}         [ "journalistic", "press", "newspaper" ],

    FuCuL |< Iy               `noun`    {- SuHufiy~ -}         [ "journalist", "reporter" ]
                              `plural`     FuCuL |< Iy |< Un,

    FuCuL |< Iy |< aT         `noun`    {- DERIVED -}          [ "journalist", "reporter" ]
                              `plural`     FuCuL |< Iy |< At,

    FaCAL |< aT               `noun`    {- SaHAfap -}          [ "journalism", "press" ],

    FaCAL |< Iy               `adj`     {- SaHAfiy~ -}         [ "journalistic", "press" ],

    FaCAL |< Iy               `noun`    {- SaHAfiy~ -}         [ "journalist", "reporter" ]
                              `plural`     FaCAL |< Iy |< Un,

    FaCAL |< Iy |< aT         `noun`    {- DERIVED -}          [ "journalist", "reporter" ]
                              `plural`     FaCAL |< Iy |< At,

    FaCCAL                    `noun`    {- SaH~Af -}           [ "Sahhaf" ],

    FaCCAL                    `noun`    {- SaH~Af -}           [ unwords [ "newspaper", "seller" ] ],

    MuFCaL                    `noun`    {- muSoHaf -}          [ "Quran", unwords [ "Quranic", "manuscript" ] ],

    MuFCaL                    `noun`    {- muSoHaf -}          [ "volume", "copies" ]
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- taSoHiyf -}         [ "mispronunciation", "misspelling", "misrepresentation" ]
                              `plural`     TaFCIL |< At ]

 |> ".s .h l" <| [

    FaCaL                     `adj`     {- SaHal -}            [ "raucous", "hoarse" ],

    HaFCaL                    `noun`    {- OaSoHal -}          [ "raucous", "hoarse" ]
                              `plural`     FaCLA' ]

 |> ".s .h n" <| [

    FaCL                      `noun`    {- SaHon -}            [ "bowl", "plate" ]
                              `plural`     FuCUL,

    FaCL                      `noun`    {- SaHon -}            [ unwords [ "dish", "antenna" ], unwords [ "concave", "mirror" ] ]
                              `plural`     FuCUL,

    MaFCUL                    `adj`     {- maSoHuwn -}         [ "grated", "crushed", "ground" ] ]


cluster_7   = listing "Lexicon's properties"


 |> ".s .h r" <| [

    HaFCaL                    `noun`    {- OaSoHar -}          [ unwords [ "desert", "-", "like" ], "desolate" ],

    FaCLA'                    `noun`    {- SaHorA' -}          [ "Sahara" ],

    FaCLA'                    `noun`    {- SaHorA' -}          [ "desert" ]
                              `plural`     FaCALY,

    FaCCAL |< aT              `noun`    {- SaH~Arap -}         [ "crate", "box" ]
                              `plural`     FaCACIL,

    TaFCIL                    `noun`    {- taSoHiyr -}         [ "desertification" ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- taSaH~ur -}         [ "desertification" ]
                              `plural`     TaFaCCuL |< At,

    FaCALI                    `noun`    {- SaHAriy -}          [ "deserts" ],

    FaCLA' |< Iy              `adj`     {- SaHorAwiy~ -}       [ "desert", "Saharan" ] ]

 |> ".s .h w" <| [

    FaCA                      `verb`    {- SaHA-u -}           [ unwords [ "wake", "up" ], unwords [ "regain", "consciousness" ] ]
                              `imperf`     FCU,

    FaCL                      `noun`    {- SaHow -}            [ "serenity", "clarity" ],

    FaCL |< aT                `noun`    {- SaHowap -}          [ "resurgence", "revival" ],

    FaCI                      `verb`    {- SaHiy-a -}          [ unwords [ "wake", "up" ], unwords [ "regain", "consciousness" ] ]
                              `imperf`     FCY,

    FaCCY                     `verb`    {- SaH~aY -}           [ "awaken", "rouse" ],

    HaFCY                     `verb`    {- OaSoHaY -}          [ "awaken", "rouse" ],

    FACI                      `adj`     {- SAHiy -}            [ "bright", "serene" ],

    FACI                      `adj`     {- SAHiy -}            [ "vigilant", "wakeful" ]
                              `plural`     FACI |< At ]

 |> ".s .h y" <| [

    FaCI                      `verb`    {- SaHiy-a -}          [ unwords [ "wake", "up" ], unwords [ "regain", "consciousness" ] ]
                              `imperf`     FCY,

    FACI                      `adj`     {- SAHiy -}            [ "bright", "serene" ],

    FACI                      `adj`     {- SAHiy -}            [ "vigilant", "wakeful" ]
                              `plural`     FACI |< At ]

 |> ".s .t b" <| [

    MaFCaL |< aT              `noun`    {- maSoTabap -}        [ "mastaba", unwords [ "stone", "bench" ] ]
                              `plural`     MaFACiL ]


cluster_8   = listing "Lexicon's properties"


 |> ".s .t l" <| [

    MaFCUL                    `noun`    {- maSoTuwl -}         [ "fool" ] ]

 |> ".s _h b" <| [

    FaCiL                     `verb`    {- Saxib-a -}          [ "shout", "clamor" ]
                              `imperf`     FCaL,

    IFtaCaL                   `verb`    {- AiSoTaxab -}        [ "shout", "clamor" ],

    FaCaL                     `noun`    {- Saxab -}            [ "shouting", "tumult" ],

    FaCiL                     `adj`     {- Saxib -}            [ "vociferous", "noisy" ],

    FaCCAL                    `adj`     {- Sax~Ab -}           [ "noisy", "vociferous" ],

    IFtiCAL                   `noun`    {- AiSoTixAb -}        [ "noise", "tumult" ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `adj`     {- SAxib -}            [ "noisy", "tumultuous" ],

    MuFtaCaL                  `noun`    {- muSoTaxab -}        [ "noise", "tumult" ] ]

 |> ".s _h r" <| [

    FaCCaL                    `verb`    {- Sax~ar -}           [ "petrify" ],

    TaFaCCaL                  `verb`    {- taSax~ar -}         [ unwords [ "be", "petrified" ] ],

    FaCL                      `noun`    {- Saxor -}            [ "rock" ]
                              `plural`     FuCUL |< aT
                              `plural`     FaCaL |< At,

    FaCL                      `noun`    {- Saxor -}            [ "Sakhr" ],

    FaCL |< Iy                `adj`     {- Saxoriy~ -}         [ "rock", "rupestral" ],

    TaFCIL                    `noun`    {- taSoxiyr -}         [ "petrification" ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- taSax~ur -}         [ "petrification" ]
                              `plural`     TaFaCCuL |< At ]


cluster_9   = listing "Lexicon's properties"


 |> ".s ` b" <| [

    FaCuL                     `verb`    {- SaEub-u -}          [ unwords [ "be", "difficult" ] ]
                              `imperf`     FCuL,

    TaFaCCaL                  `verb`    {- taSaE~ab -}         [ unwords [ "become", "difficult" ] ],

    TaFACaL                   `verb`    {- taSAEab -}          [ unwords [ "be", "difficult" ] ],

    IstaFCaL                  `verb`    {- AisotaSoEab -}      [ unwords [ "consider", "difficult" ] ],

    FaCL                      `adj`     {- SaEob -}            [ "difficult", "arduous" ]
                              `plural`     FiCAL,

    HaFCaL                    `noun`    {- OaSoEab -}          [ unwords [ "more", "/", "most", "difficult" ], unwords [ "more", "/", "most", "arduous" ] ],

    FuCUL |< aT               `noun`    {- SuEuwbap -}         [ "difficulty" ],

    MaFACiL                   `noun`    {- maSAEib -}          [ "difficulties" ],

    MutaFACiL                 `adj`     {- mutaSAEib -}        [ "difficult", unwords [ "hard", "to", "please" ] ] ]


cluster_10  = listing "Lexicon's properties"


 |> ".s ` d" <| [

    FaCiL                     `verb`    {- SaEid-a -}          [ "rise", "climb" ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- SaE~ad -}           [ "escalate", "intensify" ],

    HaFCaL                    `verb`    {- OaSoEad -}          [ unwords [ "make", "rise" ], unwords [ "be", "raised" ] ],

    TaFaCCaL                  `verb`    {- taSaE~ad -}         [ "escalate", "evaporate" ],

    TaFACaL                   `verb`    {- taSAEad -}          [ "climb", "increase" ],

    FuCUL                     `noun`    {- SuEuwd -}           [ "ascent", "rising" ],

    FaCL |< aT                `noun`    {- SaEodap -}          [ "rise", "incline" ]
                              `plural`     FaCaL |< At,

    FaCIL                     `noun`    {- SaEiyd -}           [ "level", "plane", "domains" ]
                              `plural`     HaFCiL |< aT
                              `plural`     FuCuL,

    FaCIL                     `noun`    {- SaEiyd -}           [ "highlands", unwords [ "Upper", "Egypt" ] ],

    FaCIL |< Iy               `adj`     {- SaEiydiy~ -}        [ "Saidi", unwords [ "Upper", "Egyptian" ] ]
                              `plural`     FaCA'iL |< aT,

    FuCaLA'                   `noun`    {- SuEadA' -}          [ unwords [ "deep", "sigh" ] ]
                           {- `others`  [ ".sa`adA' Nh N0_Nh Nhy" ] -},

    MaFCaL                    `noun`    {- maSoEad -}          [ "anode" ]
                              `plural`     MaFACiL,

    MiFCaL                    `noun`    {- miSoEad -}          [ "elevator" ]
                              `plural`     MiFCaL |< At
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- taSoEiyd -}         [ "escalation", "intensification" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- taSoEiydiy~ -}      [ "escalation" ],

    HiFCAL                    `noun`    {- IiSoEAd -}          [ "raise", "increase" ]
                              `plural`     HiFCAL |< At,

    TaFACuL                   `noun`    {- taSAEud -}          [ "growth", "increase", "escalation" ]
                              `plural`     TaFACuL |< At,

    TaFACuL |< Iy             `adj`     {- taSAEudiy~ -}       [ "growing", "increasing", "escalating" ],

    FACiL                     `noun`    {- SAEid -}            [ "rising", "ascending" ],

    FACiL |< aN               `adv`     {- SAEidAF -}          [ "henceforth", "upward" ]
                              `plural`     FACiL,

    MutaFACiL                 `noun`    {- mutaSAEid -}        [ "rising", "ascending" ],

    FACiL |< aT               `noun`    {- SAEidap -}          [ "stalagmite" ]
                              `plural`     FawACiL,

    FaCCAL                    `noun`    {- SaE~Ad -}           [ "escalator" ] ]


cluster_11  = listing "Lexicon's properties"


 |> ".s ` l" <| [

    FaCL                      `noun`    {- SaEol -}            [ "microcephalic" ]
                              `plural`     HaFCaL ]

 |> ".s ` l k" <| [

    KaRDaS |< aT              `noun`    {- SaEolakap -}        [ "vagrancy", "misery" ],

    KuRDUS                    `noun`    {- SuEoluwk -}         [ "vagrant", "vagabond" ]
                              `plural`     KaRADIS ]

 |> ".s ` q" <| [

    FaCaL                     `verb`    {- SaEaq-a -}          [ "stun", "strike" ]
                              `imperf`     FCaL,

    FaCiL                     `verb`    {- SaEiq-a -}          [ unwords [ "be", "stunned" ], unwords [ "be", "thunderstruck" ] ]
                              `imperf`     FCaL,

    HaFCaL                    `verb`    {- OaSoEaq -}          [ "stun", "strike", unwords [ "be", "stunned" ], unwords [ "be", "struck" ] ],

    InFaCaL                   `verb`    {- AinoSaEaq -}        [ unwords [ "be", "stunned" ], unwords [ "be", "thunderstruck" ] ],

    FaCaL                     `noun`    {- SaEaq -}            [ "electrocution", "detonation" ],

    FACiL                     `noun`    {- SAEiq -}            [ "detonator", "stunning" ],

    FACiL |< aT               `noun`    {- SAEiqap -}          [ "Sa'iqah", "Saiqa" ],

    FACiL |< aT               `noun`    {- SAEiqap -}          [ "lightning" ]
                              `plural`     FawACiL,

    MaFCUL                    `adj`     {- maSoEuwq -}         [ "thunderstruck", "stunned" ],

    MunFaCiL                  `adj`     {- munoSaEiq -}        [ "thunderstruck", "stunned" ] ]

 |> ".s ` r" <| [

    FaCiL                     `verb`    {- SaEir-a -}          [ unwords [ "have", "torticollis" ], unwords [ "have", "wryneck" ], "grimace" ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- SaE~ar -}           [ "grimace" ],

    FaCL                      `noun`    {- SaEor -}            [ "torticollis", "wryneck" ],

    TaFCIL                    `noun`    {- taSoEiyr -}         [ "grimace", "pouting" ]
                              `plural`     TaFCIL |< At ]

 |> ".s ` t r" <| [

    KaRDaS                    `noun`    {- SaEotar -}          [ "thyme" ] ]


cluster_12  = listing "Lexicon's properties"


 |> ".s b '" <| [

    FaCaL                     `verb`    {- SabaO-a -}          [ "grow", "sprout" ]
                              `imperf`     FCaL,

    FuCUL                     `noun`    {- Subuw' -}           [ "growing", "sprouting" ],

    FACiL |< Iy               `adj`     {- SAbi}iy~ -}         [ "Sabian", "Mandean" ],

    FiCAL                     `noun`    {- SibA' -}            [ "childhood", "youth" ]
                              `plural`     FaCAL ]

 |> ".s b .g" <| [

    FaCaL                     `verb`    {- Sabag-u -}          [ "color", "tint", "dye" ]
                              `imperf`     FCuL,

    InFaCaL                   `verb`    {- AinoSabag -}        [ unwords [ "be", "colored" ], unwords [ "be", "tinted" ], unwords [ "be", "dyed" ] ],

    IFtaCaL                   `verb`    {- AiSoTabag -}        [ unwords [ "be", "colored" ], unwords [ "be", "tinted" ], unwords [ "be", "dyed" ] ],

    FiCL                      `noun`    {- Sibog -}            [ "color", "dye" ]
                              `plural`     HaFCAL,

    FiCL |< aT                `noun`    {- Sibogap -}          [ "color", "dye", "tint" ],

    FiCL |< aT                `noun`    {- Sibogap -}          [ unwords [ "Sibghat", "(", "in", "\"Sibghatullah\"", ")" ] ],

    FiCL |< Iy                `adj`     {- Sibogiy~ -}         [ "chromosome" ],

    FiCL |< Iy |< aT          `noun`    {- Sibogiy~ap -}       [ "chromosome" ],

    FiCAL                     `noun`    {- SibAg -}            [ "color", "dye", "condiment" ]
                              `plural`     HaFCiL |< aT,

    FaCCAL                    `noun`    {- Sab~Ag -}           [ "dyer" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ "dyer" ]
                              `plural`     FaCCAL |< At,

    FaCCAL                    `noun`    {- Sab~Ag -}           [ "Sabbagh" ],

    FiCAL |< aT               `noun`    {- SibAgap -}          [ "dyeing", "staining" ],

    MaFCaL |< aT              `noun`    {- maSobagap -}        [ unwords [ "dye", "-", "house" ] ],

    FACiL                     `noun`    {- SAbig -}            [ "dyer", "baptist" ],

    MaFCUL                    `adj`     {- maSobuwg -}         [ "dyed", "tinted", "colored" ],

    IFtiCAL                   `noun`    {- AiSoTibAg -}        [ "coloration", "pigmentation" ]
                              `plural`     IFtiCAL |< At ]


cluster_13  = listing "Lexicon's properties"


 |> ".s b .h" <| [

    FaCCaL                    `verb`    {- Sab~aH -}           [ "greet" ],

    HaFCaL                    `verb`    {- OaSobaH -}          [ "become", "begin", "start" ],

    IstaFCaL                  `verb`    {- AisotaSobaH -}      [ "light", "illuminate" ],

    FuCL                      `noun`    {- SuboH -}            [ "morning", "daybreak" ]
                              `plural`     HaFCAL,

    FaCAL                     `adv`     {- SabAH -}            [ "morning", unwords [ "in", "the", "morning" ] ],

    FaCAL |< Iy               `adj`     {- SabAHiy~ -}         [ "morning", "matinal" ],

    FaCAL                     `noun`    {- SabAH -}            [ "Sabah" ],

    FuCAL                     `noun`    {- SubAH -}            [ "Subah" ],

    FuCAL                     `noun`    {- SubAH -}            [ "cute", "comely", "attractive" ],

    FuCLAn                    `noun`    {- SuboHAn -}          [ "cute", "comely", "attractive" ]
                              `plural`     FaCLY,

    FaCIL                     `adj`     {- SabiyH -}           [ "cute", "comely", "attractive" ],

    FaCAL |< aT               `noun`    {- SabAHap -}          [ "beauty" ],

    FaCIL |< aT               `noun`    {- SabiyHap -}         [ "morning" ],

    FaCUL                     `adj`     {- SabuwH -}           [ "radiant" ],

    MiFCAL                    `noun`    {- miSobAH -}          [ "lamp", "light" ]
                              `plural`     MaFACIL,

    MiFCAL                    `noun`    {- miSobAH -}          [ "Misbah" ],

    IstiFCAL                  `noun`    {- AisotiSobAH -}      [ "illumination", "illuminating" ]
                              `plural`     IstiFCAL |< At,

    TaFCIL                    `noun`    {- taSobiyH -}         [ "greeting", "hellos" ]
                              `plural`     TaFCIL |< At ]

 |> ".s b .h y" <| [

    KuRDI                     `noun`    {- SuboHiy -}          [ "Subhi", "Sobhi" ] ]


cluster_14  = listing "Lexicon's properties"


 |> ".s b `" <| [

    FaCaL                     `verb`    {- SabaE-a -}          [ unwords [ "point", "the", "finger", "at" ] ]
                              `imperf`     FCaL,

    HiFCaL                    `noun`    {- IiSobaE -}          [ "finger" ]
                              `plural`     HaFACiL,

    HiFCaL |< Iy              `adj`     {- IiSobaEiy~ -}       [ unwords [ "finger", "-", "related" ], "digital" ],

    HiFCaL |< Iy |< At        `noun`    {- IiSobaEiy~At -}     [ unwords [ "digitigrades", "(", "walking", "on", "the", "toes", ")" ] ],

    FuCAL                     `noun`    {- SubAE -}            [ "finger", "toe" ]
                              `plural`     FawACiL,

    HuFCUL                    `noun`    {- OuSobuwE -}         [ "finger", "toe" ]
                              `plural`     HaFACIL,

    MuFaCCaL                  `noun`    {- muSab~aE -}         [ "grill", "gridiron" ]
                              `plural`     MuFaCCaL |< At ]


cluster_15  = listing "Lexicon's properties"


 |> ".s b b" <| [

    FaCL                      `verb`    {- Sab~-u -}           [ "pour", "flow" ]
                              `imperf`     FuCL
                              `pfirst`     FaCaL,

    FaCL                      `verb`    {- Sab~-a -}           [ unwords [ "love", "passionately" ] ]
                              `pfirst`     FaCiL,

    TaFaCCaL                  `verb`    {- taSab~ab -}         [ "flow", "perspire", "drip" ],

    InFaCL                    `verb`    {- AinoSab~ -}         [ unwords [ "be", "poured", "out" ], "flow" ],

    FaCL                      `noun`    {- Sab~ -}             [ "pouring", "flowing" ],

    FaCaL                     `noun`    {- Sabab -}            [ "declivity", "slope", "hillsides" ]
                              `plural`     HaFCAL,

    FaCIL                     `noun`    {- Sabiyb -}           [ unwords [ "poured", "out" ], "spilled" ],

    FaCAL |< aT               `noun`    {- SabAbap -}          [ unwords [ "passionate", "love" ] ],

    FuCAL |< aT               `noun`    {- SubAbap -}          [ "remainder" ],

    MaFaCL                    `noun`    {- maSab~ -}           [ "outlet", "drain", "funnel" ]
                              `plural`     MaFaCL |< At
                              `plural`     MaFACL,

    MaFCUL                    `noun`    {- maSobuwb -}         [ unwords [ "lead", "(", "metal", ")" ], unwords [ "cast", "-", "metal", "goods" ], unwords [ "foundry", "products" ], unwords [ "hot", "lead" ] ]
                              `plural`     MaFCUL |< At,

    FaCCAL                    `noun`    {- Sab~Ab -}           [ "spout", unwords [ "pouring", "lip" ] ],

    InFiCAL                   `noun`    {- AinoSibAb -}        [ unwords [ "pouring", "out" ], unwords [ "throwing", "out" ] ]
                              `plural`     InFiCAL |< At,

    FaL                       `noun`    {- Sab -}              [ unwords [ "east", "wind" ] ]
                              `plural`     HaFCA'
                              `plural`     FaCA,

    FiL |< aN                 `noun`    {- SibAF -}            [ "youth", "childhood" ]
                           {- `others`  [ ".sibA N0_Nhy" ] -},

    FaL |<< "awIy"            `adj`     {- Sabawiy~ -}         [ "juvenile" ] ]


cluster_16  = listing "Lexicon's properties"


 |> ".s b n" <| [

    FaCCaL                    `verb`    {- Sab~an -}           [ "soap", unwords [ "rub", "with", "soap" ], "saponify" ],

    FACUL                     `noun`    {- SAbuwn -}           [ "soap" ],

    FACUL |< Iy               `adj`     {- SAbuwniy~ -}        [ "soapy" ],

    FaCCAL                    `noun`    {- Sab~An -}           [ unwords [ "soap", "boiler" ] ],

    MaFCaL |< aT              `noun`    {- maSobanap -}        [ unwords [ "soap", "factory" ] ],

    TaFCIL                    `noun`    {- taSobiyn -}         [ unwords [ "soap", "-", "making" ] ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- taSab~un -}         [ unwords [ "soap", "-", "making" ] ]
                              `plural`     TaFaCCuL |< At ]


cluster_17  = listing "Lexicon's properties"


 |> ".s b r" <| [

    FaCaL                     `verb`    {- Sabar-i -}          [ unwords [ "be", "patient" ], "endure" ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- Sab~ar -}           [ "preserve", "comfort", "stabilize", "ballast" ],

    FACaL                     `verb`    {- SAbar -}            [ unwords [ "endure", "patiently" ] ],

    TaFaCCaL                  `verb`    {- taSab~ar -}         [ unwords [ "be", "patient" ], "endure" ],

    IFtaCaL                   `verb`    {- AiSoTabar -}        [ unwords [ "be", "patient" ], "endure" ],

    FaCL                      `noun`    {- Sabor -}            [ "patience", "endurance" ],

    FaCL |< aT                `noun`    {- Saborap -}          [ unwords [ "severe", "cold" ] ],

    FuCL |< aT                `noun`    {- Suborap -}          [ "pile", "heap" ],

    FuCL |< aT |< aN          `adv`     {- SuborapF -}         [ unwords [ "on", "the", "whole" ], "summarily" ],

    FaCUL                     `noun`    {- Sabuwr -}           [ "patient", "enduring" ],

    FaCUL                     `noun`    {- Sabuwr -}           [ "Sabour" ],

    FaCAL |< aT               `noun`    {- SabArap -}          [ unwords [ "severe", "cold" ] ],

    HaFCaL                    `adj`     {- OaSobar -}          [ unwords [ "more", "/", "most", "patient" ], unwords [ "more", "/", "most", "persistent" ] ],

    TaFCIL                    `noun`    {- taSobiyr -}         [ "preservation", "comforting", "stabilizing" ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- muSAbarap -}        [ "perseverance", "patience" ],

    IFtiCAL                   `noun`    {- AiSoTibAr -}        [ "perseverance", "patience" ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `noun`    {- SAbir -}            [ "Sabir", "Saber" ],

    FACiL                     `adj`     {- SAbir -}            [ "patient", "steadfast", "enduring" ],

    MuFaCCaL |< At            `noun`    {- muSab~arAt -}       [ unwords [ "canned", "goods" ] ]
                              `plural`     MuFaCCaL |< At,

    FACUL |< aT               `noun`    {- SAbuwrap -}         [ unwords [ "ballast", "(", "ship", ")" ] ] ]


cluster_18  = listing "Lexicon's properties"


 |> ".s b r h" <| [

    KaRDaS                    `noun`    {- Saborah -}          [ "Sabroh" ] ]

 |> ".s b r y" <| [

    KaRDI                     `noun`    {- Saboriy -}          [ "Sabri", "Sabry" ] ]

 |> ".s b w" <| [

    FaCA                      `verb`    {- SabA-u -}           [ unwords [ "strive", "for" ], unwords [ "aspire", "to" ], unwords [ "be", "strived", "for" ], unwords [ "be", "aspired", "to" ] ]
                              `imperf`     FCU,

    FaCI                      `verb`    {- Sabiy-a -}          [ unwords [ "be", "childish" ], unwords [ "be", "infantile" ] ]
                              `imperf`     FCY,

    FaCCY                     `verb`    {- Sab~aY -}           [ "rejuvenate" ],

    TaFaCCY                   `verb`    {- taSab~aY -}         [ unwords [ "be", "childish" ], unwords [ "be", "infantile" ] ],

    TaFACY                    `verb`    {- taSAbaY -}          [ unwords [ "be", "childish" ], unwords [ "be", "infantile" ] ],

    IstaFCY                   `verb`    {- AisotaSobaY -}      [ unwords [ "be", "childish" ], unwords [ "be", "infantile" ] ],

    FaC                       `noun`    {- Sab -}              [ unwords [ "east", "wind" ] ]
                              `plural`     HaFCA'
                              `plural`     FaCaL |< At
                              `plural`     FaCA,

    FiC |< aN                 `noun`    {- SibAF -}            [ "youth", "childhood" ]
                           {- `others`  [ ".sibA N0_Nhy" ] -},

    FiCA'                     `noun`    {- SibA' -}            [ "childhood", "youth" ]
                              `plural`     FaCA',

    FuCUL                     `noun`    {- Subuw~ -}           [ unwords [ "youthful", "zeal" ], unwords [ "childish", "manners" ] ],

    FuCUL |< aT               `noun`    {- Subuw~ap -}         [ "childhood", "youth" ],

    FaCL |< aT                `noun`    {- Sabowap -}          [ unwords [ "youthful", "zeal" ], unwords [ "childish", "manners" ] ],

    FaC |<< "awIy"            `adj`     {- Sabawiy~ -}         [ "juvenile" ],

    FACI                      `adj`     {- SAbiy -}            [ "youthful" ] ]


cluster_19  = listing "Lexicon's properties"


 |> ".s b y" <| [

    FaCI                      `verb`    {- Sabiy-a -}          [ unwords [ "be", "childish" ], unwords [ "be", "infantile" ] ]
                              `imperf`     FCY,

    FaCIL                     `noun`    {- Sabiy~ -}           [ unwords [ "young", "boy" ], "youth" ]
                              `plural`     HaFCI |< aT
                              `plural`     FiCL |< aT
                              `plural`     FiCLAn,

    FaCIL |< aT               `noun`    {- Sabiy~ap -}         [ "girl", unwords [ "young", "girl" ] ]
                              `plural`     FaCALY,

    FiCLAn |< Iy              `adj`     {- SiboyAniy~ -}       [ "infantile", "puerile" ],

    FACI                      `adj`     {- SAbiy -}            [ "youthful" ] ]

 |> ".s b y n" <| [

    KiRDAS |< Iy              `adj`     {- SiboyAniy~ -}       [ "infantile", "puerile" ] ]

 |> ".s d '" <| [

    FaCiL                     `verb`    {- Sadi}-a -}          [ "oxidize", unwords [ "be", "rusty" ] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- SaduW-u -}          [ "oxidize", unwords [ "be", "rusty" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sad~aO -}           [ "oxidize", unwords [ "make", "rusty" ] ],

    HaFCaL                    `verb`    {- OaSodaO -}          [ "oxidize", unwords [ "make", "rusty" ], unwords [ "get", "rusty" ], unwords [ "become", "oxidized" ] ],

    FaCaL                     `noun`    {- SadaO -}            [ "rust", "oxidation", "rusty", "oxidized" ]
                              `plural`     FaCAL |< aT
                              `plural`     FaCiL |< aT,

    MuFCaL                    `adj`     {- muSodaO -}          [ "rusty", "oxidized" ],

    HaFCaL                    `adj`     {- OaSodaO -}          [ "rusty", "oxidized" ]
                           {- `others`  [ ".sad'A N0_Nh" ] -},

    HiFCAL                    `noun`    {- IiSodA' -}          [ "oxidation" ]
                              `plural`     HiFCAL |< At ]

 |> ".s d .g" <| [

    FuCL                      `noun`    {- Sudog -}            [ unwords [ "temple", "(", "anat", ")" ], unwords [ "temples", "(", "anat", ")" ] ]
                              `plural`     HaFCAL,

    FuCL |< Iy                `adj`     {- Sudogiy~ -}         [ unwords [ "temporal", "(", "anat", ")" ] ] ]


cluster_20  = listing "Lexicon's properties"


 |> ".s d .h" <| [

    FaCaL                     `verb`    {- SadaH-a -}          [ "chant", "sing" ]
                              `imperf`     FCaL,

    FuCAL                     `noun`    {- SudAH -}            [ "chanting", "singing" ],

    FaCaL                     `noun`    {- SadaH -}            [ "banner" ],

    FaCaL |< At               `noun`    {- SadaHAt -}          [ unwords [ "musical", "strains" ] ]
                              `plural`     FaCaL |< At,

    FACiL                     `noun`    {- SAdiH -}            [ "semitone" ],

    FACiL                     `noun`    {- SAdiH -}            [ "singer", "tenor" ]
                              `plural`     FACiL |< Un,

    FACiL |< aT               `noun`    {- DERIVED -}          [ "singer", "tenor" ]
                              `plural`     FACiL |< At ]

 |> ".s d `" <| [

    FaCaL                     `verb`    {- SadaE-a -}          [ "break", unwords [ "expose", "clearly" ] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- Sad~aE -}           [ "break", unwords [ "give", "headaches", "to" ] ],

    TaFaCCaL                  `verb`    {- taSad~aE -}         [ unwords [ "be", "cracked" ] ],

    InFaCaL                   `verb`    {- AinoSadaE -}        [ unwords [ "be", "cracked" ] ],

    FaCL                      `noun`    {- SadoE -}            [ "crevice", "break" ]
                              `plural`     FuCUL,

    FaCL |< aT                `noun`    {- SadoEap -}          [ "split", "divergence" ]
                              `plural`     FaCaL |< At,

    FuCAL                     `noun`    {- SudAE -}            [ "headache", "migraine" ],

    FuCAL |< Iy               `adj`     {- SudAEiy~ -}         [ "migraine" ],

    TaFCIL                    `noun`    {- taSodiyE -}         [ "rupture" ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- taSad~uE -}         [ "breach", "rift" ]
                              `plural`     TaFaCCuL |< At,

    MaFCUL                    `adj`     {- maSoduwE -}         [ "cracked", "broken" ],

    MutaFaCCiL                `adj`     {- mutaSad~iE -}       [ "cracked", "brittle" ] ]


cluster_21  = listing "Lexicon's properties"


 |> ".s d d" <| [

    FaCL                      `verb`    {- Sad~-u -}           [ "repel", "deter", "resist" ]
                              `imperf`     FuCL
                              `pfirst`     FaCaL,

    HaFaCL                    `verb`    {- OaSad~ -}           [ "suppurate", "fester" ],

    FaCL                      `noun`    {- Sad~ -}             [ "resistance", "deterrence", "rejection" ],

    FaCaL                     `noun`    {- Sadad -}            [ "respect", "regard", "purpose" ],

    FuCUL                     `noun`    {- Suduwd -}           [ "interception", "refusal", "rejection" ],

    FaCIL                     `noun`    {- Sadiyd -}           [ "pus", "matter" ],

    FaCIL |< Iy               `adj`     {- Sadiydiy~ -}        [ "festering", "suppurating" ],

    MaFaCL                    `noun`    {- maSad~ -}           [ "wall", "barrier" ]
                              `plural`     MaFaCL |< At,

    MiFaCL                    `noun`    {- miSad~ -}           [ "bumper", "stopper", "cork" ]
                              `plural`     MiFaCL |< At,

    HiFCAL                    `noun`    {- IiSodAd -}          [ "suppuration" ]
                              `plural`     HiFCAL |< At ]


cluster_22  = listing "Lexicon's properties"


 |> ".s d f" <| [

    FaCaL                     `verb`    {- Sadaf-i -}          [ "avoid" ]
                              `imperf`     FCiL,

    FACaL                     `verb`    {- SAdaf -}            [ "coincide", "concur", "encounter" ],

    FaCaL                     `noun`    {- Sadaf -}            [ "pearl" ]
                              `plural`     HaFCAL,

    FaCaL                     `noun`    {- Sadaf -}            [ "psoriasis" ],

    FaCaL |< Iy               `adj`     {- Sadafiy~ -}         [ "pearly", unwords [ "mother", "-", "of", "-", "pearl" ] ],

    FaCaL |< Iy |< At         `noun`    {- Sadafiy~At -}       [ "mollusks" ],

    FuCL |< aT                `adv`     {- Sudofap -}          [ "chance", "coincidence", unwords [ "by", "chance" ], unwords [ "by", "coincidence" ] ]
                              `plural`     FuCaL
                           {- `others`  [ ".sudfaTaN FW-Wa" ] -},

    FuCL |< Iy                `adj`     {- Sudofiy~ -}         [ "occasional" ],

    MuFACaL |< aT             `noun`    {- muSAdafap -}        [ "coincidence" ],

    MuFACiL                   `adj`     {- muSAdif -}          [ unwords [ "coinciding", "with" ], unwords [ "corresponding", "with" ] ],

    TaFACuL |< Iy             `adj`     {- taSAdufiy~ -}       [ "occasional" ] ]


cluster_23  = listing "Lexicon's properties"


 |> ".s d m" <| [

    FaCaL                     `verb`    {- Sadam-i -}          [ unwords [ "collide", "with" ], unwords [ "crash", "into" ], "shock" ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- Sad~am -}           [ "oppose" ],

    FACaL                     `verb`    {- SAdam -}            [ "oppose", "clash" ],

    TaFACaL                   `verb`    {- taSAdam -}          [ "collide" ],

    IFtaCaL                   `verb`    {- AiSoTadam -}        [ "collide", "crash" ],

    FaCL |< aT                `noun`    {- Sadomap -}          [ "shock", "blow" ]
                              `plural`     FaCaL |< At,

    FiCAL                     `noun`    {- SidAm -}            [ "collision", "collapse", "clashes", "confrontations" ]
                              `plural`     FiCAL |< At,

    FiCAL |< Iy               `adj`     {- SidAmiy~ -}         [ "collision", "shock" ],

    FaCCAL                    `noun`    {- Sad~Am -}           [ "Saddam" ],

    MuFACaL |< aT             `noun`    {- muSAdamap -}        [ unwords [ "hostile", "encounter" ], "collision" ],

    TaFACuL                   `noun`    {- taSAdum -}          [ "collision", "shock" ]
                              `plural`     TaFACuL |< At,

    IFtiCAL                   `noun`    {- AiSoTidAm -}        [ "collision", "shock", "impact" ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `adj`     {- SAdim -}            [ "shocking", "explosive", "percussion" ],

    FuCAL                     `noun`    {- SudAm -}            [ "influenza" ],

    MuFACiL                   `adj`     {- muSAdim -}          [ "percussive", "explosive" ] ]


cluster_24  = listing "Lexicon's properties"


 |> ".s d q" <| [

    FaCaL                     `verb`    {- Sadaq-u -}          [ unwords [ "be", "sincere" ], unwords [ "tell", "the", "truth" ], unwords [ "be", "correct" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sad~aq -}           [ "believe", unwords [ "give", "credence", "to" ], "confirm" ],

    FACaL                     `verb`    {- SAdaq -}            [ "befriend" ],

    TaFaCCaL                  `verb`    {- taSad~aq -}         [ unwords [ "give", "alms" ] ],

    TaFACaL                   `verb`    {- taSAdaq -}          [ "trust", unwords [ "live", "harmoniously", "with" ] ],

    FiCL                      `noun`    {- Sidoq -}            [ "sincerity", "candor" ],

    FiCL |< Iy |< aT          `noun`    {- Sidoqiy~ap -}       [ "honesty", "integrity" ],

    FaCaL |< aT               `noun`    {- Sadaqap -}          [ "alms", "charity" ],

    FaCAL                     `noun`    {- SadAq -}            [ unwords [ "marriage", "contract" ], unwords [ "bridal", "dower" ] ],

    FaCAL |< aT               `noun`    {- SadAqap -}          [ "friendship" ],

    FaCIL                     `noun`    {- Sadiyq -}           [ "friend" ]
                              `plural`     FuCaLA'
                              `plural`     FuCLAn
                              `plural`     HaFCiLA',

    FaCUL                     `noun`    {- Saduwq -}           [ "truthful", "veracious" ],

    FiCCIL                    `noun`    {- Sid~iyq -}          [ "Siddiq" ],

    FiCCIL                    `noun`    {- Sid~iyq -}          [ "honest", "veracious" ],

    HaFCaL                    `noun`    {- OaSodaq -}          [ unwords [ "truer", "/", "truest" ], unwords [ "more", "/", "most", "reliable" ] ],

    MiFCAL                    `noun`    {- miSodAq -}          [ "confirmation", "substantiation" ],

    MiFCAL |< Iy |< aT        `noun`    {- miSodAqiy~ap -}     [ "credibility" ],

    TaFCIL                    `noun`    {- taSodiyq -}         [ "belief", "credence" ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- muSAdaqap -}        [ "approval", "certification" ],

    TaFACuL                   `noun`    {- taSAduq -}          [ "legalization", "authentication" ]
                              `plural`     TaFACuL |< At,

    TaFACuL                   `noun`    {- taSAduq -}          [ "entente", "harmony" ]
                              `plural`     TaFACuL |< At,

    FACiL                     `noun`    {- SAdiq -}            [ "veracious", "truthful" ],

    FACiL                     `noun`    {- SAdiq -}            [ "Sadiq" ],

    MuFaCCiL |< aT            `noun`    {- muSad~iqap -}       [ "certificate" ],

    MuFaCCaL                  `adj`     {- muSad~aq -}         [ "credible", "reliable" ],

    MutaFaCCiL                `adj`     {- mutaSad~iq -}       [ unwords [ "giving", "alms" ], unwords [ "alms", "-", "giver" ] ],

    MutaFaCCiL                `noun`    {- mutaSad~iq -}       [ "Mutasaddiq", "Motassadek" ] ]


cluster_25  = listing "Lexicon's properties"


 |> ".s d q y" <| [

    KiRDI                     `noun`    {- Sidoqiy -}          [ "Sidqi", "Sidqy" ] ]


cluster_26  = listing "Lexicon's properties"


 |> ".s d r" <| [

    FaCaL                     `verb`    {- Sadar-u -}          [ unwords [ "be", "published" ], unwords [ "be", "issued" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sad~ar -}           [ "export", "publish", "dispatch" ],

    FACaL                     `verb`    {- SAdar -}            [ "confiscate" ],

    HaFCaL                    `verb`    {- OaSodar -}          [ "issue", "publish" ],

    TaFaCCaL                  `verb`    {- taSad~ar -}         [ "preside", "lead", "head" ],

    IstaFCaL                  `verb`    {- AisotaSodar -}      [ "issue" ],

    FaCL                      `noun`    {- Sador -}            [ "chest", "bosom" ]
                              `plural`     FuCUL,

    FaCL |< Iy                `adj`     {- Sadoriy~ -}         [ "chest", "pectoral" ],

    FuCL |< aT                `noun`    {- Sudorap -}          [ "vest" ],

    FuCayL |< Iy              `adj`     {- Sudayoriy~ -}       [ "vest" ],

    FiCAL                     `noun`    {- SidAr -}            [ "vest" ],

    FaCAL |< aT               `noun`    {- SadArap -}          [ "chairmanship", "precedence" ],

    FuCUL                     `noun`    {- Suduwr -}           [ "appearance", "issuance" ],

    MaFCaL                    `noun`    {- maSodar -}          [ "source" ]
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- taSodiyr -}         [ "export", "exporting" ],

    TaFCIL |< At              `noun`    {- taSodiyrAt -}       [ "exports" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- taSodiyriy~ -}      [ "export" ],

    MuFACaL |< aT             `noun`    {- muSAdarap -}        [ "confiscation", "embargo" ],

    HiFCAL                    `noun`    {- IiSodAr -}          [ "export", "issuance" ],

    HiFCAL |< At              `noun`    {- IiSodArAt -}        [ "exports" ]
                              `plural`     HiFCAL |< At,

    TaFaCCuL                  `noun`    {- taSad~ur -}         [ "chairmanship", "preeminence" ]
                              `plural`     TaFaCCuL |< At,

    IFtiCAL                   `noun`    {- AiSoTidAr -}        [ "issuance" ]
                              `plural`     IFtiCAL |< At,

    IstiFCAL                  `noun`    {- AisotiSodAr -}      [ "issuing" ]
                              `plural`     IstiFCAL |< At,

    FACiL                     `adj`     {- SAdir -}            [ "issued", "published", "exports" ]
                              `plural`     FACiL |< At,

    MaFCUL                    `noun`    {- maSoduwr -}         [ "tubercular" ],

    MuFaCCiL                  `adj`     {- muSad~ir -}         [ "exporter", "exporting" ],

    MuFaCCaL                  `adj`     {- muSad~ar -}         [ "exported", "exports" ]
                              `plural`     MuFaCCaL |< At,

    MuFACaL                   `noun`    {- muSAdar -}          [ "confiscated", "requisitioned" ],

    MutaFaCCiL                `adj`     {- mutaSad~ir -}       [ "presiding", "leading", "heading" ] ]


cluster_27  = listing "Lexicon's properties"


 |> ".s d y" <| [

    FaCI                      `verb`    {- Sadiy-a -}          [ unwords [ "be", "thirsty" ] ]
                              `imperf`     FCY,

    FaCCY                     `verb`    {- Sad~aY -}           [ "echo" ],

    HaFCY                     `verb`    {- OaSodaY -}          [ "echo", "reverberate" ],

    TaFaCCY                   `verb`    {- taSad~aY -}         [ "resist", "confront" ],

    FaCY                      `noun`    {- SadaY -}            [ "echo" ]
                              `plural`     FaCA
                              `plural`     HaFCA'
                           {- `others`  [ ".sadaN FW-WaBi" ] -},

    TaFCI |< aT               `noun`    {- taSodiyap -}        [ unwords [ "hand", "clapping" ] ],

    MuFACY |< aT              `noun`    {- muSAdAp -}          [ unwords [ "echolalia", "(", "compulsive", "repetition", "of", "someone", "else's", "words", ")" ] ],

    TaFaCCI                   `noun`    {- taSad~iy -}         [ "resistance", "confrontation" ]
                              `plural`     TaFaCCI |< At ]

 |> ".s f '" <| [

    FaCAL                     `noun`    {- SafA' -}            [ "Safaa", "Safa" ],

    FaCAL                     `noun`    {- SafA' -}            [ "purity", "clarity", "sincerity" ],

    IFtiCAL                   `noun`    {- AiSoTifA' -}        [ "selection" ]
                              `plural`     IFtiCAL |< At,

    IstiFCAL                  `noun`    {- AisotiSofA' -}      [ "sequestration", "confiscation" ]
                              `plural`     IstiFCAL |< At ]


cluster_28  = listing "Lexicon's properties"


 |> ".s f .h" <| [

    FaCaL                     `verb`    {- SafaH-a -}          [ "pardon", "flatten" ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- Saf~aH -}           [ "foliate" ],

    FACaL                     `verb`    {- SAfaH -}            [ unwords [ "shake", "hands", "with" ] ],

    TaFaCCaL                  `verb`    {- taSaf~aH -}         [ unwords [ "page", "through" ], "examine", "read" ],

    TaFACaL                   `verb`    {- taSAfaH -}          [ unwords [ "shake", "hands" ] ],

    IstaFCaL                  `verb`    {- AisotaSofaH -}      [ "apologize" ],

    FaCL                      `noun`    {- SafoH -}            [ "pardon" ],

    FaCUL                     `adj`     {- SafuwH -}           [ "clement", "forgiving" ],

    FaCL |< aT                `noun`    {- SafoHap -}          [ "page", "leaf" ]
                              `plural`     FaCaL |< At,

    FaCIL                     `noun`    {- SafiyH -}           [ unwords [ "tin", "plate" ] ],

    FaCIL |< aT               `noun`    {- SafiyHap -}         [ "plate", "sheet" ]
                              `plural`     FaCA'iL,

    FuCCAL                    `noun`    {- Suf~AH -}           [ "tinplate", "flagstone" ]
                              `plural`     FuCCAL |< At
                              `plural`     FaCACIL,

    TaFCIL                    `noun`    {- taSofiyH -}         [ "plating" ]
                              `plural`     TaFCIL |< At,

    FaCCAL                    `noun`    {- Saf~AH -}           [ "tinsmith" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ "tinsmith" ]
                              `plural`     FaCCAL |< At,

    TaFaCCuL                  `noun`    {- taSaf~uH -}         [ "examination", "perusal" ]
                              `plural`     TaFaCCuL |< At,

    MuFaCCaL                  `noun`    {- muSaf~aH -}         [ "ironclad", "armored", "plated" ],

    MuFaCCaL |< aT            `noun`    {- muSaf~aHap -}       [ unwords [ "armored", "vehicle" ] ],

    MuFACaL |< aT             `noun`    {- muSAfaHap -}        [ "handshake" ] ]

 |> ".s f .s f" <| [

    KaRDAS |< aT              `noun`    {- SafoSAfap -}        [ "willow" ],

    KaRDaS                    `noun`    {- SafoSaf -}          [ "barren", "desolate" ] ]


cluster_29  = listing "Lexicon's properties"


 |> ".s f .t" <| [

    FaCLA' |< Iy              `adj`     {- SafoTAwiy~ -}       [ "Saftawi" ],

    FaCLA' |< Iy              `adj`     {- SafoTAwiy~ -}       [ unwords [ "from", "/", "of", "Safta" ] ] ]

 |> ".s f `" <| [

    FaCaL                     `verb`    {- SafaE-a -}          [ "slap" ]
                              `imperf`     FCaL,

    TaFACaL                   `verb`    {- taSAfaE -}          [ unwords [ "slap", "one", "another" ] ],

    FaCL |< aT                `noun`    {- SafoEap -}          [ "slap", "blow" ]
                              `plural`     FaCaL |< At,

    FaCCAL                    `noun`    {- Saf~AE -}           [ "buffoon" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ "buffoon" ]
                              `plural`     FaCCAL |< At ]

 |> ".s f d" <| [

    FaCCaL                    `verb`    {- Saf~ad -}           [ "shackle", "handcuff" ],

    HaFCaL                    `verb`    {- OaSofad -}          [ "shackle", "handcuff" ],

    FaCaL                     `noun`    {- Safad -}            [ "Safad" ],

    FaCaL                     `noun`    {- Safad -}            [ "handcuff", "shackle" ]
                              `plural`     HaFCAL,

    FiCAL                     `noun`    {- SifAd -}            [ "handcuffs", "shackles" ]
                              `plural`     HaFCAL,

    TaFCIL                    `noun`    {- taSofiyd -}         [ "handcuffing", "shackling" ]
                              `plural`     TaFCIL |< At,

    HiFCAL                    `noun`    {- IiSofAd -}          [ "handcuffing", "shackling" ]
                              `plural`     HiFCAL |< At ]


cluster_30  = listing "Lexicon's properties"


 |> ".s f f" <| [

    FaCL                      `verb`    {- Saf~-u -}           [ "arrange", "classify" ]
                              `imperf`     FuCL
                              `pfirst`     FaCaL,

    FaCCaL                    `verb`    {- Saf~af -}           [ "arrange" ],

    TaFACL                    `verb`    {- taSAf~ -}           [ unwords [ "be", "arranged" ], unwords [ "get", "in", "line" ] ],

    IFtaCL                    `verb`    {- AiSoTaf~ -}         [ unwords [ "get", "in", "line" ], unwords [ "stand", "in", "formation" ] ],

    FaCL                      `noun`    {- Saf~ -}             [ "line", "row", "class" ]
                              `plural`     FuCUL,

    FuCUL                     `noun`    {- Sufuwf -}           [ unwords [ "non", "-", "commissioned", "officers" ] ],

    FuCL |< aT                `noun`    {- Suf~ap -}           [ "ledge", "bench" ],

    FaCCAL                    `noun`    {- Saf~Af -}           [ "typesetter" ],

    MaFaCL                    `noun`    {- maSaf~ -}           [ "rank" ],

    MaFaCL                    `noun`    {- maSaf~ -}           [ unwords [ "battle", "line" ], "position" ]
                              `plural`     MaFACL,

    TaFCIL                    `noun`    {- taSofiyf -}         [ "arranging", "hairdressing" ]
                              `plural`     TaFCIL |< At,

    MaFCUL                    `adj`     {- maSofuwf -}         [ "arranged", "combed" ],

    MaFCUL |< aT              `noun`    {- maSofuwfap -}       [ "matrix" ],

    FaL |< aN                 `noun`    {- SafAF -}            [ "stones", "rocks" ]
                              `plural`     FaCA ]

 |> ".s f n" <| [

    FaCaL                     `verb`    {- Safan-i -}          [ "ponder" ]
                              `imperf`     FCiL,

    FaCaL                     `noun`    {- Safan -}            [ "scrotum", "scrota" ]
                              `plural`     HaFCAL,

    FaCIL                     `noun`    {- Safiyn -}           [ "savin" ] ]


cluster_31  = listing "Lexicon's properties"


 |> ".s f q" <| [

    FaCaL                     `verb`    {- Safaq-i -}          [ "slam", "clap" ]
                              `imperf`     FCiL,

    FaCaL                     `verb`    {- Safaq-u -}          [ "transfuse" ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Saf~aq -}           [ "applaud", "clap" ],

    HaFCaL                    `verb`    {- OaSofaq -}          [ "transfuse" ],

    FaCL                      `noun`    {- Safoq -}            [ "slamming", "clapping", "transfusion" ],

    FaCL |< aT                `noun`    {- Safoqap -}          [ "deal", "transaction" ]
                              `plural`     FaCaL |< At,

    FiCAL                     `noun`    {- SifAq -}            [ "dermis" ],

    FaCIL                     `adj`     {- Safiyq -}           [ "thick", "impudent" ],

    FaCAL |< aT               `noun`    {- SafAqap -}          [ "insolence" ],

    TaFCIL                    `noun`    {- taSofiyq -}         [ "applause", "clapping" ]
                              `plural`     TaFCIL |< At ]

 |> ".s f q s" <| [

    KaRADiS                   `noun`    {- SafAqis -}          [ "Sfax" ] ]


cluster_32  = listing "Lexicon's properties"


 |> ".s f r" <| [

    FaCaL                     `verb`    {- Safar-i -}          [ "whistle" ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- Saf~ar -}           [ "whistle" ],

    FaCCaL                    `verb`    {- Saf~ar -}           [ unwords [ "dye", "yellow" ] ],

    HaFCaL                    `verb`    {- OaSofar -}          [ "empty" ],

    HaFCaL                    `verb`    {- OaSofar -}          [ unwords [ "be", "empty", "-", "handed" ] ],

    IFCaLL                    `verb`    {- AiSofar~ -}         [ unwords [ "turn", "yellow" ], unwords [ "become", "pale" ] ],

    FuCayL                    `noun`    {- Sufayor -}          [ "Sfeir" ],

    FaCL |< aT                `noun`    {- Saforap -}          [ "whistle" ]
                              `plural`     FaCaL |< At,

    FaCIL                     `noun`    {- Safiyr -}           [ "whistling", "whistles", "sapphire" ],

    FaCCAL |< aT              `noun`    {- Saf~Arap -}         [ "whistle", "siren" ]
                              `plural`     FaCACIL,

    TaFCIL                    `noun`    {- taSofiyr -}         [ "whistling" ]
                              `plural`     TaFCIL |< At,

    TaFCIL                    `noun`    {- taSofiyr -}         [ "yellowing" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< aT              `noun`    {- taSofiyrap -}       [ "whistling", "whistles" ],

    FACiL |< aT               `noun`    {- SAfirap -}          [ "whistle" ],

    MuFaCCiL                  `noun`    {- muSaf~ir -}         [ "whistling", "whistler" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "whistling", "whistler" ]
                              `plural`     MuFaCCiL |< At,

    FaCaL                     `noun`    {- Safar -}            [ "jaundice" ],

    FaCaL                     `noun`    {- Safar -}            [ unwords [ "Safar", "(", "month", ")" ] ],

    FuCL |< aT                `noun`    {- Suforap -}          [ "yellowness", "pallor" ],

    FaCAL                     `noun`    {- SafAr -}            [ "yellowness", "pallor" ],

    HaFCaL                    `noun`    {- OaSofar -}          [ "yellow" ]
                              `plural`     FuCL
                              `plural`     FaCLA',

    HaFCaL                    `noun`    {- OaSofar -}          [ "empty" ],

    FaCLA' |< Iy              `adj`     {- SaforAwiy~ -}       [ "yellow" ],

    MiFCAL                    `noun`    {- miSofAr -}          [ unwords [ "deep", "yellow" ] ],

    IFCiLAL                   `noun`    {- AiSofirAr -}        [ "yellowing", "pallor" ]
                              `plural`     IFCiLAL |< At,

    MuFCaLL                   `adj`     {- muSofar~ -}         [ "yellow", "pale" ],

    FiCL                      `noun`    {- Sifor -}            [ "zero" ]
                              `plural`     FiCL |< At,

    FiCL |< Iy                `adj`     {- Siforiy~ -}         [ "null", "negative" ],

    MuFCiL                    `noun`    {- muSofir -}          [ unwords [ "empty", "-", "handed" ] ]
                              `plural`     MuFCiL |< Un,

    MuFCiL |< aT              `noun`    {- DERIVED -}          [ unwords [ "empty", "-", "handed" ] ]
                              `plural`     MuFCiL |< At ]


cluster_33  = listing "Lexicon's properties"


 |> ".s f w" <| [

    FaCA                      `verb`    {- SafA-u -}           [ unwords [ "be", "pure" ] ]
                              `imperf`     FCU,

    FaCCY                     `verb`    {- Saf~aY -}           [ "clarify", "purify", "liquidate" ],

    FACY                      `verb`    {- SAfaY -}            [ unwords [ "be", "sincere", "with" ], unwords [ "deal", "honestly", "with" ], unwords [ "be", "treated", "with", "sincerity" ], unwords [ "be", "dealt", "with", "honestly" ] ],

    HaFCY                     `verb`    {- OaSofaY -}          [ "choose", "prefer", unwords [ "be", "chosen" ], unwords [ "be", "preferred" ] ],

    TaFACY                    `verb`    {- taSAfaY -}          [ unwords [ "be", "sincere", "with", "each", "other" ], unwords [ "deal", "honestly", "with", "each", "other" ] ],

    IFtaCY                    `verb`    {- AiSoTafaY -}        [ "choose", "select" ],

    IstaFCY                   `verb`    {- AisotaSofaY -}      [ "confiscate", "sequester" ],

    FaC |< aN                 `noun`    {- SafAF -}            [ "stones", "rocks" ]
                              `plural`     FaCA,

    FaCY |< aT                `noun`    {- SafAp -}            [ "stone", "rock" ]
                              `plural`     FaCaL |< At,

    FaCL                      `noun`    {- Safow -}            [ "clarity", "purity" ],

    FaCL |< aT                `noun`    {- Safowap -}          [ "choicest", "elite", "selection", "best" ],

    FiCL |< aT                `noun`    {- Sifowap -}          [ unwords [ "sincere", "friend" ], unwords [ "bosom", "friend" ] ],

    FaCLAn                    `noun`    {- SafowAn -}          [ "stones", "rocks" ],

    FaCA'                     `noun`    {- SafA' -}            [ "Safaa", "Safa" ],

    FaCA'                     `noun`    {- SafA' -}            [ "purity", "clarity", "sincerity" ],

    MaFCY                     `noun`    {- maSofaY -}          [ "refinery" ],

    MiFCY                     `noun`    {- miSofaY -}          [ "sieve" ],

    MiFCY |< aT               `noun`    {- miSofAp -}          [ "refinery", unwords [ "purification", "plant" ] ]
                              `plural`     MaFACI,

    MiFCY |< aT               `noun`    {- miSofAp -}          [ "filter", "strainer" ]
                              `plural`     MaFACI,

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "settlement" ],

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "clearing", "liquidation", unwords [ "elimination", "series", "(", "sports", ")" ] ],

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "purification", "filtration" ],

    MuFACY |< aT              `noun`    {- muSAfAp -}          [ unwords [ "good", "-", "will" ], "cordiality" ],

    TaFACI                    `noun`    {- taSAfiy -}          [ unwords [ "peaceful", "settlement" ], "compromise" ]
                              `plural`     TaFACI |< At,

    IFtiCA'                   `noun`    {- AiSoTifA' -}        [ "selection" ]
                              `plural`     IFtiCA' |< At,

    IstiFCA'                  `noun`    {- AisotiSofA' -}      [ "sequestration", "confiscation" ]
                              `plural`     IstiFCA' |< At,

    FACI                      `noun`    {- SAfiy -}            [ "net" ],

    FACI                      `adj`     {- SAfiy -}            [ "pure", "clear", "sincere" ],

    MuFaCCI                   `noun`    {- muSaf~iy -}         [ unwords [ "receiver", "in", "equity" ], unwords [ "clarifying", "agent" ] ],

    MuFaCCY                   `adj`     {- muSaf~aY -}         [ "purified", "clear" ],

    MuFACaL |< At             `noun`    {- muSAfawAt -}        [ "goodwill", "cordiality" ]
                              `plural`     MuFACaL |< At,

    MuFtaCY                   `noun`    {- muSoTafaY -}        [ "Mustafa", "Moustapha" ],

    MuFtaCY                   `adj`     {- muSoTafaY -}        [ "chosen", "selected" ]
                              `plural`     MuFtaCY |< At,

    MuFtaCY                   `noun`    {- muSoTafaY -}        [ unwords [ "chosen", "one", "(", "Muhammad", ")" ] ] ]


cluster_34  = listing "Lexicon's properties"


 |> ".s f y" <| [

    FaCIL                     `adj`     {- Safiy~ -}           [ "pure", "clear", "sincere" ]
                              `plural`     HaFCiLA',

    FaCIL |< aT               `noun`    {- Safiy~ap -}         [ unwords [ "lion's", "share" ] ]
                              `plural`     FaCALY,

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "settlement" ],

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "clearing", "liquidation", unwords [ "elimination", "series", "(", "sports", ")" ] ],

    TaFCI |< aT               `noun`    {- taSofiyap -}        [ "purification", "filtration" ],

    TaFACI                    `noun`    {- taSAfiy -}          [ unwords [ "peaceful", "settlement" ], "compromise" ]
                              `plural`     TaFACI |< At,

    FACI                      `noun`    {- SAfiy -}            [ "net" ],

    FACI                      `adj`     {- SAfiy -}            [ "pure", "clear", "sincere" ],

    MuFaCCI                   `noun`    {- muSaf~iy -}         [ unwords [ "receiver", "in", "equity" ], unwords [ "clarifying", "agent" ] ] ]

 |> ".s h b" <| [

    FaCiL                     `verb`    {- Sahib-a -}          [ unwords [ "become", "reddish" ] ]
                              `imperf`     FCaL,

    IFCaLL                    `verb`    {- AiSohab~ -}         [ unwords [ "become", "reddish" ] ],

    IFCALL                    `verb`    {- AiSohAb~ -}         [ unwords [ "become", "reddish" ] ],

    FuCL |< aT                `noun`    {- Suhobap -}          [ "redness", "gingery" ],

    HaFCaL                    `noun`    {- OaSohab -}          [ "reddish", "ginger" ]
                              `plural`     FuCL
                              `plural`     FaCLA' ]

 |> ".s h d" <| [

    FaCaL                     `verb`    {- Sahad-a -}          [ "scorch", "burn" ]
                              `imperf`     FCaL,

    FaCL                      `noun`    {- Sahod -}            [ "heat", "blaze" ],

    FaCIL                     `noun`    {- Sahiyd -}           [ "heat", "blaze" ],

    FuCUL                     `noun`    {- Suhuwd -}           [ "heat", "blaze" ] ]

 |> ".s h l" <| [

    FaCaL                     `verb`    {- Sahal-a -}          [ "neigh" ]
                              `imperf`     FCaL,

    FaCIL                     `noun`    {- Sahiyl -}           [ "neighing" ] ]


cluster_35  = listing "Lexicon's properties"


 |> ".s h r" <| [

    FaCaL                     `verb`    {- Sahar-a -}          [ "forge", "fuse", "melt" ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- SAhar -}            [ unwords [ "become", "related", "by", "marriage" ] ],

    TaFACaL                   `verb`    {- taSAhar -}          [ unwords [ "become", "related", "by", "marriage" ] ],

    InFaCaL                   `verb`    {- AinoSahar -}        [ unwords [ "be", "forged" ], unwords [ "be", "fused" ], unwords [ "be", "melted" ] ],

    FaCL                      `noun`    {- Sahor -}            [ "melting", "forging", "fusing" ],

    FiCL                      `noun`    {- Sihor -}            [ unwords [ "family", "tie" ] ]
                              `plural`     HaFCAL,

    FiCL                      `noun`    {- Sihor -}            [ unwords [ "brother", "-", "in", "-", "law" ], unwords [ "son", "-", "in", "-", "law" ], unwords [ "brothers", "-", "in", "-", "law" ], unwords [ "sons", "-", "in", "-", "law" ] ]
                              `plural`     HaFCAL,

    FiCL |< aT                `noun`    {- Sihorap -}          [ unwords [ "sister", "-", "in", "-", "law" ] ],

    FaCIL                     `adj`     {- Sahiyr -}           [ "molten", unwords [ "in", "fusion" ] ],

    FaCIL |< aT               `noun`    {- Sahiyrap -}         [ "fusible", "fuse" ],

    FuCAL |< aT               `noun`    {- SuhArap -}          [ "casting", "magma" ],

    FaCUL                     `noun`    {- Sahuwr -}           [ "fusible" ],

    MaFCUL                    `adj`     {- maSohuwr -}         [ "melted", "burned" ],

    MaFCaL                    `noun`    {- maSohar -}          [ "foundry", "furnace" ]
                              `plural`     MaFACiL,

    MiFCaL                    `noun`    {- miSohar -}          [ "fuse" ]
                              `plural`     MiFCaL |< At,

    MuFACaL |< aT             `noun`    {- muSAharap -}        [ "affinity", unwords [ "family", "tie" ] ],

    InFiCAL                   `noun`    {- AinoSihAr -}        [ unwords [ "melting", "process" ], "fusion" ]
                              `plural`     InFiCAL |< At,

    MunFaCaL                  `noun`    {- munoSahar -}        [ "fuse" ]
                              `plural`     MunFaCaL |< At ]

 |> ".s h r ^g" <| [

    KiRDIS                    `noun`    {- Sihoriyj -}         [ "cistern", "reservoir" ]
                              `plural`     KaRADIS ]

 |> ".s h w" <| [

    FaCL |< aT                `noun`    {- Sahowap -}          [ "rump", "back" ]
                              `plural`     HaFCA'
                              `plural`     FaCaL |< At ]


cluster_36  = listing "Lexicon's properties"


 |> ".s h y n" <| [

    KaRDUS                    `noun`    {- Sahoyuwn -}         [ "Zion" ],

    KaRDUS                    `noun`    {- Sahoyuwn -}         [ "Sahyoun" ],

    KaRDUS |< Iy              `adj`     {- Sahoyuwniy~ -}      [ "Zionist" ]
                              `plural`     KaRADiS |< aT
                              `plural`     KaRDUS |< Iy |< Un,

    KaRDUS |< Iy |< aT        `noun`    {- Sahoyuwniy~ap -}    [ "Zionism" ] ]

 |> ".s k k" <| [

    FaCL                      `verb`    {- Sak~-u -}           [ "strike", "ring" ]
                              `imperf`     FuCL
                              `pfirst`     FaCaL,

    IFtaCL                    `verb`    {- AiSoTak~ -}         [ "tremble", "shake" ],

    FaCL                      `noun`    {- Sak~ -}             [ "deed", "document" ]
                              `plural`     FiCAL
                              `plural`     FuCUL,

    IFtiCAL                   `noun`    {- AiSoTikAk -}        [ "trembling", "shaking" ]
                              `plural`     IFtiCAL |< At,

    MuFtaCL                   `adj`     {- muSoTak~ -}         [ "trembling", "shaking" ] ]

 |> ".s l '" <| [

    IFtiCAL                   `noun`    {- AiSoTilA' -}        [ "burning" ]
                              `plural`     IFtiCAL |< At ]


cluster_37  = listing "Lexicon's properties"


 |> ".s l .h" <| [

    FaCuL                     `verb`    {- SaluH-u -}          [ unwords [ "be", "suitable" ], unwords [ "be", "fitting" ], unwords [ "be", "valid" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sal~aH -}           [ unwords [ "put", "in", "order" ], "repair" ],

    FACaL                     `verb`    {- SAlaH -}            [ "reconcile", "compromise" ],

    HaFCaL                    `verb`    {- OaSolaH -}          [ unwords [ "put", "in", "order" ], "repair", unwords [ "be", "put", "in", "order" ] ],

    TaFACaL                   `verb`    {- taSAlaH -}          [ unwords [ "be", "reconciled" ] ],

    InFaCaL                   `verb`    {- AinoSalaH -}        [ unwords [ "be", "arranged" ], unwords [ "be", "put", "in", "order" ] ],

    IFtaCaL                   `verb`    {- AiSoTalaH -}        [ "concur", "agree" ],

    IstaFCaL                  `verb`    {- AisotaSolaH -}      [ unwords [ "deem", "suitable" ], unwords [ "put", "in", "order" ] ],

    FuCL                      `noun`    {- SuloH -}            [ "peace", "reconciliation" ],

    FuCL |< Iy                `adj`     {- SuloHiy~ -}         [ "conciliatory" ],

    FaCAL                     `noun`    {- SalAH -}            [ "Salah" ],

    FaCAL                     `noun`    {- SalAH -}            [ "proper", unwords [ "good", "condition" ] ],

    FaCAL |< Iy |< aT         `noun`    {- SalAHiy~ap -}       [ "practicability", "viability", "competence" ],

    FuCUL |< Iy |< aT         `noun`    {- SuluwHiy~ap -}      [ "practicability", "viability", "competence" ],

    HaFCaL                    `noun`    {- OaSolaH -}          [ unwords [ "more", "/", "most", "convenient" ], unwords [ "better", "/", "best" ] ],

    MaFCaL                    `noun`    {- maSolaH -}          [ "garage" ],

    MaFCaL |< aT              `noun`    {- maSolaHap -}        [ "interest", "advantage", "agency" ]
                              `plural`     MaFACiL,

    MaFCaL |< Iy              `adj`     {- maSolaHiy~ -}       [ "administrational", "governmental" ],

    TaFCIL                    `noun`    {- taSoliyH -}         [ "repair" ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- muSAlaHap -}        [ "conciliation", "compromise" ],

    HiFCAL                    `noun`    {- IiSolAH -}          [ "reform", "restoration" ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< At              `noun`    {- IiSolAHAt -}        [ "corrections", "amendments" ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< Iy              `adj`     {- IiSolAHiy~ -}       [ "reformative", "corrective" ],

    HiFCAL |< Iy              `adj`     {- IiSolAHiy~ -}       [ "reformer", "revisionist" ],

    HiFCAL |< Iy |< aT        `noun`    {- IiSolAHiy~ap -}     [ "reformism", "rehabilitation" ],

    TaFACuL                   `noun`    {- taSAluH -}          [ "reconciliation", "settlement" ]
                              `plural`     TaFACuL |< At,

    IFtiCAL                   `noun`    {- AiSoTilAH -}        [ "convention", unwords [ "technical", "term" ] ]
                              `plural`     IFtiCAL |< At,

    IFtiCAL |< Iy             `adj`     {- AiSoTilAHiy~ -}     [ "conventional", "technical" ],

    IstiFCAL                  `noun`    {- AisotiSolAH -}      [ "reclamation", "cultivation" ]
                              `plural`     IstiFCAL |< At,

    FACiL                     `noun`    {- SAliH -}            [ "Salih", "Saleh" ],

    FACiL                     `noun`    {- SAliH -}            [ "suitable", "applicable" ],

    FACiL                     `noun`    {- SAliH -}            [ "advantage", "interest" ]
                              `plural`     FawACiL,

    FawACiL |< aT             `noun`    {- SawAliHap -}        [ "Sawalheh" ],

    FACiL                     `adj`     {- SAliH -}            [ "pious", "upright" ]
                              `plural`     FACiL |< Un
                              `plural`     FuCaLA'
                              `plural`     FuCCAL,

    FACiL |< At               `noun`    {- SAliHAt -}          [ unwords [ "good", "deeds" ] ]
                              `plural`     FACiL |< At,

    MuFaCCiL                  `noun`    {- muSal~iH -}         [ "repairman" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "repairman" ]
                              `plural`     MuFaCCiL |< At,

    MuFACiL                   `noun`    {- muSAliH -}          [ "peacemaker", "conciliator" ]
                              `plural`     MuFACiL |< Un
                              `plural`     MuFCiL |< Un,

    MuFACiL |< aT             `noun`    {- DERIVED -}          [ "peacemaker", "conciliator" ]
                              `plural`     MuFACiL |< At,

    MutaFACiL                 `adj`     {- mutaSAliH -}        [ unwords [ "at", "peace" ], "reconciled" ],

    MuFtaCaL                  `noun`    {- muSoTalaH -}        [ unwords [ "technical", "term" ], "terminology" ]
                              `plural`     MuFtaCaL |< At ]


cluster_38  = listing "Lexicon's properties"


 |> ".s l .s" <| [

    FaCL |< aT                `noun`    {- SaloSap -}          [ "sauce", "salsa" ] ]

 |> ".s l .s l" <| [

    KaRDaS                    `verb`    {- SaloSal -}          [ "jingle", "rattle", "clink" ],

    TaKaRDaS                  `verb`    {- taSaloSal -}        [ "jingle", "rattle", "clink" ],

    KaRDaS |< aT              `noun`    {- SaloSalap -}        [ "jingle", "rattle", "clink" ],

    KaRDAS                    `noun`    {- SaloSAl -}          [ unwords [ "dry", "clay" ] ],

    MuKaRDiS                  `adj`     {- muSaloSil -}        [ "noisy", "resounding" ] ]

 |> ".s l .t .h" <| [

    MuKaRDaS                  `noun`    {- muSaloTaH -}        [ "shallow", "shoal", "flat" ] ]

 |> ".s l ^g" <| [

    FuCCaL                    `noun`    {- Sul~aj -}           [ "cocoon" ]
                              `plural`     FuCCaL |< At ]

 |> ".s l _h" <| [

    HaFCaL                    `adj`     {- OaSolax -}          [ "deaf" ]
                              `plural`     FaCLA' ]

 |> ".s l `" <| [

    FaCiL                     `verb`    {- SaliE-a -}          [ unwords [ "be", "bald" ] ]
                              `imperf`     FCaL,

    FaCaL                     `noun`    {- SalaE -}            [ "baldness" ],

    FaCL |< aT                `noun`    {- SaloEap -}          [ unwords [ "bald", "spot" ], unwords [ "bald", "head" ] ],

    HaFCaL                    `noun`    {- OaSolaE -}          [ "bald" ]
                              `plural`     FuCLAn
                              `plural`     FaCLA'
                              `plural`     FuCL ]


cluster_39  = listing "Lexicon's properties"


 |> ".s l b" <| [

    FaCuL                     `verb`    {- Salub-u -}          [ "harden", "stiffen" ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- Salab-i -}          [ "crucify" ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- Sal~ab -}           [ "harden", "stiffen", "crucify" ],

    TaFaCCaL                  `verb`    {- taSal~ab -}         [ "harden", "stiffen" ],

    TaFACaL                   `verb`    {- taSAlab -}          [ unwords [ "be", "crucified" ] ],

    FuCL                      `noun`    {- Sulob -}            [ "heart", "core", "crux", "center", "essence" ],

    FuCL                      `adj`     {- Sulob -}            [ "hard", "solid", "robust" ],

    FaCAL |< aT               `noun`    {- SalAbap -}          [ "firmness", "consistency" ],

    FuCL                      `noun`    {- Sulob -}            [ unwords [ "spinal", "column" ] ]
                              `plural`     HaFCAL,

    FuCL |< Iy                `adj`     {- Sulobiy~ -}         [ "dorsal", "lumbar" ],

    FACiL                     `noun`    {- SAlib -}            [ "keel" ]
                              `plural`     FawACiL,

    TaFaCCuL                  `noun`    {- taSal~ub -}         [ "hardness", "hardening", "sclerosis" ]
                              `plural`     TaFaCCuL |< At,

    TaFaCCuL |< Iy            `adj`     {- taSal~ubiy~ -}      [ "cataleptic", "sclerous" ],

    MutaFaCCiL                `adj`     {- mutaSal~ib -}       [ "inflexible" ],

    FaCL                      `noun`    {- Salob -}            [ "crucifixion" ],

    FaCIL                     `noun`    {- Saliyb -}           [ "cross", "crucifix" ]
                              `plural`     FuCLAn,

    FaCIL |< Iy               `adj`     {- Saliybiy~ -}        [ "crusade", unwords [ "cross", "-", "like" ] ],

    FaCIL |< Iy |< aT         `noun`    {- Saliybiy~ap -}      [ "crusade", "campaign" ],

    MaFCUL                    `adj`     {- maSoluwb -}         [ "crucified" ],

    TaFCIL                    `noun`    {- taSoliyb -}         [ "crossing", "intersecting" ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL |< aT            `noun`    {- muSal~abap -}       [ "crosspiece", "crossbar" ],

    TaFACuL                   `noun`    {- taSAlub -}          [ "crossing", "intersecting" ]
                              `plural`     TaFACuL |< At ]


cluster_40  = listing "Lexicon's properties"


 |> ".s l b t" <| [

    KaRDUS                    `noun`    {- Salobuwt -}         [ "crucifix" ] ]

 |> ".s l d" <| [

    FaCaL                     `verb`    {- Salad-u -}          [ unwords [ "be", "solid" ], unwords [ "be", "compact" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sal~ad -}           [ "harden" ],

    HaFCaL                    `verb`    {- OaSolad -}          [ unwords [ "be", "solid" ], unwords [ "be", "compact" ] ],

    FaCL                      `noun`    {- Salod -}            [ "firm", "solid" ]
                              `plural`     HaFCAL,

    FaCAL |< aT               `noun`    {- SalAdap -}          [ "solidity", "hardness", "resistance" ],

    FuCUL |< aT               `noun`    {- Suluwdap -}         [ "solidity", "hardness", "resistance" ],

    TaFCIL                    `noun`    {- taSoliyd -}         [ "tempering" ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL                  `adj`     {- muSal~ad -}         [ "tempered" ] ]

 |> ".s l f" <| [

    FaCiL                     `verb`    {- Salif-a -}          [ "brag" ]
                              `imperf`     FCaL,

    TaFaCCaL                  `verb`    {- taSal~af -}         [ "brag" ],

    FaCaL                     `noun`    {- Salaf -}            [ "bragging" ],

    FaCiL                     `adj`     {- Salif -}            [ "boastful" ]
                              `plural`     FuCaLA',

    TaFaCCuL                  `noun`    {- taSal~uf -}         [ "boasting", "conceit" ]
                              `plural`     TaFaCCuL |< At,

    MutaFaCCiL                `adj`     {- mutaSal~if -}       [ "boastful", "conceited" ] ]

 |> ".s l l" <| [

    FaCIL                     `noun`    {- Saliyl -}           [ "clanging" ],

    HaFCAL                    `noun`    {- OaSolAl -}          [ "vipers" ]
                              `plural`     FiCAL,

    FaL |< At                 `noun`    {- SalAt -}            [ "Salat" ] ]

 |> ".s l n" <| [

    FACUL                     `noun`    {- SAluwn -}           [ "salon", "parlor" ]
                              `plural`     FaCUL |< At
                              `plural`     FACUL |< At ]


cluster_41  = listing "Lexicon's properties"


 |> ".s l t" <| [

    FaCuL                     `verb`    {- Salut-u -}          [ unwords [ "be", "glossy" ], unwords [ "be", "shining" ] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- OaSolat -}          [ "unsheathe" ],

    FuCUL |< aT               `noun`    {- Suluwtap -}         [ "glossy", "shining" ] ]

 |> ".s l w" <| [

    FaCCY                     `verb`    {- Sal~aY -}           [ "pray", "worship" ],

    FaCY |< aT                `noun`    {- SalAp -}            [ "prayer", "salat" ]
                              `plural`     FaCaL |< At,

    FaC |< At                 `noun`    {- SalAt -}            [ "Salat" ],

    MuFaCCI                   `noun`    {- muSal~iy -}         [ "praying", "worshipping", unwords [ "people", "praying" ], unwords [ "people", "worshipping" ] ]
                              `plural`     MuFaCCI |< At,

    MuFaCCY                   `noun`    {- muSal~aY -}         [ unwords [ "place", "of", "prayer" ], "oratory", unwords [ "places", "of", "prayer" ] ]
                              `plural`     MuFaCCY |< At ]

 |> ".s l y" <| [

    MuFaCCI                   `noun`    {- muSal~iy -}         [ "praying", "worshipping", unwords [ "people", "praying" ], unwords [ "people", "worshipping" ] ]
                              `plural`     MuFaCCI |< At,

    FaCY                      `verb`    {- SalaY-i -}          [ "roast" ]
                              `imperf`     FCI,

    FaCI                      `verb`    {- Saliy-a -}          [ unwords [ "get", "burned" ] ]
                              `imperf`     FCY,

    HaFCY                     `verb`    {- OaSolaY -}          [ "burn" ],

    TaFaCCY                   `verb`    {- taSal~aY -}         [ unwords [ "warm", "up" ] ],

    IFtaCY                    `verb`    {- AiSoTalaY -}        [ unwords [ "get", "burned" ] ],

    IFtiCA'                   `noun`    {- AiSoTilA' -}        [ "burning" ]
                              `plural`     IFtiCA' |< At,

    MuFtaCY                   `noun`    {- muSoTalaY -}        [ "fireplace", "foyer" ] ]


cluster_42  = listing "Lexicon's properties"


 |> ".s m .g" <| [

    FaCCaL                    `verb`    {- Sam~ag -}           [ "paste", "glue" ],

    HaFCaL                    `verb`    {- OaSomag -}          [ "paste", "glue" ],

    FaCL                      `noun`    {- Samog -}            [ "gum", "resin" ]
                              `plural`     FuCUL,

    FaCL |< Iy                `adj`     {- Samogiy~ -}         [ unwords [ "gum", "-", "like" ], "resinous" ],

    TaFCIL                    `noun`    {- taSomiyg -}         [ "gumming", "resinification" ]
                              `plural`     TaFCIL |< At ]

 |> ".s m .s m" <| [

    KaRDaS                    `verb`    {- SamoSam -}          [ "persist" ],

    KaRDaS |< aT              `noun`    {- SamoSamap -}        [ "persistence" ],

    MuKaRDiS                  `adj`     {- muSamoSim -}        [ "persisting", "persistent" ] ]

 |> ".s m _h" <| [

    FiCAL                     `noun`    {- SimAx -}            [ unwords [ "ear", "canal" ], "eardrum" ]
                              `plural`     HaFCiL |< aT ]

 |> ".s m d" <| [

    FaCaL                     `verb`    {- Samad-u -}          [ "resist", unwords [ "be", "steadfast" ] ]
                              `imperf`     FCuL,

    FaCL                      `noun`    {- Samod -}            [ "blocking", "plugging" ],

    FaCaL                     `noun`    {- Samad -}            [ unwords [ "everlasting", "(", "God", ")" ] ],

    FaCaLAn |< Iy             `adj`     {- SamadAniy~ -}       [ unwords [ "everlasting", "(", "God", ")" ] ],

    FuCUL                     `noun`    {- Sumuwd -}           [ "steadfastness", "determination" ],

    FaCUL                     `adj`     {- Samuwd -}           [ unwords [ "holding", "steady" ], unwords [ "remaining", "stable" ] ],

    FACiL                     `adj`     {- SAmid -}            [ "steadfast", "resistant" ],

    FiCAL                     `noun`    {- SimAd -}            [ "cover" ]
                              `plural`     FaCA'iL,

    FiCAL |< aT               `noun`    {- SimAdap -}          [ "headcloth" ]
                              `plural`     FaCA'iL ]


cluster_43  = listing "Lexicon's properties"


 |> ".s m l" <| [

    FaCaL                     `verb`    {- Samal-u -}          [ unwords [ "stand", "firm" ] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- OaSomal -}          [ "dehydrate", "harden" ],

    FaCL                      `noun`    {- Samol -}            [ "rigidity", "stiffness" ],

    FACUL |< aT               `noun`    {- SAmuwlap -}         [ "nut", "rivet" ]
                              `plural`     FawACIL,

    FaCUL |< aT               `noun`    {- Samuwlap -}         [ "nut", "rivet" ]
                              `plural`     FawACiL ]

 |> ".s m l _h" <| [

    KiRDAS                    `noun`    {- SimolAx -}          [ "earwax" ]
                              `plural`     KaRADIS ]


cluster_44  = listing "Lexicon's properties"


 |> ".s m m" <| [

    FaCL                      `verb`    {- Sam~-u -}           [ "plug", unwords [ "stop", "up" ] ]
                              `imperf`     FuCL
                              `pfirst`     FaCaL,

    FaCCaL                    `verb`    {- Sam~am -}           [ unwords [ "be", "determined" ], unwords [ "be", "persistent" ] ],

    HaFaCL                    `verb`    {- OaSam~ -}           [ "deafen", unwords [ "make", "deaf" ], unwords [ "become", "deaf" ] ],

    TaFACL                    `verb`    {- taSAm~ -}           [ unwords [ "give", "a", "deaf", "ear", "to" ] ],

    FiCL |< aT                `noun`    {- Sim~ap -}           [ "plug", "stopper" ],

    FaCaL                     `noun`    {- Samam -}            [ "deafness" ],

    FiCAL                     `noun`    {- SimAm -}            [ "plug", "valve", "stopper" ]
                              `plural`     FiCAL |< At
                              `plural`     HaFiCL |< aT,

    FiCAL |< aT               `noun`    {- SimAmap -}          [ "embolism" ]
                              `plural`     FaCA'iL,

    FiCAL |< aT               `noun`    {- SimAmap -}          [ "tube" ],

    FaCIL                     `noun`    {- Samiym -}           [ "depth", "innermost" ],

    FaCIL                     `adj`     {- Samiym -}           [ "true", "sincere" ],

    FaCIL |< Iy               `adj`     {- Samiymiy~ -}        [ "cordial", "intimate" ],

    FaCIL |< Iy |< aT         `noun`    {- Samiymiy~ap -}      [ "cordiality", "intimacy" ],

    HaFaCL                    `adj`     {- OaSam~ -}           [ "deaf", "plugged" ]
                              `plural`     FuCLAn
                              `plural`     FuCL
                              `plural`     FaCLA',

    TaFCIL                    `noun`    {- taSomiym -}         [ "determination", "perseverance" ]
                              `plural`     TaFCIL |< At,

    TaFCIL                    `noun`    {- taSomiym -}         [ "plan", "sketch", "design" ]
                              `plural`     TaFACIL,

    MuFaCCiL                  `adj`     {- muSam~im -}         [ "determined", "resolute" ],

    MuFaCCiL                  `noun`    {- muSam~im -}         [ "designer", "stylist" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "designer", "stylist" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCaL                  `adj`     {- muSam~am -}         [ "designed", "resolved" ],

    HiFCAL                    `noun`    {- IiSomAm -}          [ "obstruction", "deafening" ]
                              `plural`     HiFCAL |< At,

    MutaFACL                  `adj`     {- mutaSAm~ -}         [ unwords [ "deaf", "to" ] ] ]


cluster_45  = listing "Lexicon's properties"


 |> ".s m t" <| [

    FaCaL                     `verb`    {- Samat-u -}          [ unwords [ "be", "silent" ], unwords [ "be", "quiet" ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- Sam~at -}           [ "silence" ],

    HaFCaL                    `verb`    {- OaSomat -}          [ "silence" ],

    FaCL                      `noun`    {- Samot -}            [ "silence" ],

    FuCUL                     `noun`    {- Sumuwt -}           [ "silence", "silent" ],

    FaCiL                     `adj`     {- Samit -}            [ "silent" ],

    FACiL                     `adj`     {- SAmit -}            [ "silent" ]
                              `plural`     FawACiL,

    MuFCaL                    `adj`     {- muSomat -}          [ "plain", "uniform" ],

    MuFCaL                    `adj`     {- muSomat -}          [ "solid", "massive" ] ]

 |> ".s m y" <| [

    FaCY                      `verb`    {- SamaY-i -}          [ unwords [ "be", "fatally", "wounded" ] ]
                              `imperf`     FCI,

    HaFCY                     `verb`    {- OaSomaY -}          [ unwords [ "deal", "a", "fatal", "blow" ], unwords [ "be", "dealt", "a", "fatal", "blow" ] ] ]

 |> ".s n ^g" <| [

    FaCL                      `noun`    {- Sanoj -}            [ "cymbal", "gong" ]
                              `plural`     FuCUL,

    FiCL |< aT                `noun`    {- Sinojap -}          [ "weight" ],

    FaCCAL                    `noun`    {- San~Aj -}           [ "castanets" ]
                              `plural`     FaCCAL |< At,

    FaCCAL                    `noun`    {- San~Aj -}           [ "cymbalist" ],

    FaCCAL |< aT              `noun`    {- San~Ajap -}         [ "castanets" ] ]

 |> ".s n ^g q" <| [

    KaRDaS |< Iy |< aT        `noun`    {- Sanojaqiy~ap -}     [ "banner", unwords [ "administrative", "district" ] ] ]


cluster_46  = listing "Lexicon's properties"


 |> ".s n `" <| [

    FaCaL                     `verb`    {- SanaE-a -}          [ "fabricate", "design", "build" ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- San~aE -}           [ "manufacture", "industrialize" ],

    FACaL                     `verb`    {- SAnaE -}            [ "flatter", "cajole" ],

    TaFaCCaL                  `verb`    {- taSan~aE -}         [ "simulate" ],

    IFtaCaL                   `verb`    {- AiSoTanaE -}        [ "manufacture", "fabricate" ],

    FaCL                      `noun`    {- SanoE -}            [ "manufacture" ],

    FaCL |< Iy                `adj`     {- SanoEiy~ -}         [ "artificial", "synthetic" ],

    FaCL |< aT                `noun`    {- SanoEap -}          [ "craft", "workmanship" ],

    FaCLA'                    `noun`    {- SanoEA' -}          [ "Sana", "Sanaa" ],

    FaCAL                     `adj`     {- SanAE -}            [ "skilled", "dexterous" ],

    FiCAL |< aT               `noun`    {- SinAEap -}          [ "manufacture", "industry", "trade", "craft" ]
                              `plural`     FaCA'iL,

    FiCAL |< Iy               `adj`     {- SinAEiy~ -}         [ "industrial", "artificial" ],

    FaCA'iL |< Iy             `adj`     {- SanA}iEiy~ -}       [ "artificial", "artisanship", "artisan" ],

    FaCIL                     `noun`    {- SaniyE -}           [ "action", "deed" ],

    FaCIL |< aT               `noun`    {- SaniyEap -}         [ unwords [ "good", "deed" ] ]
                              `plural`     FaCA'iL,

    MaFCaL                    `noun`    {- maSonaE -}          [ "factory", unwords [ "industrial", "plant" ] ]
                              `plural`     MaFACiL,

    MaFCaL |< Iy |< aT        `noun`    {- maSonaEiy~ap -}     [ "wages", "pay" ],

    TaFCIL                    `noun`    {- taSoniyE -}         [ "fabrication", "industrialization", "processing" ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- taSan~uE -}         [ "dissimulation", "affectation" ]
                              `plural`     TaFaCCuL |< At,

    TaFaCCuL |< Iy            `adj`     {- taSan~uEiy~ -}      [ "simulator" ],

    IFtiCAL                   `noun`    {- AiSoTinAE -}        [ "manufacture", "synthesis" ]
                              `plural`     IFtiCAL |< At,

    IFtiCAL |< Iy             `adj`     {- AiSoTinAEiy~ -}     [ "artificial", "synthetic" ],

    FACiL                     `noun`    {- SAniE -}            [ "manufacturer", "producer", "artisan" ]
                              `plural`     FACiL |< Un
                              `plural`     FuCCAL,

    FACiL |< aT               `noun`    {- DERIVED -}          [ "manufacturer", "producer", "artisan" ]
                              `plural`     FACiL |< At,

    FACiL                     `adj`     {- SAniE -}            [ "manufacturing", "producing" ],

    MaFCUL                    `adj`     {- maSonuwE -}         [ "manufactured", "produced" ],

    MaFCUL                    `noun`    {- maSonuwE -}         [ unwords [ "manufactured", "goods" ] ]
                              `plural`     MaFCUL |< At,

    MuFaCCiL                  `noun`    {- muSan~iE -}         [ "manufacturer", "designer" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "manufacturer", "designer" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCaL                  `adj`     {- muSan~aE -}         [ "processed", "manufactured", "designed" ],

    MutaFaCCiL                `adj`     {- mutaSan~iE -}       [ "affected", "unnatural", "industrialized" ],

    MuFtaCaL                  `adj`     {- muSoTanaE -}        [ "artificial", "synthetic", "fabricated" ] ]


cluster_47  = listing "Lexicon's properties"


 |> ".s n b r" <| [

    KuRDUS                    `noun`    {- Sunobuwr -}         [ "faucet" ]
                              `plural`     KaRADIS ]

 |> ".s n d d" <| [

    KiRDIS                    `adj`     {- Sinodiyd -}         [ "brave", "courageous" ]
                              `plural`     KaRADIS ]

 |> ".s n d l" <| [

    KaRDaS                    `noun`    {- Sanodal -}          [ "sandalwood", "sandals" ],

    KaRDaS                    `noun`    {- Sanodal -}          [ "barge" ]
                              `plural`     KaRADiS ]

 |> ".s n d q" <| [

    KuRDUS                    `noun`    {- Sunoduwq -}         [ "box", "bin" ]
                              `plural`     KaRADIS,

    KuRDUS                    `noun`    {- Sunoduwq -}         [ "treasury", "fund" ] ]

 |> ".s n d y" <| [

    KaRDAS                    `noun`    {- SanodAy -}          [ "Sunday" ],

    KaRDI                     `noun`    {- Sanodiy -}          [ "Sandy" ] ]

 |> ".s n f" <| [

    FaCCaL                    `verb`    {- San~af -}           [ "sort", "classify", "compile" ],

    FiCL                      `noun`    {- Sinof -}            [ "class", "category" ]
                              `plural`     HaFCAL,

    TaFCIL                    `noun`    {- taSoniyf -}         [ "classification", "sorting", "compilation" ]
                              `plural`     TaFACIL
                              `plural`     TaFCIL |< At,

    TaFCIL |< aT              `noun`    {- taSoniyfap -}       [ "assortment", "typology" ],

    TaFCIL |< Iy              `adj`     {- taSoniyfiy~ -}      [ "typological" ],

    MuFaCCiL                  `noun`    {- muSan~if -}         [ "compiler", "classifier" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "compiler", "classifier" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCiL |< At            `noun`    {- muSan~ifAt -}       [ unwords [ "file", "holder" ], "binder" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCaL                  `adj`     {- muSan~af -}         [ "classified", "ranked", "seeded" ],

    MuFaCCaL                  `noun`    {- muSan~af -}         [ "compilation" ]
                              `plural`     MuFaCCaL |< At ]

 |> ".s n f r" <| [

    KaRDaS                    `verb`    {- Sanofar -}          [ "sandpaper", "emery" ],

    KaRDaS                    `noun`    {- Sanofar -}          [ "emery" ],

    KaRDaS |< aT              `noun`    {- Sanofarap -}        [ "emery" ] ]


cluster_48  = listing "Lexicon's properties"


 |> ".s n m" <| [

    FaCaL                     `noun`    {- Sanam -}            [ "idol", "image" ]
                              `plural`     HaFCAL,

    FaCaL |< Iy               `adj`     {- Sanamiy~ -}         [ "fetish" ] ]

 |> ".s n n" <| [

    FaCL                      `noun`    {- San~ -}             [ "basket" ],

    FiCL |< aT                `noun`    {- Sin~ap -}           [ unwords [ "body", "odor" ] ],

    FuCAL                     `noun`    {- SunAn -}            [ unwords [ "body", "odor" ] ] ]

 |> ".s n q" <| [

    FaCiL                     `verb`    {- Saniq-a -}          [ "stink", "smell" ]
                              `imperf`     FCaL,

    FaCL                      `noun`    {- Sanoq -}            [ unwords [ "foul", "(", "odor", ")" ], "stinky", "smelly" ],

    FaCL |< aT                `noun`    {- Sanoqap -}          [ unwords [ "foul", "odor" ], "stench" ] ]

 |> ".s n r" <| [

    FiCCAL |< aT              `noun`    {- Sin~Arap -}         [ "fishhook", unwords [ "crochet", "needle" ] ]
                              `plural`     FaCACIL,

    MuFaCCaL                  `adj`     {- muSan~ar -}         [ "crocheted" ] ]

 |> ".s n t m" <| [

    KaRDIS                    `noun`    {- Sanotiym -}         [ "centime" ]
                              `plural`     KaRDIS |< At ]

 |> ".s n w" <| [

    FiCL                      `noun`    {- Sinow -}            [ "twin" ]
                              `plural`     HaFCA',

    FiCL |< aT                `noun`    {- Sinowap -}          [ "sister" ] ]

 |> ".s n z" <| [

    FaCL                      `noun`    {- Sanoz -}            [ "Suns" ] ]


cluster_49  = listing "Lexicon's properties"


 |> ".s q `" <| [

    FaCaL                     `verb`    {- SaqaE-a -}          [ unwords [ "be", "eloquent" ], unwords [ "speak", "well" ] ]
                              `imperf`     FCaL,

    FaCaL                     `verb`    {- SaqaE-a -}          [ unwords [ "be", "frozen" ] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- Saq~aE -}           [ unwords [ "be", "frozen" ] ],

    FuCL                      `noun`    {- SuqoE -}            [ "region" ]
                              `plural`     HaFCAL,

    FaCL |< aT                `noun`    {- SaqoEap -}          [ unwords [ "severe", "cold" ], unwords [ "cold", "spell" ] ],

    FaCIL                     `noun`    {- SaqiyE -}           [ "frost", "ice" ],

    HaFCaL                    `noun`    {- OaSoqaE -}          [ unwords [ "more", "/", "most", "eloquent" ] ],

    MiFCaL                    `adj`     {- miSoqaE -}          [ "eloquent", "stentorian" ]
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- taSoqiyE -}         [ unwords [ "deep", "-", "freezing" ] ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL                  `adj`     {- muSaq~aE -}         [ "frozen" ],

    MaFCUL                    `adj`     {- maSoquwE -}         [ "frozen" ] ]

 |> ".s q b" <| [

    FaCiL                     `verb`    {- Saqib-a -}          [ "approach" ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- SAqab -}            [ "approach", unwords [ "be", "adjacent" ] ],

    MuFACaL |< aT             `noun`    {- muSAqabap -}        [ "affinity" ] ]


cluster_50  = listing "Lexicon's properties"


 |> ".s q l" <| [

    FaCaL                     `verb`    {- Saqal-u -}          [ "smooth", "polish", "refine" ]
                              `imperf`     FCuL,

    InFaCaL                   `verb`    {- AinoSaqal -}        [ unwords [ "be", "smooth" ], unwords [ "be", "polished" ], unwords [ "be", "refined" ] ],

    FaCL                      `noun`    {- Saqol -}            [ "polishing", "burnishing" ],

    FaCIL                     `noun`    {- Saqiyl -}           [ "polished", "burnished" ],

    FaCCAL                    `noun`    {- Saq~Al -}           [ "polisher", "smoother" ],

    FaCCAL                    `noun`    {- Saq~Al -}           [ "Saqqal", "Sakkal" ],

    MiFCaL |< aT              `noun`    {- miSoqalap -}        [ "burnisher" ]
                              `plural`     MaFACiL,

    MaFCUL                    `adj`     {- maSoquwl -}         [ "polished", "burnished" ],

    FiCAL |< aT               `noun`    {- SiqAlap -}          [ "tier", "terrace", "scaffolding", "scaffolds" ]
                              `plural`     FaCA'iL ]

 |> ".s q l b" <| [

    KaRDaS |< Iy              `adj`     {- Saqolabiy~ -}       [ "Slavic", "Slavs" ]
                              `plural`     KaRADiS |< aT ]

 |> ".s q r" <| [

    FaCL                      `noun`    {- Saqor -}            [ "Saqr" ],

    FaCL                      `noun`    {- Saqor -}            [ "falcon", "hawk" ]
                              `plural`     FuCUL
                              `plural`     HaFCuL,

    FaCL |< Iy                `adj`     {- Saqoriy~ -}         [ unwords [ "falcon", "-", "like" ], unwords [ "hawk", "-", "like" ] ],

    FaCCAL                    `noun`    {- Saq~Ar -}           [ "falconer" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ "falconer" ]
                              `plural`     FaCCAL |< At,

    FACUL                     `noun`    {- SAquwr -}           [ unwords [ "stone", "ax" ] ],

    FaCaLAn                   `noun`    {- SaqarAn -}          [ "Saqaran" ],

    FaCaLAn |< Iy             `adj`     {- SaqarAniy~ -}       [ "Saqarani" ] ]


cluster_51  = listing "Lexicon's properties"


 |> ".s r .h" <| [

    FaCuL                     `verb`    {- SaruH-u -}          [ unwords [ "be", "frank" ], unwords [ "be", "candid" ] ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- SaraH-a -}          [ "clarify" ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- Sar~aH -}           [ "declare", "announce" ],

    FACaL                     `verb`    {- SAraH -}            [ unwords [ "speak", "frankly" ], "declare" ],

    TaFACaL                   `verb`    {- taSAraH -}          [ "clarify" ],

    InFaCaL                   `verb`    {- AinoSaraH -}        [ "clarify" ],

    FaCL                      `noun`    {- SaroH -}            [ "structure", "edifice" ]
                              `plural`     FuCUL,

    FuCAL                     `noun`    {- SurAH -}            [ "pure", "distinct" ],

    FaCIL                     `adj`     {- SariyH -}           [ "candid", "sincere" ]
                              `plural`     FaCA'iL
                              `plural`     FuCaLA',

    FaCAL |< aT               `noun`    {- SarAHap -}          [ "sincerity", "candor", "frankness" ],

    TaFCIL                    `noun`    {- taSoriyH -}         [ "declaration", "statement" ]
                              `plural`     TaFCIL |< At
                              `plural`     TaFACIL,

    TaFCIL                    `noun`    {- taSoriyH -}         [ "permit", "license" ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- muSAraHap -}        [ "openness", "sincerity", "declaration" ],

    MuFaCCaL                  `adj`     {- muSar~aH -}         [ "licensed", "permitted" ],

    FuCAL |< Iy |< aT         `noun`    {- SurAHiy~ap -}       [ "demijohn" ] ]


cluster_52  = listing "Lexicon's properties"


 |> ".s r .s r" <| [

    KaRDaS                    `verb`    {- SaroSar -}          [ "chirp", "squeak", "scream" ],

    KaRDaS                    `noun`    {- SaroSar -}          [ "gale" ],

    KuRDuS                    `noun`    {- SuroSur -}          [ "cockroach" ]
                              `plural`     KaRADiS,

    KaRDUS                    `noun`    {- SaroSuwr -}         [ "Sarsour" ],

    KuRDUS                    `noun`    {- SuroSuwr -}         [ "cockroach" ]
                              `plural`     KaRADIS,

    KaRDAS                    `noun`    {- SaroSAr -}          [ "cricket" ],

    MuKaRDiS                  `adj`     {- muSaroSir -}        [ "screaming", "piercing" ] ]

 |> ".s r .t" <| [

    FiCAL                     `noun`    {- SirAT -}            [ "way", "path" ]
                              `plural`     FuCuL ]

 |> ".s r _h" <| [

    FaCaL                     `verb`    {- Sarax-u -}          [ "shout", "scream" ]
                              `imperf`     FCuL,

    IstaFCaL                  `verb`    {- AisotaSorax -}      [ unwords [ "call", "for", "help" ] ],

    FaCL |< aT                `noun`    {- Saroxap -}          [ "shout", "scream" ]
                              `plural`     FaCaL |< At,

    FuCAL                     `noun`    {- SurAx -}            [ "shouting", "screaming" ],

    FuCAL |< Iy               `adj`     {- SurAxiy~ -}         [ "shouting", "screaming" ],

    FaCIL                     `noun`    {- Sariyx -}           [ "shouting", "screaming" ],

    FaCCAL                    `noun`    {- Sar~Ax -}           [ "shouter" ],

    FACUL                     `noun`    {- SAruwx -}           [ "missile", "rocket" ]
                              `plural`     FawACIL,

    FACUL |< Iy               `adj`     {- SAruwxiy~ -}        [ "missile", "rocket" ],

    IstiFCAL                  `noun`    {- AisotiSorAx -}      [ unwords [ "cry", "for", "help" ] ]
                              `plural`     IstiFCAL |< At,

    FACiL                     `adj`     {- SArix -}            [ "loud", "noisy", "shouter" ] ]


cluster_53  = listing "Lexicon's properties"


 |> ".s r `" <| [

    FaCaL                     `verb`    {- SaraE-a -}          [ unwords [ "throw", "down" ], "dismay" ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- SAraE -}            [ unwords [ "fight", "against" ], unwords [ "struggle", "with" ] ],

    TaFACaL                   `verb`    {- taSAraE -}          [ "fight", "struggle" ],

    InFaCaL                   `verb`    {- AinoSaraE -}        [ unwords [ "be", "dismayed" ] ],

    IFtaCaL                   `verb`    {- AiSoTaraE -}        [ "fight", "struggle" ],

    FaCL                      `noun`    {- SaroE -}            [ "epilepsy", unwords [ "knock", "-", "out" ] ],

    FaCL |< Iy                `adj`     {- SaroEiy~ -}         [ "epileptic" ],

    FaCIL                     `adj`     {- SariyE -}           [ "collapsed", "fallen", "overcome" ]
                              `plural`     FaCLY,

    MaFCaL                    `noun`    {- maSoraE -}          [ "death", "fatality" ]
                              `plural`     MaFACiL,

    MiFCAL                    `noun`    {- miSorAE -}          [ "hemistich", unwords [ "door", "panel" ] ]
                              `plural`     MaFACIL,

    FiCAL                     `noun`    {- SirAE -}            [ "struggle", "fight" ]
                              `plural`     FiCAL |< At,

    MuFACaL |< aT             `noun`    {- muSAraEap -}        [ "wrestling", "struggle" ],

    IFtiCAL                   `noun`    {- AiSoTirAE -}        [ "wrestling", "struggle" ]
                              `plural`     IFtiCAL |< At,

    MaFCUL                    `adj`     {- maSoruwE -}         [ "collapsed", unwords [ "knocked", "out" ] ],

    MaFCUL                    `adj`     {- maSoruwE -}         [ "epileptic", "demented" ],

    MuFACiL                   `noun`    {- muSAriE -}          [ "fighter", "combatant", "wrestler" ]
                              `plural`     MuFACiL |< Un,

    MuFACiL |< aT             `noun`    {- DERIVED -}          [ "fighter", "combatant", "wrestler" ]
                              `plural`     MuFACiL |< At ]

 |> ".s r b" <| [

    FiCL                      `noun`    {- Sirob -}            [ "Serbia" ],

    FiCL |< Iy                `adj`     {- Sirobiy~ -}         [ "Serbian" ]
                              `plural`     FiCL |< Iy |< Un
                              `plural`     FiCL ]


cluster_54  = listing "Lexicon's properties"


 |> ".s r d" <| [

    FaCL                      `noun`    {- Sarod -}            [ "plateau" ]
                              `plural`     FuCUL,

    FaCL                      `noun`    {- Sarod -}            [ "pure", "sincere" ],

    FuCCAL                    `noun`    {- Sur~Ad -}           [ unwords [ "drifting", "clouds" ] ]
                           {- `others`  [ ".surrayd N" ] -} ]


cluster_55  = listing "Lexicon's properties"


 |> ".s r f" <| [

    FaCaL                     `verb`    {- Saraf-i -}          [ "divert", "spend" ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- Sar~af -}           [ "exchange", "expedite" ],

    TaFaCCaL                  `verb`    {- taSar~af -}         [ "behave" ],

    InFaCaL                   `verb`    {- AinoSaraf -}        [ unwords [ "go", "away" ] ],

    FaCL                      `noun`    {- Sarof -}            [ "diverting", "spending" ],

    FiCL                      `noun`    {- Sirof -}            [ "mere", "pure" ],

    FaCL |< Iy                `adj`     {- Sarofiy~ -}         [ "morphological" ],

    FaCL |< Iy |< At          `noun`    {- Sarofiy~At -}       [ "disbursements" ],

    FaCIL                     `noun`    {- Sariyf -}           [ "squeaking", "squealing" ],

    FaCCAL                    `noun`    {- Sar~Af -}           [ unwords [ "money", "changer" ], "cashier" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ unwords [ "money", "changer" ], "cashier" ]
                              `plural`     FaCCAL |< At,

    FaCIL |< aT               `noun`    {- Sariyfap -}         [ unwords [ "reed", "-", "mat", "hut" ] ]
                              `plural`     FaCA'iL,

    MaFCiL                    `noun`    {- maSorif -}          [ "bank" ]
                              `plural`     MaFACiL,

    MaFCiL |< Iy              `adj`     {- maSorifiy~ -}       [ "bank", "banking" ],

    TaFCIL                    `noun`    {- taSoriyf -}         [ "selling", "passing" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- taSoriyfiy~ -}      [ "selling", "passing" ],

    TaFaCCuL                  `noun`    {- taSar~uf -}         [ "behavior", "conduct", "disposal" ]
                              `plural`     TaFaCCuL |< At,

    TaFaCCuL |< At            `noun`    {- taSar~ufAt -}       [ "measures", "regulations" ]
                              `plural`     TaFaCCuL |< At,

    InFiCAL                   `noun`    {- AinoSirAf -}        [ "departure" ]
                              `plural`     InFiCAL |< At,

    MaFCUL                    `noun`    {- maSoruwf -}         [ "expenditure", "expense" ]
                              `plural`     MaFACIL,

    MutaFaCCiL                `noun`    {- mutaSar~if -}       [ unwords [ "provincial", "governor" ] ],

    MutaFaCCiL |< Iy          `noun`    {- mutaSar~ifiy~ -}    [ "province", "jurisdiction" ]
                              `plural`     MutaFaCCiL |< Iy |< At,

    MunFaCiL                  `adj`     {- munoSarif -}        [ "departing", "leaving" ],

    MunFaCaL                  `noun`    {- munoSaraf -}        [ "departure", "conclusion", "end" ] ]


cluster_56  = listing "Lexicon's properties"


 |> ".s r m" <| [

    FaCaL                     `verb`    {- Saram-i -}          [ unwords [ "cut", "off" ], "sever", "separate" ]
                              `imperf`     FCiL,

    FACaL                     `verb`    {- SAram -}            [ unwords [ "be", "estranged" ], unwords [ "brake", "off", "with" ] ],

    InFaCaL                   `verb`    {- AinoSaram -}        [ "expire", "elapse" ],

    FaCL                      `noun`    {- Sarom -}            [ "severance", "separation" ],

    FaCAL |< aT               `noun`    {- SarAmap -}          [ "severity", "harshness" ],

    FACiL                     `adj`     {- SArim -}            [ "severe", "rigorous", "strict" ],

    FaCIL |< aT               `noun`    {- Sariymap -}         [ "energy", "firmness" ],

    MuFACaL |< aT             `noun`    {- muSAramap -}        [ "estrangement", "hostility" ],

    InFiCAL                   `noun`    {- AinoSirAm -}        [ "expiration", "end" ]
                              `plural`     InFiCAL |< At,

    MunFaCiL                  `adj`     {- munoSarim -}        [ unwords [ "gone", "by" ], "elapsed" ],

    FaCL |< aT                `noun`    {- Saromap -}          [ "shoes" ],

    FaCALI                    `noun`    {- SarAmiy -}          [ "shoes" ],

    FuCaL |< At |< Iy         `noun`    {- SuramAtiy~ -}       [ "cobbler" ] ]

 |> ".s r m y" <| [

    KuRDAS |< aT              `noun`    {- SuromAyap -}        [ "shoes" ] ]


cluster_57  = listing "Lexicon's properties"


 |> ".s r r" <| [

    FaCL                      `verb`    {- Sar~-i -}           [ "screech", "chirp" ]
                              `imperf`     FiCL
                              `pfirst`     FaCaL,

    HaFaCL                    `verb`    {- OaSar~ -}           [ "insist", "assert" ],

    FuCL |< aT                `noun`    {- Sur~ap -}           [ unwords [ "money", "bag" ], "bundle" ],

    FaCIL                     `noun`    {- Sariyr -}           [ "screeching", "scratching", "chirping" ],

    FaCCAL                    `noun`    {- Sar~Ar -}           [ "cricket" ],

    HiFCAL                    `noun`    {- IiSorAr -}          [ "insistence", "determination", "premeditation" ]
                              `plural`     HiFCAL |< At,

    MuFiCL                    `adj`     {- muSir~ -}           [ "determined", "insistent" ],

    FACL |< aT                `noun`    {- SAr~ap -}           [ "need", "thirst" ] ]

 |> ".s r y" <| [

    FACI                      `noun`    {- SAriy -}            [ "mast", "pole" ]
                              `plural`     FawACI
                              `plural`     FACI |< At ]

 |> ".s t m" <| [

    FuCL |< aT                `noun`    {- Sutomap -}          [ unwords [ "hard", "rock" ] ]
                              `plural`     FaCA'iL
                              `plural`     FuCaL,

    FiCAL |< aT               `noun`    {- SitAmap -}          [ "petrography", "lithology" ],

    FiCAL |< Iy               `adj`     {- SitAmiy~ -}         [ "petrographic", "lithological" ] ]

 |> ".s w .g" <| [

    FAL                       `verb`    {- SAg-u -}            [ "forge", "create", "formulate" ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FaCL                      `noun`    {- Sawog -}            [ "molding", "shaping" ],

    FIL |< aT                 `noun`    {- Siygap -}           [ "form", "shape", "formula" ]
                           {- `others`  [ ".siya.g N" ] -},

    FiyAL |< aT               `noun`    {- SiyAgap -}          [ "drafting", "formulation", "constructing" ],

    MaFAL                     `noun`    {- maSAg -}            [ "jewelry" ],

    FA'iL                     `noun`    {- SA}ig -}            [ "jeweler" ]
                              `plural`     FuyyAL
                              `plural`     FUCAL
                              `plural`     FAL |< aT,

    MaFUL                     `adj`     {- maSuwg -}           [ "fine", "crafted" ],

    MaFUL |< aT               `noun`    {- maSuwgap -}         [ "jewel", "jewelry" ]
                              `plural`     MaFUL |< At ]


cluster_58  = listing "Lexicon's properties"


 |> ".s w .h" <| [

    FaCCaL                    `verb`    {- Saw~aH -}           [ "wither" ],

    FuCAL                     `noun`    {- SuwAH -}            [ "pollen" ],

    TaFCIL                    `noun`    {- taSowiyH -}         [ "withering" ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL                  `adj`     {- muSaw~aH -}         [ "withered" ] ]

 |> ".s w .s" <| [

    FUL                       `noun`    {- SuwS -}             [ "chick" ]
                              `plural`     FILAn ]

 |> ".s w .s y" <| [

    KaRDY                     `verb`    {- SawoSaY -}          [ "squeak", "cheep" ] ]

 |> ".s w ^g" <| [

    FaCL |< aT                `noun`    {- Sawojap -}          [ "soya" ] ]

 |> ".s w _h" <| [

    FAL                       `verb`    {- SAx-u -}            [ unwords [ "be", "slippery" ], "sink", "swoon" ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FaCL                      `noun`    {- Sawox -}            [ "sinking", "swooning" ],

    HaFAL                     `verb`    {- OaSAx -}            [ "listen", unwords [ "lend", "an", "ear" ], unwords [ "be", "listened", "to" ], unwords [ "be", "lent", "an", "ear" ] ],

    HiFAL |< aT               `noun`    {- IiSAxap -}          [ "listening", unwords [ "lending", "an", "ear" ] ] ]

 |> ".s w `" <| [

    InFAL                     `verb`    {- AinoSAE -}          [ "submit", "obey" ],

    FAL                       `noun`    {- SAE -}              [ unwords [ "saa", "(", "cubic", "dry", "measure", ")" ], unwords [ "saas", "(", "cubic", "dry", "measure", ")" ] ]
                              `plural`     HaFCAL,

    FaCAL                     `noun`    {- SawAE -}            [ "dish", "jar" ],

    InFiyAL                   `noun`    {- AinoSiyAE -}        [ "obeisance", "submission" ]
                              `plural`     InFiyAL |< At,

    MunFAL                    `adj`     {- munoSAE -}          [ "submissive", "obedient" ] ]


cluster_59  = listing "Lexicon's properties"


 |> ".s w b" <| [

    FaCCaL                    `verb`    {- Saw~ab -}           [ "rectify", "correct" ],

    HaFAL                     `verb`    {- OaSAb -}            [ "strike", "afflict", unwords [ "be", "hit" ], unwords [ "be", "struck" ] ],

    FaCL                      `noun`    {- Sawob -}            [ "direction", "quarter" ],

    FaCAL                     `adj`     {- SawAb -}            [ "correct", "true" ],

    FaCAL                     `noun`    {- SawAb -}            [ "reason", unwords [ "good", "sense" ] ],

    FaCAL                     `noun`    {- SawAb -}            [ "consciousness", "awareness" ],

    FaCAL |< Iy               `adj`     {- SawAbiy~ -}         [ unwords [ "well", "-", "founded" ], "reasoned" ],

    FA'iL                     `noun`    {- SA}ib -}            [ "Saeb", "Sa'ib" ],

    FA'iL                     `adj`     {- SA}ib -}            [ "correct", "accurate", unwords [ "on", "target" ], "opportune" ],

    HiFAL |< aT               `noun`    {- IiSAbap -}          [ "casualty", "injury" ]
                              `plural`     HiFAL |< At,

    HiFAL |< aT               `noun`    {- IiSAbap -}          [ "affliction" ],

    MuFIL |< aT               `noun`    {- muSiybap -}         [ "accident", "misfortune" ]
                              `plural`     MaFA'iL,

    MuFAL                     `adj`     {- muSAb -}            [ "afflicted", "injured", "wounded" ],

    HaFCaL                    `adj`     {- OaSowab -}          [ "pertinent", "apropos" ],

    HaFCaL |< Iy |< aT        `noun`    {- OaSowabiy~ap -}     [ "advisability", "expediency" ],

    TaFCIL                    `noun`    {- taSowiyb -}         [ "correction", "rectifying" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- taSowiybiy~ -}      [ "corrective" ],

    MuFaCCiL                  `noun`    {- muSaw~ib -}         [ unwords [ "gun", "-", "layer" ] ],

    MuFaCCiL |< aT            `noun`    {- muSaw~ibap -}       [ "sight", "viewfinder" ],

    IstiFCAL                  `noun`    {- AisotiSowAb -}      [ "approval", "sanction" ]
                              `plural`     IstiFCAL |< At,

    FaCL |< aT                `noun`    {- Sawobap -}          [ "stove" ] ]


cluster_60  = listing "Lexicon's properties"


 |> ".s w b n" <| [

    KaRDaS                    `verb`    {- Sawoban -}          [ "soap", unwords [ "apply", "soap" ] ],

    KaRDaS |< aT              `noun`    {- Sawobanap -}        [ "soaping", unwords [ "application", "of", "soap" ] ] ]

 |> ".s w f" <| [

    TaFaCCaL                  `verb`    {- taSaw~af -}         [ unwords [ "become", "a", "Sufi" ], unwords [ "become", "a", "mystic" ] ],

    FUL                       `noun`    {- Suwf -}             [ "wool" ]
                              `plural`     HaFCAL,

    FUL |< Iy                 `adj`     {- Suwfiy~ -}          [ "Sufi", unwords [ "Muslim", "mystic" ] ],

    FUL |< Iy                 `adj`     {- Suwfiy~ -}          [ "woolen" ],

    FUL |< Iy |< aT           `noun`    {- Suwfiy~ap -}        [ "Sufism", unwords [ "Islamic", "mysticism" ] ],

    HaFCaL                    `adj`     {- OaSowaf -}          [ "woolen" ]
                              `plural`     FaCLA',

    FULAn                     `noun`    {- SuwfAn -}           [ "touchwood", "tinder" ],

    FaCCAL                    `noun`    {- Saw~Af -}           [ unwords [ "wool", "merchant" ] ],

    FaCCAL                    `noun`    {- Saw~Af -}           [ "Sawwaf" ],

    TaFaCCuL                  `noun`    {- taSaw~uf -}         [ "Sufism", unwords [ "Islamic", "mysticism" ] ]
                              `plural`     TaFaCCuL |< At,

    MutaFaCCiL                `noun`    {- mutaSaw~if -}       [ "Sufi", unwords [ "Muslim", "mystic" ] ]
                              `plural`     MutaFaCCiL |< Un,

    MutaFaCCiL |< aT          `noun`    {- DERIVED -}          [ "Sufi", unwords [ "Muslim", "mystic" ] ]
                              `plural`     MutaFaCCiL |< At ]


cluster_61  = listing "Lexicon's properties"


 |> ".s w l" <| [

    FAL                       `verb`    {- SAl-u -}            [ "attack", unwords [ "jump", "on" ] ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FACaL                     `verb`    {- SAwal -}            [ "attack", unwords [ "jump", "on" ] ],

    FaCL                      `noun`    {- Sawol -}            [ "attack", "assault" ],

    FaCL |< aT                `noun`    {- Sawolap -}          [ "attack", "assault" ],

    FiyAL                     `noun`    {- SiyAl -}            [ "struggle", "fight" ],

    MuFACaL |< aT             `noun`    {- muSAwalap -}        [ "attack", "assault" ],

    FA'iL |< aT               `noun`    {- SA}ilap -}          [ unwords [ "hostile", "act" ] ]
                              `plural`     FawA'iL,

    FA'iL |< aT               `noun`    {- SA}ilap -}          [ "violence" ]
                              `plural`     FawA'iL,

    FUL                       `noun`    {- Suwl -}             [ "sergeant", unwords [ "warrant", "officer" ] ] ]

 |> ".s w m" <| [

    FAL                       `verb`    {- SAm-u -}            [ "abstain", "fast" ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FaCL                      `noun`    {- Sawom -}            [ "fasting", "abstinence" ],

    FiyAL                     `noun`    {- SiyAm -}            [ "fasting", "abstinence" ],

    FiyAL |< Iy               `adj`     {- SiyAmiy~ -}         [ "fasting" ],

    FA'iL                     `adj`     {- SA}im -}            [ "fasting", "abstinent" ]
                              `plural`     FA'iL |< Un
                              `plural`     FiyAL
                              `plural`     FuyyaL
                              `plural`     FUCaL ]

 |> ".s w m `" <| [

    KaRDaS |< aT              `noun`    {- SawomaEap -}        [ "hermitage", "silo", "minaret" ]
                              `plural`     KaRADiS ]

 |> ".s w m l" <| [

    KaRDaS |< aT              `noun`    {- Sawomalap -}        [ "Somalization" ],

    KuRDAS                    `noun`    {- SuwmAl -}           [ "Somalia" ],

    KuRDAS |< Iy              `adj`     {- SuwmAliy~ -}        [ "Somali" ]
                              `plural`     KaRADiS |< aT
                              `plural`     KuRDAS |< Iy |< Un ]


cluster_62  = listing "Lexicon's properties"


 |> ".s w n" <| [

    FAL                       `verb`    {- SAn-u -}            [ "maintain", "preserve", "protect" ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FaCL                      `noun`    {- Sawon -}            [ "maintenance", "preservation", "protection" ],

    FiCAL                     `noun`    {- SiwAn -}            [ "cupboard", "wardrobe" ]
                              `plural`     HaFCiL |< aT,

    FaCCAL                    `noun`    {- Saw~An -}           [ "granite", "quartz" ],

    FaCCAL |< Iy              `adj`     {- Saw~Aniy~ -}        [ "granite" ],

    FaCCAL |< aT              `noun`    {- Saw~Anap -}         [ "flint" ],

    FiyAL |< aT               `noun`    {- SiyAnap -}          [ "maintenance", "preservation" ],

    FA'iL                     `noun`    {- SA}in -}            [ "preserver", "protector", "maintainer" ]
                              `plural`     FA'iL |< Un,

    FA'iL |< aT               `noun`    {- DERIVED -}          [ "preserver", "protector", "maintainer" ]
                              `plural`     FA'iL |< At,

    MaFUL                     `adj`     {- maSuwn -}           [ "preserved", "protected", "maintained" ] ]


cluster_63  = listing "Lexicon's properties"


 |> ".s w r" <| [

    FaCCaL                    `verb`    {- Saw~ar -}           [ "draw", "portray" ],

    FaCCaL                    `verb`    {- Saw~ar -}           [ "film", "photograph" ],

    TaFaCCaL                  `verb`    {- taSaw~ar -}         [ "imagine", "envision", "ponder" ],

    FUL                       `noun`    {- Suwr -}             [ "Tyre" ],

    FUL |< aT                 `noun`    {- Suwrap -}           [ "picture", "image", "illustration", "photo", "photographs" ]
                              `plural`     FuCaL,

    FUL |< aT                 `noun`    {- Suwrap -}           [ "manner", "way", "form" ],

    FuCaL |< Iy               `adj`     {- Suwariy~ -}         [ "formal" ],

    FuCaL |< Iy               `adj`     {- Suwariy~ -}         [ "imaginary", "deceptive" ],

    FuCaL |< Iy |< aT         `noun`    {- Suwariy~ap -}       [ "formalism" ],

    TaFCIL                    `noun`    {- taSowiyr -}         [ "photography", "illustration", "depiction", "characterization" ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< aT              `noun`    {- taSowiyrap -}       [ "image", "pictorial" ]
                              `plural`     TaFACIL,

    TaFCIL |< Iy              `adj`     {- taSowiyriy~ -}      [ "photographic", "pictorial" ],

    MuFaCCiL                  `noun`    {- muSaw~ir -}         [ "photographer" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "photographer" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCiL                  `noun`    {- muSaw~ir -}         [ "Musawwir" ],

    MuFaCCiL |< aT            `noun`    {- muSaw~irap -}       [ "camera" ],

    MuFaCCaL                  `adj`     {- muSaw~ar -}         [ "illustrated", "photographed" ],

    TaFaCCuL                  `noun`    {- taSaw~ur -}         [ "conception", "imagination" ]
                              `plural`     TaFaCCuL |< At,

    TaFaCCuL |< Iy            `adj`     {- taSaw~uriy~ -}      [ "imaginary", "ideal" ] ]


cluster_64  = listing "Lexicon's properties"


 |> ".s w t" <| [

    FAL                       `verb`    {- SAt-u -}            [ "shout", unwords [ "sound", "out" ] ]
                              `imperf`     FUL
                              `pfirst`     FuL,

    FaCCaL                    `verb`    {- Saw~at -}           [ "vote" ],

    FaCL                      `noun`    {- Sawot -}            [ "vote" ]
                              `plural`     HaFCAL,

    FaCL                      `noun`    {- Sawot -}            [ "voice", "sound" ]
                              `plural`     HaFCAL,

    FaCL |< Iy                `adj`     {- Sawotiy~ -}         [ "acoustic", "vocalic" ],

    FaCL |< Iy |< aT          `noun`    {- Sawotiy~ap -}       [ "acoustics", "resonance" ],

    FaCL |< Iy |< At          `noun`    {- Sawotiy~At -}       [ "phonetics" ],

    FIL                       `noun`    {- Siyt -}             [ "renown", "reputation" ],

    FayyiL                    `adj`     {- Say~it -}           [ "loud" ],

    FayyiL                    `noun`    {- Say~it -}           [ "megaphone" ],

    TaFCIL                    `noun`    {- taSowiyt -}         [ "voting" ]
                              `plural`     TaFCIL |< At,

    FA'iL                     `adj`     {- SA}it -}            [ "voiced", "sounded" ],

    MuFaCCiL                  `noun`    {- muSaw~it -}         [ "voter" ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ "voter" ]
                              `plural`     MuFaCCiL |< At,

    MuFaCCiL                  `adj`     {- muSaw~it -}         [ "loud" ] ]

 |> ".s w t m" <| [

    KaRDaS                    `noun`    {- Sawotam -}          [ "phoneme" ]
                              `plural`     KaRADiS,

    KaRDaS |< Iy              `adj`     {- Sawotamiy~ -}       [ "phonemic" ]
                              `plural`     KaRDaS |< Iy |< At ]

 |> ".s w w" <| [

    FUL |< aT                 `noun`    {- Suw~ap -}           [ "landmark", unwords [ "road", "sign" ] ]
                           {- `others`  [ ".suway N" ] -} ]

 |> ".s w y" <| [

    FUw |< aT                 `noun`    {- Suw~ap -}           [ "landmark", unwords [ "road", "sign" ] ]
                              `plural`     FuCaL ]


cluster_65  = listing "Lexicon's properties"


 |> ".s y .g" <| [

    FIL |< aT                 `noun`    {- Siygap -}           [ "form", "shape", "formula" ]
                              `plural`     FiCaL,

    FiCaL |< Iy               `adj`     {- Siyagiy~ -}         [ "modal" ],

    FiCAL |< aT               `noun`    {- SiyAgap -}          [ "drafting", "formulation", "constructing" ],

    FACiL                     `noun`    {- SAyig -}            [ "Sayigh", "Sayegh" ] ]

 |> ".s y .h" <| [

    FAL                       `verb`    {- SAH-i -}            [ "scream", unwords [ "call", "out" ] ]
                              `imperf`     FIL
                              `pfirst`     FiL,

    FaCCaL                    `verb`    {- Say~aH -}           [ "scream", unwords [ "call", "out" ] ],

    TaFACaL                   `verb`    {- taSAyaH -}          [ unwords [ "shout", "at", "each", "other" ], unwords [ "raise", "a", "din" ] ],

    FaCL                      `noun`    {- SayoH -}            [ "shouting", "screaming" ],

    FaCL |< aT                `noun`    {- SayoHap -}          [ "shout", "cry" ]
                              `plural`     FiCAL,

    FA'iL                     `adj`     {- SA}iH -}            [ "shouting", "screaming" ],

    FaCCAL                    `adj`     {- Say~AH -}           [ "shouting", "screaming" ],

    TaFACuL                   `noun`    {- taSAyuH -}          [ "shouting", "screaming" ]
                              `plural`     TaFACuL |< At ]

 |> ".s y .s" <| [

    FIL |< aT                 `noun`    {- SiySap -}           [ "spur" ],

    FIL |< Iy |< aT           `noun`    {- SiySiy~ap -}        [ "spur" ]
                              `plural`     FaCALI ]

 |> ".s y `" <| [

    InFiCAL                   `noun`    {- AinoSiyAE -}        [ "obeisance", "submission" ]
                              `plural`     InFiCAL |< At,

    FACiL                     `noun`    {- SAyiE -}            [ "vagabond", "tramp" ] ]

 |> ".s y b" <| [

    MuFIL |< aT               `noun`    {- muSiybap -}         [ "accident", "misfortune" ]
                              `plural`     MaFA'iL ]


cluster_66  = listing "Lexicon's properties"


 |> ".s y d" <| [

    FAL                       `verb`    {- SAd-i -}            [ "hunt", "fish", "trap" ]
                              `imperf`     FIL
                              `pfirst`     FiL,

    TaFaCCaL                  `verb`    {- taSay~ad -}         [ "hunt", "fish", "trap" ],

    IFtAL                     `verb`    {- AiSoTAd -}          [ "hunt", "fish", "trap", unwords [ "be", "trapped" ] ],

    FaCL                      `noun`    {- Sayod -}            [ "hunting", "fishing", "trapping" ],

    FaCCAL                    `noun`    {- Say~Ad -}           [ "hunter", "fisherman" ]
                              `plural`     FaCCAL |< Un,

    FaCCAL |< aT              `noun`    {- DERIVED -}          [ "hunter", "fisherman" ]
                              `plural`     FaCCAL |< At,

    FaCCAL                    `noun`    {- Say~Ad -}           [ "Sayyad" ],

    MaFIL                     `adj`     {- maSiyd -}           [ "hunted", "fished", "trapped" ],

    MaFAL                     `noun`    {- maSAd -}            [ "hunting", "fishing", "trapping" ]
                              `plural`     MaFA'iL,

    MaFCaL |< aT              `noun`    {- maSoyadap -}        [ unwords [ "hunting", "ground" ], unwords [ "fishing", "ground" ] ],

    MiFCaL |< aT              `noun`    {- miSoyadap -}        [ "trap", "net" ]
                              `plural`     MaFACiL ]

 |> ".s y d l" <| [

    KaRDaS |< aT              `noun`    {- Sayodalap -}        [ "pharmacology", "pharmaceutics" ],

    KaRDaS |< Iy              `adj`     {- Sayodaliy~ -}       [ "pharmacist" ]
                              `plural`     KaRADiS |< aT
                              `plural`     KaRDaS |< Iy |< Un,

    KaRDaS |< Iy              `adj`     {- Sayodaliy~ -}       [ "pharmaceutical" ],

    KaRDaS |< Iy |< aT        `noun`    {- Sayodaliy~ap -}     [ "pharmacy" ] ]


cluster_67  = listing "Lexicon's properties"


 |> ".s y f" <| [

    FaCCaL                    `verb`    {- Say~af -}           [ "estivate", unwords [ "spend", "the", "summer" ] ],

    TaFaCCaL                  `verb`    {- taSay~af -}         [ "estivate", unwords [ "spend", "the", "summer" ] ],

    IFtAL                     `verb`    {- AiSoTAf -}          [ "estivate", unwords [ "spend", "the", "summer" ] ],

    FaCL                      `noun`    {- Sayof -}            [ "summer" ]
                              `plural`     FuCUL
                              `plural`     HaFCAL,

    FaCL |< Iy                `adj`     {- Sayofiy~ -}         [ "summer", "estival" ],

    MaFIL                     `noun`    {- maSiyf -}           [ unwords [ "summer", "resort" ] ]
                              `plural`     MaFA'iL,

    TaFCIL                    `noun`    {- taSoyiyf -}         [ "summering", unwords [ "summer", "vacationing" ] ]
                              `plural`     TaFCIL |< At,

    MuFaCCiL                  `noun`    {- muSay~if -}         [ unwords [ "summer", "vacationer" ] ]
                              `plural`     MuFaCCiL |< Un,

    MuFaCCiL |< aT            `noun`    {- DERIVED -}          [ unwords [ "summer", "vacationer" ] ]
                              `plural`     MuFaCCiL |< At,

    IFtiCAL                   `noun`    {- AiSotiyAf -}        [ "summering", unwords [ "summer", "vacationing" ] ]
                              `plural`     IFtiCAL |< At,

    MuFtAL                    `noun`    {- muSoTAf -}          [ "summering", "vacationer" ]
                              `plural`     MuFtAL |< Un,

    MuFtAL |< aT              `noun`    {- DERIVED -}          [ "summering", "vacationer" ]
                              `plural`     MuFtAL |< At,

    MuFtAL                    `noun`    {- muSoTAf -}          [ unwords [ "summer", "resort" ] ],

    MaFCaL                    `noun`    {- maSoyaf -}          [ unwords [ "summer", "resort" ] ]
                              `plural`     MaFACiL ]

 |> ".s y l" <| [

    FiCAL                     `noun`    {- SiyAl -}            [ "struggle", "fight" ] ]

 |> ".s y m" <| [

    FiCAL                     `noun`    {- SiyAm -}            [ "fasting", "abstinence" ],

    FiCAL |< Iy               `adj`     {- SiyAmiy~ -}         [ "fasting" ] ]

 |> ".s y n" <| [

    FiCAL |< aT               `noun`    {- SiyAnap -}          [ "maintenance", "preservation" ],

    FIL                       `noun`    {- Siyn -}             [ "China" ],

    FIL |< Iy                 `adj`     {- Siyniy~ -}          [ "Chinese" ],

    FIL |< Iy |< aT           `noun`    {- Siyniy~ap -}        [ "porcelain", "china" ]
                              `plural`     FawALI ]

 |> ".s y q l" <| [

    KaRDaS                    `noun`    {- Sayoqal -}          [ "polisher", "smoother" ]
                              `plural`     KaRADiS |< aT ]


cluster_68  = listing "Lexicon's properties"


 |> ".s y r" <| [

    FAL                       `verb`    {- SAr-i -}            [ "become", unwords [ "begin", "to" ] ]
                              `imperf`     FIL
                              `pfirst`     FiL,

    FaCCaL                    `verb`    {- Say~ar -}           [ "induce", unwords [ "cause", "to", "do" ] ],

    FaCLUL |< aT              `noun`    {- Sayoruwrap -}       [ "becoming", "outcome", "result" ],

    MaFIL                     `noun`    {- maSiyr -}           [ "path", "destiny", "fate" ]
                              `plural`     MaFACiL
                              `plural`     MaFA'iL,

    MaFIL |< Iy               `adj`     {- maSiyriy~ -}        [ "crucial", "decisive", "fateful" ],

    TaFCIL                    `noun`    {- taSoyiyr -}         [ "transfer", "cession" ]
                              `plural`     TaFCIL |< At ]

 |> ".s y r f" <| [

    KaRDaS                    `noun`    {- Sayoraf -}          [ unwords [ "money", "changer" ], "cashier" ]
                              `plural`     KaRADiS,

    KaRDaS |< Iy              `noun`    {- Sayorafiy~ -}       [ unwords [ "money", "changer" ], "cashier" ]
                              `plural`     KaRADiS |< aT ]

 |> ".s y t" <| [

    FIL                       `noun`    {- Siyt -}             [ "renown", "reputation" ],

    FaCCiL                    `adj`     {- Say~it -}           [ "loud" ],

    FaCCiL                    `noun`    {- Say~it -}           [ "megaphone" ] ]

 |> ".sUdA" <| [

    Identity                  `noun`    {- SuwdA -}            [ "soda" ] ]

 |> ".sUdiyUm" <| [

    Identity                  `noun`    {- Suwdiyuwm -}        [ "sodium" ] ]

 |> ".sUfA" <| [

    Identity                  `noun`    {- SuwfA -}            [ "sofa" ] ]

 |> ".sUfiyA" <| [

    Identity                  `noun`    {- SuwfiyA -}          [ "Sofia" ] ]

 |> ".sUyA" <| [

    Identity                  `noun`    {- SuwyA -}            [ "soya" ] ]

 |> ".sabrA" <| [

    Identity                  `noun`    {- SaborA -}           [ "Sabra" ] ]

 |> ".samU'Il" <| [

    Identity                  `noun`    {- Samuw}iyl -}        [ "Samuel", "Samu'il" ] ]

 |> ".sanawbar" <| [

    Identity                  `noun`    {- Sanawobar -}        [ unwords [ "stone", "pine" ] ],

    Identity |< Iy            `adj`     {- Sanawobariy~ -}     [ "pine", "coniferous" ] ]


cluster_69  = listing "Lexicon's properties"


 |> ".santimitr" <| [

    Identity                  `noun`    {- Sanotimitr -}       [ "centimeter" ] ]

 |> ".sardA" <| [

    Identity                  `noun`    {- SarodA -}           [ "Sarda" ] ]

 |> ".sawla^gAn" <| [

    Identity                  `noun`    {- SawolajAn -}        [ "scepter", unwords [ "golf", "club" ], unwords [ "polo", "mallet" ] ] ]

 |> ".saydA" <| [

    Identity                  `noun`    {- SayodA -}           [ "Sidon" ] ]

 |> ".saydalAn" <| [

    Identity |< Iy            `adj`     {- SayodalAniy~ -}     [ "pharmacist" ] ]

 |> ".siddIqI" <| [

    Identity                  `noun`    {- Sid~iyqiy -}        [ "Siddiqi" ] ]

 |> ".siqill" <| [

    Identity |< Iy            `adj`     {- Siqil~iy~ -}        [ "Sicilian", "Sicily" ] ]

 |> ".sl`m" <| [

    Identity                  `noun`    {- SlEm -}             [ unwords [ "Peace", "Be", "Upon", "Him" ] ] ]

 |> ".subbayr" <| [

    Identity                  `noun`    {- Sub~ayor -}         [ unwords [ "Indian", "fig" ] ] ]

 |> "ta.sfY" <| [

    Identity |< Iy            `adj`     {- taSofawiy~ -}       [ "definitive", "radical" ] ]


section = [ cluster_1,
            cluster_2,
            cluster_3,
            cluster_4,
            cluster_5,
            cluster_6,
            cluster_7,
            cluster_8,
            cluster_9,
            cluster_10,
            cluster_11,
            cluster_12,
            cluster_13,
            cluster_14,
            cluster_15,
            cluster_16,
            cluster_17,
            cluster_18,
            cluster_19,
            cluster_20,
            cluster_21,
            cluster_22,
            cluster_23,
            cluster_24,
            cluster_25,
            cluster_26,
            cluster_27,
            cluster_28,
            cluster_29,
            cluster_30,
            cluster_31,
            cluster_32,
            cluster_33,
            cluster_34,
            cluster_35,
            cluster_36,
            cluster_37,
            cluster_38,
            cluster_39,
            cluster_40,
            cluster_41,
            cluster_42,
            cluster_43,
            cluster_44,
            cluster_45,
            cluster_46,
            cluster_47,
            cluster_48,
            cluster_49,
            cluster_50,
            cluster_51,
            cluster_52,
            cluster_53,
            cluster_54,
            cluster_55,
            cluster_56,
            cluster_57,
            cluster_58,
            cluster_59,
            cluster_60,
            cluster_61,
            cluster_62,
            cluster_63,
            cluster_64,
            cluster_65,
            cluster_66,
            cluster_67,
            cluster_68,
            cluster_69 ]

