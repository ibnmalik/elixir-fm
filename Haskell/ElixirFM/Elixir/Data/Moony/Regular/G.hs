
module Elixir.Data.Moony.Regular.G where

import Elixir.Lexicon


version = revised "$Revision$"

lexicon = include section


cluster_1   = cluster

 |> "^g b s" <| [

    FaCCaL                    `verb`    {- <^gabbas> -}        [ ['p','l','a','s','t','e','r'], unwords [ ['p','u','t'], ['i','n'], "a", ['c','a','s','t'] ] ],

    FiCL                      `noun`    {- <^gibs> -}          [ ['g','y','p','s','u','m'], unwords [ ['p','l','a','s','t','e','r'], ['o','f'], ['P','a','r','i','s'] ] ],

    FaCCAL |< aT              `noun`    {- <^gabbAsaT> -}      [ unwords [ ['g','y','p','s','u','m'], ['q','u','a','r','r','y'] ], unwords [ ['p','l','a','s','t','e','r'], ['k','i','l','n'] ] ] ]


cluster_2   = cluster

 |> "^g b r n" <| [

    KuRDAS                    `noun`    {- <^gubrAn> -}        [ ['G','i','b','r','a','n'], ['J','u','b','r','a','n'] ] ]


cluster_3   = cluster

 |> "^g d b" <| [

    FaCuL                     `verb`    {- <^gadub> -}         [ unwords [ ['b','e'], ['b','a','r','r','e','n'] ], unwords [ ['b','e'], ['a','r','i','d'] ] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'a^gdab> -}        [ unwords [ ['b','e'], ['b','a','r','r','e','n'] ], unwords [ ['e','x','p','l','o','d','e'], ['i','n'], ['t','h','e'], ['b','a','r','r','e','l'] ] ],

    FuCUL |< aT               `noun`    {- <^gudUbaT> -}       [ ['b','a','r','r','e','n','n','e','s','s'], ['s','t','e','r','i','l','i','t','y'] ],

    FaCL                      `noun`    {- <^gadb> -}          [ ['d','r','o','u','g','h','t'], ['s','t','e','r','i','l','i','t','y'] ],

    FaCIL                     `noun`    {- <^gadIb> -}         [ ['b','a','r','r','e','n'], ['s','t','e','r','i','l','e'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    HaFCaL                    `adj`     {- <'a^gdab> -}        [ ['b','a','r','r','e','n'], ['s','t','e','r','i','l','e'] ]
                              `femini`     FaCLA',

    MuFCiL                    `noun`    {- <mu^gdib> -}        [ ['b','a','r','r','e','n'], ['s','t','e','r','i','l','e'], ['u','n','p','r','o','d','u','c','t','i','v','e'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise ]


cluster_4   = cluster

 |> "^g d ` n" <| [

    KiRDUS                    `noun`    {- <^gid`Un> -}        [ ['G','i','d','e','o','n'] ] ]


cluster_5   = cluster

 |> "^g d `" <| [

    FaCaL                     `verb`    {- <^gada`> -}         [ ['a','m','p','u','t','a','t','e'] ]
                              `imperf`     FCaL,

    HaFCaL                    `noun`    {- <'a^gda`> -}        [ ['m','u','t','i','l','a','t','e','d'] ],

    FaCaL                     `noun`    {- <^gada`> -}         [ ['y','o','u','t','h'] ]
                              `plural`     FiCLAn,

    FaCaL                     `adj`     {- <^gada`> -}         [ ['s','t','r','o','n','g'], ['m','a','c','h','o'] ]
                              `plural`     FiCLAn ]


cluster_6   = cluster

 |> "^g d f" <| [

    FaCaL                     `verb`    {- <^gadaf> -}         [ ['r','o','w'] ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <^gaddaf> -}        [ ['r','o','w'], ['b','l','a','s','p','h','e','m','e'], ['c','u','r','s','e'] ],

    TaFCIL                    `noun`    {- <ta^gdIf> -}        [ ['r','o','w','i','n','g'], ['i','m','p','r','e','c','a','t','i','o','n'], ['b','l','a','s','p','h','e','m','y'] ]
                              `plural`     TaFCIL |< At,

    MuFaCCiL                  `noun`    {- <mu^gaddif> -}      [ ['r','o','w','e','r'], ['b','l','a','s','p','h','e','m','e','r'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MiFCAL                    `noun`    {- <mi^gdAf> -}        [ ['o','a','r'] ]
                              `plural`     MaFACIL ]


cluster_7   = cluster

 |> "^g d l" <| [

    FaCaL                     `verb`    {- <^gadal> -}         [ ['t','w','i','s','t'], ['b','r','a','i','d'] ]
                              `imperf`     FCiL
                              `imperf`     FCuL,

    FaCiL                     `verb`    {- <^gadil> -}         [ unwords [ ['b','r','o','a','d','e','n'], ['o','u','t'] ], ['g','r','o','w'], unwords [ ['b','e'], ['r','e','i','n','f','o','r','c','e','d'] ] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gaddal> -}        [ ['b','r','a','i','d'] ],

    FACaL                     `verb`    {- <^gAdal> -}         [ ['q','u','a','r','r','e','l'], ['d','e','b','a','t','e'] ],

    TaFACaL                   `verb`    {- <ta^gAdal> -}       [ ['q','u','a','r','r','e','l'], ['d','e','b','a','t','e'] ],

    FaCaL                     `noun`    {- <^gadal> -}         [ ['c','o','n','t','r','o','v','e','r','s','y'], ['q','u','a','r','r','e','l'], ['d','e','b','a','t','e'] ],

    FaCaL |< Iy               `adj`     {- <^gadalIy> -}       [ ['c','o','n','t','r','o','v','e','r','s','i','a','l'] ],

    FaCaL |< Iy |< aT         `noun`    {- <^gadalIyaT> -}     [ ['d','i','a','l','e','c','t','i','c','s'] ],

    FaCCAL                    `noun`    {- <^gaddAl> -}        [ ['d','i','s','p','u','t','a','t','i','o','u','s'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    MiFCAL                    `noun`    {- <mi^gdAl> -}        [ ['d','i','s','p','u','t','a','t','i','o','u','s'], ['f','l','a','g','s','t','o','n','e'] ]
                              `plural`     MiFCAL |< Un
                           
    `derives` otherwise,

    FaCIL |< aT               `noun`    {- <^gadIlaT> -}       [ ['b','r','a','i','d'], ['t','r','e','s','s'] ]
                              `plural`     FaCA'iL,

    MaFACIL                   `noun`    {- <ma^gAdIl> -}       [ ['f','l','a','g','s','t','o','n','e'] ],

    FiCAL                     `noun`    {- <^gidAl> -}         [ ['d','e','b','a','t','e'], ['q','u','a','r','r','e','l'] ],

    MuFACaL |< aT             `noun`    {- <mu^gAdalaT> -}     [ ['d','e','b','a','t','e'], ['q','u','a','r','r','e','l'] ],

    MaFCUL                    `noun`    {- <ma^gdUl> -}        [ ['t','w','i','s','t','e','d'], ['b','r','a','i','d','e','d'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MuFACiL                   `noun`    {- <mu^gAdil> -}       [ ['d','i','s','p','u','t','a','n','t'], ['o','p','p','o','n','e','n','t'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise ]


cluster_8   = cluster

 |> "^g d r" <| [

    FaCuL                     `verb`    {- <^gadur> -}         [ unwords [ ['b','e'], ['s','u','i','t','a','b','l','e'] ], ['b','e','h','o','o','v','e'] ]
                              `imperf`     FCuL,

    FaCiL                     `verb`    {- <^gadir> -}         [ unwords [ ['h','a','v','e'], ['s','m','a','l','l','p','o','x'] ] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gaddar> -}        [ unwords [ ['h','a','v','e'], ['s','m','a','l','l','p','o','x'] ] ],

    HaFCaL                    `verb`    {- <'a^gdar> -}        [ unwords [ ['m','a','k','e'], ['w','o','r','t','h','y'] ], unwords [ ['b','e','c','o','m','e'], ['w','o','r','t','h','y'] ] ],

    FaCIL                     `noun`    {- <^gadIr> -}         [ ['w','o','r','t','h','y'], ['a','p','p','r','o','p','r','i','a','t','e'] ]
                              `plural`     FuCaLA'
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    FaCIL                     `noun`    {- <^gadIr> -}         [ ['J','a','d','i','r'] ],

    HaFCaL                    `noun`    {- <'a^gdar> -}        [ ['w','o','r','t','h','i','e','r'], unwords [ ['m','o','r','e'], ['s','u','i','t','a','b','l','e'] ] ],

    FaCAL |< aT               `noun`    {- <^gadAraT> -}       [ ['w','o','r','t','h','i','n','e','s','s'], ['a','p','t','i','t','u','d','e'], ['m','e','r','i','t'] ],

    FiCAL                     `noun`    {- <^gidAr> -}         [ ['w','a','l','l'] ]
                              `plural`     FuCLAn,

    FiCAL |< Iy               `adj`     {- <^gidArIy> -}       [ ['m','u','r','a','l'], ['w','a','l','l'] ],

    FuCaL |< Iy               `adj`     {- <^gudarIy> -}       [ ['s','m','a','l','l','p','o','x'] ],

    MaFCUL                    `noun`    {- <ma^gdUr> -}        [ unwords [ ['h','a','v','i','n','g'], ['s','m','a','l','l','p','o','x'] ], unwords [ ['p','o','c','k'], "-", ['m','a','r','k','e','d'] ] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^gaddar> -}      [ unwords [ ['h','a','v','i','n','g'], ['s','m','a','l','l','p','o','x'] ], unwords [ ['p','o','c','k'], "-", ['m','a','r','k','e','d'] ] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MuFaCCaL |< aT            `noun`    {- <mu^gaddaraT> -}    [ unwords [ ['m','u','j','a','d','d','a','r','a'], "(", ['c','r','a','c','k','e','d'], ['w','h','e','a','t'], ['c','o','o','k','e','d'], ['w','i','t','h'], ['l','e','n','t','i','l','s'], ['a','n','d'], ['b','r','o','w','n','e','d'], ['c','a','n','d','i','e','d'], ['o','n','i','o','n','s'], ")" ] ] ]


cluster_9   = cluster

 |> "^g f r" <| [

    FaCL                      `noun`    {- <^gafr> -}          [ ['f','o','r','t','u','n','e','t','e','l','l','i','n','g'], ['d','i','v','i','n','a','t','i','o','n'] ],

    FiCL |< Iy                `adj`     {- <^gifrIy> -}        [ ['J','i','f','r','i'] ],

    FuCL |< aT                `noun`    {- <^gufraT> -}        [ ['p','i','t'], ['h','o','l','e'] ]
                              `plural`     FuCaL ]


cluster_10  = cluster

 |> "^g f n" <| [

    FaCL                      `noun`    {- <^gafn> -}          [ ['e','y','e','l','i','d'] ]
                              `plural`     HaFCAL
                              `plural`     FuCUL,

    FaCL |< aT                `noun`    {- <^gafnaT> -}        [ ['b','o','w','l'], ['g','r','a','p','e','v','i','n','e'] ]
                              `plural`     FiCAL
                              `plural`     FaCaL |< At ]


cluster_11  = cluster

 |> "^g f l" <| [

    FaCaL                     `verb`    {- <^gafal> -}         [ unwords [ ['b','e'], ['s','t','a','r','t','l','e','d'] ], unwords [ ['j','u','m','p'], ['w','i','t','h'], ['f','r','i','g','h','t'] ], unwords [ ['b','e'], ['s','h','y'] ] ]
                              `imperf`     FCiL
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gaffal> -}        [ ['s','c','a','r','e'], ['s','t','a','r','t','l','e'] ],

    HaFCaL                    `verb`    {- <'a^gfal> -}        [ ['b','o','l','t'], unwords [ ['b','e'], ['s','t','a','r','t','l','e','d'] ] ],

    FaCL                      `noun`    {- <^gafl> -}          [ ['f','r','i','g','h','t'], ['a','l','a','r','m'] ],

    FuCUL                     `noun`    {- <^gufUl> -}         [ ['f','r','i','g','h','t'], ['a','l','a','r','m'] ] ]


cluster_12  = cluster

 |> "^g h b _d" <| [

    KaRDaS                    `noun`    {- <^gahba_d> -}       [ ['b','r','i','l','l','i','a','n','t'], ['e','r','u','d','i','t','e'], ['g','e','n','i','u','s'] ]
                              `plural`     KaRADiS |< aT
                              `plural`     KiRDIS |< aT
                              `plural`     KiRDiS |< aT
                           
    `derives` otherwise ]


cluster_13  = cluster

 |> "^g h ^s" <| [

    HaFCaL                    `verb`    {- <'a^gha^s> -}       [ ['s','o','b'] ],

    FaCL |< aT                `noun`    {- <^gah^saT> -}       [ ['s','o','b'] ]
                              `plural`     FaCaL |< At,

    HiFCAL                    `noun`    {- <'i^ghA^s> -}       [ ['s','o','b'] ]
                              `plural`     HiFCAL |< At ]


cluster_14  = cluster

 |> "^g h .d" <| [

    HaFCaL                    `verb`    {- <'a^gha.d> -}       [ ['a','b','o','r','t'], ['m','i','s','c','a','r','r','y'] ],

    FiCL                      `noun`    {- <^gih.d> -}         [ unwords [ ['m','i','s','c','a','r','r','i','e','d'], ['f','e','t','u','s'] ] ],

    FaCIL                     `noun`    {- <^gahI.d> -}        [ unwords [ ['m','i','s','c','a','r','r','i','e','d'], ['f','e','t','u','s'] ] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    HiFCAL                    `noun`    {- <'i^ghA.d> -}       [ ['a','b','o','r','t','i','o','n'], ['m','i','s','c','a','r','r','i','a','g','e'] ]
                              `plural`     HiFCAL |< At ]


cluster_15  = cluster

 |> "^g f t" <| [

    FiCL                      `noun`    {- <^gift> -}          [ ['f','o','r','c','e','p','s'], ['t','w','e','e','z','e','r','s'] ] ]


cluster_16  = cluster

 |> "^g h d" <| [

    FaCaL                     `verb`    {- <^gahad> -}         [ ['e','n','d','e','a','v','o','r'], ['s','t','r','i','v','e'] ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- <^gAhad> -}         [ unwords [ ['w','a','g','e'], ['w','a','r'], ['o','r'], ['J','i','h','a','d'], "(", ['a','g','a','i','n','s','t'], ")" ] ],

    HaFCaL                    `verb`    {- <'a^ghad> -}        [ ['f','a','t','i','g','u','e'], ['s','t','r','a','i','n'] ],

    IFtaCaL                   `verb`    {- <i^gtahad> -}       [ unwords [ ['w','o','r','k'], ['h','a','r','d'] ], ['s','t','r','i','v','e'] ],

    FaCL                      `noun`    {- <^gahd> -}          [ ['e','x','e','r','t','i','o','n'], ['e','n','d','e','a','v','o','r'], ['e','f','f','o','r','t','s'] ]
                              `plural`     FuCUL,

    FuCL                      `noun`    {- <^guhd> -}          [ ['s','t','r','a','i','n'], ['e','x','e','r','t','i','o','n'] ],

    FaCIL                     `noun`    {- <^gahId> -}         [ ['s','o','u','g','h','t'], unwords [ ['w','o','r','k','e','d'], ['h','a','r','d'], ['f','o','r'] ] ],

    FiCAL                     `noun`    {- <^gihAd> -}         [ ['J','i','h','a','d'], unwords [ ['h','o','l','y'], ['w','a','r'] ], ['s','t','r','u','g','g','l','e'] ],

    FiCAL |< Iy               `adj`     {- <^gihAdIy> -}       [ ['J','i','h','a','d','i','s','t'], unwords [ ['h','o','l','y'], ['w','a','r','f','a','r','e'] ] ],

    MuFACaL |< aT             `noun`    {- <mu^gAhadaT> -}     [ ['f','i','g','h','t'], ['b','a','t','t','l','e'] ],

    HiFCAL                    `noun`    {- <'i^ghAd> -}        [ ['e','f','f','o','r','t'], ['e','x','e','r','t','i','o','n'] ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< Iy              `adj`     {- <'i^ghAdIy> -}      [ ['s','t','r','e','n','u','o','u','s'], ['g','r','u','e','l','i','n','g'] ],

    IFtiCAL                   `noun`    {- <i^gtihAd> -}       [ ['e','f','f','o','r','t'], unwords [ ['i','n','d','e','p','e','n','d','e','n','t'], ['t','h','i','n','k','i','n','g'] ] ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `noun`    {- <^gAhid> -}         [ ['s','t','r','i','v','i','n','g'], ['e','n','d','e','a','v','o','r','i','n','g'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MaFCUL                    `adj`     {- <ma^ghUd> -}        [ ['e','f','f','o','r','t','s'], ['e','n','d','e','a','v','o','r','s'], unwords [ ['e','x','p','e','n','d','e','d'], "(", ['e','f','f','o','r','t','s'], ")" ] ]
                              `plural`     MaFCUL |< At,

    MuFACiL                   `noun`    {- <mu^gAhid> -}       [ ['f','i','g','h','t','e','r'], ['w','a','r','r','i','o','r'], ['M','u','j','a','h','i','d'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise,

    MuFCiL                    `noun`    {- <mu^ghid> -}        [ ['s','t','r','e','n','u','o','u','s'], ['g','r','u','e','l','i','n','g'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise,

    MuFCaL                    `noun`    {- <mu^ghad> -}        [ ['e','x','h','a','u','s','t','e','d'] ]
                              `plural`     MuFCaL |< Un
                           
    `derives` otherwise,

    MuFtaCiL                  `noun`    {- <mu^gtahid> -}      [ ['i','n','d','u','s','t','r','i','o','u','s'], ['d','i','l','i','g','e','n','t'] ]
                              `plural`     MuFtaCiL |< Un
                           
    `derives` otherwise ]


cluster_17  = cluster

 |> "^g h l" <| [

    FaCiL                     `verb`    {- <^gahil> -}         [ ['i','g','n','o','r','e'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gahhal> -}        [ unwords [ ['c','a','l','l'], ['s','t','u','p','i','d'] ], ['s','t','u','p','e','f','y'] ],

    TaFACaL                   `verb`    {- <ta^gAhal> -}       [ unwords [ ['f','e','i','g','n'], ['i','g','n','o','r','a','n','c','e'], ['o','f'] ], ['d','i','s','r','e','g','a','r','d'] ],

    IstaFCaL                  `verb`    {- <ista^ghal> -}      [ unwords [ ['c','o','n','s','i','d','e','r'], ['i','g','n','o','r','a','n','t'] ] ],

    FaCL                      `noun`    {- <^gahl> -}          [ ['i','g','n','o','r','a','n','c','e'] ],

    FaCAL |< aT               `noun`    {- <^gahAlaT> -}       [ ['i','g','n','o','r','a','n','c','e'] ],

    FaCUL                     `adj`     {- <^gahUl> -}         [ ['i','g','n','o','r','a','n','t'], ['f','o','o','l','i','s','h'] ],

    MaFCaL                    `noun`    {- <ma^ghal> -}        [ unwords [ ['u','n','k','n','o','w','n'], ['t','e','r','r','i','t','o','r','y'] ] ]
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- <ta^ghIl> -}        [ ['s','t','u','l','t','i','f','i','c','a','t','i','o','n'] ]
                              `plural`     TaFCIL |< At,

    TaFACuL                   `noun`    {- <ta^gAhul> -}       [ ['i','g','n','o','r','i','n','g'], ['d','i','s','r','e','g','a','r','d'], unwords [ ['f','e','i','g','n','e','d'], ['i','g','n','o','r','a','n','c','e'] ] ]
                              `plural`     TaFACuL |< At,

    FACiL                     `adj`     {- <^gAhil> -}         [ ['i','g','n','o','r','a','n','t'], ['f','o','o','l','i','s','h'] ]
                              `plural`     FaCaL |< aT
                              `plural`     FuCCaL
                              `plural`     FuCaLA'
                              `plural`     FuCCAL,

    FACiL |< Iy               `adj`     {- <^gAhilIy> -}       [ ['p','a','g','a','n'], unwords [ ['J','a','h','i','l','i'], "(", ['p','r','e'], "-", ['I','s','l','a','m','i','c'], ")" ] ],

    FACiL |< Iy |< aT         `noun`    {- <^gAhilIyaT> -}     [ unwords [ ['J','a','h','i','l','i','y','a'], "(", ['p','r','e'], "-", ['I','s','l','a','m'], ")" ], ['i','g','n','o','r','a','n','c','e'] ],

    MaFCUL                    `adj`     {- <ma^ghUl> -}        [ ['u','n','k','n','o','w','n'], ['u','n','i','d','e','n','t','i','f','i','e','d'], unwords [ ['u','n','i','d','e','n','t','i','f','i','e','d'], "(", ['p','e','r','s','o','n','s'], ")" ] ],

    MaFCUL |< Iy |< aT        `noun`    {- <ma^ghUlIyaT> -}    [ ['a','n','o','n','y','m','i','t','y'] ] ]


cluster_18  = cluster

 |> "^g h m" <| [

    FaCuL                     `verb`    {- <^gahum> -}         [ ['f','r','o','w','n'] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'a^gham> -}        [ ['f','r','o','w','n'], unwords [ ['b','e'], ['g','l','o','o','m','y'] ] ],

    TaFaCCaL                  `verb`    {- <ta^gahham> -}      [ ['s','c','o','w','l'], unwords [ ['b','e'], ['g','l','o','o','m','y'] ] ],

    FaCL                      `noun`    {- <^gahm> -}          [ ['s','u','l','l','e','n'], ['g','l','o','o','m','y'] ],

    FaCAL                     `noun`    {- <^gahAm> -}         [ ['c','l','o','u','d','s'] ],

    FaCAL |< aT               `noun`    {- <^gahAmaT> -}       [ ['s','u','l','l','e','n'], ['g','l','o','o','m','y'] ],

    FuCUL |< aT               `noun`    {- <^guhUmaT> -}       [ ['s','u','l','l','e','n'], ['g','l','o','o','m','y'] ],

    MutaFaCCiL                `noun`    {- <muta^gahhim> -}    [ ['f','r','o','w','n','i','n','g'], ['s','c','o','w','l','i','n','g'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise ]


cluster_19  = cluster

 |> "^g h r" <| [

    FaCaL                     `verb`    {- <^gahar> -}         [ ['a','p','p','e','a','r'] ]
                              `imperf`     FCaL,

    FaCaL                     `verb`    {- <^gahar> -}         [ ['a','n','n','o','u','n','c','e'], ['p','r','o','c','l','a','i','m'] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <^gahur> -}         [ unwords [ ['b','e'], ['l','o','u','d'] ], unwords [ ['b','e'], ['a','u','d','i','b','l','e'] ] ]
                              `imperf`     FCuL,

    FACaL                     `verb`    {- <^gAhar> -}         [ ['c','o','n','f','e','s','s'], ['e','x','p','r','e','s','s'] ],

    FaCL                      `noun`    {- <^gahr> -}          [ ['n','o','t','o','r','i','e','t','y'], ['p','u','b','l','i','c','l','y'], ['o','p','e','n','l','y'] ],

    FiCAL                     `noun`    {- <^gihAr> -}         [ ['n','o','t','o','r','i','e','t','y'], ['p','u','b','l','i','c','l','y'] ],

    FaCL |< Iy                `adj`     {- <^gahrIy> -}        [ ['p','u','b','l','i','c'], unwords [ ['o','u','t'], "-", ['l','o','u','d'] ] ],

    FaCIL                     `noun`    {- <^gahIr> -}         [ ['l','o','u','d'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    HaFCaL                    `noun`    {- <'a^ghar> -}        [ unwords [ ['l','o','u','d','e','r'], "/", ['l','a','u','d','e','s','t'] ] ],

    HaFCaL                    `noun`    {- <'a^ghar> -}        [ unwords [ ['d','a','y'], "-", ['b','l','i','n','d'] ] ],

    MiFCaL                    `noun`    {- <mi^ghar> -}        [ ['m','i','c','r','o','s','c','o','p','e'] ]
                              `plural`     MaFACiL,

    MiFCaL |< Iy              `adj`     {- <mi^gharIy> -}      [ ['m','i','c','r','o','s','c','o','p','i','c'] ],

    MiFCAL                    `noun`    {- <mi^ghAr> -}        [ unwords [ ['l','o','u','d'], "-", ['v','o','i','c','e','d'] ], ['m','e','g','a','p','h','o','n','e'] ]
                              `plural`     MaFACIL,

    MuFACaL |< aT             `noun`    {- <mu^gAharaT> -}     [ ['f','r','a','n','k','n','e','s','s'], ['c','a','n','d','o','r'] ],

    MaFCUL                    `noun`    {- <ma^ghUr> -}        [ ['p','e','r','c','e','p','t','i','b','l','e'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MuFACiL                   `noun`    {- <mu^gAhir> -}       [ ['o','p','e','n','l','y'], ['p','u','b','l','i','c'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise ]


cluster_20  = cluster

 |> "^g k t" <| [

    FACiL                     `noun`    {- <^gAkit> -}         [ ['j','a','c','k','e','t'] ] ]


cluster_21  = cluster

 |> "^g k r" <| [

    FACaL                     `verb`    {- <^gAkar> -}         [ ['t','e','a','s','e'], ['f','o','o','l'] ] ]


cluster_22  = cluster

 |> "^g h z" <| [

    FaCaL                     `verb`    {- <^gahaz> -}         [ unwords [ ['f','i','n','i','s','h'], ['o','f','f'] ], unwords [ ['g','i','v','e'], "a", ['c','o','u','p'], ['d','e'], ['g','r','a','c','e'] ] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gahhaz> -}        [ ['e','q','u','i','p'], ['s','u','p','p','l','y'], ['p','r','e','p','a','r','e'] ],

    HaFCaL                    `verb`    {- <'a^ghaz> -}        [ unwords [ ['f','i','n','i','s','h'], ['o','f','f'] ], unwords [ ['g','i','v','e'], "a", ['c','o','u','p'], ['d','e'], ['g','r','a','c','e'] ] ],

    TaFaCCaL                  `verb`    {- <ta^gahhaz> -}      [ unwords [ ['b','e'], ['e','q','u','i','p','p','e','d'] ], unwords [ ['b','e'], ['f','u','r','n','i','s','h','e','d'] ], unwords [ ['g','e','t'], ['r','e','a','d','y'] ] ],

    FiCAL                     `noun`    {- <^gihAz> -}         [ ['a','p','p','a','r','a','t','u','s'], ['e','q','u','i','p','m','e','n','t'], ['a','p','p','l','i','a','n','c','e'], ['s','y','s','t','e','m'] ]
                              `plural`     HaFCiL |< At |< aT,

    FuCUL |< Iy |< aT         `noun`    {- <^guhUzIyaT> -}     [ ['p','r','e','p','a','r','e','d','n','e','s','s'], ['f','i','t','n','e','s','s'] ],

    TaFCIL                    `noun`    {- <ta^ghIz> -}        [ ['e','q','u','i','p','m','e','n','t'], ['g','e','a','r'] ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- <ta^ghIzIy> -}      [ ['p','r','e','p','a','r','a','t','o','r','y'], ['e','q','u','i','p','p','i','n','g'] ],

    FACiL                     `adj`     {- <^gAhiz> -}         [ ['p','r','e','p','a','r','e','d'], ['e','q','u','i','p','p','e','d'] ],

    FACiL |< Iy |< aT         `noun`    {- <^gAhizIyaT> -}     [ ['r','e','a','d','i','n','e','s','s'], ['p','r','e','p','a','r','e','d','n','e','s','s'] ],

    MuFaCCiL                  `noun`    {- <mu^gahhiz> -}      [ ['s','u','p','p','l','i','e','r'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `adj`     {- <mu^gahhaz> -}      [ ['e','q','u','i','p','p','e','d'], ['s','u','p','p','l','i','e','d'] ] ]


cluster_23  = cluster

 |> "^g l ` d" <| [

    KiRDAS                    `noun`    {- <^gil`Ad> -}        [ ['G','i','l','e','a','d'] ] ]


cluster_24  = cluster

 |> "^g l _h" <| [

    FaCaL                     `verb`    {- <^gala_h> -}        [ ['s','h','a','r','p','e','n'], ['s','t','r','e','t','c','h'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^galla_h> -}       [ ['s','h','a','r','p','e','n'], ['s','t','r','e','t','c','h'] ],

    FaCL                      `noun`    {- <^gal_h> -}         [ ['g','r','i','n','d','s','t','o','n','e'] ] ]


cluster_25  = cluster

 |> "^g l ^g l" <| [

    KaRDaS                    `verb`    {- <^gal^gal> -}       [ ['r','e','s','o','u','n','d'], ['r','e','v','e','r','b','e','r','a','t','e'], ['r','a','t','t','l','e'] ],

    KuRDuS                    `noun`    {- <^gul^gul> -}       [ ['b','e','l','l'], ['j','i','n','g','l','e'] ]
                              `plural`     KaRADiS,

    KaRDaS |< aT              `noun`    {- <^gal^galaT> -}     [ ['j','i','n','g','l','e'], ['r','i','n','g'] ],

    MuKaRDiS                  `noun`    {- <mu^gal^gil> -}     [ ['r','i','n','g','i','n','g'] ]
                              `plural`     MuKaRDiS |< Un
                           
    `derives` otherwise,

    KuRDuS |< aT              `noun`    {- <^gul^gulaT> -}     [ ['G','o','l','g','o','t','h','a'] ] ]


cluster_26  = cluster

 |> "^g l .t n" <| [

    TaKaRDaS                  `verb`    {- <ta^gal.tan> -}     [ ['g','e','l','a','t','i','n','i','z','e'] ] ]


cluster_27  = cluster

 |> "^g l .t" <| [

    FaCaL                     `verb`    {- <^gala.t> -}        [ ['s','h','a','v','e'], ['c','h','a','f','e'] ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <^galla.t> -}       [ ['c','o','a','g','u','l','a','t','e'] ],

    TaFaCCaL                  `verb`    {- <ta^galla.t> -}     [ ['c','o','a','g','u','l','a','t','e'], ['c','o','n','g','e','a','l'] ],

    FuCL |< aT                `noun`    {- <^gul.taT> -}       [ ['c','l','o','t'] ]
                              `plural`     FaCaL |< At,

    TaFaCCuL                  `noun`    {- <ta^gallu.t> -}     [ ['c','o','a','g','u','l','a','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At ]


cluster_28  = cluster

 |> "^g l .s" <| [

    FACUL                     `noun`    {- <^gAlU.s> -}        [ unwords [ ['l','a','r','g','e'], ['h','e','a','p'] ] ],

    FiCAL |< Iy               `adj`     {- <^gilA.sIy> -}      [ ['J','e','l','a','s','s','i'] ] ]


cluster_29  = cluster

 |> "^g l .h" <| [

    FaCiL                     `verb`    {- <^gali.h> -}        [ unwords [ ['b','e','c','o','m','e'], ['b','a','l','d'] ], unwords [ ['l','o','s','e'], ['h','a','i','r'] ] ]
                              `imperf`     FCaL,

    HaFCaL                    `adj`     {- <'a^gla.h> -}       [ ['b','a','l','d'] ]
                              `plural`     FuCL
                              `femini`     FaCLA' ]


cluster_30  = cluster

 |> "^g l b" <| [

    FaCaL                     `verb`    {- <^galab> -}         [ ['b','r','i','n','g'], ['a','t','t','r','a','c','t'] ]
                              `imperf`     FCuL
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <^gallab> -}        [ ['c','l','a','m','o','r'], ['s','h','o','u','t'] ],

    HaFCaL                    `verb`    {- <'a^glab> -}        [ ['e','a','r','n'], ['a','c','q','u','i','r','e'] ],

    InFaCaL                   `verb`    {- <in^galab> -}       [ unwords [ ['b','e'], ['b','r','o','u','g','h','t'] ], unwords [ ['b','e'], ['p','r','o','c','u','r','e','d'] ] ],

    IFtaCaL                   `verb`    {- <i^gtalab> -}       [ ['b','r','i','n','g'], ['p','r','o','c','u','r','e'] ],

    IstaFCaL                  `verb`    {- <ista^glab> -}      [ ['a','t','t','r','a','c','t'], ['f','e','t','c','h'] ],

    FuCAL                     `noun`    {- <^gulAb> -}         [ unwords [ ['r','o','s','e'], ['w','a','t','e','r'] ], ['j','u','l','e','p'] ],

    FuCCAL                    `noun`    {- <^gullAb> -}        [ unwords [ ['r','o','s','e'], ['w','a','t','e','r'] ], ['j','u','l','e','p'] ],

    FaCL                      `noun`    {- <^galb> -}          [ ['a','c','q','u','i','s','i','t','i','o','n'], ['a','r','r','a','i','g','n','m','e','n','t'] ],

    FaCaL                     `noun`    {- <^galab> -}         [ ['c','l','a','m','o','r'] ],

    FuCL |< aT                `noun`    {- <^gulbaT> -}        [ ['s','c','a','r'] ],

    FaCIL                     `noun`    {- <^galIb> -}         [ ['i','m','p','o','r','t','e','d'] ],

    FaCLY                     `noun`    {- <^galbY> -}         [ unwords [ ['f','o','r','e','i','g','n'], ['s','l','a','v','e'] ] ]
                              `plural`     FuCaLA',

    FaCCAL                    `noun`    {- <^gallAb> -}        [ ['a','t','t','r','a','c','t','i','v','e'], ['t','r','a','d','e','r'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FaCCAL |< Iy |< aT        `noun`    {- <^gallAbIyaT> -}    [ unwords [ ['g','a','l','l','a','b','i','y','a'], "(", ['g','o','w','n'], ")" ] ],

    HaFCaL                    `noun`    {- <'a^glab> -}        [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['a','t','t','r','a','c','t','i','v','e'] ] ],

    MaFCaL |< aT              `noun`    {- <ma^glabaT> -}      [ ['c','a','u','s','e'], ['m','o','t','i','v','e'] ]
                              `plural`     MaFACiL,

    IstiFCAL                  `noun`    {- <isti^glAb> -}      [ ['a','c','q','u','i','s','i','t','i','o','n'], ['p','r','o','m','o','t','i','o','n'] ]
                              `plural`     IstiFCAL |< At,

    FACiL                     `noun`    {- <^gAlib> -}         [ ['c','a','u','s','e'], ['m','o','t','i','v','e'] ],

    MaFCUL                    `noun`    {- <ma^glUb> -}        [ ['i','m','p','o','r','t','e','d'], ['e','x','o','t','i','c'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise ]


cluster_31  = cluster

 |> "^g l b t" <| [

    KaRDUS                    `noun`    {- <^galbUt> -}        [ ['b','o','a','t'] ]
                              `plural`     KaRADIS ]


cluster_32  = cluster

 |> "^g l d" <| [

    FaCaL                     `verb`    {- <^galad> -}         [ ['f','l','o','g'] ]
                              `imperf`     FCiL,

    FaCiL                     `verb`    {- <^galid> -}         [ ['f','r','e','e','z','e'] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <^galud> -}         [ unwords [ ['b','e'], ['s','t','e','a','d','f','a','s','t'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gallad> -}        [ ['b','i','n','d'], ['f','r','e','e','z','e'] ],

    FACaL                     `verb`    {- <^gAlad> -}         [ ['f','i','g','h','t'] ],

    HaFCaL                    `verb`    {- <'a^glad> -}        [ ['f','r','e','e','z','e'], unwords [ ['b','e'], ['f','r','o','z','e','n'] ] ],

    TaFaCCaL                  `verb`    {- <ta^gallad> -}      [ unwords [ ['b','e'], ['p','a','t','i','e','n','t'] ], unwords [ ['b','e'], ['f','r','o','z','e','n'] ] ],

    TaFACaL                   `verb`    {- <ta^gAlad> -}       [ unwords [ ['c','r','o','s','s'], ['s','w','o','r','d','s'] ] ],

    FaCL                      `noun`    {- <^gald> -}          [ ['f','l','o','g','g','i','n','g'] ],

    FiCL                      `noun`    {- <^gild> -}          [ ['s','k','i','n'], ['l','e','a','t','h','e','r'] ],

    FiCL |< Iy                `adj`     {- <^gildIy> -}        [ ['d','e','r','m','a','l'], ['c','u','t','a','n','e','o','u','s'] ],

    FuCayL |< aT              `noun`    {- <^gulaydaT> -}      [ ['m','e','m','b','r','a','n','e'], ['f','i','l','m'] ],

    FaCaL                     `noun`    {- <^galad> -}         [ ['e','n','d','u','r','a','n','c','e'], ['p','a','t','i','e','n','c','e'] ],

    FaCL |< aT                `noun`    {- <^galdaT> -}        [ ['l','a','s','h'] ]
                              `plural`     FaCaL |< At,

    FaCIL                     `noun`    {- <^galId> -}         [ ['s','t','e','a','d','f','a','s','t'] ]
                              `plural`     FuCaLA'
                           
    `derives` otherwise,

    FaCIL                     `noun`    {- <^galId> -}         [ ['i','c','e'] ],

    FaCIL |< Iy               `adj`     {- <^galIdIy> -}       [ ['i','c','y'], ['i','c','e'] ],

    FaCUL                     `noun`    {- <^galUd> -}         [ ['p','a','t','i','e','n','t'] ],

    FaCCAL                    `noun`    {- <^gallAd> -}        [ unwords [ ['l','e','a','t','h','e','r'], ['m','e','r','c','h','a','n','t'] ] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    MiFCaL |< aT              `noun`    {- <mi^gladaT> -}      [ ['w','h','i','p'] ],

    TaFCIL                    `noun`    {- <ta^glId> -}        [ ['f','r','e','e','z','i','n','g'], ['b','o','o','k','b','i','n','d','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    FiCAL                     `noun`    {- <^gilAd> -}         [ ['b','a','t','t','l','e'] ],

    TaFaCCuL                  `noun`    {- <ta^gallud> -}      [ ['p','a','t','i','e','n','c','e'], ['e','n','d','u','r','a','n','c','e'] ]
                              `plural`     TaFaCCuL |< At,

    MuFaCCiL                  `noun`    {- <mu^gallid> -}      [ ['b','o','o','k','b','i','n','d','e','r'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^gallad> -}      [ ['f','r','o','z','e','n'], ['b','o','u','n','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MuFACiL                   `noun`    {- <mu^gAlid> -}       [ ['g','l','a','d','i','a','t','o','r'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise,

    MutaFaCCiL                `noun`    {- <muta^gallid> -}    [ ['p','a','t','i','e','n','t'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise ]


cluster_33  = cluster

 |> "^g l f n" <| [

    KaRDaS                    `verb`    {- <^galfan> -}        [ ['g','a','l','v','a','n','i','z','e'] ],

    TaKaRDaS                  `verb`    {- <ta^galfan> -}      [ unwords [ ['b','e'], ['g','a','l','v','a','n','i','z','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <^galfanaT> -}      [ ['g','a','l','v','a','n','i','z','a','t','i','o','n'] ],

    MuKaRDaS                  `noun`    {- <mu^galfan> -}      [ ['g','a','l','v','a','n','i','z','e','d'] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise ]


cluster_34  = cluster

 |> "^g l f .t" <| [

    KaRDaS                    `verb`    {- <^galfa.t> -}       [ ['c','a','u','l','k'] ],

    KaRDaS |< aT              `noun`    {- <^galfa.taT> -}     [ ['c','a','u','l','k','i','n','g'] ] ]


cluster_35  = cluster

 |> "^g l f" <| [

    FaCaL                     `verb`    {- <^galaf> -}         [ ['s','c','r','a','t','c','h'], ['s','c','r','a','p','e'], ['g','r','a','z','e'] ]
                              `imperf`     FCuL,

    InFaCaL                   `verb`    {- <in^galaf> -}       [ unwords [ ['b','e'], ['s','c','r','a','t','c','h','e','d'] ], unwords [ ['b','e'], ['g','r','a','z','e','d'] ] ],

    FiCL                      `noun`    {- <^gilf> -}          [ ['r','u','d','e'] ]
                              `plural`     HaFCAL ]


cluster_36  = cluster

 |> "^g l t n" <| [

    TaKaRDaS                  `verb`    {- <ta^galtan> -}      [ ['g','e','l','a','t','i','n','i','z','e'] ] ]


cluster_37  = cluster

 |> "^g l t" <| [

    FACUL                     `noun`    {- <^gAlUt> -}         [ ['G','o','l','i','a','t','h'] ] ]


cluster_38  = cluster

 |> "^g l s" <| [

    FaCaL                     `verb`    {- <^galas> -}         [ ['s','i','t'] ]
                              `imperf`     FCiL,

    FACaL                     `verb`    {- <^gAlas> -}         [ unwords [ ['s','i','t'], ['w','i','t','h'] ] ],

    HaFCaL                    `verb`    {- <'a^glas> -}        [ ['s','e','a','t'], unwords [ ['m','a','k','e'], ['s','i','t'] ] ],

    FaCL |< aT                `noun`    {- <^galsaT> -}        [ ['s','e','s','s','i','o','n'], ['m','e','e','t','i','n','g'] ]
                              `plural`     FaCaL |< At,

    FaCIL                     `noun`    {- <^galIs> -}         [ ['p','a','r','t','i','c','i','p','a','n','t'], ['c','o','m','p','a','n','i','o','n'], ['s','i','t','t','e','r'] ]
                              `plural`     FuCaLA'
                           
    `derives` otherwise,

    FuCUL                     `noun`    {- <^gulUs> -}         [ ['s','i','t','t','i','n','g'] ],

    FACiL                     `noun`    {- <^gAlis> -}         [ ['s','i','t','t','i','n','g'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MaFCiL                    `noun`    {- <ma^glis> -}        [ ['c','o','u','n','c','i','l'], ['b','o','a','r','d'], ['M','a','j','l','i','s'] ]
                              `plural`     MaFACiL,

    MuFACaL |< aT             `noun`    {- <mu^gAlasaT> -}     [ unwords [ ['s','o','c','i','a','l'], ['i','n','t','e','r','c','o','u','r','s','e'] ] ],

    FuCCAL                    `noun`    {- <^gullAs> -}        [ ['p','a','r','t','i','c','i','p','a','n','t','s'] ] ]


cluster_39  = cluster

 |> "^g l q" <| [

    FuCAL |< aT               `noun`    {- <^gulAqaT> -}       [ ['b','u','m','p','k','i','n'] ] ]


cluster_40  = cluster

 |> "^g l n" <| [

    FACUL                     `noun`    {- <^gAlUn> -}         [ ['g','a','l','l','o','n'] ]
                              `plural`     FACUL |< At,

    FaCUL                     `noun`    {- <^galUn> -}         [ ['g','a','l','l','o','n'] ]
                              `plural`     FaCUL |< At ]


cluster_41  = cluster

 |> "^g l m d" <| [

    TaKaRDaS                  `verb`    {- <ta^galmad> -}      [ unwords [ ['b','e'], ['p','e','t','r','i','f','i','e','d'] ] ],

    KaRDaS                    `noun`    {- <^galmad> -}        [ ['b','o','u','l','d','e','r'], ['r','o','c','k'] ]
                              `plural`     KaRADiS,

    KuRDUS                    `noun`    {- <^gulmUd> -}        [ ['b','o','u','l','d','e','r'], ['r','o','c','k'] ]
                              `plural`     KaRADIS ]


cluster_42  = cluster

 |> "^g l m" <| [

    FaCaL                     `verb`    {- <^galam> -}         [ ['c','l','i','p'], ['s','h','e','a','r'] ]
                              `imperf`     FCiL,

    FaCaL                     `noun`    {- <^galam> -}         [ ['s','h','e','a','r','s'] ]
                              `plural`     HaFCAL ]


cluster_43  = cluster

 |> "^g m .h" <| [

    FaCaL                     `verb`    {- <^gama.h> -}        [ unwords [ ['b','e'], ['d','e','f','i','a','n','t'] ], unwords [ ['b','e'], ['o','b','s','t','i','n','a','t','e'] ] ]
                              `imperf`     FCaL,

    FiCAL                     `noun`    {- <^gimA.h> -}        [ ['d','e','f','i','a','n','c','e'], ['o','b','s','t','i','n','a','c','y'] ],

    FuCUL                     `noun`    {- <^gumU.h> -}        [ ['d','e','f','i','a','n','c','e'], ['o','b','s','t','i','n','a','c','y'] ],

    FaCUL                     `noun`    {- <^gamU.h> -}        [ ['d','e','f','i','a','n','t'], ['o','b','s','t','i','n','a','t','e'] ],

    FACiL                     `noun`    {- <^gAmi.h> -}        [ ['d','e','f','i','a','n','t'], ['o','b','s','t','i','n','a','t','e'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise ]


cluster_44  = cluster

 |> "^g m ^s" <| [

    FaCaL                     `verb`    {- <^gama^s> -}        [ ['s','h','a','v','e'], ['c','a','r','e','s','s'] ]
                              `imperf`     FCiL
                              `imperf`     FCuL,

    FaCL                      `noun`    {- <^gam^s> -}         [ ['s','h','a','v','i','n','g'], ['c','a','r','e','s','s','i','n','g'] ],

    FaCCaL                    `verb`    {- <^gamma^s> -}       [ ['c','a','r','e','s','s'] ],

    TaFCIL                    `noun`    {- <ta^gmI^s> -}       [ ['c','a','r','e','s','s','i','n','g'] ]
                              `plural`     TaFCIL |< At ]


cluster_45  = cluster

 |> "^g m ^g m" <| [

    KaRDaS                    `verb`    {- <^gam^gam> -}       [ ['s','t','a','m','m','e','r'], ['m','u','m','b','l','e'] ],

    TaKaRDaS                  `verb`    {- <ta^gam^gam> -}     [ ['s','t','a','m','m','e','r'], ['m','u','m','b','l','e'] ],

    KaRDaS |< aT              `noun`    {- <^gam^gamaT> -}     [ ['s','t','a','m','m','e','r','i','n','g'], ['m','u','m','b','l','i','n','g'] ],

    KuRDuS |< aT              `noun`    {- <^gum^gumaT> -}     [ ['s','k','u','l','l'], ['c','r','a','n','i','u','m'], ['c','r','a','n','i','a'] ]
                              `plural`     KaRADiS,

    KuRDuS |< Iy              `adj`     {- <^gum^gumIy> -}     [ ['c','r','a','n','i','a','l'] ] ]


cluster_46  = cluster

 |> "^g m `" <| [

    FaCaL                     `verb`    {- <^gama`> -}         [ ['g','a','t','h','e','r'], ['a','s','s','e','m','b','l','e'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gamma`> -}        [ ['a','c','c','u','m','u','l','a','t','e'], ['a','s','s','e','m','b','l','e'] ],

    FACaL                     `verb`    {- <^gAma`> -}         [ unwords [ ['m','a','k','e'], ['l','o','v','e'], ['t','o'] ] ],

    HaFCaL                    `verb`    {- <'a^gma`> -}        [ ['c','o','n','c','u','r'], unwords [ ['a','g','r','e','e'], ['u','n','a','n','i','m','o','u','s','l','y'] ], unwords [ ['b','e'], ['a','g','r','e','e','d'], ['u','n','a','n','i','m','o','u','s','l','y'] ] ],

    TaFaCCaL                  `verb`    {- <ta^gamma`> -}      [ ['g','a','t','h','e','r'], ['r','a','l','l','y'] ],

    IFtaCaL                   `verb`    {- <i^gtama`> -}       [ ['m','e','e','t'], ['c','o','n','f','e','r'] ],

    IstaFCaL                  `verb`    {- <ista^gma`> -}      [ ['g','a','t','h','e','r'], ['s','u','m','m','a','r','i','z','e'] ],

    FaCL                      `noun`    {- <^gam`> -}          [ ['g','a','t','h','e','r','i','n','g'], ['c','o','l','l','e','c','t','i','o','n'], ['j','o','i','n','i','n','g'] ],

    FuCUL                     `noun`    {- <^gumU`> -}         [ ['c','r','o','w','d','s'], ['m','a','s','s','e','s'] ],

    FaCL |< Iy                `adj`     {- <^gam`Iy> -}        [ ['c','o','l','l','e','c','t','i','v','e'] ],

    FuCL |< aT                `noun`    {- <^gum`aT> -}        [ ['F','r','i','d','a','y'] ],

    FaCL |< Iy |< aT          `noun`    {- <^gam`IyaT> -}      [ ['a','s','s','o','c','i','a','t','i','o','n'], ['s','o','c','i','e','t','y'] ],

    FaCIL                     `noun`    {- <^gamI`> -}         [ unwords [ ['a','l','l'], ['o','f'] ], unwords [ ['e','v','e','r','y'], ['o','n','e'], ['o','f'] ], ['e','v','e','r','y','o','n','e'], ['a','l','l'], ['e','n','t','i','r','e','l','y'], ['t','o','g','e','t','h','e','r'] ],

    HaFCaL                    `adj`     {- <'a^gma`> -}        [ ['e','n','t','i','r','e'], ['w','h','o','l','e'] ]
                              `femini`     FaCLA',

    FaCAL |< aT               `noun`    {- <^gamA`aT> -}       [ ['p','a','r','t','y'], ['g','r','o','u','p'], ['s','q','u','a','d'] ],

    FaCAL |< Iy               `adj`     {- <^gamA`Iy> -}       [ ['s','o','c','i','a','l'], ['c','o','m','m','o','n'], ['c','o','l','l','e','c','t','i','v','e'] ],

    FuCLAn                    `noun`    {- <^gum`An> -}        [ ['J','u','m','\'','a','n'] ],

    MaFCaL                    `noun`    {- <ma^gma`> -}        [ ['a','s','s','e','m','b','l','y'], ['a','c','a','d','e','m','y'] ]
                              `plural`     MaFACiL,

    MaFCaL |< Iy              `adj`     {- <ma^gma`Iy> -}      [ ['a','c','a','d','e','m','y'], ['a','c','a','d','e','m','i','c','i','a','n'] ],

    TaFCIL                    `noun`    {- <ta^gmI`> -}        [ ['g','a','t','h','e','r','i','n','g'], ['j','o','i','n','i','n','g'], ['a','s','s','e','m','b','l','e'] ]
                              `plural`     TaFCIL |< At,

    FiCAL                     `noun`    {- <^gimA`> -}         [ unwords [ ['s','e','x','u','a','l'], ['i','n','t','e','r','c','o','u','r','s','e'] ], ['c','o','m','b','i','n','a','t','i','o','n'] ],

    HiFCAL                    `noun`    {- <'i^gmA`> -}        [ ['c','o','n','s','e','n','s','u','s'], ['a','g','r','e','e','m','e','n','t'] ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< Iy              `adj`     {- <'i^gmA`Iy> -}      [ ['u','n','a','n','i','m','o','u','s'], ['c','o','l','l','e','c','t','i','v','e'] ],

    TaFaCCuL                  `noun`    {- <ta^gammu`> -}      [ ['g','a','t','h','e','r','i','n','g'], ['a','s','s','e','m','b','l','y'] ]
                              `plural`     TaFaCCuL |< At,

    IFtiCAL                   `noun`    {- <i^gtimA`> -}       [ ['m','e','e','t','i','n','g'], ['g','a','t','h','e','r','i','n','g'] ]
                              `plural`     IFtiCAL |< At,

    IFtiCAL |< Iy             `adj`     {- <i^gtimA`Iy> -}     [ ['s','o','c','i','a','l'] ],

    lA >| IFtiCAL |< Iy       `adj`     {- <lA-i^gtimA`Iy> -}  [ ['a','n','t','i','s','o','c','i','a','l'], ['a','s','o','c','i','a','l'] ],

    FACiL                     `noun`    {- <^gAmi`> -}         [ ['c','o','m','p','r','e','h','e','n','s','i','v','e'], ['e','x','t','e','n','s','i','v','e'] ],

    FACiL                     `noun`    {- <^gAmi`> -}         [ ['m','o','s','q','u','e'] ]
                              `plural`     FawACiL,

    FACiL |< aT               `noun`    {- <^gAmi`aT> -}       [ ['u','n','i','v','e','r','s','i','t','y'], ['l','e','a','g','u','e'] ],

    FACiL |< Iy               `adj`     {- <^gAmi`Iy> -}       [ ['u','n','i','v','e','r','s','i','t','y'] ],

    MaFCUL                    `noun`    {- <ma^gmU`> -}        [ ['g','a','t','h','e','r','e','d'], ['t','o','t','a','l'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MaFCUL |< aT              `noun`    {- <ma^gmU`aT> -}      [ ['c','o','l','l','e','c','t','i','o','n'], ['g','r','o','u','p'], ['b','l','o','c'] ]
                              `plural`     MaFACIL,

    MuFaCCiL                  `noun`    {- <mu^gammi`> -}      [ ['c','o','l','l','e','c','t','o','r'], ['b','a','t','t','e','r','y'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^gamma`> -}      [ ['c','o','m','b','i','n','e','d'], ['c','o','l','l','e','c','t','i','v','e'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MuFCaL                    `noun`    {- <mu^gma`> -}        [ ['u','n','a','n','i','m','o','u','s'], unwords [ ['i','n'], ['a','g','r','e','e','m','e','n','t'] ] ]
                              `plural`     MuFCaL |< Un
                           
    `derives` otherwise,

    MuFtaCiL                  `noun`    {- <mu^gtami`> -}      [ ['p','a','r','t','i','c','i','p','a','n','t'], ['a','s','s','e','m','b','l','a','g','e'] ]
                              `plural`     MuFtaCiL |< Un
                           
    `derives` otherwise,

    MuFtaCaL                  `noun`    {- <mu^gtama`> -}      [ ['s','o','c','i','e','t','y'] ]
                              `plural`     MuFtaCaL |< At,

    MuFtaCaL |< Iy            `adj`     {- <mu^gtama`Iy> -}    [ ['s','o','c','i','e','t','a','l'] ] ]


cluster_47  = cluster

 |> "^g m d" <| [

    FaCaL                     `verb`    {- <^gamad> -}         [ ['f','r','e','e','z','e'], unwords [ ['b','e','c','o','m','e'], ['h','a','r','d'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gammad> -}        [ ['f','r','e','e','z','e'], ['h','a','r','d','e','n'] ],

    TaFaCCaL                  `verb`    {- <ta^gammad> -}      [ unwords [ ['b','e'], ['s','t','i','f','f'] ], ['h','a','r','d','e','n'] ],

    InFaCaL                   `verb`    {- <in^gamad> -}       [ ['f','r','e','e','z','e'] ],

    FaCL                      `noun`    {- <^gamd> -}          [ ['f','r','e','e','z','i','n','g'] ],

    FaCAL                     `noun`    {- <^gamAd> -}         [ ['s','o','l','i','d'], ['i','n','o','r','g','a','n','i','c'] ]
                              `plural`     FaCAL |< At,

    FuCAL                     `noun`    {- <^gumAd> -}         [ ['J','u','m','a','d'] ],

    FuCALY                    `noun`    {- <^gumAdY> -}        [ unwords [ ['J','u','m','a','d','a'], "(", ['p','a','r','t'], ['o','f'], ['5','t','h'], ['a','n','d'], ['6','t','h'], ['m','o','n','t','h'], ['n','a','m','e','s'], ['i','n'], ['I','s','l','a','m','i','c'], ['c','a','l','e','n','d','a','r'], ")" ], unwords [ ['J','u','m','a','d','a','s'], "(", ['5','t','h'], ['a','n','d'], ['6','t','h'], ['m','o','n','t','h','s'], ['i','n'], ['I','s','l','a','m','i','c'], ['c','a','l','e','n','d','a','r'], ")" ] ]
                              `plural`     FuCALY |< At,

    FuCUL                     `noun`    {- <^gumUd> -}         [ ['f','r','e','e','z','i','n','g'], ['i','n','e','r','t','i','a'], ['d','e','a','d','l','o','c','k'] ],

    FuCUL |< aT               `noun`    {- <^gumUdaT> -}       [ ['s','o','l','i','d','i','t','y'] ],

    TaFCIL                    `noun`    {- <ta^gmId> -}        [ ['f','r','e','e','z','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- <ta^gammud> -}      [ ['f','r','e','e','z','i','n','g'] ]
                              `plural`     TaFaCCuL |< At,

    InFiCAL                   `noun`    {- <in^gimAd> -}       [ ['f','r','e','e','z','i','n','g'] ]
                              `plural`     InFiCAL |< At,

    FACiL                     `noun`    {- <^gAmid> -}         [ ['f','r','o','z','e','n'], ['r','i','g','i','d'], ['s','o','l','i','d','s'], ['i','n','o','r','g','a','n','i','c'] ]
                              `plural`     FACiL |< Un
                              `plural`     FawACiL
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^gammad> -}      [ ['f','r','o','z','e','n'], ['h','a','r','d','e','n','e','d'], unwords [ ['f','r','o','z','e','n'], ['f','o','o','d','s'] ] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MutaFaCCiL                `noun`    {- <muta^gammid> -}    [ ['f','r','o','z','e','n'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise,

    MunFaCiL                  `noun`    {- <mun^gamid> -}      [ ['f','r','o','z','e','n'] ]
                              `plural`     MunFaCiL |< Un
                           
    `derives` otherwise ]


cluster_48  = cluster

 |> "^g m b r" <| [

    KaRDaS |< Iy              `adj`     {- <^gambarIy> -}      [ ['s','h','r','i','m','p'] ] ]


cluster_49  = cluster

 |> "^g m k" <| [

    FACaL |< Iy |< aT         `noun`    {- <^gAmakIyaT> -}     [ ['p','a','y'] ]
                              `plural`     FawACiL ]


cluster_50  = cluster

 |> "^g m h r" <| [

    KaRDaS                    `verb`    {- <^gamhar> -}        [ ['a','s','s','e','m','b','l','e'], ['g','a','t','h','e','r'] ],

    TaKaRDaS                  `verb`    {- <ta^gamhar> -}      [ ['a','s','s','e','m','b','l','e'], ['g','a','t','h','e','r'] ],

    KaRDaS |< aT              `noun`    {- <^gamharaT> -}      [ ['g','a','t','h','e','r','i','n','g'], ['t','h','r','o','n','g'] ],

    TaKaRDuS                  `noun`    {- <ta^gamhur> -}      [ ['g','a','t','h','e','r','i','n','g'], ['c','r','o','w','d'] ]
                              `plural`     TaKaRDuS |< At,

    KuRDUS                    `noun`    {- <^gumhUr> -}        [ ['m','u','l','t','i','t','u','d','e'], ['p','u','b','l','i','c'], ['m','a','s','s','e','s'] ]
                              `plural`     KaRADIS,

    KuRDUS |< Iy              `adj`     {- <^gumhUrIy> -}      [ ['r','e','p','u','b','l','i','c','a','n'], ['r','e','p','u','b','l','i','c'] ],

    KaRADIS |< Iy             `adj`     {- <^gamAhIrIy> -}     [ ['m','a','s','s'], ['t','h','r','o','n','g'] ],

    KaRADIS |< Iy |< aT       `noun`    {- <^gamAhIrIyaT> -}   [ ['J','a','m','a','h','i','r','i','y','a'] ] ]


cluster_51  = cluster

 |> "^g m l" <| [

    FaCaL                     `verb`    {- <^gamal> -}         [ ['s','u','m','m','a','r','i','z','e'] ]
                              `imperf`     FCuL,

    FaCuL                     `verb`    {- <^gamul> -}         [ unwords [ ['b','e'], ['b','e','a','u','t','i','f','u','l'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gammal> -}        [ ['e','m','b','e','l','l','i','s','h'], ['a','d','o','r','n'] ],

    FACaL                     `verb`    {- <^gAmal> -}         [ unwords [ ['b','e'], ['c','o','u','r','t','e','o','u','s'] ], unwords [ ['b','e'], ['a','m','i','a','b','l','e'] ] ],

    HaFCaL                    `verb`    {- <'a^gmal> -}        [ ['a','d','d'], ['s','u','m','m','a','r','i','z','e'] ],

    TaFaCCaL                  `verb`    {- <ta^gammal> -}      [ unwords [ ['a','d','o','r','n'], ['o','n','e','s','e','l','f'] ] ],

    TaFACaL                   `verb`    {- <ta^gAmal> -}       [ unwords [ ['b','e'], ['c','o','u','r','t','e','o','u','s'] ] ],

    FuCL |< aT                `noun`    {- <^gumlaT> -}        [ ['s','e','n','t','e','n','c','e'], ['c','l','a','u','s','e'] ]
                              `plural`     FuCaL,

    FuCL |< aT |<< "aN"       `noun`    {- <^gumlaTaN> -}      [ ['c','o','m','p','l','e','t','e','l','y'], unwords [ ['o','n'], ['t','h','e'], ['w','h','o','l','e'] ] ],

    FuCL |< aT |<< "a"        `prep`    {- <^gumlaTa> -}       [ unwords [ ['a','l','l'], ['o','f'] ] ],

    FaCaL                     `noun`    {- <^gamal> -}         [ ['c','a','m','e','l'] ]
                              `plural`     FiCAL
                              `plural`     HaFCAL,

    FaCaL                     `noun`    {- <^gamal> -}         [ ['J','a','m','a','l'], ['G','a','m','a','l'] ],

    FaCCAL                    `noun`    {- <^gammAl> -}        [ unwords [ ['c','a','m','e','l'], ['d','r','i','v','e','r'] ] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FaCAL                     `noun`    {- <^gamAl> -}         [ ['J','a','m','a','l'], ['G','a','m','a','l'] ],

    FaCAL                     `noun`    {- <^gamAl> -}         [ ['b','e','a','u','t','y'] ],

    FaCAL |< Iy               `adj`     {- <^gamAlIy> -}       [ ['a','e','s','t','h','e','t','i','c'] ],

    FaCIL                     `noun`    {- <^gamIl> -}         [ ['b','e','a','u','t','i','f','u','l'], ['n','i','c','e'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    FaCIL                     `noun`    {- <^gamIl> -}         [ ['J','a','m','e','e','l'], ['J','a','m','i','l'], ['G','a','m','e','e','l'] ],

    HaFCaL                    `noun`    {- <'a^gmal> -}        [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['b','e','a','u','t','i','f','u','l'] ] ],

    TaFCIL                    `noun`    {- <ta^gmIl> -}        [ ['e','m','b','e','l','l','i','s','h','m','e','n','t'] ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- <ta^gmIlIy> -}      [ ['c','o','s','m','e','t','i','c'] ],

    MuFACaL |< aT             `noun`    {- <mu^gAmalaT> -}     [ ['c','o','u','r','t','e','s','y'], ['f','l','a','t','t','e','r','y'] ],

    HiFCAL                    `noun`    {- <'i^gmAl> -}        [ ['s','u','m','m','a','t','i','o','n'] ]
                              `plural`     HiFCAL |< At,

    HiFCAL |<< "aN"           `noun`    {- <'i^gmAlaN> -}      [ unwords [ ['i','n'], ['g','e','n','e','r','a','l'] ], unwords [ ['o','n'], ['t','h','e'], ['w','h','o','l','e'] ] ],

    HiFCAL |< Iy              `adj`     {- <'i^gmAlIy> -}      [ ['c','o','m','p','r','e','h','e','n','s','i','v','e'], ['c','o','l','l','e','c','t','i','v','e'], ['f','u','l','l'] ],

    HiFCAL |< Iy              `noun`    {- <'i^gmAlIy> -}      [ ['t','o','t','a','l'], unwords [ ['f','u','l','l'], ['a','m','o','u','n','t'] ], ['t','o','t','a','l','i','t','y'] ],

    MuFCiL                    `noun`    {- <mu^gmil> -}        [ ['w','h','o','l','e','s','a','l','e','r'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise,

    MuFCaL                    `noun`    {- <mu^gmal> -}        [ ['s','u','m','m','a','r','y'], ['t','o','t','a','l'] ]
                              `plural`     MuFCaL |< At ]


cluster_52  = cluster

 |> "^g m r k" <| [

    KuRDuS                    `noun`    {- <^gumruk> -}        [ ['c','u','s','t','o','m','s'], ['c','o','n','t','r','o','l'], ['t','a','r','i','f','f','s'] ]
                              `plural`     KaRADiS,

    KuRDuS |< Iy              `adj`     {- <^gumrukIy> -}      [ ['c','u','s','t','o','m','s'], ['t','a','r','i','f','f'] ],

    MuKaRDaS                  `noun`    {- <mu^gamrak> -}      [ unwords [ ['d','u','t','y'], ['p','a','i','d'] ] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise ]


cluster_53  = cluster

 |> "^g m r" <| [

    FaCCaL                    `verb`    {- <^gammar> -}        [ ['r','o','a','s','t'] ],

    IFtaCaL                   `verb`    {- <i^gtamar> -}       [ unwords [ ['b','u','r','n'], ['i','n','c','e','n','s','e'] ] ],

    FaCL                      `noun`    {- <^gamr> -}          [ ['e','m','b','e','r'], ['p','e','b','b','l','e'] ]
                              `plural`     FaCaL |< At,

    FuCCAL                    `noun`    {- <^gummAr> -}        [ unwords [ ['p','a','l','m'], ['c','o','r','e'] ] ],

    MiFCaL |< aT              `noun`    {- <mi^gmaraT> -}      [ ['b','r','a','z','i','e','r'] ]
                              `plural`     MaFACiL ]


cluster_54  = cluster

 |> "^g m n" <| [

    FuCAL                     `noun`    {- <^gumAn> -}         [ ['J','u','m','a','n'] ],

    FuCAL                     `noun`    {- <^gumAn> -}         [ ['p','e','a','r','l'] ],

    FuCAL |< aT               `noun`    {- <^gumAnaT> -}       [ ['J','u','m','a','n','a'] ] ]


cluster_55  = cluster

 |> "^g n .h" <| [

    FaCaL                     `verb`    {- <^gana.h> -}        [ ['i','n','c','l','i','n','e'], ['t','e','n','d'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^ganna.h> -}       [ ['i','n','c','l','i','n','e'], unwords [ ['g','i','v','e'], ['w','i','n','g','s'] ] ],

    HaFCaL                    `verb`    {- <'a^gna.h> -}       [ unwords [ ['b','e'], ['i','n','c','l','i','n','e','d'] ], ['t','e','n','d'], unwords [ ['b','e'], ['s','t','r','a','n','d','e','d'] ] ],

    FiCL                      `noun`    {- <^gin.h> -}         [ ['d','a','r','k','n','e','s','s'] ],

    FiCL                      `noun`    {- <^gin.h> -}         [ ['s','i','d','e'] ],

    FuCL |< aT                `noun`    {- <^gun.haT> -}       [ ['m','i','s','d','e','m','e','a','n','o','r'] ]
                              `plural`     FuCaL,

    FaCAL                     `noun`    {- <^ganA.h> -}        [ ['w','i','n','g'], ['f','l','a','n','k'] ]
                              `plural`     HaFCuL
                              `plural`     HaFCiL |< aT,

    FuCAL                     `noun`    {- <^gunA.h> -}        [ ['m','i','s','d','e','m','e','a','n','o','r'] ],

    FuCAL |< Iy               `adj`     {- <^gunA.hIy> -}      [ ['d','e','l','i','n','q','u','e','n','t'], ['c','r','i','m','i','n','a','l'] ],

    HaFCaL                    `noun`    {- <'a^gna.h> -}       [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['i','n','c','l','i','n','e','d'] ] ],

    FaCUL                     `noun`    {- <^ganU.h> -}        [ ['i','n','c','l','i','n','e','d'] ],

    FuCUL                     `noun`    {- <^gunU.h> -}        [ ['i','n','c','l','i','n','a','t','i','o','n'], ['t','e','n','d','e','n','c','y'] ],

    FACiL                     `noun`    {- <^gAni.h> -}        [ ['w','i','n','g'], ['f','l','a','n','k'] ]
                              `plural`     FawACiL,

    FACiL                     `noun`    {- <^gAni.h> -}        [ ['d','e','l','i','n','q','u','e','n','t'], ['c','r','i','m','i','n','a','l'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    FACiL |< aT               `noun`    {- <^gAni.haT> -}      [ ['r','i','b'], ['i','n','s','i','d','e'], ['h','e','a','r','t'], ['s','o','u','l'] ]
                              `plural`     FawACiL,

    MuFaCCaL                  `adj`     {- <mu^ganna.h> -}     [ ['w','i','n','g','e','d'], ['f','l','a','n','k','e','d'] ] ]


cluster_56  = cluster

 |> "^g m s" <| [

    FACUL                     `noun`    {- <^gAmUs> -}         [ ['b','u','f','f','a','l','o'] ],

    FACUL |< aT               `noun`    {- <^gAmUsaT> -}       [ unwords [ ['b','u','f','f','a','l','o'], ['c','o','w'] ], ['b','u','f','f','a','l','o','e','s'] ]
                              `plural`     FawACIL ]


cluster_57  = cluster

 |> "^g n b" <| [

    FaCaL                     `verb`    {- <^ganab> -}         [ ['a','v','e','r','t'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gannab> -}        [ unwords [ ['h','e','l','p'], ['a','v','o','i','d'] ], unwords [ ['s','h','i','e','l','d'], ['f','r','o','m'] ] ],

    FACaL                     `verb`    {- <^gAnab> -}         [ ['f','l','a','n','k'], ['s','k','i','r','t'], ['a','v','o','i','d'] ],

    TaFaCCaL                  `verb`    {- <ta^gannab> -}      [ ['a','v','o','i','d'] ],

    TaFACaL                   `verb`    {- <ta^gAnab> -}       [ ['a','v','o','i','d'] ],

    IFtaCaL                   `verb`    {- <i^gtanab> -}       [ ['a','v','o','i','d'] ],

    FaCL |<< "a"              `prep`    {- <^ganba> -}         [ unwords [ ['n','e','x','t'], ['t','o'] ] ],

    FaCL                      `noun`    {- <^ganb> -}          [ ['s','i','d','e'] ]
                              `plural`     HaFCAL,

    FaCL |< aT                `noun`    {- <^ganbaT> -}        [ ['s','i','d','e'] ]
                              `plural`     FaCaL |< At,

    FaCL |< Iy                `adj`     {- <^ganbIy> -}        [ ['l','a','t','e','r','a','l'] ],

    FaCL |< Iy |< aT          `noun`    {- <^ganbIyaT> -}      [ ['d','a','g','g','e','r'] ],

    FaCAL                     `noun`    {- <^ganAb> -}         [ unwords [ ['R','i','g','h','t'], ['H','o','n','o','r','a','b','l','e'] ] ],

    FaCAL |< aT               `noun`    {- <^ganAbaT> -}       [ unwords [ ['r','i','t','u','a','l'], ['i','m','p','u','r','i','t','y'] ] ],

    FaCUL                     `noun`    {- <^ganUb> -}         [ ['s','o','u','t','h'] ],

    FaCUL |< Iy               `adj`     {- <^ganUbIy> -}       [ ['s','o','u','t','h','e','r','n'], ['s','o','u','t','h'] ],

    FACiL                     `noun`    {- <^gAnib> -}         [ ['s','i','d','e'], ['a','s','p','e','c','t'] ]
                              `plural`     FawACiL,

    FACiL |< Iy               `adj`     {- <^gAnibIy> -}       [ ['s','i','d','e'], ['l','a','t','e','r','a','l'], ['m','a','r','g','i','n','a','l'] ],

    HaFCaL |< Iy              `adj`     {- <'a^gnabIy> -}      [ ['f','o','r','e','i','g','n'] ]
                              `plural`     HaFCaL |< Iy |< Un
                              `plural`     HaFACiL,

    HaFCaL |< Iy              `noun`    {- <'a^gnabIy> -}      [ ['f','o','r','e','i','g','n','e','r'] ]
                              `plural`     HaFCaL |< Iy |< Un
                              `plural`     HaFACiL
                           
    `derives` otherwise,

    FaCCAL |< Iy |< aT        `noun`    {- <^gannAbIyaT> -}    [ ['e','m','b','a','n','k','m','e','n','t'], unwords [ ['s','i','d','e'], ['c','h','a','n','n','e','l'] ] ],

    TaFCIL                    `noun`    {- <ta^gnIb> -}        [ unwords [ ['h','e','l','p','i','n','g'], ['a','v','o','i','d'] ], unwords [ ['s','h','i','e','l','d','i','n','g'], ['f','r','o','m'] ] ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- <ta^gannub> -}      [ ['a','v','o','i','d','a','n','c','e'], ['a','v','o','i','d','i','n','g'] ]
                              `plural`     TaFaCCuL |< At,

    IFtiCAL                   `noun`    {- <i^gtinAb> -}       [ ['a','v','o','i','d','a','n','c','e'] ]
                              `plural`     IFtiCAL |< At,

    MuFaCCiL |< aT            `noun`    {- <mu^gannibaT> -}    [ ['f','l','a','n','k'], unwords [ ['w','i','n','g'], "(", ['m','i','l','.'], ")" ] ] ]


cluster_58  = cluster

 |> "^g n f .s" <| [

    KuRDAS                    `noun`    {- <^gunfA.s> -}       [ ['r','a','g'], ['s','a','c','k','c','l','o','t','h'] ] ]


cluster_59  = cluster

 |> "^g n f" <| [

    TaFACaL                   `verb`    {- <ta^gAnaf> -}       [ ['d','e','v','i','a','t','e'], ['i','n','c','l','i','n','e'] ] ]


cluster_60  = cluster

 |> "^g n d r" <| [

    KaRDaS                    `verb`    {- <^gandar> -}        [ ['r','e','t','o','u','c','h'], ['m','a','n','g','l','e'] ],

    KiRDAS |< Iy              `adj`     {- <^gindArIy> -}      [ unwords [ ['s','t','a','n','d','a','r','d'], "-", ['b','e','a','r','e','r'] ], ['c','o','r','n','e','t','i','s','t'] ]
                              `plural`     KaRADiS |< aT
                              `plural`     KiRDAS |< Iy |< Un ]


cluster_61  = cluster

 |> "^g n d l" <| [

    KaRDaS                    `verb`    {- <^gandal> -}        [ unwords [ ['b','r','i','n','g'], ['d','o','w','n'] ], ['f','e','l','l'] ],

    KaRDaS                    `noun`    {- <^gandal> -}        [ ['c','a','t','a','r','a','c','t'] ]
                              `plural`     KaRDaS |< At
                              `plural`     KaRADiS,

    KuRDUS                    `noun`    {- <^gundUl> -}        [ ['g','o','n','d','o','l','a'] ]
                              `plural`     KuRDUS |< At
                              `plural`     KaRADIS ]


cluster_62  = cluster

 |> "^g n d b" <| [

    KuRDuS                    `noun`    {- <^gundub> -}        [ ['g','r','a','s','s','h','o','p','p','e','r'] ]
                              `plural`     KaRADiS ]


cluster_63  = cluster

 |> "^g n d" <| [

    FaCCaL                    `verb`    {- <^gannad> -}        [ ['r','e','c','r','u','i','t'], ['e','n','l','i','s','t'], ['m','o','b','i','l','i','z','e'] ],

    TaFaCCaL                  `verb`    {- <ta^gannad> -}      [ unwords [ ['b','e'], ['r','e','c','r','u','i','t','e','d'] ], unwords [ ['b','e'], ['e','n','l','i','s','t','e','d'] ], unwords [ ['b','e'], ['m','o','b','i','l','i','z','e','d'] ] ],

    FuCL                      `noun`    {- <^gund> -}          [ ['a','r','m','y'] ],

    FuCL |< Iy                `noun`    {- <^gundIy> -}        [ ['s','o','l','d','i','e','r'], ['p','r','i','v','a','t','e'] ]
                              `plural`     FuCUL
                              `plural`     HaFCAL
                           
    `derives` otherwise,

    FuCL |< Iy |< aT          `noun`    {- <^gundIyaT> -}      [ ['m','i','l','i','t','a','r','i','s','m'], unwords [ ['m','i','l','i','t','a','r','y'], ['a','r','t'] ] ],

    TaFCIL                    `noun`    {- <ta^gnId> -}        [ ['r','e','c','r','u','i','t','m','e','n','t'], ['e','n','l','i','s','t','m','e','n','t'], ['m','o','b','i','l','i','z','a','t','i','o','n'] ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- <ta^gannud> -}      [ unwords [ ['m','i','l','i','t','a','r','y'], ['s','e','r','v','i','c','e'] ] ]
                              `plural`     TaFaCCuL |< At,

    MuFaCCaL                  `adj`     {- <mu^gannad> -}      [ ['r','e','c','r','u','i','t','e','d'], ['e','n','l','i','s','t','e','d'], ['r','e','c','r','u','i','t'], ['d','r','a','f','t','e','e'], ['c','o','n','s','c','r','i','p','t'] ] ]


cluster_64  = cluster

 |> "^g n b z" <| [

    KuRDAS                    `noun`    {- <^gunbAz> -}        [ ['g','y','m','n','a','s','t','i','c','s'], ['c','a','l','i','s','t','h','e','n','i','c','s'] ],

    KuRDAS |< Iy              `adj`     {- <^gunbAzIy> -}      [ ['g','y','m','n','a','s','t','i','c'], ['c','a','l','i','s','t','h','e','n','i','c'] ] ]


cluster_65  = cluster

 |> "^g n b r" <| [

    KaRDaS |< Iy              `adj`     {- <^ganbarIy> -}      [ ['s','h','r','i','m','p'] ] ]


cluster_66  = cluster

 |> "^g n b n" <| [

    KaRDUS                    `noun`    {- <^ganbUn> -}        [ unwords [ ['h','a','m'], "(", ['F','r','.'], ['j','a','m','b','o','n'], ")" ] ] ]


cluster_67  = cluster

 |> "^g n k" <| [

    FuCL                      `noun`    {- <^gunk> -}          [ ['h','a','r','p'] ]
                              `plural`     FuCUL,

    FuCL |< Iy                `adj`     {- <^gunkIy> -}        [ ['h','a','r','p','i','s','t'] ] ]


cluster_68  = cluster

 |> "^g n q" <| [

    FaCaL                     `verb`    {- <^ganaq> -}         [ ['c','a','t','a','p','u','l','t'] ]
                              `imperf`     FCiL ]


cluster_69  = cluster

 |> "^g n s" <| [

    FaCCaL                    `verb`    {- <^gannas> -}        [ ['c','l','a','s','s','i','f','y'], ['h','o','m','o','g','e','n','i','z','e'], ['a','s','s','i','m','i','l','a','t','e'] ],

    FACaL                     `verb`    {- <^gAnas> -}         [ unwords [ ['b','e'], ['r','e','l','a','t','e','d'], ['t','o'] ], unwords [ ['b','e'], ['o','f'], ['s','a','m','e'], ['k','i','n','d'], ['a','s'] ] ],

    TaFaCCaL                  `verb`    {- <ta^gannas> -}      [ unwords [ ['b','e'], ['n','a','t','u','r','a','l','i','z','e','d'] ] ],

    TaFACaL                   `verb`    {- <ta^gAnas> -}       [ unwords [ ['b','e'], ['h','o','m','o','g','e','n','o','u','s'] ], unwords [ ['b','e'], ['r','e','l','a','t','e','d'] ] ],

    FiCL                      `noun`    {- <^gins> -}          [ ['g','e','n','d','e','r'], ['s','e','x'] ]
                              `plural`     HaFCAL,

    FiCL                      `noun`    {- <^gins> -}          [ ['t','y','p','e'], ['k','i','n','d'] ]
                              `plural`     HaFCAL,

    FiCL |< Iy                `adj`     {- <^ginsIy> -}        [ ['s','e','x','u','a','l'] ],

    FiCL |< Iy                `adj`     {- <^ginsIy> -}        [ ['e','t','h','n','i','c'], ['r','a','c','i','a','l'], ['n','a','t','i','o','n','a','l'] ],

    FiCL |< Iy |< aT          `noun`    {- <^ginsIyaT> -}      [ ['n','a','t','i','o','n','a','l','i','t','y'], ['c','i','t','i','z','e','n','s','h','i','p'] ],

    lA >| FiCL |< Iy |< aT    `noun`    {- <lA-^ginsIyaT> -}   [ ['s','t','a','t','e','l','e','s','s','n','e','s','s'] ],

    TaFCIL                    `noun`    {- <ta^gnIs> -}        [ ['n','a','t','u','r','a','l','i','z','a','t','i','o','n'] ]
                              `plural`     TaFCIL |< At,

    FiCAL                     `noun`    {- <^ginAs> -}         [ ['a','s','s','o','n','a','n','c','e'], ['p','u','n'] ],

    MuFACaL |< aT             `noun`    {- <mu^gAnasaT> -}     [ ['s','i','m','i','l','a','r','i','t','y'], ['r','e','s','e','m','b','l','a','n','c','e'] ],

    TaFaCCuL                  `noun`    {- <ta^gannus> -}      [ ['n','a','t','u','r','a','l','i','z','a','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At,

    TaFACuL                   `noun`    {- <ta^gAnus> -}       [ ['h','o','m','o','g','e','n','e','i','t','y'], ['r','e','s','e','m','b','l','a','n','c','e'] ]
                              `plural`     TaFACuL |< At,

    MuFaCCiL                  `noun`    {- <mu^gannis> -}      [ ['m','o','n','g','r','e','l'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFACiL                   `noun`    {- <mu^gAnis> -}       [ ['h','o','m','o','g','e','n','e','o','u','s'], ['r','e','l','a','t','e','d'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise,

    MutaFaCCiL                `noun`    {- <muta^gannis> -}    [ ['n','a','t','u','r','a','l','i','z','e','d'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise,

    MutaFACiL                 `noun`    {- <muta^gAnis> -}     [ ['h','o','m','o','g','e','n','e','o','u','s'], ['r','e','l','a','t','e','d'] ]
                              `plural`     MutaFACiL |< Un
                           
    `derives` otherwise ]


cluster_70  = cluster

 |> "^g n z" <| [

    FaCCaL                    `verb`    {- <^gannaz> -}        [ ['b','u','r','y'] ],

    FiCAL |< aT               `noun`    {- <^ginAzaT> -}       [ ['f','u','n','e','r','a','l'] ]
                              `plural`     FiCAL |< At
                              `plural`     FaCA'iL,

    FiCAL |< Iy               `adj`     {- <^ginAzIy> -}       [ ['f','u','n','e','r','a','r','y'] ],

    FuCCAL                    `noun`    {- <^gunnAz> -}        [ ['f','u','n','e','r','a','l'] ]
                              `plural`     FaCACIL,

    TaFCIL                    `noun`    {- <ta^gnIz> -}        [ ['b','u','r','y','i','n','g'] ]
                              `plural`     TaFCIL |< At ]


cluster_71  = cluster

 |> "^g n z r" <| [

    KaRDaS                    `verb`    {- <^ganzar> -}        [ unwords [ ['b','e'], ['v','e','r','d','i','g','r','i','s'] ] ],

    KiRDAS                    `noun`    {- <^ginzAr> -}        [ ['v','e','r','d','i','g','r','i','s'], ['c','o','p','p','e','r'] ],

    KiRDIS                    `noun`    {- <^ginzIr> -}        [ ['c','h','a','i','n'], ['t','r','a','c','k'] ]
                              `plural`     KaRADIS,

    KiRDIS                    `noun`    {- <^ginzIr> -}        [ unwords [ ['j','i','n','z','e','e','r'], "(", ['c','.'], ['2','0'], ['m','e','t','e','r','s'], ")" ], unwords [ ['j','i','n','z','e','e','r','s'], "(", ['c','.'], ['2','0'], ['m','e','t','e','r','s'], ")" ] ]
                              `plural`     KaRADIS,

    KaRDUS |< Iy              `adj`     {- <^ganzUrIy> -}      [ ['J','a','n','z','o','u','r','i'] ],

    MuKaRDiS                  `noun`    {- <mu^ganzir> -}      [ unwords [ ['t','r','a','c','k'], "-", ['l','a','y','i','n','g'] ] ]
                              `plural`     MuKaRDiS |< Un
                           
    `derives` otherwise,

    MuKaRDaS                  `noun`    {- <mu^ganzar> -}      [ ['t','r','a','c','k','e','d'], unwords [ ['h','a','v','i','n','g'], ['t','r','a','c','k','s'], "(", ['v','e','h','i','c','l','e'], ")" ] ] ]


cluster_72  = cluster

 |> "^g r ^g" <| [

    FaCAL                     `noun`    {- <^garA^g> -}        [ ['g','a','r','a','g','e'] ]
                              `plural`     FaCAL |< At ]


cluster_73  = cluster

 |> "^g r .h" <| [

    FaCaL                     `verb`    {- <^gara.h> -}        [ ['w','o','u','n','d'], ['i','n','j','u','r','e'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^garra.h> -}       [ ['c','h','a','l','l','e','n','g','e'], ['i','n','v','a','l','i','d','a','t','e'] ],

    InFaCaL                   `verb`    {- <in^gara.h> -}      [ unwords [ ['b','e'], ['w','o','u','n','d','e','d'] ], unwords [ ['b','e'], ['i','n','j','u','r','e','d'] ] ],

    IFtaCaL                   `verb`    {- <i^gtara.h> -}      [ ['c','o','m','m','i','t'] ],

    FuCL                      `noun`    {- <^gur.h> -}         [ ['w','o','u','n','d'], ['i','n','j','u','r','y'] ]
                              `plural`     HaFCAL
                              `plural`     FiCAL
                              `plural`     FuCUL
                              `plural`     FuCUL |< At,

    FaCCAL                    `noun`    {- <^garrA.h> -}       [ ['s','u','r','g','e','o','n'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FaCCAL                    `noun`    {- <^garrA.h> -}       [ ['J','a','r','r','a','h'] ],

    FiCAL |< aT               `noun`    {- <^girA.haT> -}      [ ['s','u','r','g','e','r','y'] ],

    FiCAL |< Iy               `adj`     {- <^girA.hIy> -}      [ ['s','u','r','g','i','c','a','l'] ],

    FaCIL                     `noun`    {- <^garI.h> -}        [ ['w','o','u','n','d','e','d'], ['i','n','j','u','r','e','d'] ]
                              `plural`     FaCLY
                           
    `derives` otherwise,

    TaFCIL                    `noun`    {- <ta^grI.h> -}       [ ['s','u','r','g','e','r','y'], ['s','l','a','n','d','e','r'] ]
                              `plural`     TaFCIL |< At,

    FACiL                     `noun`    {- <^gAri.h> -}        [ ['i','n','j','u','r','i','n','g'], ['p','r','e','d','a','t','o','r','y'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    FACiL |< aT               `noun`    {- <^gAri.haT> -}      [ ['p','r','e','d','a','t','o','r','y'], ['e','x','t','r','e','m','i','t','i','e','s'] ]
                              `plural`     FawACiL,

    MaFCUL                    `noun`    {- <ma^grU.h> -}       [ ['w','o','u','n','d','e','d'], ['i','n','j','u','r','e','d'] ]
                              `plural`     MaFCUL |< Un
                              `plural`     MaFACIL
                           
    `derives` otherwise ]


cluster_74  = cluster

 |> "^g r .d" <| [

    FaCiL                     `verb`    {- <^gari.d> -}        [ ['c','h','o','k','e'], unwords [ ['b','e'], ['u','p','s','e','t'] ], unwords [ ['b','e'], ['m','o','v','e','d'] ] ]
                              `imperf`     FCaL,

    FaCaL                     `verb`    {- <^gara.d> -}        [ ['c','h','o','k','e'], ['s','u','f','f','o','c','a','t','e'] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'a^gra.d> -}       [ ['a','l','a','r','m'], ['u','n','n','e','r','v','e'] ],

    FaCIL                     `noun`    {- <^garI.d> -}        [ ['c','h','o','k','i','n','g'] ] ]


cluster_75  = cluster

 |> "^g r _t m" <| [

    TaKaRDaS                  `verb`    {- <ta^gar_tam> -}     [ unwords [ ['t','a','k','e'], ['r','o','o','t'] ], ['g','e','r','m','i','n','a','t','e'] ],

    KuRDUS                    `noun`    {- <^gur_tUm> -}       [ ['g','e','r','m'], ['m','i','c','r','o','b','e'] ]
                              `plural`     KaRADIS,

    KuRDUS |< Iy              `adj`     {- <^gur_tUmIy> -}     [ ['g','e','r','m'], ['b','a','c','t','e','r','i','a','l'] ] ]


cluster_76  = cluster

 |> "^g r _d" <| [

    FuCaL                     `noun`    {- <^gura_d> -}        [ ['r','a','t'] ]
                              `plural`     FiCLAn
                              `plural`     FaCALIn
                              `plural`     FuCLAn ]


cluster_77  = cluster

 |> "^g r ^s" <| [

    FaCaL                     `verb`    {- <^gara^s> -}        [ ['g','r','a','t','e'], ['c','r','u','s','h'], ['g','r','i','n','d'] ]
                              `imperf`     FCuL,

    FaCL                      `noun`    {- <^gar^s> -}         [ ['g','r','a','t','i','n','g'], ['s','c','r','a','p','i','n','g'] ],

    FaCIL                     `noun`    {- <^garI^s> -}        [ ['c','r','u','s','h','e','d'], ['g','r','o','u','n','d'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    FaCCAL                    `noun`    {- <^garrA^s> -}       [ ['c','r','u','s','h','e','r'], ['g','r','i','n','d','e','r'] ]
                              `plural`     FaCCAL |< At,

    FACUL |< aT               `noun`    {- <^gArU^saT> -}      [ ['q','u','e','r','n'], unwords [ ['h','a','n','d'], ['m','i','l','l'] ] ]
                              `plural`     FawACIL,

    FaCaL                     `noun`    {- <^gara^s> -}        [ ['J','e','r','a','s','h'], ['J','a','r','a','s','h'] ] ]


cluster_78  = cluster

 |> "^g r ^g s" <| [

    KiRDiS                    `noun`    {- <^gir^gis> -}       [ ['G','i','r','g','i','s'] ] ]


cluster_79  = cluster

 |> "^g r ^g r" <| [

    KaRDaS                    `verb`    {- <^gar^gar> -}       [ ['g','a','r','g','l','e'], ['d','r','a','g'] ],

    TaKaRDaS                  `verb`    {- <ta^gar^gar> -}     [ unwords [ ['b','e'], ['g','a','r','g','l','e','d'] ], unwords [ ['b','e'], ['d','r','a','g','g','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <^gar^garaT> -}     [ ['g','a','r','g','l','i','n','g'], ['c','l','a','t','t','e','r'] ],

    KiRDiS                    `noun`    {- <^gir^gir> -}       [ ['b','e','a','n','s'] ],

    KiRDIS                    `noun`    {- <^gir^gIr> -}       [ ['w','a','t','e','r','c','r','e','s','s'] ] ]


cluster_80  = cluster

 |> "^g r b _d" <| [

    KaRDaS |< aT              `noun`    {- <^garba_daT> -}     [ ['s','w','i','n','d','l','e'], ['d','e','c','e','p','t','i','o','n'] ],

    KuRDuS                    `noun`    {- <^gurbu_d> -}       [ ['i','m','p','o','s','t','o','r'], ['s','w','i','n','d','l','e','r'] ]
                              `plural`     KaRADiS |< aT ]


cluster_81  = cluster

 |> "^g r b" <| [

    FaCiL                     `verb`    {- <^garib> -}         [ unwords [ ['b','e'], ['m','a','n','g','y'] ], ['f','a','d','e'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^garrab> -}        [ ['t','e','s','t'], ['t','r','y'], ['a','t','t','e','m','p','t'] ],

    FaCaL                     `noun`    {- <^garab> -}         [ ['m','a','n','g','e'], ['s','c','a','b','i','e','s'] ],

    FaCiL                     `noun`    {- <^garib> -}         [ ['m','a','n','g','y'], ['s','c','a','b','b','y'] ]
                              `plural`     FaCiL |< Un
                           
    `derives` otherwise,

    HaFCaL                    `adj`     {- <'a^grab> -}        [ ['m','a','n','g','y'], ['s','c','a','b','b','y'] ]
                              `plural`     FuCL
                              `plural`     FaCLAn |< aT
                              `femini`     FaCLA',

    FiCAL                     `noun`    {- <^girAb> -}         [ ['s','a','c','k'], ['c','a','s','e'] ]
                              `plural`     FuCuL
                              `plural`     HaFCiL |< aT,

    FuCAL                     `noun`    {- <^gurAb> -}         [ ['s','t','o','c','k','i','n','g','s'] ]
                              `plural`     FuCAL |< At,

    FaCIL                     `noun`    {- <^garIb> -}         [ unwords [ ['p','a','t','c','h'], ['o','f'], ['l','a','n','d'] ] ]
                              `plural`     FaCIL |< At,

    TaFCiL |< aT              `noun`    {- <ta^gribaT> -}      [ ['e','x','p','e','r','i','e','n','c','e'], ['e','x','p','e','r','i','m','e','n','t'] ]
                              `plural`     TaFACiL,

    TaFCIL                    `noun`    {- <ta^grIb> -}        [ ['t','r','i','a','l'], ['t','e','s','t'] ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< Iy              `adj`     {- <ta^grIbIy> -}      [ ['t','r','i','a','l'], ['e','x','p','e','r','i','m','e','n','t','a','l'] ],

    MuFaCCiL                  `noun`    {- <mu^garrib> -}      [ ['t','e','s','t','e','r'], ['e','x','p','e','r','i','m','e','n','t','e','r'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^garrab> -}      [ ['t','e','s','t','e','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise ]


cluster_82  = cluster

 |> "^g r `" <| [

    FaCaL                     `verb`    {- <^gara`> -}         [ ['a','b','s','o','r','b'], ['s','w','a','l','l','o','w'], ['d','e','v','o','u','r'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^garra`> -}        [ unwords [ ['m','a','k','e'], ['s','w','a','l','l','o','w'] ], unwords [ ['g','u','l','p'], ['d','o','w','n'] ] ],

    TaFaCCaL                  `verb`    {- <ta^garra`> -}      [ ['s','w','a','l','l','o','w'], ['d','r','i','n','k'] ],

    IFtaCaL                   `verb`    {- <i^gtara`> -}       [ ['s','w','a','l','l','o','w'], unwords [ ['g','u','l','p'], ['d','o','w','n'] ] ],

    FuCL |< aT                `noun`    {- <^gur`aT> -}        [ ['d','o','s','a','g','e'], ['g','u','l','p'] ] ]


cluster_83  = cluster

 |> "^g r d" <| [

    FaCaL                     `verb`    {- <^garad> -}         [ ['r','e','m','o','v','e'], ['s','t','r','i','p'], unwords [ ['t','a','k','e'], ['s','t','o','c','k'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^garrad> -}        [ ['r','e','m','o','v','e'], ['s','t','r','i','p'] ],

    TaFaCCaL                  `verb`    {- <ta^garrad> -}      [ unwords [ ['b','e'], ['s','t','r','i','p','p','e','d'] ], ['r','e','n','o','u','n','c','e'] ],

    FaCL                      `noun`    {- <^gard> -}          [ ['i','n','v','e','n','t','o','r','y'], ['s','t','o','c','k','t','a','k','i','n','g'] ],

    FaCiL                     `noun`    {- <^garid> -}         [ ['b','a','r','r','e','n'] ]
                              `plural`     FuCL,

    HaFCaL                    `adj`     {- <'a^grad> -}        [ ['b','a','r','r','e','n'] ]
                              `femini`     FaCLA',

    FaCAL                     `noun`    {- <^garAd> -}         [ ['l','o','c','u','s','t'] ],

    FaCIL                     `noun`    {- <^garId> -}         [ unwords [ ['p','a','l','m'], ['b','r','a','n','c','h','e','s'] ], ['j','a','v','e','l','i','n'] ],

    FaCIL |< aT               `noun`    {- <^garIdaT> -}       [ ['n','e','w','s','p','a','p','e','r'] ]
                              `plural`     FaCA'iL,

    TaFCIL                    `noun`    {- <ta^grId> -}        [ ['s','t','r','i','p','p','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    TaFCIL |< aT              `noun`    {- <ta^grIdaT> -}      [ unwords [ ['m','i','l','i','t','a','r','y'], ['d','e','t','a','c','h','m','e','n','t'] ], unwords [ ['e','x','p','e','d','i','t','i','o','n','a','r','y'], ['f','o','r','c','e'] ] ]
                              `plural`     TaFACIL,

    TaFCIL |< Iy              `adj`     {- <ta^grIdIy> -}      [ ['a','b','s','t','r','a','c','t'] ],

    TaFaCCuL                  `noun`    {- <ta^garrud> -}      [ ['f','r','e','e','d','o','m'], ['a','b','s','t','r','a','c','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At,

    MuFaCCaL                  `noun`    {- <mu^garrad> -}      [ unwords [ ['n','o','t','h','i','n','g'], ['b','u','t'] ], ['m','e','r','e'], unwords [ ['f','o','r'], ['n','o'], ['r','e','a','s','o','n'], ['e','x','c','e','p','t'] ], unwords [ ['f','o','r'], ['t','h','e'], ['s','o','l','e'], ['r','e','a','s','o','n'] ] ],

    MuFaCCaL                  `noun`    {- <mu^garrad> -}      [ ['b','a','r','e'], ['p','u','r','e'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MutaFaCCiL                `noun`    {- <muta^garrid> -}    [ ['i','m','p','a','r','t','i','a','l'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise,

    FuCUL                     `noun`    {- <^gurUd> -}         [ unwords [ ['b','a','r','r','e','n'], ['a','r','e','a','s'] ] ] ]


cluster_84  = cluster

 |> "^g r b z" <| [

    KaRDaS |< aT              `noun`    {- <^garbazaT> -}      [ ['s','w','i','n','d','l','e'], ['d','e','c','e','p','t','i','o','n'] ],

    KuRDuS                    `noun`    {- <^gurbuz> -}        [ ['i','m','p','o','s','t','o','r'], ['s','w','i','n','d','l','e','r'] ]
                              `plural`     KaRADiS |< aT ]


cluster_85  = cluster

 |> "^g r b `" <| [

    KaRDUS                    `noun`    {- <^garbU`> -}        [ ['j','e','r','b','o','a'] ]
                              `plural`     KaRADIS ]


cluster_86  = cluster

 |> "^g r k s" <| [

    KaRDaS                    `noun`    {- <^garkas> -}        [ ['C','i','r','c','a','s','s','i','a','n','s'] ],

    KaRDaS |< Iy              `adj`     {- <^garkasIy> -}      [ ['C','i','r','c','a','s','s','i','a','n'] ]
                              `plural`     KaRDaS |< Iy |< Un
                              `plural`     KaRADiS |< aT ]


cluster_87  = cluster

 |> "^g r f" <| [

    FaCaL                     `verb`    {- <^garaf> -}         [ unwords [ ['t','e','a','r'], ['o','u','t'] ], ['r','e','m','o','v','e'], unwords [ ['s','w','e','e','p'], ['a','w','a','y'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^garraf> -}        [ unwords [ ['c','a','r','r','y'], ['a','w','a','y'] ], unwords [ ['w','a','s','h'], ['a','w','a','y'] ] ],

    InFaCaL                   `verb`    {- <in^garaf> -}       [ unwords [ ['b','e'], ['s','w','e','p','t'], ['a','w','a','y'] ], unwords [ ['b','e'], ['c','a','r','r','i','e','d'], ['a','w','a','y'] ] ],

    IFtaCaL                   `verb`    {- <i^gtaraf> -}       [ unwords [ ['c','a','r','r','y'], ['a','w','a','y'] ], unwords [ ['w','a','s','h'], ['a','w','a','y'] ], unwords [ ['t','e','a','r'], ['o','u','t'] ] ],

    FaCL                      `noun`    {- <^garf> -}          [ ['s','h','o','v','e','l','i','n','g'], ['d','r','e','d','g','i','n','g'] ],

    FuCL                      `noun`    {- <^gurf> -}          [ ['b','l','u','f','f'], ['c','l','i','f','f'] ]
                              `plural`     FuCUL,

    FaCCAL |< aT              `noun`    {- <^garrAfaT> -}      [ ['r','a','k','e'], ['b','u','l','l','d','o','z','e','r'] ]
                              `plural`     FaCACIL,

    MaFCaL                    `noun`    {- <ma^graf> -}        [ ['t','o','r','r','e','n','t'] ],

    MiFCaL |< aT              `noun`    {- <mi^grafaT> -}      [ ['s','h','o','v','e','l'], ['t','r','o','w','e','l'], ['s','c','o','o','p'] ]
                              `plural`     MiFCAL
                              `plural`     MaFACIL,

    TaFCIL                    `noun`    {- <ta^grIf> -}        [ unwords [ ['c','a','r','r','y','i','n','g'], ['a','w','a','y'] ], unwords [ ['w','a','s','h','i','n','g'], ['a','w','a','y'] ] ]
                              `plural`     TaFCIL |< At,

    InFiCAL                   `noun`    {- <in^girAf> -}       [ ['e','r','o','s','i','o','n'] ]
                              `plural`     InFiCAL |< At,

    FACiL                     `noun`    {- <^gArif> -}         [ ['t','o','r','r','e','n','t','i','a','l'], ['s','t','o','r','m','y'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise ]


cluster_88  = cluster

 |> "^g r d l" <| [

    KaRDaS                    `noun`    {- <^gardal> -}        [ ['b','u','c','k','e','t'], ['p','a','i','l'] ]
                              `plural`     KaRADiS ]


cluster_89  = cluster

 |> "^g r n l" <| [

    KuRDAS                    `noun`    {- <^gurnAl> -}        [ ['j','o','u','r','n','a','l'], ['n','e','w','s','p','a','p','e','r'] ]
                              `plural`     KaRADIS ]


cluster_90  = cluster

 |> "^g r n" <| [

    FuCL                      `noun`    {- <^gurn> -}          [ ['b','a','s','i','n'], ['m','o','r','t','a','r'] ]
                              `plural`     HaFCAL ]


cluster_91  = cluster

 |> "^g r m z" <| [

    KaRADiS                   `noun`    {- <^garAmiz> -}       [ ['l','e','g','s'], ['l','i','m','b','s'] ]
                              `plural`     KaRADIS ]


cluster_92  = cluster

 |> "^g r m" <| [

    FaCaL                     `verb`    {- <^garam> -}         [ ['i','n','j','u','r','e'], ['w','r','o','n','g'], ['b','o','n','e'] ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <^garram> -}        [ ['c','r','i','m','i','n','a','l','i','z','e'], ['i','n','c','r','i','m','i','n','a','t','e'] ],

    HaFCaL                    `verb`    {- <'a^gram> -}        [ unwords [ ['c','o','m','m','i','t'], "a", ['c','r','i','m','e'] ], ['s','i','n'], unwords [ ['d','o'], ['w','r','o','n','g'] ], unwords [ ['b','e'], ['s','i','n','n','e','d'], ['a','g','a','i','n','s','t'] ], unwords [ ['b','e'], ['d','o','n','e'], ['w','r','o','n','g'] ] ],

    IFtaCaL                   `verb`    {- <i^gtaram> -}       [ ['i','n','c','r','i','m','i','n','a','t','e'] ],

    FaCL                      `noun`    {- <^garm> -}          [ ['b','a','r','g','e'], ['c','r','i','m','e','s'] ]
                              `plural`     FuCUL,

    FiCL                      `noun`    {- <^girm> -}          [ ['b','o','d','y'], ['b','u','l','k'], ['m','a','s','s'], ['c','r','i','m','e','s'] ]
                              `plural`     HaFCAL,

    FuCL                      `noun`    {- <^gurm> -}          [ ['c','r','i','m','e'], ['s','i','n'] ],

    FaCIL |< aT               `noun`    {- <^garImaT> -}       [ ['c','r','i','m','e'] ]
                              `plural`     FaCA'iL,

    TaFCIL                    `noun`    {- <ta^grIm> -}        [ ['c','r','i','m','i','n','a','l','i','z','a','t','i','o','n'], ['i','n','c','r','i','m','i','n','a','t','i','o','n'] ]
                              `plural`     TaFCIL |< At,

    HiFCAL                    `noun`    {- <'i^grAm> -}        [ ['d','e','l','i','n','q','u','e','n','c','y'], ['c','r','i','m','i','n','a','l','i','t','y'] ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< Iy              `adj`     {- <'i^grAmIy> -}      [ ['c','r','i','m','i','n','a','l'] ],

    MuFCiL                    `noun`    {- <mu^grim> -}        [ ['c','r','i','m','i','n','a','l'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise ]


cluster_93  = cluster

 |> "^g r s n" <| [

    KaRDUS                    `noun`    {- <^garsUn> -}        [ ['w','a','i','t','e','r'] ]
                              `plural`     KaRDUS |< At,

    KaRDUS |< aT              `noun`    {- <^garsUnaT> -}      [ ['w','a','i','t','r','e','s','s'] ] ]


cluster_94  = cluster

 |> "^g r s" <| [

    FaCaL                     `verb`    {- <^garas> -}         [ ['r','i','n','g'], ['t','o','l','l'] ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <^garras> -}        [ unwords [ ['m','a','k','e'], ['i','n','u','r','e'] ], unwords [ ['s','o','r','e','l','y'], ['t','r','y'] ], ['d','i','s','c','r','e','d','i','t'] ],

    FaCL                      `noun`    {- <^gars> -}          [ ['s','o','u','n','d'], ['t','o','n','e'] ],

    FaCaL                     `noun`    {- <^garas> -}         [ ['b','e','l','l'] ]
                              `plural`     HaFCAL,

    FuCL |< aT                `noun`    {- <^gursaT> -}        [ ['d','e','f','a','m','a','t','i','o','n'], ['s','c','a','n','d','a','l'] ],

    FiCL |< Iy |< aT          `noun`    {- <^girsIyaT> -}      [ ['j','e','r','s','e','y'] ] ]


cluster_95  = cluster

 |> "^g r z" <| [

    FaCaL                     `verb`    {- <^garaz> -}         [ ['a','n','n','i','h','i','l','a','t','e'], unwords [ ['c','u','t'], ['o','f','f'] ] ]
                              `imperf`     FCuL,

    FuCL |< aT                `noun`    {- <^gurzaT> -}        [ ['b','u','n','d','l','e'], ['p','o','n','y','t','a','i','l'] ]
                              `plural`     FuCaL ]


cluster_96  = cluster

 |> "^g s d n" <| [

    KuRDAS                    `noun`    {- <^gusdAn> -}        [ ['b','o','d','y'] ],

    KuRDAS |< Iy              `adj`     {- <^gusdAnIy> -}      [ ['b','o','d','i','l','y'] ] ]


cluster_97  = cluster

 |> "^g s d" <| [

    FaCCaL                    `verb`    {- <^gassad> -}        [ ['m','a','t','e','r','i','a','l','i','z','e'], ['e','m','b','o','d','y'] ],

    TaFaCCaL                  `verb`    {- <ta^gassad> -}      [ ['m','a','t','e','r','i','a','l','i','z','e'], unwords [ ['b','e'], ['e','m','b','o','d','i','e','d'] ] ],

    FaCaL                     `noun`    {- <^gasad> -}         [ ['b','o','d','y'] ]
                              `plural`     HaFCAL,

    FaCaL |< Iy               `adj`     {- <^gasadIy> -}       [ ['b','o','d','i','l','y'] ],

    FuCLAn                    `noun`    {- <^gusdAn> -}        [ ['b','o','d','y'] ],

    FuCLAn |< Iy              `adj`     {- <^gusdAnIy> -}      [ ['b','o','d','i','l','y'] ],

    TaFCIL                    `noun`    {- <ta^gsId> -}        [ ['m','a','t','e','r','i','a','l','i','z','a','t','i','o','n'], ['e','m','b','o','d','i','m','e','n','t'] ]
                              `plural`     TaFCIL |< At,

    TaFaCCuL                  `noun`    {- <ta^gassud> -}      [ ['m','a','t','e','r','i','a','l','i','z','a','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At,

    MuFaCCaL                  `noun`    {- <mu^gassad> -}      [ ['e','m','b','o','d','i','e','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    MutaFaCCiL                `noun`    {- <muta^gassid> -}    [ ['c','o','r','p','o','r','e','a','l'], ['i','n','c','a','r','n','a','t','e'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise ]


cluster_98  = cluster

 |> "^g s .t n" <| [

    TaKaRDaS                  `verb`    {- <ta^gas.tan> -}     [ ['r','e','l','a','x'], ['l','o','u','n','g','e'], unwords [ ['l','o','l','l'], ['b','a','c','k'] ] ] ]


cluster_99  = cluster

 |> "^g s m n" <| [

    KuRDAS                    `noun`    {- <^gusmAn> -}        [ ['b','o','d','y'], ['m','a','s','s'] ],

    KuRDAS |< Iy              `adj`     {- <^gusmAnIy> -}      [ ['b','o','d','i','l','y'], ['p','h','y','s','i','c','a','l'] ],

    KaRDAS |< Iy |< aT        `noun`    {- <^gasmAnIyaT> -}    [ ['G','e','t','h','s','e','m','a','n','e'] ] ]


cluster_100 = cluster

 |> "^g s m" <| [

    FaCuL                     `verb`    {- <^gasum> -}         [ unwords [ ['b','e'], ['l','a','r','g','e'] ], unwords [ ['b','e'], ['b','u','l','k','y'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gassam> -}        [ ['m','a','t','e','r','i','a','l','i','z','e'], ['e','n','l','a','r','g','e'] ],

    TaFaCCaL                  `verb`    {- <ta^gassam> -}      [ unwords [ ['t','a','k','e'], ['s','h','a','p','e'] ], unwords [ ['b','e'], ['m','a','t','e','r','i','a','l','i','z','e','d'] ] ],

    FiCL                      `noun`    {- <^gism> -}          [ ['b','o','d','y'], ['f','o','r','m'] ]
                              `plural`     HaFCAL,

    FiCL |< Iy                `adj`     {- <^gismIy> -}        [ ['b','o','d','i','l','y'], ['m','a','t','e','r','i','a','l'] ],

    FaCIL                     `noun`    {- <^gasIm> -}         [ ['g','r','e','a','t'], ['h','u','g','e'] ]
                              `plural`     FiCAL
                           
    `derives` otherwise,

    FuCayL                    `noun`    {- <^gusaym> -}        [ ['p','a','r','t','i','c','l','e'], ['c','o','r','p','u','s','c','l','e'] ]
                              `plural`     FuCayL |< At,

    HaFCaL                    `noun`    {- <'a^gsam> -}        [ ['l','a','r','g','e','r'], unwords [ ['m','o','r','e'], ['c','o','r','p','u','l','e','n','t'] ] ],

    FaCAL |< aT               `noun`    {- <^gasAmaT> -}       [ ['s','i','z','e'], ['c','o','r','p','u','l','e','n','c','e'] ],

    FuCLAn                    `noun`    {- <^gusmAn> -}        [ ['b','o','d','y'], ['m','a','s','s'] ],

    FuCLAn |< Iy              `adj`     {- <^gusmAnIy> -}      [ ['b','o','d','i','l','y'], ['p','h','y','s','i','c','a','l'] ],

    FaCLAn |< Iy |< aT        `noun`    {- <^gasmAnIyaT> -}    [ ['G','e','t','h','s','e','m','a','n','e'] ],

    TaFCIL                    `noun`    {- <ta^gsIm> -}        [ ['e','m','b','o','d','i','m','e','n','t'], ['e','n','l','a','r','g','e','m','e','n','t'] ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL                  `noun`    {- <mu^gassam> -}      [ ['b','o','d','i','l','y'], ['e','m','b','o','d','i','e','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise,

    FACiL                     `noun`    {- <^gAsim> -}         [ ['J','a','s','e','m'], ['J','a','s','i','m'] ] ]


cluster_101 = cluster

 |> "^g s r" <| [

    FaCaL                     `verb`    {- <^gasar> -}         [ ['v','e','n','t','u','r','e'], ['r','i','s','k'] ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- <^gasar> -}         [ ['c','r','o','s','s'], ['s','p','a','n'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <^gassar> -}        [ ['e','n','c','o','u','r','a','g','e'] ],

    TaFACaL                   `verb`    {- <ta^gAsar> -}       [ unwords [ ['b','e'], ['i','n','s','o','l','e','n','t'] ], ['d','a','r','e'] ],

    IFtaCaL                   `verb`    {- <i^gtasar> -}       [ ['c','r','o','s','s'], ['s','p','a','n'], ['t','r','a','v','e','r','s','e'] ],

    FiCL                      `noun`    {- <^gisr> -}          [ ['b','r','i','d','g','e'], unwords [ "(", ['r','e','i','n','f','o','r','c','e','m','e','n','t'], ")", ['b','e','a','m'], ['o','r'], ['b','a','r'] ], unwords [ "(", ['r','e','i','n','f','o','r','c','e','m','e','n','t'], ")", ['b','e','a','m','s'], ['o','r'], ['b','a','r','s'] ] ]
                              `plural`     FuCUL,

    HaFCuL                    `noun`    {- <'a^gsur> -}        [ ['b','r','i','d','g','e','s'] ],

    FaCUL                     `noun`    {- <^gasUr> -}         [ ['b','o','l','d'], ['d','a','r','i','n','g'] ]
                              `plural`     FaCUL |< Un
                           
    `derives` otherwise,

    FaCAL |< aT               `noun`    {- <^gasAraT> -}       [ ['b','o','l','d','n','e','s','s'], ['i','n','s','o','l','e','n','c','e'] ],

    TaFACuL                   `noun`    {- <ta^gAsur> -}       [ ['b','o','l','d','n','e','s','s'], ['i','n','s','o','l','e','n','c','e'] ]
                              `plural`     TaFACuL |< At,

    MutaFACiL                 `noun`    {- <muta^gAsir> -}     [ ['b','o','l','d'], ['i','m','p','u','d','e','n','t'] ]
                              `plural`     MutaFACiL |< Un
                           
    `derives` otherwise,

    FACiL                     `noun`    {- <^gAsir> -}         [ ['J','a','s','i','r'] ] ]


cluster_102 = cluster

 |> "^guwIl" <| [

    _____                     `noun`    {- <^guwIl> -}         [ ['J','o','e','l'] ] ]


cluster_103 = cluster

 |> "^gUn" <| [

    _____                     `noun`    {- <^gUn> -}           [ ['J','o','h','n'] ] ]



 |> "^guwAn" <| [

    _____                     `noun`    {- <^guwAn> -}         [ ['J','u','n','e'] ] ]


cluster_104 = cluster

 |> "^gU" <| [

    _____                     `noun`    {- <^gU> -}            [ ['J','o','e'] ] ]


cluster_105 = cluster

 |> "^gIb" <| [

    _____                     `noun`    {- <^gIb> -}           [ ['j','e','e','p'] ]
                              `plural`     _____ |< At ]


cluster_106 = cluster

 |> "^gIn" <| [

    _____                     `noun`    {- <^gIn> -}           [ ['g','e','n','e'] ]
                              `plural`     _____ |< At ]


cluster_107 = cluster

 |> "^g z ^g" <| [

    HaFCAL |< Iy              `adj`     {- <'a^gzA^gIy> -}     [ ['p','h','a','r','m','a','c','i','s','t'] ] ]


cluster_108 = cluster

 |> "^g z f" <| [

    FaCaL                     `verb`    {- <^gazaf> -}         [ unwords [ ['b','e'], ['r','e','c','k','l','e','s','s'] ], ['r','i','s','k'] ]
                              `imperf`     FCuL,

    FACaL                     `verb`    {- <^gAzaf> -}         [ unwords [ ['a','c','t'], ['r','a','n','d','o','m','l','y'] ], ['s','p','e','c','u','l','a','t','e'] ],

    FuCAL                     `noun`    {- <^guzAf> -}         [ ['p','u','r','c','h','a','s','e'] ],

    FuCAL |<< "aN"            `noun`    {- <^guzAfaN> -}       [ ['r','a','n','d','o','m','l','y'] ],

    MuFACaL |< aT             `noun`    {- <mu^gAzafaT> -}     [ ['r','e','c','k','l','e','s','s','n','e','s','s'], ['h','a','z','a','r','d'] ],

    MuFACiL                   `adj`     {- <mu^gAzif> -}       [ ['r','e','c','k','l','e','s','s'], ['v','e','n','t','u','r','e','s','o','m','e'] ] ]


cluster_109 = cluster

 |> "^g z d n" <| [

    KuRDAS                    `noun`    {- <^guzdAn> -}        [ ['w','a','l','l','e','t'] ]
                              `plural`     KuRDAS |< At ]


cluster_110 = cluster

 |> "^g z d" <| [

    FuCLAn                    `noun`    {- <^guzdAn> -}        [ ['w','a','l','l','e','t'] ]
                              `plural`     FuCLAn |< At ]


cluster_111 = cluster

 |> "^g z `" <| [

    FaCiL                     `verb`    {- <^gazi`> -}         [ unwords [ ['b','e'], ['c','o','n','c','e','r','n','e','d'] ], unwords [ ['b','e'], ['w','o','r','r','i','e','d'] ], ['r','e','g','r','e','t'] ]
                              `imperf`     FCaL,

    FaCaL                     `verb`    {- <^gaza`> -}         [ ['t','r','a','v','e','r','s','e'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <^gazza`> -}        [ ['l','a','c','e','r','a','t','e'], ['m','a','r','b','l','e'], ['b','r','e','a','k'] ],

    TaFaCCaL                  `verb`    {- <ta^gazza`> -}      [ unwords [ ['b','e'], ['b','r','o','k','e','n'] ], unwords [ ['b','r','e','a','k'], ['a','p','a','r','t'] ], ['s','n','a','p'] ],

    FaCL                      `noun`    {- <^gaz`> -}          [ ['o','n','y','x'] ],

    FuCL                      `noun`    {- <^guz`> -}          [ ['a','x','l','e'], ['s','h','a','f','t'] ],

    FaCaL                     `noun`    {- <^gaza`> -}         [ ['a','n','x','i','e','t','y'], ['w','o','r','r','y'] ],

    FaCiL                     `noun`    {- <^gazi`> -}         [ ['r','e','s','t','l','e','s','s'], ['a','n','x','i','o','u','s'] ]
                              `plural`     FaCiL |< Un
                              `plural`     FaCUL |< Un
                           
    `derives` otherwise,

    FaCCAL                    `noun`    {- <^gazzA`> -}        [ ['J','a','z','z','a'] ],

    FACiL                     `noun`    {- <^gAzi`> -}         [ ['r','e','s','t','l','e','s','s'], ['a','n','x','i','o','u','s'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mu^gazza`> -}      [ ['m','a','r','b','l','e','d'], ['v','a','r','i','e','g','a','t','e','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise ]


cluster_112 = cluster

 |> "^g z n" <| [

    FACUL                     `noun`    {- <^gAzUn> -}         [ ['l','a','w','n'] ] ]


cluster_113 = cluster

 |> "^g z m ^g" <| [

    KaRDaS |< Iy              `adj`     {- <^gazma^gIy> -}     [ ['s','h','o','e','m','a','k','e','r'] ] ]


cluster_114 = cluster

 |> "^g z m" <| [

    FaCaL                     `verb`    {- <^gazam> -}         [ unwords [ ['c','u','t'], ['s','h','o','r','t'] ], unwords [ ['b','e'], ['c','e','r','t','a','i','n'] ], ['i','m','p','o','s','e'] ]
                              `imperf`     FCiL,

    FaCL                      `noun`    {- <^gazm> -}          [ ['c','l','i','p','p','i','n','g'], ['d','e','c','i','s','i','o','n'] ],

    FACiL                     `adj`     {- <^gAzim> -}         [ ['d','e','c','i','s','i','v','e'], ['d','e','f','i','n','i','t','i','v','e'] ],

    FACiL |<< "aN"            `adj`     {- <^gAzimaN> -}       [ unwords [ ['w','i','t','h'], ['a','b','s','o','l','u','t','e'], ['c','e','r','t','a','i','n','t','y'] ] ],

    FACiL                     `noun`    {- <^gAzim> -}         [ unwords [ ['a','p','o','c','o','p','a','t','i','n','g'], "(", ['g','r','a','m','.'], ")" ] ]
                              `plural`     FawACiL,

    MaFCUL                    `adj`     {- <ma^gzUm> -}        [ unwords [ ['c','u','t'], ['s','h','o','r','t'] ], ['c','l','i','p','p','e','d'] ],

    FaCL |< aT                `noun`    {- <^gazmaT> -}        [ ['s','h','o','e','s'], ['b','o','o','t','s'] ]
                              `plural`     FaCaL |< At ]


cluster_115 = cluster

 |> "^g z l n" <| [

    KuRDAS                    `noun`    {- <^guzlAn> -}        [ ['w','a','l','l','e','t'] ] ]


cluster_116 = cluster

 |> "^g z l" <| [

    FaCuL                     `verb`    {- <^gazul> -}         [ unwords [ ['b','e'], ['p','r','u','d','e','n','t'] ], unwords [ ['b','e'], ['c','o','n','s','i','d','e','r','a','b','l','e'] ], unwords [ ['b','e'], ['p','l','e','n','t','i','f','u','l'] ] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'a^gzal> -}        [ unwords [ ['b','e'], ['g','e','n','e','r','o','u','s'] ], unwords [ ['g','i','v','e'], ['l','i','b','e','r','a','l','l','y'] ], unwords [ ['b','e'], ['g','i','v','e','n'], ['l','i','b','e','r','a','l','l','y'] ] ],

    IFtaCaL                   `verb`    {- <i^gtazal> -}       [ unwords [ ['w','r','i','t','e'], ['s','h','o','r','t','h','a','n','d'] ] ],

    FaCL                      `noun`    {- <^gazl> -}          [ ['a','b','u','n','d','a','n','t'] ]
                              `plural`     FiCAL
                              `plural`     FaCIL |< aT
                           
    `derives` otherwise,

    FiCL |< aT                `noun`    {- <^gizlaT> -}        [ ['p','i','e','c','e'], ['s','l','i','c','e'] ],

    FaCAL |< aT               `noun`    {- <^gazAlaT> -}       [ ['a','b','u','n','d','a','n','c','e'] ],

    FuCLAn                    `noun`    {- <^guzlAn> -}        [ ['w','a','l','l','e','t'] ] ]


cluster_117 = cluster

 |> "^g z r" <| [

    FaCaL                     `noun`    {- <^gazar> -}         [ ['c','a','r','r','o','t'] ] ]



 |> "^g z r" <| [

    FaCaL                     `verb`    {- <^gazar> -}         [ ['s','l','a','u','g','h','t','e','r'], ['b','u','t','c','h','e','r'] ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- <^gazar> -}         [ ['s','i','n','k'], ['e','b','b'] ]
                              `imperf`     FCiL,

    FaCL                      `noun`    {- <^gazr> -}          [ ['s','l','a','u','g','h','t','e','r'], ['e','b','b'] ],

    FaCL |< aT                `noun`    {- <^gazraT> -}        [ unwords [ ['b','l','o','o','d'], ['s','a','c','r','i','f','i','c','e'] ] ],

    FaCUL                     `noun`    {- <^gazUr> -}         [ unwords [ ['s','l','a','u','g','h','t','e','r'], ['c','a','m','e','l'] ] ],

    FiCAL |< aT               `noun`    {- <^gizAraT> -}       [ ['b','u','t','c','h','e','r','y'] ],

    FaCCAL                    `noun`    {- <^gazzAr> -}        [ ['b','u','t','c','h','e','r'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FaCIL |< aT               `noun`    {- <^gazIraT> -}       [ ['i','s','l','a','n','d'], ['p','e','n','i','n','s','u','l','a'] ]
                              `plural`     FuCuL,

    FaCIL |< aT               `noun`    {- <^gazIraT> -}       [ ['J','a','z','e','e','r','a'], unwords [ ['A','l'], "-", ['J','a','z','e','e','r','a'] ] ],

    FaCA'iL                   `noun`    {- <^gazA'ir> -}       [ ['A','l','g','e','r','i','a'] ],

    FaCA'iL                   `noun`    {- <^gazA'ir> -}       [ ['A','l','g','i','e','r','s'] ],

    FaCA'iL |< Iy             `adj`     {- <^gazA'irIy> -}     [ ['A','l','g','e','r','i','a','n'] ],

    FaCaL |< Iy               `adj`     {- <^gazarIy> -}       [ ['i','n','s','u','l','a','r'] ],

    MaFCiL                    `noun`    {- <ma^gzir> -}        [ ['m','a','s','s','a','c','r','e'], ['s','l','a','u','g','h','t','e','r'] ],

    MaFCaL |< aT              `noun`    {- <ma^gzaraT> -}      [ ['m','a','s','s','a','c','r','e'], ['s','l','a','u','g','h','t','e','r'] ]
                              `plural`     MaFACiL ]


cluster_118 = cluster

 |> "^gAymI" <| [

    _____                     `noun`    {- <^gAymI> -}         [ ['J','a','m','i','e'] ] ]


cluster_119 = cluster

 |> "^gAwI^s" <| [

    _____                     `noun`    {- <^gAwI^s> -}        [ ['s','e','r','g','e','a','n','t'] ] ]


cluster_120 = cluster

 |> "^gAstIn" <| [

    _____                     `noun`    {- <^gAstIn> -}        [ ['J','u','s','t','i','n'] ] ]


cluster_121 = cluster

 |> "^gArdiyAn" <| [

    _____                     `noun`    {- <^gArdiyAn> -}      [ ['G','u','a','r','d','i','a','n'] ] ]


cluster_122 = cluster

 |> "^gArAn^g" <| [

    _____                     `noun`    {- <^gArAn^g> -}       [ ['G','a','r','a','n','g'] ] ]


cluster_123 = cluster

 |> "^gArAlll_ah" <| [

    _____                     `noun`    {- <^gArAlll_ah> -}    [ ['J','a','r','a','l','l','a','h'] ] ]


cluster_124 = cluster

 |> "^gAnluwI^gI" <| [

    _____                     `noun`    {- <^gAnluwI^gI> -}    [ ['G','i','a','n','l','u','i','g','i'] ] ]


cluster_125 = cluster

 |> "^gAnlUkA" <| [

    _____                     `noun`    {- <^gAnlUkA> -}       [ ['G','i','a','n','l','u','c','a'] ] ]


cluster_126 = cluster

 |> "^gAnfrAnkU" <| [

    _____                     `noun`    {- <^gAnfrAnkU> -}     [ ['G','i','a','n','f','r','a','n','c','o'] ] ]


cluster_127 = cluster

 |> "^gAnfI" <| [

    _____                     `noun`    {- <^gAnfI> -}         [ ['J','a','n','u','a','r','y'] ] ]


cluster_128 = cluster

 |> "^gAnIrU" <| [

    _____                     `noun`    {- <^gAnIrU> -}        [ ['J','a','n','e','i','r','o'] ] ]


cluster_129 = cluster

 |> "^gAmU" <| [

    _____                     `noun`    {- <^gAmU> -}          [ ['J','a','m','m','u'] ] ]


cluster_130 = cluster

 |> "^gAmA" <| [

    _____                     `noun`    {- <^gAmA> -}          [ ['g','a','m','m','a'] ] ]


cluster_131 = cluster

 |> "^gAlA" <| [

    _____                     `noun`    {- <^gAlA> -}          [ ['J','a','l','a'] ] ]


cluster_132 = cluster

 |> "^gAksUn" <| [

    _____                     `noun`    {- <^gAksUn> -}        [ ['J','a','c','k','s','o','n'] ] ]


cluster_133 = cluster

 |> "^gAkArtA" <| [

    _____                     `noun`    {- <^gAkArtA> -}       [ ['J','a','k','a','r','t','a'] ] ]


cluster_134 = cluster

 |> "^gAdIr" <| [

    _____                     `noun`    {- <^gAdIr> -}         [ ['J','a','d','i','r'] ] ]


cluster_135 = cluster

 |> "^gAdAllh" <| [

    _____                     `noun`    {- <^gAdAllh> -}       [ ['J','a','d','a','l','l','a','h'], ['G','a','d','a','l','l','a','h'] ] ]


cluster_136 = cluster

 |> "^gA.guwAr" <| [

    _____                     `noun`    {- <^gA.guwAr> -}      [ ['J','a','g','u','a','r'] ] ]


cluster_137 = cluster

 |> "^gUfAnI" <| [

    _____                     `noun`    {- <^gUfAnI> -}        [ ['G','i','o','v','a','n','n','i'] ] ]


cluster_138 = cluster

 |> "^gUbA" <| [

    _____                     `noun`    {- <^gUbA> -}          [ ['J','u','b','a'] ] ]


cluster_139 = cluster

 |> "^gIzhU" <| [

    _____                     `noun`    {- <^gIzhU> -}         [ ['J','i','z','h','u'] ] ]


cluster_140 = cluster

 |> "^gItiks" <| [

    _____                     `noun`    {- <^gItiks> -}        [ ['G','i','t','e','x'] ] ]


cluster_141 = cluster

 |> "^gIr^gA" <| [

    _____                     `noun`    {- <^gIr^gA> -}        [ ['J','i','r','g','a'] ] ]


cluster_142 = cluster

 |> "^gIrUzAlIm" <| [

    _____                     `noun`    {- <^gIrUzAlIm> -}     [ ['J','e','r','u','s','a','l','e','m'] ] ]


cluster_143 = cluster

 |> "^gIrU" <| [

    _____                     `noun`    {- <^gIrU> -}          [ unwords [ ['e','n','d','o','r','s','e','m','e','n','t'], "(", ['c','h','e','q','u','e'], ")" ] ] ]


cluster_144 = cluster

 |> "^gInArU" <| [

    _____                     `noun`    {- <^gInArU> -}        [ ['G','e','n','a','r','o'] ] ]


cluster_145 = cluster

 |> "^gIn.g" <| [

    _____                     `noun`    {- <^gIn.g> -}         [ ['J','i','n','g'] ] ]


cluster_146 = cluster

 |> "^gIlz" <| [

    _____                     `noun`    {- <^gIlz> -}          [ ['G','i','l','e','s'] ] ]


cluster_147 = cluster

 |> "^gIlbirtU" <| [

    _____                     `noun`    {- <^gIlbirtU> -}      [ ['G','i','l','b','e','r','t','o'] ] ]


cluster_148 = cluster

 |> "^gIlbirt" <| [

    _____                     `noun`    {- <^gIlbirt> -}       [ ['G','i','l','b','e','r','t'] ] ]


cluster_149 = cluster

 |> "^gIlU" <| [

    _____                     `noun`    {- <^gIlU> -}          [ ['G','i','l','o'] ] ]


cluster_150 = cluster

 |> "^gIlAtU" <| [

    _____                     `noun`    {- <^gIlAtU> -}        [ ['g','e','l','a','t','o'], unwords [ ['i','c','e'], ['c','r','e','a','m'] ] ] ]


cluster_151 = cluster

 |> "^gIlAtI" <| [

    _____                     `noun`    {- <^gIlAtI> -}        [ ['g','e','l','a','t','i'], unwords [ ['i','c','e'], ['c','r','e','a','m'] ] ] ]


cluster_152 = cluster

 |> "^gIlArdInU" <| [

    _____                     `noun`    {- <^gIlArdInU> -}     [ ['G','i','l','a','r','d','i','n','o'] ] ]


cluster_153 = cluster

 |> "^gIl.gAl" <| [

    _____                     `noun`    {- <^gIl.gAl> -}       [ ['G','i','l','g','a','l'] ] ]


cluster_154 = cluster

 |> "^gIfsUn" <| [

    _____                     `noun`    {- <^gIfsUn> -}        [ ['J','e','p','h','s','o','n'] ] ]


cluster_155 = cluster

 |> "^gIbUtI" <| [

    _____                     `noun`    {- <^gIbUtI> -}        [ ['D','j','i','b','o','u','t','i'] ] ]


cluster_156 = cluster

 |> "^gAzAn" <| [

    _____                     `noun`    {- <^gAzAn> -}         [ ['J','a','z','a','n'] ] ]


cluster_157 = cluster

 |> "^gUwA" <| [

    _____                     `noun`    {- <^gUwA> -}          [ ['i','n','s','i','d','e'] ] ]


cluster_158 = cluster

 |> "^gUsbAn" <| [

    _____                     `noun`    {- <^gUsbAn> -}        [ ['J','o','s','p','i','n'] ] ]


cluster_159 = cluster

 |> "^gUrnAl" <| [

    _____                     `noun`    {- <^gUrnAl> -}        [ ['J','o','u','r','n','a','l'] ] ]


cluster_160 = cluster

 |> "^gUr^giyA" <| [

    _____                     `noun`    {- <^gUr^giyA> -}      [ ['G','e','o','r','g','i','a'] ] ]


cluster_161 = cluster

 |> "^gUr^gIt" <| [

    _____                     `noun`    {- <^gUr^gIt> -}       [ ['G','e','o','r','g','e','t','t','e'] ] ]


cluster_162 = cluster

 |> "^gUr^gI" <| [

    _____                     `noun`    {- <^gUr^gI> -}        [ ['G','e','o','r','g','i'], ['G','h','e','o','r','g','h','e'] ] ]


cluster_163 = cluster

 |> "^gUr^g" <| [

    _____                     `noun`    {- <^gUr^g> -}         [ ['G','e','o','r','g','e'] ] ]


cluster_164 = cluster

 |> "^gUnz" <| [

    _____                     `noun`    {- <^gUnz> -}          [ ['J','o','n','e','s'] ] ]


cluster_165 = cluster

 |> "^gUnsUn" <| [

    _____                     `noun`    {- <^gUnsUn> -}        [ ['J','o','h','n','s','o','n'] ] ]


cluster_166 = cluster

 |> "^gUniyUr" <| [

    _____                     `noun`    {- <^gUniyUr> -}       [ ['J','u','n','i','o','r'] ] ]


cluster_167 = cluster

 |> "^gUnInyU" <| [

    _____                     `noun`    {- <^gUnInyU> -}       [ ['J','u','n','i','n','i','o'] ] ]


cluster_168 = cluster

 |> "^gUnA_tAn" <| [

    _____                     `noun`    {- <^gUnA_tAn> -}      [ ['J','o','n','a','t','h','a','n'] ] ]


cluster_169 = cluster

 |> "^gUn.g" <| [

    _____                     `noun`    {- <^gUn.g> -}         [ ['J','o','n','g'] ] ]


section = [ cluster_1,
            cluster_2,
            cluster_3,
            cluster_4,
            cluster_5,
            cluster_6,
            cluster_7,
            cluster_8,
            cluster_9,
            cluster_10,
            cluster_11,
            cluster_12,
            cluster_13,
            cluster_14,
            cluster_15,
            cluster_16,
            cluster_17,
            cluster_18,
            cluster_19,
            cluster_20,
            cluster_21,
            cluster_22,
            cluster_23,
            cluster_24,
            cluster_25,
            cluster_26,
            cluster_27,
            cluster_28,
            cluster_29,
            cluster_30,
            cluster_31,
            cluster_32,
            cluster_33,
            cluster_34,
            cluster_35,
            cluster_36,
            cluster_37,
            cluster_38,
            cluster_39,
            cluster_40,
            cluster_41,
            cluster_42,
            cluster_43,
            cluster_44,
            cluster_45,
            cluster_46,
            cluster_47,
            cluster_48,
            cluster_49,
            cluster_50,
            cluster_51,
            cluster_52,
            cluster_53,
            cluster_54,
            cluster_55,
            cluster_56,
            cluster_57,
            cluster_58,
            cluster_59,
            cluster_60,
            cluster_61,
            cluster_62,
            cluster_63,
            cluster_64,
            cluster_65,
            cluster_66,
            cluster_67,
            cluster_68,
            cluster_69,
            cluster_70,
            cluster_71,
            cluster_72,
            cluster_73,
            cluster_74,
            cluster_75,
            cluster_76,
            cluster_77,
            cluster_78,
            cluster_79,
            cluster_80,
            cluster_81,
            cluster_82,
            cluster_83,
            cluster_84,
            cluster_85,
            cluster_86,
            cluster_87,
            cluster_88,
            cluster_89,
            cluster_90,
            cluster_91,
            cluster_92,
            cluster_93,
            cluster_94,
            cluster_95,
            cluster_96,
            cluster_97,
            cluster_98,
            cluster_99,
            cluster_100,
            cluster_101,
            cluster_102,
            cluster_103,
            cluster_104,
            cluster_105,
            cluster_106,
            cluster_107,
            cluster_108,
            cluster_109,
            cluster_110,
            cluster_111,
            cluster_112,
            cluster_113,
            cluster_114,
            cluster_115,
            cluster_116,
            cluster_117,
            cluster_118,
            cluster_119,
            cluster_120,
            cluster_121,
            cluster_122,
            cluster_123,
            cluster_124,
            cluster_125,
            cluster_126,
            cluster_127,
            cluster_128,
            cluster_129,
            cluster_130,
            cluster_131,
            cluster_132,
            cluster_133,
            cluster_134,
            cluster_135,
            cluster_136,
            cluster_137,
            cluster_138,
            cluster_139,
            cluster_140,
            cluster_141,
            cluster_142,
            cluster_143,
            cluster_144,
            cluster_145,
            cluster_146,
            cluster_147,
            cluster_148,
            cluster_149,
            cluster_150,
            cluster_151,
            cluster_152,
            cluster_153,
            cluster_154,
            cluster_155,
            cluster_156,
            cluster_157,
            cluster_158,
            cluster_159,
            cluster_160,
            cluster_161,
            cluster_162,
            cluster_163,
            cluster_164,
            cluster_165,
            cluster_166,
            cluster_167,
            cluster_168,
            cluster_169 ]
