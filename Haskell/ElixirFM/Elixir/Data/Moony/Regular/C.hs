
module Elixir.Data.Moony.Regular.C where

import Elixir.Lexicon


version = revised "$Revision$"

lexicon = include section


cluster_1   = cluster

 |> "b _t r" <| [

    FaCaL                     `verb`    {- <ba_tar> -}         [ ['p','u','s','t','u','l','a','t','e'] ]
                              `imperf`     FCiL,

    FaCiL                     `verb`    {- <ba_tir> -}         [ ['p','u','s','t','u','l','a','t','e'] ]
                              `imperf`     FCaL,

    TaFaCCaL                  `verb`    {- <taba_t_tar> -}     [ ['p','u','s','t','u','l','a','t','e'] ],

    FaCL                      `noun`    {- <ba_tr> -}          [ ['p','u','s','t','u','l','e'] ]
                              `plural`     FuCUL
                              `plural`     FaCaL |< At,

    FaCiL                     `noun`    {- <ba_tir> -}         [ ['p','u','s','t','u','l','a','t','e','d'] ],

    FaCIL                     `noun`    {- <ba_tIr> -}         [ ['p','u','s','t','u','l','a','t','e','d'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise ]


cluster_2   = cluster

 |> "b ` _t" <| [

    FaCaL                     `verb`    {- <ba`a_t> -}         [ ['s','e','n','d'], ['e','m','i','t'] ]
                              `imperf`     FCaL,

    InFaCaL                   `verb`    {- <inba`a_t> -}       [ ['r','e','v','i','v','e'], ['r','e','s','u','r','r','e','c','t'] ],

    InFaCaL                   `verb`    {- <inba`a_t> -}       [ unwords [ ['b','e'], ['s','e','n','t'], ['o','u','t'] ], unwords [ ['b','e'], ['r','e','v','i','v','e','d'] ] ],

    IFtaCaL                   `verb`    {- <ibta`a_t> -}       [ ['s','e','n','d'], ['d','i','s','p','a','t','c','h'] ],

    IFtaCaL                   `verb`    {- <ibta`a_t> -}       [ ['e','x','h','u','m','e'], ['r','e','v','i','v','e'] ],

    FaCL                      `noun`    {- <ba`_t> -}          [ ['e','m','i','s','s','i','o','n'], ['a','w','a','k','e','n','i','n','g'] ],

    FaCL                      `noun`    {- <ba`_t> -}          [ ['B','a','a','t','h'] ],

    FaCL                      `noun`    {- <ba`_t> -}          [ ['d','e','l','e','g','a','t','i','o','n'] ]
                              `plural`     FuCUL,

    FaCL |< Iy                `adj`     {- <ba`_tIy> -}        [ ['B','a','a','t','h','i','s','t'] ],

    FaCL |< aT                `noun`    {- <ba`_taT> -}        [ ['d','e','l','e','g','a','t','i','o','n'], ['m','i','s','s','i','o','n'] ]
                              `plural`     FiCL
                              `plural`     FaCaL |< At,

    FACUL                     `noun`    {- <bA`U_t> -}         [ ['E','a','s','t','e','r'] ],

    MaFCaL                    `noun`    {- <mab`a_t> -}        [ ['c','a','u','s','e'], ['f','a','c','t','o','r'] ]
                              `plural`     MaFACiL,

    FACiL                     `noun`    {- <bA`i_t> -}         [ ['s','e','n','d','i','n','g'], ['s','e','n','d','e','r'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    FACiL                     `noun`    {- <bA`i_t> -}         [ ['i','n','c','e','n','t','i','v','e'], ['m','o','t','i','v','e'] ]
                              `plural`     FawACiL,

    MaFCUL                    `noun`    {- <mab`U_t> -}        [ ['e','n','v','o','y'], ['r','e','p','r','e','s','e','n','t','a','t','i','v','e'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MunFaCaL                  `noun`    {- <munba`a_t> -}      [ ['s','e','n','t'] ]
                              `plural`     MunFaCaL |< Un
                           
    `derives` otherwise,

    MunFaCaL                  `noun`    {- <munba`a_t> -}      [ ['s','o','u','r','c','e'] ]
                              `plural`     MunFaCaL |< At,

    InFiCAL                   `noun`    {- <inbi`A_t> -}       [ ['r','e','v','i','v','a','l'], ['r','e','s','u','r','r','e','c','t','i','o','n'] ]
                              `plural`     InFiCAL |< At,

    MunFaCiL                  `noun`    {- <munba`i_t> -}      [ ['r','e','s','u','r','g','e','n','t'] ]
                              `plural`     MunFaCiL |< Un
                           
    `derives` otherwise ]


cluster_3   = cluster

 |> "b ` b `" <| [

    KuRDuS                    `noun`    {- <bu`bu`> -}         [ ['m','o','n','s','t','e','r'], ['s','p','e','c','t','e','r'] ]
                              `plural`     KaRADiS ]


cluster_4   = cluster

 |> "b ` _t r" <| [

    KaRDaS                    `verb`    {- <ba`_tar> -}        [ ['s','c','a','t','t','e','r'], ['d','i','s','p','e','r','s','e'] ],

    TaKaRDaS                  `verb`    {- <taba`_tar> -}      [ unwords [ ['b','e'], ['s','c','a','t','t','e','r','e','d'] ], unwords [ ['b','e'], ['d','i','s','p','e','r','s','e','d'] ] ],

    MuKaRDaS                  `noun`    {- <muba`_tar> -}      [ ['s','c','a','t','t','e','r','e','d'], ['d','i','s','p','e','r','s','e','d'] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise,

    TaKaRDuS                  `noun`    {- <taba`_tur> -}      [ ['s','c','a','t','t','e','r','i','n','g'], ['d','i','s','p','e','r','s','a','l'] ]
                              `plural`     TaKaRDuS |< At ]


cluster_5   = cluster

 |> "b ` d" <| [

    FaCL |<< "a"              `prep`    {- <ba`da> -}          [ ['a','f','t','e','r'] ],

    FaCL |<< "u"              `adv`     {- <ba`du> -}          [ ['a','f','t','e','r','w','a','r','d'], ['l','a','t','e','r'], unwords [ "(", ['n','o','t'], ")", ['y','e','t'] ] ],

    FaCuL                     `verb`    {- <ba`ud> -}          [ unwords [ ['b','e'], ['d','i','s','t','a','n','t'] ] ]
                              `imperf`     FCuL,

    FACaL                     `verb`    {- <bA`ad> -}          [ ['s','e','p','a','r','a','t','e'] ],

    HaFCaL                    `verb`    {- <'ab`ad> -}         [ ['r','e','m','o','v','e'], ['e','x','p','e','l'] ],

    HuFCiL                    `verb`    {- <'ub`id> -}         [ unwords [ ['b','e'], ['r','e','m','o','v','e','d'] ], unwords [ ['b','e'], ['e','x','p','e','l','l','e','d'] ] ],

    TaFACaL                   `verb`    {- <tabA`ad> -}        [ ['d','i','v','e','r','g','e'], ['s','e','p','a','r','a','t','e'] ],

    IFtaCaL                   `verb`    {- <ibta`ad> -}        [ unwords [ ['m','o','v','e'], ['a','w','a','y'] ], ['e','s','c','h','e','w'] ],

    IstaFCaL                  `verb`    {- <istab`ad> -}       [ unwords [ ['r','u','l','e'], ['o','u','t'] ], ['r','e','j','e','c','t'] ],

    FuCL                      `noun`    {- <bu`d> -}           [ ['d','i','m','e','n','s','i','o','n'], ['d','i','s','t','a','n','c','e'] ]
                              `plural`     HaFCAL,

    FaCIL                     `adj`     {- <ba`Id> -}          [ ['r','e','m','o','t','e'], ['d','i','s','t','a','n','t'], ['f','a','r'] ]
                              `plural`     FaCIL |< Un
                              `plural`     FuCaLA',

    FuCayL |<< "a"            `prep`    {- <bu`ayda> -}        [ unwords [ ['s','h','o','r','t','l','y'], ['a','f','t','e','r'] ], unwords [ ['s','o','o','n'], ['a','f','t','e','r'] ] ],

    HaFCaL                    `adj`     {- <'ab`ad> -}         [ ['f','a','r','t','h','e','r'], ['f','a','r','t','h','e','s','t'] ],

    HaFACiL                   `noun`    {- <'abA`id> -}        [ ['f','a','r','t','h','e','r'], ['f','a','r','t','h','e','s','t'] ],

    HaFCAL |< Iy |< aT        `noun`    {- <'ab`AdIyaT> -}     [ unwords [ ['c','o','u','n','t','r','y'], ['e','s','t','a','t','e'] ] ],

    FiCAL                     `noun`    {- <bi`Ad> -}          [ ['d','i','s','t','a','n','c','e'] ],

    TaFCIL                    `noun`    {- <tab`Id> -}         [ ['b','a','n','i','s','h','m','e','n','t'] ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- <mubA`adaT> -}      [ ['a','l','i','e','n','a','t','i','o','n'], unwords [ ['s','o','w','i','n','g'], ['d','i','s','s','e','n','t'] ] ],

    HiFCAL                    `noun`    {- <'ib`Ad> -}         [ ['d','e','p','o','r','t','a','t','i','o','n'], ['e','x','i','l','e'], ['r','e','m','o','v','a','l'] ]
                              `plural`     HiFCAL |< At,

    TaFACuL                   `noun`    {- <tabA`ud> -}        [ ['e','s','t','r','a','n','g','e','m','e','n','t'] ]
                              `plural`     TaFACuL |< At,

    IFtiCAL                   `noun`    {- <ibti`Ad> -}        [ ['a','v','o','i','d','i','n','g'], ['e','s','c','h','e','w','i','n','g'] ]
                              `plural`     IFtiCAL |< At,

    IstiFCAL                  `noun`    {- <istib`Ad> -}       [ unwords [ ['r','u','l','i','n','g'], ['o','u','t'] ], ['d','i','s','t','a','n','c','i','n','g'] ]
                              `plural`     IstiFCAL |< At,

    MuFCaL                    `noun`    {- <mub`ad> -}         [ ['d','e','p','o','r','t','e','d'], ['d','e','p','o','r','t','e','e'] ]
                              `plural`     MuFCaL |< Un
                           
    `derives` otherwise,

    MutaFACiL                 `noun`    {- <mutabA`id> -}      [ ['s','e','p','a','r','a','t','e'], ['i','n','f','r','e','q','u','e','n','t'] ]
                              `plural`     MutaFACiL |< Un
                           
    `derives` otherwise,

    MustaFCaL                 `noun`    {- <mustab`ad> -}      [ ['i','m','p','r','o','b','a','b','l','e'], ['u','n','l','i','k','e','l','y'] ]
                              `plural`     MustaFCaL |< Un
                           
    `derives` otherwise,

    MaFCaL |< aT              `noun`    {- <mab`adaT> -}       [ ['d','i','s','t','a','n','c','e'], ['r','e','m','o','t','e','n','e','s','s'] ] ]


cluster_6   = cluster

 |> "b ` z q" <| [

    KaRDaS                    `verb`    {- <ba`zaq> -}         [ ['s','c','a','t','t','e','r'], ['s','q','u','a','n','d','e','r'] ],

    KaRDaS |< aT              `noun`    {- <ba`zaqaT> -}       [ ['s','c','a','t','t','e','r','i','n','g'], ['s','q','u','a','n','d','e','r','i','n','g'] ],

    MuKaRDiS                  `noun`    {- <muba`ziq> -}       [ ['s','q','u','a','n','d','e','r','e','r'], ['s','p','e','n','d','t','h','r','i','f','t'] ]
                              `plural`     MuKaRDiS |< Un
                           
    `derives` otherwise ]


cluster_7   = cluster

 |> "b ` r" <| [

    FaCL                      `noun`    {- <ba`r> -}           [ ['d','u','n','g'], ['d','r','o','p','p','i','n','g','s'] ]
                              `plural`     HaFCAL,

    FaCIL                     `noun`    {- <ba`Ir> -}          [ ['c','a','m','e','l'] ]
                              `plural`     HaFCiL |< aT
                              `plural`     FuCLAn
                              `plural`     FaCALIn
                              `plural`     HaFACiL ]


cluster_8   = cluster

 |> "b ` q" <| [

    FaCaL                     `verb`    {- <ba`aq> -}          [ ['i','r','r','i','g','a','t','e'], ['w','a','t','e','r'] ]
                              `imperf`     FCuL,

    InFaCaL                   `verb`    {- <inba`aq> -}        [ ['s','p','r','i','n','k','l','e'] ] ]


cluster_9   = cluster

 |> "b ` l" <| [

    FaCL                      `noun`    {- <ba`l> -}           [ ['h','u','s','b','a','n','d'] ]
                              `plural`     FuCUL |< aT
                              `plural`     FiCAL,

    FaCL                      `noun`    {- <ba`l> -}           [ ['B','a','a','l'] ],

    FaCL |< aT                `noun`    {- <ba`laT> -}         [ ['w','i','f','e'] ]
                              `plural`     FaCaL |< At,

    FaCL |< Iy                `adj`     {- <ba`lIy> -}         [ ['u','n','i','r','r','i','g','a','t','e','d'] ] ]


cluster_10  = cluster

 |> "b d `" <| [

    FaCaL                     `verb`    {- <bada`> -}          [ ['i','n','n','o','v','a','t','e'], ['i','n','v','e','n','t'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <badda`> -}         [ ['e','x','c','e','l'], unwords [ ['b','e'], ['e','x','c','e','l','l','e','d'] ] ],

    HaFCaL                    `verb`    {- <'abda`> -}         [ ['i','n','n','o','v','a','t','e'], ['c','r','e','a','t','e'] ],

    IFtaCaL                   `verb`    {- <ibtada`> -}        [ ['c','o','n','t','r','i','v','e'], ['i','n','v','e','n','t'] ],

    IstaFCaL                  `verb`    {- <istabda`> -}       [ unwords [ ['f','i','n','d'], ['e','x','t','r','a','o','r','d','i','n','a','r','y'] ], unwords [ ['f','i','n','d'], ['s','t','r','a','n','g','e'] ] ],

    FaCL                      `noun`    {- <bad`> -}           [ ['i','n','n','o','v','a','t','i','o','n'], ['c','r','e','a','t','i','o','n'] ],

    FiCL                      `noun`    {- <bid`> -}           [ ['o','r','i','g','i','n','a','l'], ['w','o','n','d','e','r'] ],

    HaFCAL                    `noun`    {- <'abdA`> -}         [ ['w','o','n','d','e','r','s'], ['n','o','v','e','l','t','i','e','s'] ],

    FiCL |< aT                `noun`    {- <bid`aT> -}         [ ['h','e','r','e','s','y'], ['n','o','v','e','l','t','y'] ]
                              `plural`     FiCaL,

    FaCIL                     `noun`    {- <badI`> -}          [ ['B','a','d','i','e'] ],

    FaCIL                     `noun`    {- <badI`> -}          [ ['w','o','n','d','e','r','f','u','l'], ['m','a','r','v','e','l','o','u','s'] ],

    FaCIL |< aT               `noun`    {- <badI`aT> -}        [ ['w','o','n','d','e','r'], ['m','a','r','v','e','l'] ]
                              `plural`     FaCA'iL,

    FaCIL |< Iy               `adj`     {- <badI`Iy> -}        [ ['r','h','e','t','o','r','i','c','a','l'] ],

    HaFCaL                    `noun`    {- <'abda`> -}         [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['a','m','a','z','i','n','g'] ] ],

    HiFCAL                    `noun`    {- <'ibdA`> -}         [ ['o','r','i','g','i','n','a','l','i','t','y'], ['c','r','e','a','t','i','v','i','t','y'] ]
                              `plural`     HiFCAL |< At,

    HiFCAL |< Iy              `adj`     {- <'ibdA`Iy> -}       [ ['o','r','i','g','i','n','a','l'], ['c','r','e','a','t','i','v','e'] ],

    IFtiCAL                   `noun`    {- <ibtidA`> -}        [ ['i','n','n','o','v','a','t','i','o','n'] ]
                              `plural`     IFtiCAL |< At,

    IFtiCAL |< Iy             `adj`     {- <ibtidA`Iy> -}      [ ['i','n','n','o','v','a','t','i','v','e'] ],

    MuFCiL                    `noun`    {- <mubdi`> -}         [ ['i','n','n','o','v','a','t','o','r'], ['c','r','e','a','t','i','v','e'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise,

    MuFtaCiL                  `noun`    {- <mubtadi`> -}       [ ['i','n','n','o','v','a','t','o','r'], ['c','r','e','a','t','i','v','e'] ]
                              `plural`     MuFtaCiL |< Un
                           
    `derives` otherwise ]


cluster_11  = cluster

 |> "b d h" <| [

    FaCaL                     `verb`    {- <badah> -}          [ ['s','u','r','p','r','i','s','e'], unwords [ ['d','o'], ['s','p','o','n','t','a','n','e','o','u','s','l','y'] ] ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- <bAdah> -}          [ unwords [ ['a','r','r','i','v','e'], ['u','n','e','x','p','e','c','t','e','d','l','y'] ], ['s','u','r','p','r','i','s','e'] ],

    IFtaCaL                   `verb`    {- <ibtadah> -}        [ ['i','m','p','r','o','v','i','s','e'], ['e','x','t','e','m','p','o','r','i','z','e'] ],

    FaCAL |< aT               `noun`    {- <badAhaT> -}        [ ['s','p','o','n','t','a','n','e','i','t','y'] ],

    FaCAL |< aT |<< "aN"      `noun`    {- <badAhaTaN> -}      [ ['s','p','o','n','t','a','n','e','o','u','s','l','y'] ],

    FaCIL |< aT               `noun`    {- <badIhaT> -}        [ unwords [ ['i','n','t','u','i','t','i','v','e'], ['g','r','a','s','p'] ] ],

    FaCIL |< Iy               `adj`     {- <badIhIy> -}        [ ['o','b','v','i','o','u','s'], ['i','n','t','u','i','t','i','v','e'], unwords [ ['s','e','l','f'], "-", ['e','v','i','d','e','n','t'] ] ],

    FaCIL |< Iy |< aT         `noun`    {- <badIhIyaT> -}      [ ['p','l','a','t','i','t','u','d','e'], ['a','x','i','o','m'] ],

    FaCA'iL                   `noun`    {- <badA'ih> -}        [ unwords [ ['s','e','l','f'], "-", ['e','v','i','d','e','n','t'], ['t','r','u','t','h','s'] ] ] ]


cluster_12  = cluster

 |> "b d l" <| [

    FaCaL                     `verb`    {- <badal> -}          [ ['e','x','c','h','a','n','g','e'], ['r','e','p','l','a','c','e'], ['s','u','b','s','t','i','t','u','t','e'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <baddal> -}         [ ['c','h','a','n','g','e'], ['m','o','d','i','f','y'] ],

    FACaL                     `verb`    {- <bAdal> -}          [ ['e','x','c','h','a','n','g','e'], ['t','r','a','d','e'] ],

    HaFCaL                    `verb`    {- <'abdal> -}         [ ['s','u','b','s','t','i','t','u','t','e'], ['e','x','c','h','a','n','g','e'], ['r','e','p','l','a','c','e'] ],

    TaFaCCaL                  `verb`    {- <tabaddal> -}       [ unwords [ ['b','e'], ['e','x','c','h','a','n','g','e','d'] ] ],

    TaFACaL                   `verb`    {- <tabAdal> -}        [ ['e','x','c','h','a','n','g','e'] ],

    IstaFCaL                  `verb`    {- <istabdal> -}       [ ['r','e','p','l','a','c','e'], ['s','u','b','s','t','i','t','u','t','e'] ],

    FaCaL                     `noun`    {- <badal> -}          [ ['s','u','b','s','t','i','t','u','t','e'] ]
                              `plural`     FaCaL |< At
                              `plural`     HaFCAL,

    FaCaL |<< "aN"            `noun`    {- <badalaN> -}        [ unwords [ ['i','n','s','t','e','a','d'], ['o','f'] ], unwords [ ['i','n'], ['l','i','e','u'], ['o','f'] ] ],

    FaCL |< aT                `noun`    {- <badlaT> -}         [ ['s','u','i','t'], ['c','o','s','t','u','m','e'] ]
                              `plural`     FaCaL |< At
                              `plural`     FiCaL,

    FaCaL |< Iy |< aT         `noun`    {- <badalIyaT> -}      [ ['c','o','m','p','e','n','s','a','t','i','o','n'] ],

    FaCIL                     `noun`    {- <badIl> -}          [ ['s','u','b','s','t','i','t','u','t','e'], ['a','l','t','e','r','n','a','t','e'] ]
                              `plural`     FaCIL |< Un
                              `plural`     FuCaLA'
                              `plural`     FaCA'iL
                           
    `derives` otherwise,

    FaCCAL                    `noun`    {- <baddAl> -}         [ ['e','x','c','h','a','n','g','e','r'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FaCCAL                    `noun`    {- <baddAl> -}         [ ['p','e','d','a','l'] ]
                              `plural`     FaCCAL |< At,

    FaCCAL |< aT              `noun`    {- <baddAlaT> -}       [ unwords [ ['t','e','l','e','p','h','o','n','e'], ['e','x','c','h','a','n','g','e'] ] ],

    MaFACiL                   `noun`    {- <mabAdil> -}        [ unwords [ ['h','o','u','s','e'], ['c','l','o','t','h','e','s'] ], unwords [ ['c','a','s','u','a','l'], ['d','r','e','s','s'] ] ],

    TaFCIL                    `noun`    {- <tabdIl> -}         [ ['r','e','p','l','a','c','e','m','e','n','t'], ['e','x','c','h','a','n','g','e'] ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- <mubAdalaT> -}      [ ['e','x','c','h','a','n','g','e'] ],

    HiFCAL                    `noun`    {- <'ibdAl> -}         [ ['s','u','b','s','t','i','t','u','t','e'], ['r','e','p','l','a','c','e'] ],

    HiFCAL                    `noun`    {- <'ibdAl> -}         [ ['s','u','b','s','t','i','t','u','t','i','o','n'], ['r','e','p','l','a','c','e','m','e','n','t'] ]
                              `plural`     HiFCAL |< At,

    TaFaCCuL                  `noun`    {- <tabaddul> -}       [ ['t','r','a','n','s','f','o','r','m','a','t','i','o','n'], ['s','h','i','f','t'] ]
                              `plural`     TaFaCCuL |< At,

    TaFACuL                   `noun`    {- <tabAdul> -}        [ ['e','x','c','h','a','n','g','e'] ]
                              `plural`     TaFACuL |< At,

    IstiFCAL                  `noun`    {- <istibdAl> -}       [ ['s','u','b','s','t','i','t','u','t','i','o','n'], ['e','x','c','h','a','n','g','e'], ['r','e','p','l','a','c','e','m','e','n','t'] ]
                              `plural`     IstiFCAL |< At,

    MutaFACaL                 `adj`     {- <mutabAdal> -}      [ ['m','u','t','u','a','l'], ['r','e','c','i','p','r','o','c','a','l'] ],

    MustaFCaL                 `noun`    {- <mustabdal> -}      [ ['r','e','p','l','a','c','e','m','e','n','t'], ['r','e','s','e','r','v','e'] ]
                              `plural`     MustaFCaL |< Un
                           
    `derives` otherwise ]


cluster_13  = cluster

 |> "b d q" <| [

    FUCaL |< aT               `noun`    {- <bUdaqaT> -}        [ ['c','r','u','c','i','b','l','e'], unwords [ ['m','e','l','t','i','n','g'], ['p','o','t'] ] ]
                              `plural`     FawACiL ]


cluster_14  = cluster

 |> "b d n" <| [

    FaCuL                     `verb`    {- <badun> -}          [ unwords [ ['b','e'], ['c','o','r','p','u','l','e','n','t'] ] ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- <badan> -}          [ unwords [ ['b','e'], ['c','o','r','p','u','l','e','n','t'] ] ]
                              `imperf`     FCuL,

    FaCaL                     `noun`    {- <badan> -}          [ ['b','o','d','y'] ]
                              `plural`     HaFCuL
                              `plural`     HaFCAL,

    FaCaL |< Iy               `adj`     {- <badanIy> -}        [ ['p','h','y','s','i','c','a','l'], ['b','o','d','i','l','y'] ],

    FaCAL |< aT               `noun`    {- <badAnaT> -}        [ ['o','b','e','s','i','t','y'], ['c','o','r','p','u','l','e','n','c','e'] ],

    FaCIL                     `noun`    {- <badIn> -}          [ ['c','o','r','p','u','l','e','n','t'], ['o','b','e','s','e'] ]
                              `plural`     FaCIL |< Un
                              `plural`     FuCuL
                           
    `derives` otherwise,

    FuCUL |< aT               `noun`    {- <budUnaT> -}        [ ['c','o','r','p','u','l','e','n','c','e'], ['o','b','e','s','i','t','y'] ],

    FACiL                     `noun`    {- <bAdin> -}          [ ['c','o','r','p','u','l','e','n','t'], ['o','b','e','s','e'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise ]


cluster_15  = cluster

 |> "b d r m" <| [

    KaRDUS                    `noun`    {- <badrUm> -}         [ ['b','a','s','e','m','e','n','t'] ]
                              `plural`     KaRDUS |< At ]


cluster_16  = cluster

 |> "b d r .g" <| [

    KiRDAS                    `noun`    {- <bidrA.g> -}        [ ['P','e','d','r','a','g'] ] ]


cluster_17  = cluster

 |> "b d r" <| [

    FaCaL                     `verb`    {- <badar> -}          [ ['s','u','r','p','r','i','s','e'], unwords [ ['c','o','m','e'], ['s','u','d','d','e','n','l','y'] ] ]
                              `imperf`     FCuL,

    FACaL                     `verb`    {- <bAdar> -}          [ ['i','n','i','t','i','a','t','e'], ['s','u','r','p','r','i','s','e'] ],

    TaFACaL                   `verb`    {- <tabAdar> -}        [ unwords [ ['c','o','m','e'], ['f','i','r','s','t'] ] ],

    IFtaCaL                   `verb`    {- <ibtadar> -}        [ unwords [ ['r','u','s','h'], ['a','t'] ], unwords [ ['h','u','r','r','y'], ['t','o'] ] ],

    FuCUL                     `noun`    {- <budUr> -}          [ unwords [ ['s','u','d','d','e','n'], ['a','p','p','e','a','r','a','n','c','e'] ] ],

    FaCL                      `noun`    {- <badr> -}           [ unwords [ ['f','u','l','l'], ['m','o','o','n'] ] ]
                              `plural`     FuCUL,

    FaCL |< aT                `noun`    {- <badraT> -}         [ unwords [ ['l','a','r','g','e'], ['s','u','m'] ] ]
                              `plural`     FaCaL |< At
                              `plural`     FiCAL,

    FaCLAn                    `noun`    {- <badrAn> -}         [ ['B','a','d','r','a','n'] ],

    FaCLA' |< Iy              `adj`     {- <badrAwIy> -}       [ ['B','a','d','r','a','w','i'] ],

    FiCAL |<< "aN"            `noun`    {- <bidAraN> -}        [ ['q','u','i','c','k','l','y'], ['h','a','s','t','i','l','y'] ],

    FaCAL |<< "i"             `noun`    {- <badAri> -}         [ unwords [ ['h','u','r','r','y'], "!" ] ],

    MuFACaL |< aT             `noun`    {- <mubAdaraT> -}      [ ['i','n','i','t','i','a','t','i','v','e'], ['p','r','o','p','o','s','a','l'] ],

    FACiL                     `noun`    {- <bAdir> -}          [ ['B','a','d','e','r'] ],

    FACiL |< aT               `noun`    {- <bAdiraT> -}        [ ['g','e','s','t','u','r','e'], unwords [ ['f','i','r','s','t'], ['s','i','g','n'] ] ]
                              `plural`     FawACiL,

    FICAL                     `noun`    {- <bIdAr> -}          [ ['B','i','d','a','r'] ] ]


cluster_18  = cluster

 |> "b f t" <| [

    FaCL |< aT                `noun`    {- <baftaT> -}         [ ['c','a','l','i','c','o'] ] ]


cluster_19  = cluster

 |> "b f s" <| [

    FACL                      `noun`    {- <bAfs> -}           [ ['P','u','f','f','s'] ] ]


cluster_20  = cluster

 |> "b f l" <| [

    FACiL                     `noun`    {- <bAfil> -}          [ ['P','a','v','e','l'] ] ]


cluster_21  = cluster

 |> "b h ^g t" <| [

    KaRDaS                    `noun`    {- <bah^gat> -}        [ ['B','a','h','j','a','t'] ] ]


cluster_22  = cluster

 |> "b h ^g" <| [

    FaCiL                     `verb`    {- <bahi^g> -}         [ ['r','e','j','o','i','c','e'] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <bahu^g> -}         [ unwords [ ['b','e'], ['b','e','a','u','t','i','f','u','l'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <bahha^g> -}        [ unwords [ ['m','a','k','e'], ['h','a','p','p','y'] ], ['e','m','b','e','l','l','i','s','h'] ],

    HaFCaL                    `verb`    {- <'abha^g> -}        [ unwords [ ['m','a','k','e'], ['h','a','p','p','y'] ] ],

    IFtaCaL                   `verb`    {- <ibtaha^g> -}       [ ['r','e','j','o','i','c','e'] ],

    FaCL |< aT                `noun`    {- <bah^gaT> -}        [ ['s','p','l','e','n','d','o','r'], ['d','e','l','i','g','h','t'] ],

    FaCiL                     `noun`    {- <bahi^g> -}         [ ['m','a','g','n','i','f','i','c','e','n','t'], ['h','a','p','p','y'] ]
                              `plural`     FaCiL |< Un
                           
    `derives` otherwise,

    FaCIL                     `noun`    {- <bahI^g> -}         [ ['m','a','g','n','i','f','i','c','e','n','t'], ['h','a','p','p','y'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    MaFCaL |< aT              `noun`    {- <mabha^gaT> -}      [ ['d','e','l','i','g','h','t'] ]
                              `plural`     MaFACiL,

    IFtiCAL                   `noun`    {- <ibtihA^g> -}       [ ['r','e','j','o','i','c','i','n','g'], ['d','e','l','i','g','h','t'] ]
                              `plural`     IFtiCAL |< At,

    MuFCiL                    `noun`    {- <mubhi^g> -}        [ ['c','h','a','r','m','i','n','g'], ['d','e','l','i','g','h','t','f','u','l'] ]
                              `plural`     MuFCiL |< Un
                           
    `derives` otherwise,

    MuFtaCiL                  `noun`    {- <mubtahi^g> -}      [ ['d','e','l','i','g','h','t','e','d'] ]
                              `plural`     MuFtaCiL |< Un
                           
    `derives` otherwise ]


cluster_23  = cluster

 |> "b h .z" <| [

    FaCaL                     `verb`    {- <baha.z> -}         [ ['o','p','p','r','e','s','s'] ]
                              `imperf`     FCaL,

    HaFCaL                    `verb`    {- <'abha.z> -}        [ ['o','p','p','r','e','s','s'], ['o','v','e','r','l','o','a','d'] ],

    FACiL                     `noun`    {- <bAhi.z> -}         [ ['o','p','p','r','e','s','s','i','v','e'], ['e','x','o','r','b','i','t','a','n','t'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise ]


cluster_24  = cluster

 |> "b f t k" <| [

    KiRDIS                    `noun`    {- <biftIk> -}         [ ['b','e','e','f','s','t','e','a','k'] ] ]


cluster_25  = cluster

 |> "b h l" <| [

    FaCaL                     `verb`    {- <bahal> -}          [ ['c','u','r','s','e'] ]
                              `imperf`     FCaL,

    TaFaCCaL                  `verb`    {- <tabahhal> -}       [ ['c','u','r','s','e'] ],

    TaFACaL                   `verb`    {- <tabAhal> -}        [ unwords [ ['c','u','r','s','e'], ['e','a','c','h'], ['o','t','h','e','r'] ] ],

    IFtaCaL                   `verb`    {- <ibtahal> -}        [ ['s','u','p','p','l','i','c','a','t','e'] ],

    HaFCaL                    `noun`    {- <'abhal> -}         [ ['s','a','v','i','n'], unwords [ ['s','a','v','i','n','e'], "(", ['e','v','e','r','g','r','e','e','n'], ['s','h','r','u','b'], ")" ] ],

    MuFACaL |< aT             `noun`    {- <mubAhalaT> -}      [ ['c','u','r','s','i','n','g'] ],

    IFtiCAL                   `noun`    {- <ibtihAl> -}        [ ['s','u','p','p','l','i','c','a','t','i','o','n'], ['p','r','a','y','e','r'] ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `noun`    {- <bAhil> -}          [ ['f','r','e','e'], ['i','n','d','e','p','e','n','d','e','n','t'] ]
                              `plural`     FACiL |< Un
                              `plural`     FuCCaL
                              `plural`     FuCL
                           
    `derives` otherwise,

    FaCLY |< Iy               `adj`     {- <bahlawIy> -}       [ ['P','a','h','l','a','v','i'], ['P','a','h','l','e','v','i'] ],

    FaCLY |< Iy               `adj`     {- <bahlawIy> -}       [ ['B','a','h','l','a','w','i'], ['B','a','h','l','a','o','u','i'] ] ]


cluster_26  = cluster

 |> "b h d l" <| [

    KaRDaS                    `verb`    {- <bahdal> -}         [ ['i','n','s','u','l','t'], ['r','i','d','i','c','u','l','e'] ],

    TaKaRDaS                  `verb`    {- <tabahdal> -}       [ unwords [ ['b','e'], ['i','n','s','u','l','t','e','d'] ], unwords [ ['b','e'], ['i','n'], ['d','i','s','a','r','r','a','y'] ] ],

    KaRDaS |< aT              `noun`    {- <bahdalaT> -}       [ ['i','n','s','u','l','t'], ['a','f','f','r','o','n','t'] ],

    MuKaRDaS                  `noun`    {- <mubahdal> -}       [ ['m','i','s','t','r','e','a','t','e','d'] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise ]


cluster_27  = cluster

 |> "b h q" <| [

    FaCaL                     `noun`    {- <bahaq> -}          [ ['h','e','r','p','e','s'] ],

    HaFCaL                    `adj`     {- <'abhaq> -}         [ ['h','e','r','p','e','t','i','c'] ]
                              `femini`     FaCLA' ]


cluster_28  = cluster

 |> "b h m" <| [

    HaFCaL                    `verb`    {- <'abham> -}         [ unwords [ ['m','a','k','e'], ['o','b','s','c','u','r','e'] ], unwords [ ['b','e'], ['a','m','b','i','g','u','o','u','s'] ] ],

    TaFaCCaL                  `verb`    {- <tabahham> -}       [ unwords [ ['b','e'], ['o','b','s','c','u','r','e'] ] ],

    IstaFCaL                  `verb`    {- <istabham> -}       [ unwords [ ['b','e'], ['o','b','s','c','u','r','e'] ] ],

    FaCL |< aT                `noun`    {- <bahmaT> -}         [ ['s','h','e','e','p'] ],

    FaCIL                     `noun`    {- <bahIm> -}          [ unwords [ ['j','e','t'], "-", ['b','l','a','c','k'] ], ['d','a','r','k'] ]
                              `plural`     FuCuL,

    FaCIL |< aT               `noun`    {- <bahImaT> -}        [ ['a','n','i','m','a','l'], ['b','e','a','s','t'], ['l','i','v','e','s','t','o','c','k'], ['c','a','t','t','l','e'] ]
                              `plural`     FaCA'iL,

    FaCIL |< Iy               `adj`     {- <bahImIy> -}        [ ['a','n','i','m','a','l'], ['b','e','s','t','i','a','l'] ],

    HiFCAL                    `noun`    {- <'ibhAm> -}         [ ['o','b','s','c','u','r','i','t','y'], ['i','n','c','o','m','p','r','e','h','e','n','s','i','b','i','l','i','t','y'] ]
                              `plural`     HiFCAL |< At,

    FACiL                     `noun`    {- <bAhim> -}          [ unwords [ ['b','i','g'], ['t','o','e'] ], ['t','h','u','m','b'] ]
                              `plural`     HaFACIL,

    MuFCaL                    `noun`    {- <mubham> -}         [ ['u','n','i','n','t','e','l','l','i','g','i','b','l','e'], ['a','b','s','t','r','a','c','t'] ]
                              `plural`     MuFCaL |< Un
                           
    `derives` otherwise,

    MuFCaL |< At              `noun`    {- <mubhamAt> -}       [ unwords [ ['i','n','s','c','r','u','t','a','b','l','e'], ['t','o','p','i','c','s'] ] ]
                              `plural`     MuFCaL |< At ]


cluster_29  = cluster

 |> "b h r ^g" <| [

    KaRDaS                    `verb`    {- <bahra^g> -}        [ ['d','i','s','g','u','i','s','e'] ],

    TaKaRDaS                  `verb`    {- <tabahra^g> -}      [ ['d','i','s','g','u','i','s','e'] ],

    KaRDaS                    `noun`    {- <bahra^g> -}        [ ['s','p','u','r','i','o','u','s'], ['c','h','e','a','p'], ['f','a','l','s','e'] ]
                              `plural`     KaRADiS,

    KaRDaS |< aT              `noun`    {- <bahra^gaT> -}      [ unwords [ ['h','o','l','l','o','w'], ['p','o','m','p'] ] ],

    MuKaRDaS                  `noun`    {- <mubahra^g> -}      [ ['g','a','u','d','y'], ['t','r','a','s','h','y'] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise ]


cluster_30  = cluster

 |> "b h r" <| [

    FaCaL                     `verb`    {- <bahar> -}          [ ['b','l','i','n','d'], ['d','a','z','z','l','e'] ]
                              `imperf`     FCaL,

    HaFCaL                    `verb`    {- <'abhar> -}         [ ['d','a','z','z','l','e'] ],

    InFaCaL                   `verb`    {- <inbahar> -}        [ unwords [ ['b','e'], ['d','a','z','z','l','e','d'] ], unwords [ ['b','e'], ['b','l','i','n','d','e','d'] ] ],

    IFtaCaL                   `verb`    {- <ibtahar> -}        [ ['f','l','a','u','n','t'] ],

    FaCL                      `noun`    {- <bahr> -}           [ ['d','e','c','e','p','t','i','o','n'] ],

    FuCL                      `noun`    {- <buhr> -}           [ unwords [ ['l','a','b','o','r','e','d'], ['b','r','e','a','t','h','i','n','g'] ] ],

    FaCL |< aT                `noun`    {- <bahraT> -}         [ ['d','a','z','z','l','e','m','e','n','t'] ],

    FuCL |< aT                `noun`    {- <buhraT> -}         [ ['c','e','n','t','e','r'], ['m','i','d','d','l','e'] ],

    HaFCaL                    `noun`    {- <'abhar> -}         [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['b','r','i','l','l','i','a','n','t'] ] ],

    HaFCaL                    `noun`    {- <'abhar> -}         [ ['a','o','r','t','a'] ],

    HaFCaL |< Iy              `adj`     {- <'abharIy> -}       [ ['a','o','r','t','i','c'] ],

    FaCAL                     `noun`    {- <bahAr> -}          [ ['s','p','i','c','e'] ]
                              `plural`     FaCAL |< At,

    IFtiCAL                   `noun`    {- <ibtihAr> -}        [ ['d','i','s','p','l','a','y'], ['s','h','o','w'] ]
                              `plural`     IFtiCAL |< At,

    FACiL                     `noun`    {- <bAhir> -}          [ ['d','a','z','z','l','i','n','g'], ['s','p','l','e','n','d','i','d'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MaFCUL                    `noun`    {- <mabhUr> -}         [ unwords [ ['o','u','t'], ['o','f'], ['b','r','e','a','t','h'] ], ['p','a','n','t','i','n','g'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise ]


cluster_31  = cluster

 |> "b h t r" <| [

    KaRDaS                    `verb`    {- <bahtar> -}         [ ['l','i','e'] ],

    KaRDaS |< aT              `noun`    {- <bahtaraT> -}       [ ['l','y','i','n','g'] ] ]


cluster_32  = cluster

 |> "b h t n" <| [

    KuRDAS                    `noun`    {- <buhtAn> -}         [ ['s','l','a','n','d','e','r'] ] ]


cluster_33  = cluster

 |> "b h t" <| [

    FaCiL                     `verb`    {- <bahit> -}          [ unwords [ ['b','e','c','o','m','e'], ['p','a','l','e'] ], ['b','l','a','n','c','h','e'] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <bahut> -}          [ unwords [ ['b','e'], ['a','s','t','o','n','i','s','h','e','d'] ] ]
                              `imperf`     FCuL,

    FaCaL                     `verb`    {- <bahat> -}          [ ['a','s','s','a','i','l'], ['a','s','t','o','n','i','s','h'] ]
                              `imperf`     FCaL,

    FACaL                     `verb`    {- <bAhat> -}          [ ['s','t','a','r','t','l','e'], ['a','s','t','o','n','i','s','h'] ],

    HaFCaL                    `verb`    {- <'abhat> -}         [ ['a','s','t','o','n','i','s','h'], ['s','t','a','r','t','l','e'] ],

    InFaCaL                   `verb`    {- <inbahat> -}        [ unwords [ ['b','e'], ['a','s','t','o','n','i','s','h','e','d'] ] ],

    FuCL                      `noun`    {- <buht> -}           [ ['s','l','a','n','d','e','r'] ],

    FuCLAn                    `noun`    {- <buhtAn> -}         [ ['s','l','a','n','d','e','r'] ],

    FaCL |< aT                `noun`    {- <bahtaT> -}         [ ['a','m','a','z','e','m','e','n','t'] ],

    FACiL                     `noun`    {- <bAhit> -}          [ ['p','a','l','e'], ['a','s','t','o','n','i','s','h','e','d'], ['s','t','a','r','t','l','e','d'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MaFCUL                    `noun`    {- <mabhUt> -}         [ ['a','s','t','o','n','i','s','h','e','d'], ['s','t','a','r','t','l','e','d'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise ]


cluster_34  = cluster

 |> "b h r z" <| [

    KiRDUS                    `noun`    {- <bihrUz> -}         [ ['B','e','h','r','o','u','z'] ],

    KaRDIS                    `noun`    {- <bahrIz> -}         [ ['B','a','h','r','i','z'] ],

    KaRDIS                    `noun`    {- <bahrIz> -}         [ ['b','r','o','t','h'] ] ]


cluster_35  = cluster

 |> "b h z" <| [

    FaCaL                     `verb`    {- <bahaz> -}          [ ['p','u','s','h'], ['r','e','j','e','c','t'] ]
                              `imperf`     FCaL,

    FaCL |< aT                `noun`    {- <bahzaT> -}         [ ['r','e','j','e','c','t','i','o','n'] ]
                              `plural`     FaCaL |< At ]


cluster_36  = cluster

 |> "b k l" <| [

    FaCCaL                    `verb`    {- <bakkal> -}         [ ['b','u','c','k','l','e'], ['f','o','l','d'] ],

    FuCL |< aT                `noun`    {- <buklaT> -}         [ ['c','l','a','s','p'] ]
                              `plural`     FuCaL,

    FaL                       `conj`    {- <bal> -}            [ unwords [ ['b','u','t'], ['r','a','t','h','e','r'] ], unwords [ ['i','n'], ['f','a','c','t'] ] ] ]


cluster_37  = cluster

 |> "b k b ^s" <| [

    KiRDAS |< Iy              `adj`     {- <bikbA^sIy> -}      [ unwords [ ['l','i','e','u','t','e','n','a','n','t'], ['c','o','l','o','n','e','l'] ] ] ]


cluster_38  = cluster

 |> "b k m" <| [

    FaCiL                     `verb`    {- <bakim> -}          [ unwords [ ['b','e'], ['m','u','t','e'] ] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <bakum> -}          [ unwords [ ['b','e'], ['s','i','l','e','n','t'] ] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'abkam> -}         [ ['s','i','l','e','n','c','e'], ['m','u','t','e'] ],

    TaFaCCaL                  `verb`    {- <tabakkam> -}       [ unwords [ ['b','e'], ['s','i','l','e','n','t'] ], unwords [ ['b','e'], ['m','u','t','e'] ] ],

    FaCaL                     `noun`    {- <bakam> -}          [ ['m','u','t','e','n','e','s','s'], ['s','i','l','e','n','c','e'] ],

    HaFCaL                    `adj`     {- <'abkam> -}         [ ['s','i','l','e','n','t'], ['m','u','t','e'] ]
                              `plural`     FuCL
                              `femini`     FaCLA' ]


cluster_39  = cluster

 |> "b k r" <| [

    FaCaL                     `verb`    {- <bakar> -}          [ unwords [ ['r','i','s','e'], ['e','a','r','l','y'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <bakkar> -}         [ unwords [ ['d','o'], ['e','a','r','l','y'] ], unwords [ ['b','e'], ['d','o','n','e'], ['e','a','r','l','y'] ] ],

    FACaL                     `verb`    {- <bAkar> -}          [ ['a','n','t','i','c','i','p','a','t','e'], ['f','o','r','e','s','t','a','l','l'] ],

    HaFCaL                    `verb`    {- <'abkar> -}         [ unwords [ ['b','e'], ['t','h','e'], ['f','i','r','s','t'] ], unwords [ ['b','e'], ['p','r','e','c','o','c','i','o','u','s'] ] ],

    IFtaCaL                   `verb`    {- <ibtakar> -}        [ ['i','n','v','e','n','t'], ['o','r','i','g','i','n','a','t','e'] ],

    FaCL                      `noun`    {- <bakr> -}           [ ['B','a','k','r'] ],

    FaCL |< Iy                `adj`     {- <bakrIy> -}         [ ['B','a','k','r','i'] ],

    FaCL                      `noun`    {- <bakr> -}           [ unwords [ ['y','o','u','n','g'], ['c','a','m','e','l'] ] ]
                              `plural`     HaFCuL
                              `plural`     FuCLAn,

    FiCL                      `adj`     {- <bikr> -}           [ ['e','l','d','e','s','t'], unwords [ ['f','i','r','s','t'], "-", ['b','o','r','n'] ] ]
                              `plural`     HaFCAL,

    FiCL |< Iy                `adj`     {- <bikrIy> -}         [ unwords [ ['f','i','r','s','t'], "-", ['b','o','r','n'] ] ],

    FaCL |< aT                `noun`    {- <bakraT> -}         [ ['r','e','e','l'], ['p','u','l','l','e','y'], ['w','i','n','c','h'] ]
                              `plural`     FaCaL |< At,

    FuCL |< aT                `noun`    {- <bukraT> -}         [ ['t','o','m','o','r','r','o','w'] ],

    FaCIL                     `noun`    {- <bakIr> -}          [ ['e','a','r','l','y'], ['p','r','e','c','o','c','i','o','u','s'], ['p','r','e','m','a','t','u','r','e'] ]
                              `plural`     FaCIL |< Un
                              `plural`     FaCUL |< Un
                              `plural`     FACUL |< Un
                           
    `derives` otherwise,

    FuCUL                     `noun`    {- <bukUr> -}          [ ['e','a','r','l','i','n','e','s','s'], ['d','a','y','b','r','e','a','k'] ],

    FaCAL |< aT               `noun`    {- <bakAraT> -}        [ ['v','i','r','g','i','n','i','t','y'] ],

    FaCCAL |< aT              `noun`    {- <bakkAraT> -}       [ ['p','u','l','l','e','y'] ],

    FuCUL |< aT               `noun`    {- <bukUraT> -}        [ ['p','r','i','m','o','g','e','n','i','t','u','r','e'] ],

    FACUL |< aT               `noun`    {- <bAkUraT> -}        [ unwords [ ['f','i','r','s','t'], ['w','o','r','k','s'] ] ]
                              `plural`     FawACIL,

    HaFCaL                    `noun`    {- <'abkar> -}         [ unwords [ ['e','a','r','l','i','e','r'], "/", ['e','a','r','l','i','e','s','t'] ] ],

    MiFCAL                    `noun`    {- <mibkAr> -}         [ ['p','r','e','c','o','c','i','o','u','s'] ],

    IFtiCAL                   `noun`    {- <ibtikAr> -}        [ ['i','n','v','e','n','t','i','o','n'] ]
                              `plural`     IFtiCAL |< At,

    IFtiCAL |< Iy             `adj`     {- <ibtikArIy> -}      [ ['o','r','i','g','i','n','a','l'], ['i','n','v','e','n','t','i','v','e'] ],

    IFtiCAL |< Iy |< aT       `noun`    {- <ibtikArIyaT> -}    [ ['o','r','i','g','i','n','a','l','i','t','y'], ['i','n','v','e','n','t','i','v','e','n','e','s','s'] ],

    FACiL                     `noun`    {- <bAkir> -}          [ ['e','a','r','l','y'] ],

    FACiL |< aT               `noun`    {- <bAkiraT> -}        [ ['h','a','r','b','i','n','g','e','r'], unwords [ ['e','a','r','l','y'], ['f','r','u','i','t','s'] ] ]
                              `plural`     FawACiL,

    MuFaCCiL                  `adj`     {- <mubakkir> -}       [ ['e','a','r','l','y'] ],

    MuFtaCiL                  `noun`    {- <mubtakir> -}       [ ['i','n','v','e','n','t','o','r'], ['o','r','i','g','i','n','a','l'] ]
                              `plural`     MuFtaCiL |< Un
                           
    `derives` otherwise,

    MuFtaCaL                  `noun`    {- <mubtakar> -}       [ ['i','n','v','e','n','t','i','o','n'], ['c','r','e','a','t','i','o','n'] ]
                              `plural`     MuFtaCaL |< At,

    FUCaL                     `noun`    {- <bUkar> -}          [ ['p','o','k','e','r'] ],

    FICAL                     `noun`    {- <bIkAr> -}          [ ['c','o','m','p','a','s','s'], ['d','i','v','i','d','e','r','s'] ] ]


cluster_40  = cluster

 |> "b k t r" <| [

    KaRDIS |< Iy              `adj`     {- <baktIrIy> -}       [ ['b','a','c','t','e','r','i','a','l'] ] ]


cluster_41  = cluster

 |> "b k t" <| [

    FaCCaL                    `verb`    {- <bakkat> -}         [ ['c','e','n','s','u','r','e'], ['r','e','p','r','i','m','a','n','d'] ],

    TaFCIL                    `noun`    {- <tabkIt> -}         [ ['r','e','p','r','o','a','c','h'], ['r','e','m','o','r','s','e'] ]
                              `plural`     TaFCIL |< At ]


cluster_42  = cluster

 |> "b k r ^g" <| [

    KaRDaS                    `noun`    {- <bakra^g> -}        [ ['k','e','t','t','l','e'], unwords [ ['c','o','f','f','e','e'], ['p','o','t'] ] ]
                              `plural`     KaRADiS ]


cluster_43  = cluster

 |> "b l .g" <| [

    FaCaL                     `verb`    {- <bala.g> -}         [ ['r','e','a','c','h'], ['a','t','t','a','i','n'] ]
                              `imperf`     FCuL,

    FaCuL                     `verb`    {- <balu.g> -}         [ unwords [ ['b','e'], ['e','l','o','q','u','e','n','t'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <balla.g> -}        [ ['c','o','m','m','u','n','i','c','a','t','e'], ['c','o','n','v','e','y'] ],

    FACaL                     `verb`    {- <bAla.g> -}         [ ['e','x','a','g','g','e','r','a','t','e'] ],

    HaFCaL                    `verb`    {- <'abla.g> -}        [ ['r','e','p','o','r','t'], ['i','n','f','o','r','m'], ['n','o','t','i','f','y'] ],

    TaFaCCaL                  `verb`    {- <taballa.g> -}      [ unwords [ ['b','e'], ['i','n','f','o','r','m','e','d'] ] ],

    FuCL |< aT                `noun`    {- <bul.gaT> -}        [ ['c','o','m','p','e','t','e','n','c','y'], ['a','d','e','q','u','a','c','y'] ],

    FaCAL                     `noun`    {- <balA.g> -}         [ ['c','o','m','m','u','n','i','q','u','e'], ['r','e','p','o','r','t'], ['n','o','t','i','f','i','c','a','t','i','o','n'], ['n','o','t','i','c','e'] ]
                              `plural`     FaCAL |< At,

    FaCIL                     `noun`    {- <balI.g> -}         [ ['o','r','a','t','o','r'] ]
                              `plural`     FuCaLA'
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    FaCIL                     `adj`     {- <balI.g> -}         [ ['e','l','o','q','u','e','n','t'] ]
                              `plural`     FuCaLA'
                              `plural`     FaCIL |< Un,

    FuCUL                     `noun`    {- <bulU.g> -}         [ ['r','e','a','c','h','i','n','g'], ['a','t','t','a','i','n','m','e','n','t'] ],

    FaCAL |< aT               `noun`    {- <balA.gaT> -}       [ ['e','l','o','q','u','e','n','c','e'] ],

    HaFCaL                    `noun`    {- <'abla.g> -}        [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['e','l','o','q','u','e','n','t'] ] ],

    MaFCaL                    `noun`    {- <mabla.g> -}        [ ['a','m','o','u','n','t'], ['s','u','m'] ]
                              `plural`     MaFACiL,

    TaFCIL                    `noun`    {- <tablI.g> -}        [ ['c','o','n','v','e','y','a','n','c','e'], ['n','o','t','i','f','i','c','a','t','i','o','n'], ['i','n','f','o','r','m','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    MuFACaL |< aT             `noun`    {- <mubAla.gaT> -}     [ ['e','x','a','g','g','e','r','a','t','i','o','n'] ],

    HiFCAL                    `noun`    {- <'iblA.g> -}        [ ['r','e','p','o','r','t','i','n','g'], ['n','o','t','i','f','i','c','a','t','i','o','n'], ['i','n','f','o','r','m','i','n','g'] ]
                              `plural`     HiFCAL |< At,

    FACiL                     `adj`     {- <bAli.g> -}         [ ['e','x','t','r','e','m','e'], ['p','r','o','f','o','u','n','d'] ],

    FACiL                     `adj`     {- <bAli.g> -}         [ ['r','e','a','c','h','i','n','g'], ['a','t','t','a','i','n','i','n','g'], ['a','d','u','l','t'] ],

    FACiL                     `noun`    {- <bAli.g> -}         [ ['s','e','r','i','o','u','s'], ['c','r','i','t','i','c','a','l'] ],

    MuFaCCiL                  `noun`    {- <muballi.g> -}      [ ['i','n','f','o','r','m','e','r'], ['a','n','n','o','u','n','c','i','n','g'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFACiL                   `noun`    {- <mubAli.g> -}       [ ['e','x','a','g','g','e','r','a','t','i','n','g'] ]
                              `plural`     MuFACiL |< Un
                           
    `derives` otherwise,

    MuFACaL                   `noun`    {- <mubAla.g> -}       [ ['e','x','a','g','g','e','r','a','t','e','d'] ],

    FuCL |< aT                `noun`    {- <bul.gaT> -}        [ ['s','l','i','p','p','e','r'] ]
                              `plural`     FuCaL ]


cluster_44  = cluster

 |> "b l .t" <| [

    FaCCaL                    `verb`    {- <balla.t> -}        [ ['p','a','v','e'], ['t','i','l','e'] ],

    FaCAL                     `noun`    {- <balA.t> -}         [ ['c','o','u','r','t'], ['p','a','l','a','c','e'] ],

    FaCAL |< aT               `noun`    {- <balA.taT> -}       [ ['B','a','l','a','t','a'] ],

    FaCAL |< aT               `noun`    {- <balA.taT> -}       [ unwords [ ['f','l','o','o','r'], ['t','i','l','e'] ] ]
                              `plural`     FaCAL
                              `plural`     HaFCiL |< aT,

    TaFCIL                    `noun`    {- <tablI.t> -}        [ unwords [ ['t','i','l','e'], "-", ['l','a','y','i','n','g'] ], ['p','a','v','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    MuFaCCaL                  `noun`    {- <muballa.t> -}      [ ['t','i','l','e','d'], ['p','a','v','e','d'] ],

    FaCCUL                    `noun`    {- <ballU.t> -}        [ ['o','a','k'], ['a','c','o','r','n'] ],

    FaCCUL |< aT              `noun`    {- <ballU.taT> -}      [ ['a','c','o','r','n'], ['o','a','k'] ],

    FaCL |< aT                `noun`    {- <bal.taT> -}        [ ['a','x'] ]
                              `plural`     FaCaL |< At
                              `plural`     FuCaL ]


cluster_45  = cluster

 |> "b l .s" <| [

    FaCaL                     `verb`    {- <bala.s> -}         [ ['e','x','t','o','r','t'], ['b','l','a','c','k','m','a','i','l'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <balla.s> -}        [ ['e','x','t','o','r','t'], ['b','l','a','c','k','m','a','i','l'] ],

    FaCL                      `noun`    {- <bal.s> -}          [ ['e','x','t','o','r','t','i','o','n'], ['b','l','a','c','k','m','a','i','l'] ],

    FaCCAL                    `noun`    {- <ballA.s> -}        [ ['j','a','r'] ]
                              `plural`     FaCACIL ]


cluster_46  = cluster

 |> "b l .h" <| [

    FaCaL                     `noun`    {- <bala.h> -}         [ ['B','a','l','a','h'] ],

    FaCaL                     `verb`    {- <bala.h> -}         [ unwords [ ['d','r','y'], ['u','p'] ] ]
                              `imperf`     FCaL,

    FaCaL                     `noun`    {- <bala.h> -}         [ unwords [ ['d','a','t','e','s'], "(", ['f','r','u','i','t'], ")" ], unwords [ ['d','a','t','e'], "(", ['f','r','u','i','t'], ")" ] ]
                              `plural`     FaCaL |< At ]


cluster_47  = cluster

 |> "b l .g r" <| [

    KuRDAS                    `noun`    {- <bul.gAr> -}        [ ['B','u','l','g','a','r','i','a','n','s'] ],

    KuRDAS |< Iy              `adj`     {- <bul.gArIy> -}      [ ['B','u','l','g','a','r','i','a','n'] ] ]


cluster_48  = cluster

 |> "b l .g m" <| [

    KaRDaS                    `noun`    {- <bal.gam> -}        [ ['p','h','l','e','g','m'] ]
                              `plural`     KaRADiS,

    KaRDaS |< Iy              `adj`     {- <bal.gamIy> -}      [ ['m','u','c','o','u','s'], ['p','h','l','e','g','m','y'] ] ]


cluster_49  = cluster

 |> "b l ^s f" <| [

    KaRDaS                    `verb`    {- <bal^saf> -}        [ ['B','o','l','s','h','e','v','i','z','e'] ],

    TaKaRDaS                  `verb`    {- <tabal^saf> -}      [ unwords [ ['b','e'], ['B','o','l','s','h','e','v','i','z','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <bal^safaT> -}      [ ['B','o','l','s','h','e','v','i','z','a','t','i','o','n'] ],

    KaRDaS |< Iy              `adj`     {- <bal^safIy> -}      [ ['B','o','l','s','h','e','v','i','s','t'] ]
                              `plural`     KaRADiS |< aT
                              `plural`     KaRDaS |< Iy |< Un,

    KaRDaS |< Iy |< aT        `noun`    {- <bal^safIyaT> -}    [ ['B','o','l','s','h','e','v','i','s','m'] ] ]


cluster_50  = cluster

 |> "b l ^s" <| [

    FiCAL                     `noun`    {- <bilA^s> -}         [ ['g','r','a','t','i','s'] ],

    FiCAL                     `noun`    {- <bilA^s> -}         [ unwords [ ['n','e','v','e','r'], ['m','i','n','d'] ], unwords [ ['f','o','r','g','e','t'], ['a','b','o','u','t'] ] ],

    FaCUL |< Iy               `adj`     {- <balU^sIy> -}       [ ['B','a','l','u','c','h','i'] ] ]


cluster_51  = cluster

 |> "b l ^g k" <| [

    KiRDIS |< Iy              `adj`     {- <bil^gIkIy> -}      [ ['B','e','l','g','i','a','n'] ] ]


cluster_52  = cluster

 |> "b l ^g" <| [

    FaCaL                     `verb`    {- <bala^g> -}         [ ['d','a','w','n'], ['s','h','i','n','e'] ]
                              `imperf`     FCuL,

    FaCiL                     `verb`    {- <bali^g> -}         [ unwords [ ['b','e'], ['h','a','p','p','y'] ], unwords [ ['b','e'], ['s','e','r','e','n','e'] ] ]
                              `imperf`     FCaL,

    HaFCaL                    `verb`    {- <'abla^g> -}        [ ['d','a','w','n'], ['s','h','i','n','e'] ],

    InFaCaL                   `verb`    {- <inbala^g> -}       [ ['d','a','w','n'], ['s','h','i','n','e'] ],

    IFtaCaL                   `verb`    {- <ibtala^g> -}       [ unwords [ ['b','e'], ['h','a','p','p','y'] ], ['d','a','w','n'], ['s','h','i','n','e'] ],

    HaFCaL                    `noun`    {- <'abla^g> -}        [ ['b','r','i','g','h','t'], ['b','e','a','u','t','i','f','u','l'] ],

    InFiCAL                   `noun`    {- <inbilA^g> -}       [ ['d','a','w','n','i','n','g'], ['d','a','y','b','r','e','a','k'] ]
                              `plural`     InFiCAL |< At ]


cluster_53  = cluster

 |> "b l .z" <| [

    FACUL |< aT               `noun`    {- <bAlU.zaT> -}       [ ['c','r','e','a','m'] ] ]


cluster_54  = cluster

 |> "b l .t q" <| [

    KaRDIS                    `noun`    {- <bal.tIq> -}        [ ['B','a','l','t','i','c'] ] ]


cluster_55  = cluster

 |> "b l .t ^g" <| [

    KaRDaS |< Iy              `adj`     {- <bal.ta^gIy> -}     [ ['r','u','f','f','i','a','n'] ] ]


cluster_56  = cluster

 |> "b l b l" <| [

    KaRDaS                    `verb`    {- <balbal> -}         [ ['a','g','i','t','a','t','e'], ['d','i','s','t','u','r','b'] ],

    TaKaRDaS                  `verb`    {- <tabalbal> -}       [ unwords [ ['b','e'], ['a','g','i','t','a','t','e','d'] ], unwords [ ['b','e'], ['d','i','s','t','u','r','b','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <balbalaT> -}       [ ['c','o','n','f','u','s','i','o','n'], ['c','h','a','o','s'] ]
                              `plural`     KaRADiS,

    KaRDAS                    `noun`    {- <balbAl> -}         [ ['a','n','x','i','e','t','y'] ],

    TaKaRDuS                  `noun`    {- <tabalbul> -}       [ ['m','u','d','d','l','e'], ['c','o','n','f','u','s','i','o','n'] ]
                              `plural`     TaKaRDuS |< At,

    KuRDuS                    `noun`    {- <bulbul> -}         [ ['n','i','g','h','t','i','n','g','a','l','e'] ]
                              `plural`     KaRADiS ]


cluster_57  = cluster

 |> "b l b .t" <| [

    KaRDaS                    `verb`    {- <balba.t> -}        [ ['g','u','r','g','l','e'] ] ]


cluster_58  = cluster

 |> "b l ` m" <| [

    KaRDaS                    `noun`    {- <bal`am> -}         [ ['p','h','a','g','o','c','y','t','e'] ]
                              `plural`     KaRADiS,

    KuRDuS                    `noun`    {- <bul`um> -}         [ ['p','h','a','r','y','n','x'] ]
                              `plural`     KaRADiS,

    KuRDUS                    `noun`    {- <bul`Um> -}         [ ['p','h','a','r','y','n','x'] ]
                              `plural`     KaRADIS ]


cluster_59  = cluster

 |> "b l `" <| [

    FaCaL                     `verb`    {- <bala`> -}          [ ['s','w','a','l','l','o','w'], ['a','b','s','o','r','b'] ]
                              `imperf`     FCaL,

    FaCiL                     `verb`    {- <bali`> -}          [ ['s','w','a','l','l','o','w'], ['a','b','s','o','r','b'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <balla`> -}         [ unwords [ ['m','a','k','e'], ['s','w','a','l','l','o','w'] ] ],

    HaFCaL                    `verb`    {- <'abla`> -}         [ unwords [ ['m','a','k','e'], ['s','w','a','l','l','o','w'] ] ],

    InFaCaL                   `verb`    {- <inbala`> -}        [ unwords [ ['b','e'], ['s','w','a','l','l','o','w','e','d'] ] ],

    IFtaCaL                   `verb`    {- <ibtala`> -}        [ ['s','w','a','l','l','o','w'] ],

    FaCL                      `noun`    {- <bal`> -}           [ ['s','w','a','l','l','o','w','i','n','g'], ['a','b','s','o','r','p','t','i','o','n'] ],

    FaCL |< aT                `noun`    {- <bal`aT> -}         [ ['g','u','l','p'] ],

    FaCCAL |< aT              `noun`    {- <ballA`aT> -}       [ ['d','r','a','i','n'], ['s','i','n','k'] ]
                              `plural`     FaCCUL |< At
                              `plural`     FaCACIL,

    FACUL                     `noun`    {- <bAlU`> -}          [ ['d','r','a','i','n'], ['s','e','w','e','r'] ]
                              `plural`     FACUL |< At
                              `plural`     FawACIL ]


cluster_60  = cluster

 |> "b l f r" <| [

    KaRDUS                    `noun`    {- <balfUr> -}         [ ['B','a','l','f','o','u','r'] ],

    KaRDUS |< Iy              `adj`     {- <balfUrIy> -}       [ ['B','a','l','f','o','u','r'] ] ]


cluster_61  = cluster

 |> "b l f" <| [

    FaCaL                     `verb`    {- <balaf> -}          [ ['b','l','u','f','f'] ]
                              `imperf`     FCiL,

    FaCCaL                    `verb`    {- <ballaf> -}         [ ['b','l','u','f','f'] ],

    FaCL                      `noun`    {- <balf> -}           [ ['b','l','u','f','f'] ] ]


cluster_62  = cluster

 |> "b l d" <| [

    FaCuL                     `verb`    {- <balud> -}          [ unwords [ ['b','e'], ['s','t','u','p','i','d'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <ballad> -}         [ ['a','c','c','l','i','m','a','t','e'], ['h','a','b','i','t','u','a','t','e'] ],

    TaFaCCaL                  `verb`    {- <taballad> -}       [ unwords [ ['b','e'], ['h','a','b','i','t','u','a','t','e','d'] ], unwords [ ['b','e'], ['a','c','c','l','i','m','a','t','e','d'] ] ],

    TaFACaL                   `verb`    {- <tabAlad> -}        [ unwords [ ['f','e','i','g','n'], ['s','t','u','p','i','d','i','t','y'] ] ],

    FaCaL                     `noun`    {- <balad> -}          [ ['c','o','u','n','t','r','y'] ]
                              `plural`     FiCAL
                              `plural`     FuCLAn,

    FaCL |< aT                `noun`    {- <baldaT> -}         [ ['t','o','w','n','s','h','i','p'] ],

    FaCaL |< Iy               `adj`     {- <baladIy> -}        [ ['i','n','d','i','g','e','n','o','u','s'], ['p','o','p','u','l','a','r'] ],

    FaCaL |< Iy |< aT         `noun`    {- <baladIyaT> -}      [ ['m','u','n','i','c','i','p','a','l','i','t','y'], ['t','o','w','n','s','h','i','p'] ],

    FaCIL                     `noun`    {- <balId> -}          [ ['s','t','u','p','i','d'] ]
                              `plural`     FaCIL |< Un
                           
    `derives` otherwise,

    HaFCaL                    `noun`    {- <'ablad> -}         [ ['s','t','u','p','i','d'] ],

    FaCAL                     `noun`    {- <balAd> -}          [ ['s','t','u','p','i','d','i','t','y'] ],

    TaFaCCuL                  `noun`    {- <taballud> -}       [ ['i','d','i','o','c','y'], ['a','p','a','t','h','y'] ]
                              `plural`     TaFaCCuL |< At,

    MutaFaCCiL                `noun`    {- <mutaballid> -}     [ ['s','t','u','p','i','d'], ['a','p','a','t','h','e','t','i','c'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise,

    FUCAL                     `noun`    {- <bUlAd> -}          [ ['s','t','e','e','l'] ] ]


cluster_63  = cluster

 |> "b l k n" <| [

    KaRDUS                    `noun`    {- <balkUn> -}         [ ['b','a','l','c','o','n','y'] ]
                              `plural`     KaRDUS |< At ]


cluster_64  = cluster

 |> "b l k" <| [

    FCaL                      `noun`    {- <blak> -}           [ unwords [ ['s','p','a','r','k'], ['p','l','u','g'] ] ]
                              `plural`     FCaL |< At,

    FuCUL                     `noun`    {- <bulUk> -}          [ ['b','l','o','c','k'] ]
                              `plural`     FuCUL |< At,

    FuCuL                     `noun`    {- <buluk> -}          [ unwords [ ['c','o','m','p','a','n','y'], "(", ['m','i','l','.'], ")" ] ]
                              `plural`     FuCUL ]


cluster_65  = cluster

 |> "b l h n" <| [

    KuRDUS                    `noun`    {- <bulhUn> -}         [ ['s','p','h','i','n','x'] ]
                              `plural`     KaRADIS ]


cluster_66  = cluster

 |> "b l h" <| [

    FaCiL                     `verb`    {- <balih> -}          [ unwords [ ['b','e'], ['s','t','u','p','i','d'] ] ]
                              `imperf`     FCaL,

    TaFACaL                   `verb`    {- <tabAlah> -}        [ unwords [ ['f','e','i','g','n'], ['s','t','u','p','i','d','i','t','y'] ], unwords [ ['p','l','a','y'], ['t','h','e'], ['f','o','o','l'] ] ],

    IstaFCaL                  `verb`    {- <istablah> -}       [ unwords [ ['d','e','e','m'], ['s','t','u','p','i','d'] ] ],

    FaCaL                     `noun`    {- <balah> -}          [ ['s','t','u','p','i','d','i','t','y'] ]
                              `plural`     FaCAL |< aT,

    FaCL |<< "a"              `noun`    {- <balha> -}          [ unwords [ ['n','o','t'], ['t','o'], ['m','e','n','t','i','o','n'] ] ],

    HaFCaL                    `adj`     {- <'ablah> -}         [ ['d','o','l','t','i','s','h'], ['d','u','m','b'] ]
                              `plural`     FuCL
                              `femini`     FaCLA' ]


cluster_67  = cluster

 |> "b l s" <| [

    FaCaLAn                   `noun`    {- <balasAn> -}        [ unwords [ ['g','a','r','d','e','n'], ['b','a','l','m'] ], unwords [ ['b','a','l','m'], "-", ['t','r','e','e'] ] ] ]


cluster_68  = cluster

 |> "b l r" <| [

    FaCCUL                    `adj`     {- <ballUr> -}         [ ['c','r','y','s','t','a','l'] ]
                              `plural`     FaCCUL |< At ]


cluster_69  = cluster

 |> "b l q s" <| [

    KiRDIS                    `noun`    {- <bilqIs> -}         [ ['B','i','l','q','i','s'] ] ]


cluster_70  = cluster

 |> "b l q n" <| [

    KaRDAS                    `noun`    {- <balqAn> -}         [ ['B','a','l','k','a','n','s'] ],

    KaRDAS |< Iy              `adj`     {- <balqAnIy> -}       [ ['B','a','l','k','a','n'] ],

    KaRDaS |< aT              `noun`    {- <balqanaT> -}       [ ['B','a','l','k','a','n','i','z','a','t','i','o','n'] ] ]


cluster_71  = cluster

 |> "b l q `" <| [

    KaRDaS                    `noun`    {- <balqa`> -}         [ ['w','a','s','t','e','l','a','n','d'] ]
                              `plural`     KaRADiS ]


cluster_72  = cluster

 |> "b l q" <| [

    HaFCaL                    `adj`     {- <'ablaq> -}         [ unwords [ ['b','l','a','c','k'], ['a','n','d'], ['w','h','i','t','e'] ], ['p','i','n','t','o'], ['p','i','e','b','a','l','d'] ]
                              `femini`     FaCLA',

    FaCLAn                    `noun`    {- <balqAn> -}         [ ['B','a','l','k','a','n','s'] ],

    FaCLAn |< Iy              `adj`     {- <balqAnIy> -}       [ ['B','a','l','k','a','n'] ] ]


cluster_73  = cluster

 |> "ballUn" <| [

    _____                     `noun`    {- <ballUn> -}         [ ['b','a','l','l','o','o','n'] ] ]



 |> "bAlUn" <| [

    _____                     `noun`    {- <bAlUn> -}          [ ['b','a','l','l','o','o','n'] ]
                              `plural`     _____ |< At ]



 |> "b l n" <| [

    FaCCAL                    `noun`    {- <ballAn> -}         [ unwords [ ['b','a','t','h'], "-", ['m','a','i','d'] ] ] ]


cluster_74  = cluster

 |> "b l m r" <| [

    KaRDaS                    `verb`    {- <balmar> -}         [ ['p','o','l','y','m','e','r','i','z','e'] ],

    TaKaRDaS                  `verb`    {- <tabalmar> -}       [ unwords [ ['b','e'], ['p','o','l','y','m','e','r','i','z','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <balmaraT> -}       [ ['p','o','l','y','m','e','r','i','z','a','t','i','o','n'] ] ]


cluster_75  = cluster

 |> "b l m" <| [

    FACL                      `noun`    {- <bAlm> -}           [ ['P','a','l','m'] ],

    HaFCaL                    `verb`    {- <'ablam> -}         [ unwords [ ['b','e'], ['s','i','l','e','n','t'] ] ],

    FaCaL                     `noun`    {- <balam> -}          [ ['a','n','c','h','o','v','y'], unwords [ ['s','a','i','l','i','n','g'], ['b','a','r','g','e'] ] ],

    HaFCAL                    `noun`    {- <'ablAm> -}         [ unwords [ ['s','a','i','l','i','n','g'], ['b','a','r','g','e','s'] ] ] ]


cluster_76  = cluster

 |> "b l s m" <| [

    KaRDaS                    `noun`    {- <balsam> -}         [ ['b','a','l','s','a','m'] ]
                              `plural`     KaRADiS,

    KaRDaS |< Iy              `adj`     {- <balsamIy> -}       [ ['b','a','l','s','a','m','i','c'] ] ]


cluster_77  = cluster

 |> "b m b h" <| [

    KaRDaS                    `noun`    {- <bambah> -}         [ unwords [ ['h','o','t'], ['p','i','n','k'] ] ] ]


cluster_78  = cluster

 |> "b m b ^s" <| [

    KiRDAS |< Iy              `adj`     {- <bimbA^sIy> -}      [ unwords [ ['l','i','e','u','t','e','n','a','n','t'], ['c','o','l','o','n','e','l'] ] ] ]


cluster_79  = cluster

 |> "b m b .g" <| [

    KuRDAS                    `noun`    {- <bumbA.g> -}        [ unwords [ ['b','o','w'], ['t','i','e'] ] ] ]


cluster_80  = cluster

 |> "b m b" <| [

    FaCL |< Iy                `adj`     {- <bambIy> -}         [ unwords [ ['h','o','t'], ['p','i','n','k'] ] ] ]


cluster_81  = cluster

 |> "b m " <| [

    FiC |<< "a"               `part`    {- <!!! b m !!!a> -}   [ unwords [ ['w','i','t','h'], ['w','h','a','t'] ] ] ]


cluster_82  = cluster

 |> "b n ^g" <| [

    FaCCaL                    `verb`    {- <banna^g> -}        [ ['a','n','e','s','t','h','e','t','i','z','e'], ['n','a','r','c','o','t','i','z','e'] ],

    FaCL                      `noun`    {- <ban^g> -}          [ unwords [ ['h','e','n','b','a','n','e'], "(", ['p','o','i','s','o','n','o','u','s'], ['p','l','a','n','t'], ")" ], ['a','n','e','s','t','h','e','t','i','c'], ['n','a','r','c','o','t','i','c'], ['b','o','n','g'] ] ]


cluster_83  = cluster

 |> "b n .t" <| [

    FuCL                      `noun`    {- <bun.t> -}          [ ['p','o','i','n','t'] ]
                              `plural`     FuCUL,

    FuCL |< aT                `noun`    {- <bun.taT> -}        [ unwords [ ['d','r','i','l','l'], ['b','i','t'] ] ]
                              `plural`     FuCaL ]


cluster_84  = cluster

 |> "b n .s r" <| [

    KiRDiS                    `noun`    {- <bin.sir> -}        [ unwords [ ['l','i','t','t','l','e'], ['f','i','n','g','e','r'] ], ['p','i','n','k','i','e'] ]
                              `plural`     KaRADiS ]


cluster_85  = cluster

 |> "b n .g z" <| [

    KaRDAS |< Iy              `adj`     {- <ban.gAzIy> -}      [ ['B','e','n','g','h','a','z','i'] ] ]


cluster_86  = cluster

 |> "b n .g l" <| [

    KaRDAS                    `noun`    {- <ban.gAl> -}        [ ['B','e','n','g','a','l'] ],

    KaRDAS |< Iy              `adj`     {- <ban.gAlIy> -}      [ ['B','e','n','g','a','l','i'] ] ]


cluster_87  = cluster

 |> "b n f ^s" <| [

    KaRDaS                    `noun`    {- <banfa^s> -}        [ ['a','m','e','t','h','y','s','t'] ] ]


cluster_88  = cluster

 |> "b n d r" <| [

    KaRDaS                    `noun`    {- <bandar> -}         [ ['B','a','n','d','a','r'] ],

    KaRDaS |< Iy              `adj`     {- <bandarIy> -}       [ ['B','a','n','d','a','r','y'], ['B','a','n','d','a','r','i'] ],

    KaRDaS                    `noun`    {- <bandar> -}         [ ['s','e','a','p','o','r','t'], unwords [ ['d','i','s','t','r','i','c','t'], ['c','a','p','i','t','a','l'] ] ]
                              `plural`     KaRADiS,

    KaRDIS |< aT              `noun`    {- <bandIraT> -}       [ ['b','a','n','n','e','r'] ] ]


cluster_89  = cluster

 |> "b n d q" <| [

    KuRDuS                    `noun`    {- <bunduq> -}         [ ['h','a','z','e','l','n','u','t'], ['b','u','l','l','e','t'] ]
                              `plural`     KaRADiS,

    KuRDuS |< Iy              `adj`     {- <bunduqIy> -}       [ ['V','e','n','e','t','i','a','n'] ],

    KuRDuS |< Iy |< aT        `noun`    {- <bunduqIyaT> -}     [ ['r','i','f','l','e'] ],

    KuRDuS |< Iy |< aT        `noun`    {- <bunduqIyaT> -}     [ ['V','e','n','i','c','e'] ],

    KuRDUS                    `noun`    {- <bundUq> -}         [ ['b','a','s','t','a','r','d'] ]
                              `plural`     KaRADIS ]


cluster_90  = cluster

 |> "b n d l" <| [

    KaRDUS                    `noun`    {- <bandUl> -}         [ ['p','e','n','d','u','l','u','m'] ] ]


cluster_91  = cluster

 |> "b n d" <| [

    FaCCaL                    `verb`    {- <bannad> -}         [ ['b','a','n','d'] ],

    FaCL                      `noun`    {- <band> -}           [ ['a','r','t','i','c','l','e'], ['c','l','a','u','s','e'] ]
                              `plural`     FuCUL ]


cluster_92  = cluster

 |> "b n b ^s" <| [

    KiRDAS |< Iy              `adj`     {- <binbA^sIy> -}      [ unwords [ ['l','i','e','u','t','e','n','a','n','t'], ['c','o','l','o','n','e','l'] ] ] ]


cluster_93  = cluster

 |> "b n ^s r" <| [

    KaRDaS                    `noun`    {- <ban^sar> -}        [ ['p','u','n','c','t','u','r','e'], ['f','l','a','t'] ]
                              `plural`     KaRDaS |< At ]


cluster_94  = cluster

 |> "b n ^g r" <| [

    KaRDaS                    `noun`    {- <ban^gar> -}        [ unwords [ ['s','u','g','a','r'], ['b','e','e','t'] ], unwords [ ['r','e','d'], ['b','e','e','t'] ] ] ]


cluster_95  = cluster

 |> "b n ^g l" <| [

    KaRDAS                    `noun`    {- <ban^gAl> -}        [ ['B','e','n','g','a','l'] ],

    KaRDAS |< Iy              `adj`     {- <ban^gAlIy> -}      [ ['B','e','n','g','a','l','i'] ] ]


cluster_96  = cluster

 |> "b n ^g b" <| [

    KaRDAS                    `noun`    {- <ban^gAb> -}        [ ['P','u','n','j','a','b'] ],

    KaRDAS |< Iy              `adj`     {- <ban^gAbIy> -}      [ ['P','u','n','j','a','b','i'] ] ]


cluster_97  = cluster

 |> "b n t" <| [

    FiCL                      `noun`    {- <bint> -}           [ ['d','a','u','g','h','t','e','r'], ['g','i','r','l'] ]
                              `plural`     FaC |< At ]


cluster_98  = cluster

 |> "b n s" <| [

    FACUL                     `noun`    {- <bAnUs> -}          [ ['P','a','n','o','s'] ] ]


cluster_99  = cluster

 |> "b n r s" <| [

    KaRADiS                   `noun`    {- <banAris> -}        [ ['B','e','n','a','r','e','s'] ] ]


cluster_100 = cluster

 |> "b n r" <| [

    FaCCUL                    `noun`    {- <bannUr> -}         [ ['g','l','a','s','s'] ] ]


cluster_101 = cluster

 |> "b n q" <| [

    FiCaL |< aT               `noun`    {- <binaqaT> -}        [ ['g','o','r','e'], ['g','u','s','s','e','t'] ]
                              `plural`     FaCIL |< aT ]


cluster_102 = cluster

 |> "b n k" <| [

    FuCL                      `noun`    {- <bunk> -}           [ ['r','o','o','t'], ['c','o','r','e'] ],

    FaCL                      `noun`    {- <bank> -}           [ ['b','a','n','k'] ]
                              `plural`     FuCUL,

    FaCL |< Iy                `adj`     {- <bankIy> -}         [ ['b','a','n','k'], ['b','a','n','k','i','n','g'] ],

    MuFaCCaL                  `noun`    {- <mubannak> -}       [ ['s','t','r','a','n','d','e','d'], ['s','h','r','e','w','d'] ]
                              `plural`     MuFaCCaL |< Un
                           
    `derives` otherwise ]


cluster_103 = cluster

 |> "b n h" <| [

    FaL |< Iy                 `adj`     {- <bahIy> -}          [ ['b','e','a','u','t','i','f','u','l'], ['b','r','i','l','l','i','a','n','t'] ] ]


cluster_104 = cluster

 |> "b q `" <| [

    FaCCaL                    `verb`    {- <baqqa`> -}         [ ['s','t','a','i','n'], ['s','o','i','l'] ],

    TaFaCCaL                  `verb`    {- <tabaqqa`> -}       [ unwords [ ['b','e'], ['s','t','a','i','n','e','d'] ], unwords [ ['b','e'], ['s','p','o','t','t','e','d'] ] ],

    FuCL |< aT                `noun`    {- <buq`aT> -}         [ ['s','p','o','t'], ['s','t','a','i','n'] ]
                              `plural`     FuCaL,

    FiCAL                     `noun`    {- <biqA`> -}          [ ['B','e','k','a','a'], ['B','i','q','a'] ],

    FiCAL                     `noun`    {- <biqA`> -}          [ ['r','e','g','i','o','n','s'], ['s','p','o','t','s'] ],

    HaFCaL                    `noun`    {- <'abqa`> -}         [ ['s','p','o','t','t','e','d'], ['s','p','e','c','k','l','e','d'] ],

    FACiL |< aT               `noun`    {- <bAqi`aT> -}        [ ['s','l','y'], ['s','h','r','e','w','d'] ]
                              `plural`     FawACiL ]


cluster_105 = cluster

 |> "b q ^s" <| [

    FuCL |< aT                `noun`    {- <buq^saT> -}        [ unwords [ ['b','u','q','s','h','a'], "(", ['Y','e','m','e','n','i'], ['c','o','p','p','e','r'], ['c','o','i','n'], ")" ] ] ]


cluster_106 = cluster

 |> "b q ^g" <| [

    FuCL |< aT                `noun`    {- <buq^gaT> -}        [ ['b','u','n','d','l','e'], ['p','a','c','k','a','g','e'] ]
                              `plural`     FuCaL ]


cluster_107 = cluster

 |> "b n z n" <| [

    KiRDIS                    `noun`    {- <binzIn> -}         [ ['g','a','s','o','l','i','n','e'], ['b','e','n','z','i','n','e'], ['b','e','n','z','e','n','e'] ] ]


cluster_108 = cluster

 |> "b n z l" <| [

    KiRDUS                    `noun`    {- <binzUl> -}         [ ['b','e','n','z','o','l'], ['b','e','n','z','e','n','e'] ] ]


cluster_109 = cluster

 |> "b q m" <| [

    FaCCaL                    `noun`    {- <baqqam> -}         [ ['b','r','a','z','i','l','w','o','o','d'] ],

    FaCaL |< Iy               `adj`     {- <baqamIy> -}        [ ['B','a','q','a','m','i'] ],

    FaCL |< Iy                `adj`     {- <baqmIy> -}         [ ['B','a','q','m','i'] ] ]


cluster_110 = cluster

 |> "b q l" <| [

    FaCaL                     `verb`    {- <baqal> -}          [ ['s','p','r','o','u','t'] ]
                              `imperf`     FCuL,

    FaCL                      `noun`    {- <baql> -}           [ ['h','e','r','b'], ['l','e','g','u','m','e'] ]
                              `plural`     FuCUL
                              `plural`     FaCaL |< At,

    FaCL |< Iy                `adj`     {- <baqlIy> -}         [ ['l','e','g','u','m','i','n','o','u','s'] ],

    FaCCAL                    `noun`    {- <baqqAl> -}         [ unwords [ ['g','r','e','e','n'], "-", ['g','r','o','c','e','r'] ], ['g','r','o','c','e','r'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FiCAL |< aT               `noun`    {- <biqAlaT> -}        [ ['g','r','o','c','e','r','y'] ],

    FUCAL                     `noun`    {- <bUqAl> -}          [ ['m','u','g'] ]
                              `plural`     FawACIL
                              `plural`     FUCAL |< At ]


cluster_111 = cluster

 |> "b q b q" <| [

    KaRDaS                    `verb`    {- <baqbaq> -}         [ ['p','r','a','t','t','l','e'], ['c','h','a','t','t','e','r'] ],

    KaRDaS |< aT              `noun`    {- <baqbaqaT> -}       [ ['g','u','r','g','l','i','n','g'], ['p','r','a','t','t','l','e'] ],

    KaRDAS                    `noun`    {- <baqbAq> -}         [ ['l','o','q','u','a','c','i','o','u','s'] ]
                              `plural`     KaRDAS |< Un
                           
    `derives` otherwise,

    KaRDUS |< aT              `noun`    {- <baqbUqaT> -}       [ ['b','l','i','s','t','e','r'] ] ]


cluster_112 = cluster

 |> "b q s m" <| [

    KuRDuS |< At              `noun`    {- <buqsumAt> -}       [ ['r','u','s','k'], ['b','i','s','c','u','i','t'] ]
                              `plural`     KuRDuS |< At ]


cluster_113 = cluster

 |> "b q s" <| [

    FaCL                      `noun`    {- <baqs> -}           [ ['b','o','x'] ] ]


cluster_114 = cluster

 |> "b q r ^g" <| [

    KaRDaS                    `noun`    {- <baqra^g> -}        [ ['k','e','t','t','l','e'], unwords [ ['c','o','f','f','e','e'], ['p','o','t'] ] ]
                              `plural`     KaRADiS ]


cluster_115 = cluster

 |> "b q r .t" <| [

    KuRDAS                    `noun`    {- <buqrA.t> -}        [ ['H','i','p','p','o','c','r','a','t','e','s'] ],

    KuRDAS |< Iy              `adj`     {- <buqrA.tIy> -}      [ ['H','i','p','p','o','c','r','a','t','i','c'] ] ]


cluster_116 = cluster

 |> "b q r" <| [

    FaCaL                     `verb`    {- <baqar> -}          [ unwords [ ['s','p','l','i','t'], ['o','p','e','n'] ], ['d','i','s','e','m','b','o','w','e','l'] ]
                              `imperf`     FCaL
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'abqar> -}         [ unwords [ ['s','p','l','i','t'], ['o','p','e','n'] ], ['d','i','s','e','m','b','o','w','e','l'], unwords [ ['b','e'], ['s','p','l','i','t'], ['o','p','e','n'] ] ],

    FaCaL                     `noun`    {- <baqar> -}          [ ['c','o','w'] ]
                              `plural`     FaCaL |< At
                              `plural`     FuCUL
                              `plural`     HaFCAL,

    FaCaL |< Iy               `adj`     {- <baqarIy> -}        [ ['b','o','v','i','n','e'], ['c','o','w'] ],

    FaCCAL                    `noun`    {- <baqqAr> -}         [ ['c','o','w','b','o','y'], ['c','o','w','h','a','n','d'] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FACiL                     `noun`    {- <bAqir> -}          [ ['B','a','q','i','r'] ],

    FACiL |< Iy               `adj`     {- <bAqirIy> -}        [ ['B','a','q','i','r','i'], ['B','a','g','h','e','r','i'] ] ]


cluster_117 = cluster

 |> "b r .g ^s" <| [

    KaRDaS                    `noun`    {- <bar.ga^s> -}       [ ['g','n','a','t'], ['m','i','d','g','e'] ] ]


cluster_118 = cluster

 |> "b r .d" <| [

    FaCaL                     `verb`    {- <bara.d> -}         [ ['g','e','r','m','i','n','a','t','e'], ['s','p','r','o','u','t'] ]
                              `imperf`     FCuL,

    FuCUL                     `noun`    {- <burU.d> -}         [ ['g','e','r','m','i','n','a','t','i','o','n'], ['s','p','r','o','u','t','i','n','g'] ] ]


cluster_119 = cluster

 |> "b r .h" <| [

    FaCiL                     `verb`    {- <bari.h> -}         [ ['q','u','i','t'], ['l','e','a','v','e'], ['f','i','n','i','s','h'], ['e','n','d'] ]
                              `imperf`     FCaL,

    FaCCaL                    `verb`    {- <barra.h> -}        [ ['o','v','e','r','w','h','e','l','m'], ['a','f','f','l','i','c','t'], ['t','o','r','m','e','n','t'] ],

    FACaL                     `verb`    {- <bAra.h> -}         [ unwords [ ['g','i','v','e'], ['u','p'] ] ],

    HaFCaL                    `verb`    {- <'abra.h> -}        [ unwords [ ['d','o'], ['g','o','o','d'] ] ],

    FaCAL                     `noun`    {- <barA.h> -}         [ unwords [ ['e','m','p','t','y'], ['l','a','n','d'] ], ['c','e','s','s','a','t','i','o','n'] ],

    FaCAL |<< "aN"            `noun`    {- <barA.haN> -}       [ ['o','p','e','n','l','y'], ['p','a','t','e','n','t','l','y'] ],

    TaFACIL                   `noun`    {- <tabArI.h> -}       [ ['a','g','o','n','i','e','s'], ['t','o','r','m','e','n','t','s'] ],

    MuFACaL |< aT             `noun`    {- <mubAra.haT> -}     [ ['d','e','p','a','r','t','u','r','e'] ],

    FACiL                     `noun`    {- <bAri.h> -}         [ ['o','m','i','n','o','u','s'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    FACiL |< aT               `noun`    {- <bAri.haT> -}       [ ['y','e','s','t','e','r','d','a','y'], ['y','e','s','t','e','r','y','e','a','r'] ],

    MuFaCCiL                  `noun`    {- <mubarri.h> -}      [ ['a','g','o','n','i','z','i','n','g'], ['v','i','o','l','e','n','t'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mubarra.h> -}      [ ['a','f','f','l','i','c','t','e','d'], ['t','o','r','m','e','n','t','e','d'] ] ]


cluster_120 = cluster

 |> "b r .g r" <| [

    KaRDaS                    `noun`    {- <bar.gar> -}        [ ['B','e','r','g','e','r'], ['B','u','r','g','e','r'] ] ]


cluster_121 = cluster

 |> "b r .g l" <| [

    KuRDuS                    `noun`    {- <bur.gul> -}        [ ['b','u','l','g','u','r'] ] ]


cluster_122 = cluster

 |> "b r .g _t" <| [

    KaRDUS                    `noun`    {- <bar.gU_t> -}       [ ['f','l','e','a'] ]
                              `plural`     KaRADIS,

    KaRDUS |< Iy              `adj`     {- <bar.gU_tIy> -}     [ ['B','a','r','g','h','o','u','t','h','i'] ] ]


cluster_123 = cluster

 |> "b r ^g m" <| [

    KuRDuS |< aT              `noun`    {- <bur^gumaT> -}      [ ['k','n','u','c','k','l','e'] ]
                              `plural`     KaRADiS ]


cluster_124 = cluster

 |> "b r ^g l" <| [

    KaRDaS                    `noun`    {- <bar^gal> -}        [ ['c','o','m','p','a','s','s'], ['d','i','v','i','d','e','r','s'] ]
                              `plural`     KaRADiS ]


cluster_125 = cluster

 |> "b r ^g" <| [

    TaFaCCaL                  `verb`    {- <tabarra^g> -}      [ ['a','d','o','r','n'], ['d','e','c','k'] ],

    FuCL                      `noun`    {- <bur^g> -}          [ ['t','o','w','e','r'], ['c','o','n','s','t','e','l','l','a','t','i','o','n'] ]
                              `plural`     FuCUL
                              `plural`     HaFCAL,

    FuCL                      `noun`    {- <bur^g> -}          [ ['B','u','r','j'] ],

    FuCayL                    `noun`    {- <buray^g> -}        [ ['B','u','r','e','i','j'], ['B','o','u','r','e','i','j'] ],

    FuCayL                    `noun`    {- <buray^g> -}        [ unwords [ ['s','m','a','l','l'], ['t','o','w','e','r'] ], unwords [ ['s','m','a','l','l'], ['c','o','n','s','t','e','l','l','a','t','i','o','n'] ] ]
                              `plural`     FuCayL |< At,

    FACiL |< aT               `noun`    {- <bAri^gaT> -}       [ ['b','a','t','t','l','e','s','h','i','p'], ['b','a','r','g','e'] ]
                              `plural`     FawACiL,

    FuCUL |< Iy               `adj`     {- <burU^gIy> -}       [ ['t','r','u','m','p','e','t','e','r'], ['b','u','g','l','e','r'] ] ]


cluster_126 = cluster

 |> "b r .t z" <| [

    KaRDUS                    `noun`    {- <bar.tUz> -}        [ unwords [ ['c','r','e','w','\'','s'], ['q','u','a','r','t','e','r','s'] ] ] ]


cluster_127 = cluster

 |> "b r .t m" <| [

    KaRDaS                    `verb`    {- <bar.tam> -}        [ unwords [ ['t','a','l','k'], ['n','o','n','s','e','n','s','e'] ] ],

    KuRDUS                    `noun`    {- <bur.tUm> -}        [ unwords [ ['e','l','e','p','h','a','n','t'], ['t','r','u','n','k'] ] ]
                              `plural`     KaRDUS ]


cluster_128 = cluster

 |> "b r .t l" <| [

    KaRDaS                    `verb`    {- <bar.tal> -}        [ ['b','r','i','b','e'], ['c','o','r','r','u','p','t'] ],

    TaKaRDaS                  `verb`    {- <tabar.tal> -}      [ unwords [ ['t','a','k','e'], ['b','r','i','b','e','s'] ], unwords [ ['b','e'], ['c','o','r','r','u','p','t','e','d'] ] ],

    KiRDIS                    `noun`    {- <bir.tIl> -}        [ ['b','r','i','b','e'] ]
                              `plural`     KaRADIS ]


cluster_129 = cluster

 |> "b r .t `" <| [

    KaRDaS                    `verb`    {- <bar.ta`> -}        [ ['g','a','l','l','o','p'] ] ]


cluster_130 = cluster

 |> "b r .s" <| [

    FaCiL                     `verb`    {- <bari.s> -}         [ unwords [ ['h','a','v','e'], ['l','e','p','r','o','s','y'] ] ]
                              `imperf`     FCaL,

    FuCL                      `noun`    {- <bur.s> -}          [ ['g','e','c','k','o'] ],

    FaCaL                     `noun`    {- <bara.s> -}         [ ['l','e','p','r','o','s','y'], ['l','e','p','r','o','u','s'], ['l','e','p','e','r'] ]
                              `plural`     HaFCaL,

    FuCL |< aT                `noun`    {- <bur.saT> -}        [ unwords [ ['s','t','o','c','k'], ['e','x','c','h','a','n','g','e'] ] ] ]


cluster_131 = cluster

 |> "b r _t n" <| [

    KuRDuS                    `noun`    {- <bur_tun> -}        [ ['c','l','a','w'], ['t','a','l','o','n'] ]
                              `plural`     KaRADiS ]


cluster_132 = cluster

 |> "b r _d `" <| [

    KaRDaS |< aT              `noun`    {- <bar_da`aT> -}      [ unwords [ ['s','a','d','d','l','e'], ['c','l','o','t','h'] ], unwords [ ['p','a','c','k'], "-", ['s','a','d','d','l','e'] ] ]
                              `plural`     KaRADiS,

    KaRADiS |< Iy             `adj`     {- <barA_di`Iy> -}     [ unwords [ ['s','a','d','d','l','e'], ['m','a','k','e','r'] ] ] ]


cluster_133 = cluster

 |> "b r ^s m" <| [

    KaRDaS                    `verb`    {- <bar^sam> -}        [ ['s','t','a','r','e'], ['g','a','z','e'] ],

    KaRDaS                    `verb`    {- <bar^sam> -}        [ ['r','i','v','e','t'], ['h','e','m'] ],

    KaRDaS |< aT              `noun`    {- <bar^samaT> -}      [ ['r','i','v','e','t','i','n','g'] ],

    KuRDAS |< aT              `noun`    {- <bur^sAmaT> -}      [ ['r','i','v','e','t'] ],

    KuRDAS                    `noun`    {- <bur^sAm> -}        [ unwords [ ['c','o','m','m','u','n','i','o','n'], ['w','a','f','e','r'] ] ],

    KaRDUS |< Iy              `adj`     {- <bar^sUmIy> -}      [ unwords [ ['p','r','i','c','k','l','y'], ['p','e','a','r'] ] ] ]


cluster_134 = cluster

 |> "b r ^s" <| [

    FuCL                      `noun`    {- <bur^s> -}          [ ['m','a','t'] ]
                              `plural`     HaFCAL,

    HaFCaL                    `noun`    {- <'abra^s> -}        [ ['s','p','o','t','t','e','d'], ['s','p','e','c','k','l','e','d'] ],

    FaCAL                     `noun`    {- <barA^s> -}         [ ['B','a','r','a','s','h'] ] ]


cluster_135 = cluster

 |> "b r ^g z" <| [

    TaKaRDaS                  `verb`    {- <tabar^gaz> -}      [ unwords [ ['b','e','c','o','m','e'], ['b','o','u','r','g','e','o','i','s'] ] ],

    TaKaRDuS                  `noun`    {- <tabar^guz> -}      [ unwords [ ['b','e','c','o','m','i','n','g'], ['b','o','u','r','g','e','o','i','s'] ] ]
                              `plural`     TaKaRDuS |< At,

    MutaKaRDiS                `noun`    {- <mutabar^giz> -}    [ ['b','o','u','r','g','e','o','i','s'] ]
                              `plural`     MutaKaRDiS |< Un
                           
    `derives` otherwise ]


cluster_136 = cluster

 |> "b r ^g s" <| [

    KiRDAS                    `noun`    {- <bir^gAs> -}        [ unwords [ ['b','i','r','j','a','s'], "(", ['e','q','u','e','s','t','r','i','a','n'], ['j','o','u','s','t'], ")" ] ],

    KiRDIS                    `noun`    {- <bir^gIs> -}        [ ['J','u','p','i','t','e','r'] ] ]


cluster_137 = cluster

 |> "b r b _h" <| [

    KaRDaS                    `noun`    {- <barba_h> -}        [ unwords [ ['w','a','t','e','r'], ['p','i','p','e'] ], ['c','u','l','v','e','r','t'] ]
                              `plural`     KaRADiS ]


cluster_138 = cluster

 |> "b r b ^s" <| [

    KaRDUS                    `noun`    {- <barbU^s> -}        [ ['c','o','u','s','c','o','u','s'] ],

    KaRDIS                    `noun`    {- <barbI^s> -}        [ ['t','u','b','e'] ] ]


cluster_139 = cluster

 |> "b r b .t" <| [

    KaRDaS                    `verb`    {- <barba.t> -}        [ ['s','p','l','a','s','h'], ['p','a','d','d','l','e'] ] ]


cluster_140 = cluster

 |> "b r b" <| [

    FiCL |< aT                `noun`    {- <birbaT> -}         [ ['l','a','b','y','r','i','n','t','h'] ]
                              `plural`     FaCALI
                              `plural`     FiCLY ]


cluster_141 = cluster

 |> "b r ` m" <| [

    KaRDaS                    `verb`    {- <bar`am> -}         [ ['b','u','r','g','e','o','n'], ['b','u','d'] ],

    TaKaRDaS                  `verb`    {- <tabar`am> -}       [ ['b','u','r','g','e','o','n'], ['b','u','d'] ],

    KuRDuS                    `noun`    {- <bur`um> -}         [ ['b','u','d'], ['b','l','o','s','s','o','m'] ]
                              `plural`     KaRADiS,

    KuRDUS                    `noun`    {- <bur`Um> -}         [ ['b','u','d'], ['b','l','o','s','s','o','m'] ]
                              `plural`     KaRADIS,

    TaKaRDuS                  `noun`    {- <tabar`um> -}       [ ['g','e','m','m','a','t','i','o','n'], ['b','u','d','d','i','n','g'], ['b','l','o','s','s','o','m','i','n','g'] ]
                              `plural`     TaKaRDuS |< At ]


cluster_142 = cluster

 |> "b r `" <| [

    FaCaL                     `verb`    {- <bara`> -}          [ ['e','x','c','e','l'], unwords [ ['b','e'], ['d','i','s','t','i','n','g','u','i','s','h','e','d'] ] ]
                              `imperf`     FCaL,

    FaCuL                     `verb`    {- <baru`> -}          [ ['e','x','c','e','l'], unwords [ ['b','e'], ['d','i','s','t','i','n','g','u','i','s','h','e','d'] ] ]
                              `imperf`     FCuL,

    FACaL                     `verb`    {- <bAra`> -}          [ ['s','t','r','i','v','e'], ['w','o','r','k'] ],

    TaFaCCaL                  `verb`    {- <tabarra`> -}       [ ['c','o','n','t','r','i','b','u','t','e'], ['d','o','n','a','t','e'], ['g','i','v','e'] ],

    FaCAL |< aT               `noun`    {- <barA`aT> -}        [ ['s','k','i','l','l'], ['p','r','o','f','i','c','i','e','n','c','y'] ],

    FuCUL |< aT               `noun`    {- <burU`aT> -}        [ ['e','m','i','n','e','n','c','e'] ],

    HaFCaL                    `noun`    {- <'abra`> -}         [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['e','m','i','n','e','n','t'] ], unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['s','k','i','l','l','f','u','l'] ] ],

    TaFaCCuL                  `noun`    {- <tabarru`> -}       [ ['d','o','n','a','t','i','o','n'], ['c','o','n','t','r','i','b','u','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At,

    FACiL                     `adj`     {- <bAri`> -}          [ ['s','k','i','l','l','e','d'], ['p','r','o','f','i','c','i','e','n','t'] ],

    MutaFaCCiL                `noun`    {- <mutabarri`> -}     [ ['d','o','n','o','r'], ['c','o','n','t','r','i','b','u','t','o','r'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise ]


cluster_143 = cluster

 |> "b r b s" <| [

    KaRDIS                    `noun`    {- <barbIs> -}         [ ['b','a','r','b','e','l'] ] ]


cluster_144 = cluster

 |> "b r b r" <| [

    KaRDaS                    `verb`    {- <barbar> -}         [ ['b','a','b','b','l','e'], ['j','a','b','b','e','r'] ],

    KaRDaS |< Iy              `adj`     {- <barbarIy> -}       [ ['B','e','r','b','e','r'] ]
                              `plural`     KaRDaS
                              `plural`     KaRADiS |< aT
                              `plural`     KaRDaS |< Iy |< Un,

    KaRDaS |< Iy              `adj`     {- <barbarIy> -}       [ ['b','a','r','b','a','r','i','c'] ],

    KaRDaS |< Iy |< aT        `noun`    {- <barbarIyaT> -}     [ ['b','a','r','b','a','r','i','s','m'], ['s','a','v','a','g','e','r','y'] ],

    MutaKaRDiS                `noun`    {- <mutabarbir> -}     [ ['b','a','r','b','a','r','i','c'] ]
                              `plural`     MutaKaRDiS |< Un
                           
    `derives` otherwise ]


cluster_145 = cluster

 |> "b r d" <| [

    FaCaL                     `verb`    {- <barad> -}          [ ['c','a','l','m'], ['c','o','o','l'] ]
                              `imperf`     FCuL,

    FaCuL                     `verb`    {- <barud> -}          [ unwords [ ['b','e','c','o','m','e'], ['c','o','l','d'] ] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <barrad> -}         [ ['r','e','f','r','i','g','e','r','a','t','e'], unwords [ ['m','a','k','e'], ['c','o','l','d'] ], unwords [ ['b','e'], ['c','o','o','l','e','d'] ] ],

    HaFCaL                    `verb`    {- <'abrad> -}         [ ['m','a','i','l'] ],

    TaFaCCaL                  `verb`    {- <tabarrad> -}       [ unwords [ ['b','e'], ['c','o','l','d'] ] ],

    IFtaCaL                   `verb`    {- <ibtarad> -}        [ unwords [ ['c','o','o','l'], ['o','f','f'] ] ],

    FaCL                      `noun`    {- <bard> -}           [ ['c','o','l','d'], ['c','o','o','l','i','n','g'] ],

    FaCaL                     `noun`    {- <barad> -}          [ ['h','a','i','l'], ['h','a','i','l','s','t','o','n','e'] ],

    FaCaL |< aT               `noun`    {- <baradaT> -}        [ ['h','a','i','l','s','t','o','n','e'] ],

    FuCL                      `noun`    {- <burd> -}           [ ['g','a','r','m','e','n','t'] ]
                              `plural`     HaFCAL,

    FaCUL                     `noun`    {- <barUd> -}          [ unwords [ ['c','o','l','l','y','r','i','u','m'], "(", ['m','e','d','i','c','a','t','e','d'], ['s','o','l','u','t','i','o','n'], ['f','o','r'], ['t','h','e'], ['e','y','e','s'], ")" ] ],

    FuCUL                     `noun`    {- <burUd> -}          [ ['c','o','l','d','n','e','s','s'] ],

    FaCL |< Iy |< aT          `noun`    {- <bardIyaT> -}       [ ['a','g','u','e'], unwords [ ['f','e','v','e','r'], ['a','t','t','a','c','k'] ] ]
                              `plural`     FuCaLA',

    FaCCAL                    `noun`    {- <barrAd> -}         [ ['r','e','f','r','i','g','e','r','a','t','o','r'] ],

    FaCCAL                    `noun`    {- <barrAd> -}         [ ['t','e','a','p','o','t'] ],

    FaCCAL |< aT              `noun`    {- <barrAdaT> -}       [ ['r','e','f','r','i','g','e','r','a','t','o','r'] ],

    FaCCAL                    `noun`    {- <barrAd> -}         [ unwords [ ['t','o','o','l'], ['c','u','t','t','e','r'] ] ]
                              `plural`     FaCCAL |< Un
                           
    `derives` otherwise,

    FiCAL |< aT               `noun`    {- <birAdaT> -}        [ unwords [ ['t','o','o','l'], ['c','u','t','t','i','n','g'] ] ],

    FuCAL |< aT               `noun`    {- <burAdaT> -}        [ unwords [ ['i','r','o','n'], ['f','i','l','i','n','g','s'] ] ],

    MiFCaL                    `noun`    {- <mibrad> -}         [ ['f','i','l','e'], unwords [ ['t','o','o','l'], ['c','u','t','t','e','r'] ] ],

    TaFCIL                    `noun`    {- <tabrId> -}         [ ['r','e','f','r','i','g','e','r','a','t','i','o','n'], ['c','o','o','l','i','n','g'] ]
                              `plural`     TaFCIL |< At,

    FACiL                     `noun`    {- <bArid> -}          [ ['c','o','l','d'], ['f','r','i','g','i','d'] ]
                              `plural`     FACiL |< Un
                           
    `derives` otherwise,

    MuFaCCiL                  `noun`    {- <mubarrid> -}       [ ['c','o','o','l','e','r'], ['r','e','f','r','e','s','h','i','n','g'] ]
                              `plural`     MuFaCCiL |< Un
                           
    `derives` otherwise,

    MuFaCCaL                  `noun`    {- <mubarrad> -}       [ ['c','o','o','l','e','d'], ['c','h','i','l','l','e','d'] ],

    FaCIL                     `noun`    {- <barId> -}          [ ['m','a','i','l'], unwords [ ['p','o','s','t'], ['o','f','f','i','c','e'] ] ],

    FaCIL |< Iy               `adj`     {- <barIdIy> -}        [ ['p','o','s','t','a','l'] ],

    FACUL                     `noun`    {- <bArUd> -}          [ ['g','u','n','p','o','w','d','e','r'] ],

    FACUL |< Iy               `adj`     {- <bArUdIy> -}        [ ['B','a','r','o','u','d','i'] ],

    FACUL |< aT               `noun`    {- <bArUdaT> -}        [ ['r','i','f','l','e'], ['c','a','r','b','i','n','e'] ]
                              `plural`     FawACIL,

    FaCL |< Iy                `adj`     {- <bardIy> -}         [ ['p','a','p','y','r','u','s'] ],

    FaCL |< Iy |< aT          `noun`    {- <bardIyaT> -}       [ ['p','a','p','y','r','o','l','o','g','y'] ],

    FaCaLY                    `noun`    {- <baradY> -}         [ ['B','a','r','a','d','a'] ],

    FuCayL |< aT              `noun`    {- <buraydaT> -}       [ ['B','u','r','e','i','d','a'] ] ]


cluster_146 = cluster

 |> "b r h n" <| [

    KaRDaS                    `verb`    {- <barhan> -}         [ ['p','r','o','v','e'], ['d','e','m','o','n','s','t','r','a','t','e'] ],

    KaRDaS |< aT              `noun`    {- <barhanaT> -}       [ ['p','r','o','v','i','n','g'], ['d','e','m','o','n','s','t','r','a','t','i','o','n'] ],

    KuRDAS                    `noun`    {- <burhAn> -}         [ ['B','u','r','h','a','n'] ],

    KuRDAS                    `noun`    {- <burhAn> -}         [ ['p','r','o','o','f'] ]
                              `plural`     KaRADIS ]


cluster_147 = cluster

 |> "b r h m" <| [

    KaRDaS |< aT              `noun`    {- <barhamaT> -}       [ ['B','r','a','h','m','a','n','i','s','m'] ],

    KaRDaS |< Iy              `adj`     {- <barhamIy> -}       [ ['B','r','a','h','m','a','n'] ],

    KaRDaS |< Iy |< aT        `noun`    {- <barhamIyaT> -}     [ ['B','r','a','h','m','a','n','i','s','m'] ] ]


cluster_148 = cluster

 |> "b r h" <| [

    FuCL |< aT                `noun`    {- <burhaT> -}         [ ['i','n','s','t','a','n','t'], ['m','o','m','e','n','t'] ]
                              `plural`     FuCaL |< At,

    FuCayL |< aT              `noun`    {- <burayhaT> -}       [ unwords [ "a", ['l','i','t','t','l','e'], ['w','h','i','l','e'] ] ],

    FuCL |< Iy                `adj`     {- <burhIy> -}         [ ['m','o','m','e','n','t','a','r','i','l','y'] ] ]


cluster_149 = cluster

 |> "b r f z" <| [

    KaRDIS                    `noun`    {- <barfIz> -}         [ ['P','a','r','v','e','z'] ] ]


cluster_150 = cluster

 |> "b r f r" <| [

    KiRDIS                    `noun`    {- <birfIr> -}         [ ['p','u','r','p','l','e'] ]
                              `plural`     KaRADIS ]


cluster_151 = cluster

 |> "b r d r" <| [

    KaRDUS |< aT              `noun`    {- <bardUraT> -}       [ ['c','u','r','b','s','t','o','n','e'], ['c','u','r','b'] ] ]


cluster_152 = cluster

 |> "b r d q" <| [

    KaRDAS                    `noun`    {- <bardAq> -}         [ ['p','i','t','c','h','e','r'], ['j','u','g'] ]
                              `plural`     KaRADIS ]


cluster_153 = cluster

 |> "b r d `" <| [

    KaRDaS |< aT              `noun`    {- <barda`aT> -}       [ unwords [ ['s','a','d','d','l','e'], ['c','l','o','t','h'] ], unwords [ ['p','a','c','k'], "-", ['s','a','d','d','l','e'] ] ]
                              `plural`     KaRADiS,

    KaRADiS |< Iy             `adj`     {- <barAdi`Iy> -}      [ unwords [ ['s','a','d','d','l','e'], ['m','a','k','e','r'] ] ] ]


cluster_154 = cluster

 |> "b r d _h" <| [

    KaRDaS                    `verb`    {- <barda_h> -}        [ ['p','o','l','i','s','h'], ['b','u','r','n','i','s','h'] ],

    KaRDaS |< aT              `noun`    {- <barda_haT> -}      [ ['p','o','l','i','s','h','i','n','g'], ['b','u','r','n','i','s','h','i','n','g'] ] ]


cluster_155 = cluster

 |> "b r k" <| [

    FaCaL                     `verb`    {- <barak> -}          [ ['k','n','e','e','l'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <barrak> -}         [ unwords [ ['m','a','k','e'], ['k','n','e','e','l'] ] ],

    FACaL                     `verb`    {- <bArak> -}          [ ['b','l','e','s','s'], ['c','o','n','g','r','a','t','u','l','a','t','e'] ],

    HaFCaL                    `verb`    {- <'abrak> -}         [ unwords [ ['m','a','k','e'], ['k','n','e','e','l'] ] ],

    TaFaCCaL                  `verb`    {- <tabarrak> -}       [ unwords [ ['b','e'], ['b','l','e','s','s','e','d'] ], unwords [ ['b','e'], ['p','r','o','s','p','e','r','o','u','s'] ] ],

    TaFACaL                   `verb`    {- <tabArak> -}        [ unwords [ ['b','e'], ['b','l','e','s','s','e','d'] ] ],

    IstaFCaL                  `verb`    {- <istabrak> -}       [ unwords [ ['b','e'], ['b','l','e','s','s','e','d'] ] ],

    FiCL |< aT                `noun`    {- <birkaT> -}         [ ['p','o','o','l'] ]
                              `plural`     FiCaL,

    FaCaL |< aT               `noun`    {- <barakaT> -}        [ ['b','l','e','s','s','i','n','g'] ]
                              `plural`     FaCaL |< At,

    FaCaL |< At               `noun`    {- <barakAt> -}        [ ['B','a','r','a','k','a','a','t'], ['B','a','r','a','k','a','t'] ],

    HaFCaL                    `noun`    {- <'abrak> -}         [ unwords [ ['m','o','r','e'], "/", ['m','o','s','t'], ['b','l','e','s','s','e','d'] ] ],

    MaFCUL                    `noun`    {- <mabrUk> -}         [ unwords [ ['c','o','n','g','r','a','t','u','l','a','t','i','o','n','s'], "!" ] ],

    MaFCUL                    `noun`    {- <mabrUk> -}         [ ['b','l','e','s','s','e','d'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    TaFCIL                    `noun`    {- <tabrIk> -}         [ ['b','l','e','s','s','i','n','g'], ['b','e','n','e','d','i','c','t','i','o','n'] ]
                              `plural`     TaFCIL |< At,

    MuFACaL                   `noun`    {- <mubArak> -}        [ ['M','u','b','a','r','a','k'] ],

    MuFACaL                   `noun`    {- <mubArak> -}        [ ['b','l','e','s','s','e','d'], ['f','o','r','t','u','n','a','t','e'] ]
                              `plural`     MuFACaL |< Un
                           
    `derives` otherwise,

    MuFACaL |< Iy             `adj`     {- <mubArakIy> -}      [ ['M','o','u','b','a','r','k','i'] ],

    FaCACIL                   `noun`    {- <barArIk> -}        [ ['b','a','r','r','a','c','k','s'] ],

    FuCLAn                    `noun`    {- <burkAn> -}         [ ['v','o','l','c','a','n','o'] ]
                              `plural`     FaCALIn,

    FuCLAn |< Iy              `adj`     {- <burkAnIy> -}       [ ['v','o','l','c','a','n','i','c'] ] ]


cluster_156 = cluster

 |> "b r l n" <| [

    KaRDIS                    `noun`    {- <barlIn> -}         [ ['B','e','r','l','i','n'] ] ]


cluster_157 = cluster

 |> "b r k r" <| [

    KiRDAS                    `noun`    {- <birkAr> -}         [ ['c','o','m','p','a','s','s'], ['d','i','v','i','d','e','r','s'] ] ]


cluster_158 = cluster

 |> "b r k n" <| [

    KuRDAS                    `noun`    {- <burkAn> -}         [ ['v','o','l','c','a','n','o'] ]
                              `plural`     KaRADIS,

    KuRDAS |< Iy              `adj`     {- <burkAnIy> -}       [ ['v','o','l','c','a','n','i','c'] ] ]


cluster_159 = cluster

 |> "b r m" <| [

    FaCiL                     `verb`    {- <barim> -}          [ unwords [ ['b','e'], ['d','i','s','c','o','n','t','e','n','t','e','d'] ], unwords [ ['b','e'], ['b','o','r','e','d'] ] ]
                              `imperf`     FCaL,

    FaCaL                     `verb`    {- <baram> -}          [ ['b','r','a','i','d'], ['t','w','i','s','t'] ]
                              `imperf`     FCuL,

    FaCCaL                    `verb`    {- <barram> -}         [ ['t','w','i','s','t'], ['t','w','i','n','e'] ],

    HaFCaL                    `verb`    {- <'abram> -}         [ ['c','o','n','c','l','u','d','e'], ['r','a','t','i','f','y'] ],

    TaFaCCaL                  `verb`    {- <tabarram> -}       [ unwords [ ['b','e'], ['b','o','r','e','d'] ], unwords [ ['b','e'], ['f','e','d'], ['u','p'] ] ],

    InFaCaL                   `verb`    {- <inbaram> -}        [ unwords [ ['b','e'], ['s','e','t','t','l','e','d'] ], unwords [ ['b','e'], ['t','w','i','s','t','e','d'] ] ],

    FaCiL                     `noun`    {- <barim> -}          [ ['w','e','a','r','y'], ['t','i','r','e','d'] ],

    TaFaCCuL                  `noun`    {- <tabarrum> -}       [ ['b','o','r','e','d','o','m'], ['d','i','s','s','a','t','i','s','f','a','c','t','i','o','n'] ]
                              `plural`     TaFaCCuL |< At,

    MutaFaCCiL                `noun`    {- <mutabarrim> -}     [ ['a','n','n','o','y','e','d'] ]
                              `plural`     MutaFaCCiL |< Un
                           
    `derives` otherwise,

    FaCCAL |< aT              `noun`    {- <barrAmaT> -}       [ ['d','r','i','l','l'], unwords [ ['d','r','i','l','l','i','n','g'], ['m','a','c','h','i','n','e'] ] ],

    FaCIL                     `noun`    {- <barIm> -}          [ ['r','o','p','e'], ['t','w','i','n','e'] ],

    MaFCUL                    `noun`    {- <mabrUm> -}         [ ['c','r','o','o','k','e','d'], ['t','w','i','s','t','e','d'] ]
                              `plural`     MaFCUL |< Un
                           
    `derives` otherwise,

    MuFCiL                    `noun`    {- <mubrim> -}         [ ['c','o','n','c','l','u','s','i','o','n'], ['r','a','t','i','f','i','c','a','t','i','o','n'] ]
                              `plural`     MuFCiL |< At,

    MuFCaL                    `noun`    {- <mubram> -}         [ ['c','o','n','c','l','u','d','e','d'], ['r','a','t','i','f','i','e','d'] ],

    MuFCaL                    `noun`    {- <mubram> -}         [ ['e','s','t','a','b','l','i','s','h','e','d'], ['i','r','r','e','v','o','c','a','b','l','e'] ]
                              `plural`     MuFCaL |< Un
                           
    `derives` otherwise,

    HiFCAL                    `noun`    {- <'ibrAm> -}         [ ['r','a','t','i','f','i','c','a','t','i','o','n'], ['c','o','n','c','l','u','s','i','o','n'] ]
                              `plural`     HiFCAL |< At,

    FuCL |< aT                `noun`    {- <burmaT> -}         [ unwords [ ['e','a','r','t','h','e','n','w','a','r','e'], ['p','o','t'] ] ]
                              `plural`     FiCAL
                              `plural`     FuCaL,

    FuCL |< Iy                `adj`     {- <burmIy> -}         [ ['B','u','r','m','e','s','e'] ],

    FaCLA' |< Iy              `adj`     {- <barmAwIy> -}       [ ['a','m','p','h','i','b','i','o','u','s'] ] ]


cluster_160 = cluster

 |> "b r n s" <| [

    KuRDuS                    `noun`    {- <burnus> -}         [ ['b','u','r','n','o','o','s','e'] ]
                              `plural`     KaRADiS
                              `plural`     KuRDUS
                              `plural`     KaRADIS,

    KaRADiS                   `noun`    {- <barAnis> -}        [ ['P','y','r','e','n','e','e','s'] ] ]


cluster_161 = cluster

 |> "b r n q" <| [

    KaRDaS                    `verb`    {- <barnaq> -}         [ ['v','a','r','n','i','s','h'] ],

    KaRDaS |<< "a"            `noun`    {- <barnaqa> -}        [ ['v','a','r','n','i','s','h','i','n','g'] ]
                              `plural`     KaRDaS |< aT ]


cluster_162 = cluster

 |> "b r n .t" <| [

    TaKaRDaS                  `verb`    {- <tabarna.t> -}      [ unwords [ ['w','e','a','r'], "/", ['p','u','t'], ['o','n'], "a", "(", ['W','e','s','t','e','r','n'], ")", ['h','a','t'] ] ] ]


cluster_163 = cluster

 |> "b r n" <| [

    FACUL                     `noun`    {- <bArUn> -}          [ ['B','a','r','o','n'], ['b','a','r','o','n'] ],

    FaCCAL |< Iy              `adj`     {- <barrAnIy> -}       [ ['e','x','t','e','r','i','o','r'], ['e','x','t','e','r','n','a','l'] ],

    FiCL                      `noun`    {- <birn> -}           [ ['B','e','r','n'] ],

    FaCL |< Iy |< aT          `noun`    {- <barnIyaT> -}       [ unwords [ ['c','l','a','y'], ['v','e','s','s','e','l'] ] ]
                              `plural`     FaCALI ]


cluster_164 = cluster

 |> "b r m q" <| [

    KaRDaS                    `noun`    {- <barmaq> -}         [ ['b','a','l','u','s','t','e','r'], ['p','o','s','t'], ['s','p','i','k','e'] ]
                              `plural`     KaRADiS ]


cluster_165 = cluster

 |> "b r m l" <| [

    KaRDIS                    `noun`    {- <barmIl> -}         [ ['b','a','r','r','e','l'] ]
                              `plural`     KaRADIS ]


cluster_166 = cluster

 |> "b r m d" <| [

    KiRDUS |< Iy              `adj`     {- <birmUdIy> -}       [ ['B','e','r','m','u','d','a','n'] ],

    KaRDUS |< aT              `noun`    {- <barmUdaT> -}       [ ['B','a','r','m','u','d','a','h'] ] ]


cluster_167 = cluster

 |> "b r m ^g" <| [

    KaRDaS                    `verb`    {- <barma^g> -}        [ ['p','r','o','g','r','a','m'] ],

    TaKaRDaS                  `verb`    {- <tabarma^g> -}      [ unwords [ ['b','e'], ['p','r','o','g','r','a','m','m','e','d'] ] ],

    KaRDaS |< aT              `noun`    {- <barma^gaT> -}      [ ['p','r','o','g','r','a','m','m','i','n','g'] ],

    MuKaRDaS                  `noun`    {- <mubarma^g> -}      [ ['p','r','o','g','r','a','m','m','e','d'], ['s','c','h','e','d','u','l','e','d'] ],

    MuKaRDiS                  `noun`    {- <mubarmi^g> -}      [ ['p','r','o','g','r','a','m','m','e','r'] ]
                              `plural`     MuKaRDiS |< Un
                           
    `derives` otherwise,

    KaRDaS |< Iy              `adj`     {- <barma^gIy> -}      [ ['p','r','o','g','r','a','m','m','i','n','g'], ['s','o','f','t','w','a','r','e'] ],

    KaRDaS |< Iy |< At        `noun`    {- <barma^gIyAt> -}    [ ['s','o','f','t','w','a','r','e'] ] ]


cluster_168 = cluster

 |> "b r q" <| [

    FaCaL                     `verb`    {- <baraq> -}          [ ['f','l','a','s','h'], ['s','h','i','n','e'] ]
                              `imperf`     FCuL,

    HaFCaL                    `verb`    {- <'abraq> -}         [ ['f','l','a','s','h'], ['s','h','i','n','e'] ],

    FaCL                      `noun`    {- <barq> -}           [ ['l','i','g','h','t','n','i','n','g'], ['t','e','l','e','g','r','a','p','h'] ],

    FuCUL                     `noun`    {- <burUq> -}          [ ['l','i','g','h','t','n','i','n','g'], ['f','l','a','s','h'] ],

    FaCL |< Iy                `adj`     {- <barqIy> -}         [ ['t','e','l','e','g','r','a','p','h'], ['t','e','l','e','g','r','a','p','h','i','c'] ],

    FaCL |< Iy |< aT          `noun`    {- <barqIyaT> -}       [ ['t','e','l','e','g','r','a','m'] ],

    FaCIL                     `noun`    {- <barIq> -}          [ ['g','l','i','t','t','e','r'] ]
                              `plural`     FaCA'iL,

    FuCAL                     `noun`    {- <burAq> -}          [ ['B','u','r','a','q'] ],

    FaCCAL                    `noun`    {- <barrAq> -}         [ ['s','h','i','n','i','n','g'], ['f','l','a','s','h','i','n','g'] ],

    MaFCaL                    `noun`    {- <mabraq> -}         [ ['g','l','i','t','t','e','r'], ['f','l','a','s','h'] ],

    FACiL                     `noun`    {- <bAriq> -}          [ ['g','l','i','m','p','s','e'], ['g','l','i','t','t','e','r'], ['g','l','e','a','m'], ['t','w','i','n','k','l','e'] ]
                              `plural`     FACiL |< Un
                              `plural`     FawACiL
                           
    `derives` otherwise,

    MuFCiL                    `noun`    {- <mubriq> -}         [ ['t','e','l','e','t','y','p','e'] ],

    FaCL |< aT                `noun`    {- <barqaT> -}         [ ['C','y','r','e','n','a','i','c','a'] ] ]


cluster_169 = cluster

 |> "b r n z" <| [

    KaRDaS                    `verb`    {- <barnaz> -}         [ ['b','r','o','n','z','e'] ],

    KaRDaS |< aT              `noun`    {- <barnazaT> -}       [ ['b','r','o','n','z','i','n','g'] ],

    MuKaRDaS                  `noun`    {- <mubarnaz> -}       [ ['b','r','o','n','z','e','d'] ]
                              `plural`     MuKaRDaS |< Un
                           
    `derives` otherwise ]


section = [ cluster_1,
            cluster_2,
            cluster_3,
            cluster_4,
            cluster_5,
            cluster_6,
            cluster_7,
            cluster_8,
            cluster_9,
            cluster_10,
            cluster_11,
            cluster_12,
            cluster_13,
            cluster_14,
            cluster_15,
            cluster_16,
            cluster_17,
            cluster_18,
            cluster_19,
            cluster_20,
            cluster_21,
            cluster_22,
            cluster_23,
            cluster_24,
            cluster_25,
            cluster_26,
            cluster_27,
            cluster_28,
            cluster_29,
            cluster_30,
            cluster_31,
            cluster_32,
            cluster_33,
            cluster_34,
            cluster_35,
            cluster_36,
            cluster_37,
            cluster_38,
            cluster_39,
            cluster_40,
            cluster_41,
            cluster_42,
            cluster_43,
            cluster_44,
            cluster_45,
            cluster_46,
            cluster_47,
            cluster_48,
            cluster_49,
            cluster_50,
            cluster_51,
            cluster_52,
            cluster_53,
            cluster_54,
            cluster_55,
            cluster_56,
            cluster_57,
            cluster_58,
            cluster_59,
            cluster_60,
            cluster_61,
            cluster_62,
            cluster_63,
            cluster_64,
            cluster_65,
            cluster_66,
            cluster_67,
            cluster_68,
            cluster_69,
            cluster_70,
            cluster_71,
            cluster_72,
            cluster_73,
            cluster_74,
            cluster_75,
            cluster_76,
            cluster_77,
            cluster_78,
            cluster_79,
            cluster_80,
            cluster_81,
            cluster_82,
            cluster_83,
            cluster_84,
            cluster_85,
            cluster_86,
            cluster_87,
            cluster_88,
            cluster_89,
            cluster_90,
            cluster_91,
            cluster_92,
            cluster_93,
            cluster_94,
            cluster_95,
            cluster_96,
            cluster_97,
            cluster_98,
            cluster_99,
            cluster_100,
            cluster_101,
            cluster_102,
            cluster_103,
            cluster_104,
            cluster_105,
            cluster_106,
            cluster_107,
            cluster_108,
            cluster_109,
            cluster_110,
            cluster_111,
            cluster_112,
            cluster_113,
            cluster_114,
            cluster_115,
            cluster_116,
            cluster_117,
            cluster_118,
            cluster_119,
            cluster_120,
            cluster_121,
            cluster_122,
            cluster_123,
            cluster_124,
            cluster_125,
            cluster_126,
            cluster_127,
            cluster_128,
            cluster_129,
            cluster_130,
            cluster_131,
            cluster_132,
            cluster_133,
            cluster_134,
            cluster_135,
            cluster_136,
            cluster_137,
            cluster_138,
            cluster_139,
            cluster_140,
            cluster_141,
            cluster_142,
            cluster_143,
            cluster_144,
            cluster_145,
            cluster_146,
            cluster_147,
            cluster_148,
            cluster_149,
            cluster_150,
            cluster_151,
            cluster_152,
            cluster_153,
            cluster_154,
            cluster_155,
            cluster_156,
            cluster_157,
            cluster_158,
            cluster_159,
            cluster_160,
            cluster_161,
            cluster_162,
            cluster_163,
            cluster_164,
            cluster_165,
            cluster_166,
            cluster_167,
            cluster_168,
            cluster_169 ]
