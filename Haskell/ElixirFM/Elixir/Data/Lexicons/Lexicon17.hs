...
lexicon = listing "Lexicon properties"
 ...
 |> ".zby" <| [
    ...
    noun     FaCiL                     {- Zabiy -}          `gloss`  [ "Dhabi" ],

    -- ;; Zaboyap_1
    -- Zby     Zaboy   Napdu   gazelle
    -- Zby     Zabay   NAt     gazelles

    noun     FaCL |< aT                {- Zaboyap -}        `others` [ ".zabay NAt", ".zaby Napdu" ]
                                                            `gloss`  [ "gazelle", "gazelles" ] ]
 ...
 |> ".zrf" <| [
    ...
    verb     FaCuL                     {- Zaruf-u -}        `imperf` [ FCuL ]
                                                            `others` [ ".zaruf PV_intr", ".zruf IV_intr" ]
    ...
    noun     FaCL |< Iy                {- Zarofiy~ -}       `gloss`  [ "circumstantial" ],
    ...
    noun     FaCAL |< aT               {- ZarAfap -}        `others` [ ".zarAf Nap" ]
                                                            `gloss`  [ "elegance", "wittiness", "courtesy" ],

    -- ;; >aZoraf_1
    -- >Zrf    >aZoraf Nel     more/most elegant;wittier/wittiest
    -- AZrf    >aZoraf Nel     more/most elegant;wittier/wittiest

    noun     HaFCaL                    {- >aZoraf -}        `gloss`  [ "more / most elegant", "wittier / wittiest" ],
    ...
    noun     MutaFaCCiL                {- mutaZar~if -}     `gloss`  [ "elegant", "courteous" ],

    -- ;; musotaZoraf_1
    -- mstZrf  musotaZoraf     Nall    elegant;fancy     [[musotaZoraf/ADJ]]

    noun     MustaFCaL                 {- musotaZoraf -}    `gloss`  [ "elegant", "fancy" ] ]
 ...
 |> ".z`n" <| [
    ...
    noun     FaCIL |< aT               {- ZaEiynap -}       `others` [ ".za`A'in Ndip", "'a.z`An N", ".za`iyn Nap" ]
    ...
    noun     FACiL                     {- ZAEin -}          `gloss`  [ "ephemeral", "transitory" ] ]
 ...
 |> ".zfr" <| [
    ...
    verb     HaFCaL                    {- >aZofar -}        `others` [ ".zfir IV_yu", ".zfar IV_Pass_yu" ]
 ...
 |> "'u.zfuwr" <| [
    ...
    noun     Identity                  {- >uZofuwr -}       `others` [ "'a.zAfiyr Ndip" ]
                                                            `gloss`  [ "nail", "claw", "talon", "nails", "claws", "talons" ],
 ...
 |> ".zll" <| [
    ...
    verb     HaFaCL                    {- >aZal~ -}         `others` [ ".zall IV_V_Pass_yu", ".zill IV_V_yu", ".zlil IV_C_yu", "'a.zlal PV_C" ]
    ...
    noun     FuCL |< aT                {- Zul~ap -}         `others` [ ".zull Nap", ".zulal N" ]
    ...
    noun     MuFaCCiL                  {- muZal~il -}       `gloss`  [ "shady", "shadowy" ],

    -- ;; muZil~_1
    -- mZl     muZil~  N-ap    shady;shadowy     [[muZil~/ADJ]]

    noun     MuFiCL                    {- muZil~ -}         `gloss`  [ "shady", "shadowy" ] ]
 ...
 |> ".zlm" <| [
    ...
    verb     FaCaL                     {- Zalam-ia -}       `imperf` [ FCiL, FCaL ]
                                                            `others` [ ".zlim IV_intr_yu", ".zalam PV_intr", ".zlam IV_Pass_yu" ]
    ...
    verb     HaFCaL                    {- >aZolam -}        `others` [ ".zlim IV_intr_yu", ".zlam IV_Pass_yu" ]
 ...
 |> "i.z.zalam" <| [
    ...
    noun     FuCL |< aT                {- Zulomap -}        `others` [ ".zalmA' Nh Nhy N0_Nh", ".zulm NapAt" ]
                                                            `gloss`  [ "darkness" ],
    ...
    noun     FaCAL |< Iy               {- ZalAmiy~ -}       `gloss`  [ "obscurantist" ],
    ...
    noun     HaFCaL                    {- >aZolam -}        `gloss`  [ "darker / darkest", "viler / vilest" ],

    -- ;; maZolimap_1
    -- mZlm    maZolim Napdu   injustice;misdeed
    -- mZAlm   maZAlim Ndip    injustices;misdeeds

    noun     MaFCiL |< aT              {- maZolimap -}      `others` [ "ma.zAlim Ndip", "ma.zlim Napdu" ]
    ...
    noun     HiFCAL                    {- <iZolAm -}        `gloss`  [ "darkness", "gloom" ],
    ...
    noun     MaFCUL                    {- maZoluwm -}       `gloss`  [ "oppressed", "treated unjustly" ],

    -- ;; muZolim_1
    -- mZlm    muZolim Nall    dark;gloomy     [[muZolim/ADJ]]

    noun     MuFCiL                    {- muZolim -}        `gloss`  [ "dark", "gloomy" ] ]
 ...
 |> ".zm'" <| [
    ...
    verb     FaCiL                     {- Zami}-a -}        `imperf` [ FCaL ]
                                                            `others` [ ".zami' PV_intr", ".zma' IV_wn IV_intr IV_yn", ".zma'A IV-|" ]
                                                            `gloss`  [ "be thirsty" ],
    ...
    verb     FaCCaL                    {- Zam~a> -}         `others` [ ".zamma'A PV-|", ".zammi' IV_yu" ]
                                                            `gloss`  [ "make thirsty" ],
    ...
    verb     HaFCaL                    {- >aZoma> -}        `others` [ ".zmi' IV_yu", ".zma' IV_Pass_yu", "'a.zma'A PV-|" ]
 ...
 |> ".znn" <| [
    ...
    verb     FaCL                      {- Zan~-u -}         `imperf` [ FCuL ]
                                                            `others` [ ".znun IV-n", ".zanan PV_Cn", ".zann PV_V", ".zunn IV_V" ]
    ...
    noun     FaCL |< Iy                {- Zan~iy~ -}        `gloss`  [ "hypothetical", "supposed" ],

    -- ;; Zin~ap_1
    -- Zn      Zin~    Nap     distrust
    -- Znn     Zinan   N       misgivings

    noun     FiCL |< aT                {- Zin~ap -}         `others` [ ".zinan N", ".zinn Nap" ]
                                                            `gloss`  [ "distrust", "misgivings" ],

    -- ;; Zaniyn_1
    -- Znyn    Zaniyn  Nall    suspicious;suspected     [[Zaniyn/ADJ]]

    noun     FaCIL                     {- Zaniyn -}         `gloss`  [ "suspicious", "suspected" ],

    -- ;; Zanuwn_1
    -- Znwn    Zanuwn  Nall    suspicious;suspected     [[Zanuwn/ADJ]]

    noun     FaCUL                     {- Zanuwn -}         `gloss`  [ "suspicious", "suspected" ] ]
 ...
 |> "ma.zinn" <| [
    ...
    noun     Identity |< aT            {- maZin~ap -}       `others` [ "ma.zAnn Ndip", "ma.zinn Nap" ]
                                                            `gloss`  [ "presumption", "prejudice", "presumptions", "prejudices" ],

    -- ;; maZonuwn_1
    -- mZnwn   maZonuwn        Nall    presumed;suspected;suspicious     [[maZonuwn/ADJ]]

    noun     MaFCUL                    {- maZonuwn -}       `gloss`  [ "presumed", "suspected", "suspicious" ] ]
 ...
 |> ".zunbuwb" <| [
    ...
    noun     Identity |< Iy            {- Zunobuwbiy~ -}    `gloss`  [ "tibial", "shin" ] ]
 ...
 |> ".zhr" <| [
    ...
    verb     HaFCaL                    {- >aZohar -}        `others` [ ".zhar IV_Pass_yu", ".zhir IV_yu" ]
    ...
    noun     FaCL |< Iy                {- Zahoriy~ -}       `gloss`  [ "dorsal" ] ]
 ...
 |> "'a.zhur" <| [
    ...
    noun     Identity                  {- >aZohur -}        `gloss`  [ "rear part" ],
    ...
    noun     FuCL                      {- Zuhor -}          `others` [ "'a.zhAr N" ]
                                                            `gloss`  [ "noon", "afternoon", "in the afternoon", "at noon", "afternoons" ],

    -- ;; Zuhoriy~_1
    -- Zhry    Zuhoriy~        N-ap    afternoon;meridian     [[Zuhoriy~/ADJ]]

    noun     FuCL |< Iy                {- Zuhoriy~ -}       `gloss`  [ "afternoon", "meridian" ],
    ...
    noun     FaCIL |< aT               {- Zahiyrap -}       `others` [ ".zahiyr Nap" ]
                                                            `gloss`  [ "noon", "midday" ],
    ...
    noun     FiCAL |< aT               {- ZihArap -}        `others` [ ".zihAr Nap" ]
                                                            `gloss`  [ "outside", "epithelium" ],

    -- ;; >aZohar_2
    -- >Zhr    >aZohar Nel     clearer/clearest;more/most apparent
    -- AZhr    >aZohar Nel     clearer/clearest;more/most apparent

    noun     HaFCaL                    {- >aZohar -}        `gloss`  [ "clearer / clearest", "more / most apparent" ],
    ...
    noun     FiCAL |< Iy               {- ZihAriy~ -}       `gloss`  [ "epithelial" ],

    -- ;; muZAharap_1
    -- mZAhr   muZAhar NapAt   demonstration;rally

    noun     MuFACaL |< aT             {- muZAharap -}      `others` [ "mu.zAhar NapAt" ]
    ...
    noun     HiFCAL                    {- <iZohAr -}        `gloss`  [ "expressing", "showing", "demonstrating" ],
    ...
    noun     HiFCAL |< Iy              {- <iZohAriy~ -}     `gloss`  [ "demonstrative", "exhibitionist" ],
    ...
    noun     TaFACuL |< aT             {- taZAhurap -}      `others` [ "ta.zAhur NapAt" ]
                                                            `gloss`  [ "rally", "demonstration" ],

    -- ;; taZAhuriy~_1
    -- tZAhry  taZAhuriy~      N-ap    simulated     [[taZAhuriy~/ADJ]]

    noun     TaFACuL |< Iy             {- taZAhuriy~ -}     `gloss`  [ "simulated" ],
    ...
    noun     FACiL |< Iy               {- ZAhiriy~ -}       `gloss`  [ "apparent", "external" ],
