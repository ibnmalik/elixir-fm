...
trieDict = tcompile . dict2fullform
...
imode l
    = do s <- getLine
         case (words s) of
          (x:xs) -> do case (parseCommand l (unwords (x:xs))) of
                            Bad s -> do putStrLn s
                            Ok e  -> do outputLex $ dictionary [e]
          _     -> do putStrLn "Invalid format. Write: [command] [dictionary form]"
